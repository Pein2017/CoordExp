...
RMP_CE_MEMSTRESS_CONFIG = CONFIG_ROOT / "smoke" / "rmp_ce_memstress.yaml"
...
def _load_rmp_ce_memstress_config():
    return ConfigLoader.load_materialized_training_config(str(RMP_CE_MEMSTRESS_CONFIG))
...
def test_production_profile_resolves_and_pins_common_contract() -> None:
    ...
    assert cfg.benchmark.group_id == "stage1_set_continuation_et_rmp_ce_support2_bsz16"
    assert cfg.benchmark.control_group_id == "stage1_set_continuation_full_features"
    assert cfg.benchmark.intended_variable
    assert cfg.benchmark.comparability_label == "not-comparable"
    ...
    assert (
        ...
        == "setcont-coco1024-sota1332-et-rmp-ce-support2-bsz16-v1"
    )
    ...
    assert cfg.training["per_device_train_batch_size"] == 16
    ...
    assert (
        ...
        == "smart_batched_exact_full_suffix_rows_no_ddp_padding_et_rmp_ce_support2_bsz16_v1"
    )
    assert (
    ...
    assert (
        "valid-support loss with weight 2.0"
        in report["objective_fidelity_note"]
    )
    assert "without explicit stop-token suppression" in report["objective_fidelity_note"]


def test_production_profile_enables_support_weighted_rmp_ce() -> None:
    cfg = _load_production_config()
    ...
    assert sc.objective.branch_support_weight == pytest.approx(2.0)
    assert sc.objective.branch_balance_weight == pytest.approx(1.0)
    ...
    assert sc.train_forward.branch_batching.max_branch_rows == 32
    assert sc.train_forward.branch_batching.max_branch_tokens == 65536
    ...
    assert sc.bidirectional_token_gate.coord_gate_weight == pytest.approx(0.0)
    assert sc.bidirectional_token_gate.text_gate_weight == pytest.approx(0.0)
...
def test_rmp_ce_memstress_smoke_preserves_objective_and_decode_contract() -> None:
    cfg = _load_rmp_ce_memstress_config()
    sc = cfg.custom.stage1_set_continuation

    assert cfg.custom.trainer_variant == "stage1_set_continuation"
    assert cfg.benchmark.group_id == "stage1_set_continuation_et_rmp_ce_memstress_smoke"
    assert cfg.benchmark.comparability_label == "not-comparable"
    assert cfg.training["artifact_subdir"] == "et_rmp_ce_memstress"
    assert cfg.training["run_name"] == "smoke-et-rmp-ce-memstress"
    assert cfg.training["max_steps"] == 4
    assert cfg.training["per_device_train_batch_size"] == 32
    assert cfg.training["gradient_accumulation_steps"] == 1
    assert cfg.training["effective_batch_size"] == 256
    assert cfg.training["packing"] is False
    assert cfg.training["eval_packing"] is False
    assert cfg.custom.train_sample_limit == 1024
    assert cfg.custom.val_sample_limit == 64
    assert cfg.custom.eval_detection.limit == 64
    assert cfg.custom.eval_detection.batch_size == 4
    assert cfg.custom.eval_detection.temperature == pytest.approx(0.0)
    assert cfg.custom.eval_detection.top_p == pytest.approx(1.0)
    assert cfg.custom.eval_detection.repetition_penalty == pytest.approx(1.10)
    assert sc.objective.mode == "entry_trie_rmp_ce"
...
def test_effective_runtime_records_production_set_continuation_provenance() -> None:
    ...
    assert runtime["benchmark_group_id"] == "stage1_set_continuation_et_rmp_ce_support2_bsz16"
    assert runtime["control_group_id"] == "stage1_set_continuation_full_features"
    assert runtime["intended_variable"] == cfg.benchmark.intended_variable
    assert runtime["comparability_label"] == "not-comparable"
