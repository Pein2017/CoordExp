...
def test_stage2_ab_does_not_import_private_rollout_symbols() -> None:
    ...
    stage2_path = repo_root / "src" / "trainers" / "stage2_ab_training.py"
    ...
    for node in ast.walk(tree):
        ...
        # Match both legacy trainer module and the new rollout-matching contract package.
        if "rollout_matching" not in node.module:
            continue

        for alias in node.names:
            if alias.name.startswith("_"):
                violations.append((node.module, alias.name, int(getattr(node, "lineno", 0) or 0)))

    if violations:
        formatted = "\n".join(
            f"- {mod}: {name} (line {lineno})" for mod, name, lineno in violations
        )
