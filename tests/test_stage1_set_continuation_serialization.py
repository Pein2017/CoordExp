...
def test_render_indexed_object_list_tracks_duplicate_entries_by_index() -> None:
    objects = [
        {"desc": "cat", "bbox_2d": ["<|coord_10|>", "<|coord_20|>", "<|coord_30|>", "<|coord_40|>"]},
        {"desc": "cat", "bbox_2d": ["<|coord_10|>", "<|coord_20|>", "<|coord_30|>", "<|coord_40|>"]},
        {"desc": "cat", "bbox_2d": ["<|coord_11|>", "<|coord_21|>", "<|coord_31|>", "<|coord_41|>"]},
    ]
...
def test_build_prefix_text_supports_empty_non_empty_and_full_prefixes() -> None:
    objects = [
        {"desc": "left", "bbox_2d": ["<|coord_1|>", "<|coord_2|>", "<|coord_3|>", "<|coord_4|>"]},
        {"desc": "right", "bbox_2d": ["<|coord_5|>", "<|coord_6|>", "<|coord_7|>", "<|coord_8|>"]},
    ]
    ...
    assert non_empty_prefix.text[
        non_empty_prefix.object_spans[0].start : non_empty_prefix.object_spans[0].end
    ] == entry1


def test_build_candidate_entry_text_returns_candidate_and_global_close_spans() -> None:
    objects = [
        {"desc": "first", "bbox_2d": ["<|coord_1|>", "<|coord_2|>", "<|coord_3|>", "<|coord_4|>"]},
        {"desc": "second", "bbox_2d": ["<|coord_11|>", "<|coord_12|>", "<|coord_13|>", "<|coord_14|>"]},
    ]
    rendered = render_indexed_object_list(objects, object_field_order="desc_first")
    ...
    assert full_text[offset + candidate_span.start : offset + candidate_span.end] == candidate_entry
    assert continuation.text[close_start.start : close_start.end] == "]"
    assert continuation.text[full_close.start : full_close.end] == "]}"
    assert candidate_span.end == close_start.start
...
def test_build_global_close_text_excludes_object_and_chat_end_tokens() -> None:
    ...
    assert close.text == "]}"
    assert close.text[close.global_close_start_span.start : close.global_close_start_span.end] == "]"
    assert close.text[close.global_full_close_span.start : close.global_full_close_span.end] == "]}"
    assert "}" not in close.text[
