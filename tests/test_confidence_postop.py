...
def test_confidence_postop_supports_coord_token_logprobs_with_numeric_raw_output(
    tmp_path: Path,
) -> None:
    gt_vs_pred_path = tmp_path / "gt_vs_pred.jsonl"
    trace_path = tmp_path / "pred_token_trace.jsonl"
    ...
    record = _bbox_record(
        image="img_coord_numeric_raw.png",
        x1=100,
        y1=200,
        x2=300,
        y2=400,
    ...
    assert obj["kept"] is True
    assert obj["score_geom"] == pytest.approx(0.2)
    assert obj["score_desc"] == pytest.approx(1.0)
    assert obj["score_fusion"] == pytest.approx(0.52)
    assert obj["confidence_details"]["method"] == CONFIDENCE_METHOD
    ...
    assert scored_records[0]["pred_score_version"] == PRED_SCORE_VERSION
    assert scored_records[0]["pred"][0]["score"] == pytest.approx(0.52)
