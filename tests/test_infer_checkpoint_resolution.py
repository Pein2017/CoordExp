...
def test_compact_token_embeddings_adapter_requires_saved_token_embeddings_adapter(
    tmp_path: Path,
) -> None:
    adapter_dir = tmp_path / "adapter"
...
def test_compact_token_embeddings_adapter_guard_rejects_missing_object_box_adapter(
    tmp_path: Path,
) -> None:
    adapter_dir = tmp_path / "adapter"
    _write_adapter_checkpoint(adapter_dir, base_model_name_or_path="base-model")
...
def test_compact_token_embeddings_adapter_rejects_partial_rows(
    tmp_path: Path,
) -> None:
    adapter_dir = tmp_path / "adapter"
    _write_adapter_checkpoint(
...
def test_compact_token_embeddings_adapter_accepts_exact_template_rows(
    tmp_path: Path,
    template_id: str,
    expected_count: int,
) -> None:
    ...
    token_ids = list(required_trainable_token_row_ids(template_id))
    _write_token_embeddings_adapter_weights(
        adapter_dir,
        token_ids=token_ids,
        tie_head=True,
...
def test_compact_token_embeddings_adapter_rejects_missing_or_extra_rows(
    tmp_path: Path,
    token_ids: list[int],
) -> None:
    adapter_dir = tmp_path / "adapter"
...
def test_compact_token_embeddings_adapter_rejects_missing_closure_rows(
    tmp_path: Path,
    template_id: str,
    missing_id: int,
    missing_token: str,
    ...
    _write_token_embeddings_adapter_weights(adapter_dir, token_ids=token_ids, tie_head=True)
    resolved = resolve_inference_checkpoint(model_checkpoint=str(adapter_dir))
...
def test_compact_token_embeddings_adapter_rejects_duplicate_rows(tmp_path: Path) -> None:
    adapter_dir = tmp_path / "adapter"
    _write_adapter_checkpoint(
        adapter_dir,
        base_model_name_or_path="base-model",
    ...
    token_ids = list(required_trainable_token_row_ids("compact"))
...
def test_compact_token_embeddings_adapter_rejects_tensor_shape_mismatch(
    tmp_path: Path,
) -> None:
    adapter_dir = tmp_path / "adapter"
    _write_adapter_checkpoint(
...
def test_compact_token_embeddings_adapter_guard_does_not_apply_to_non_compact_format(
    tmp_path: Path,
) -> None:
    adapter_dir = tmp_path / "adapter"
    _write_adapter_checkpoint(adapter_dir, base_model_name_or_path="base-model")
...
def test_compact_token_embeddings_adapter_allows_full_or_merged_checkpoint() -> None:
    resolved = resolve_inference_checkpoint(model_checkpoint="merged-model")
