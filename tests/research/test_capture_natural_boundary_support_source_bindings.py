...
def test_active_consumer_and_merger_are_untracked_without_owner_commit() -> None:
    receipt = capture.capture_source_bindings()
    for role in ("consumer", "merger"):
        provenance = receipt["sources"][role]["git"]
        assert provenance["state"] == "untracked"
        assert provenance["tracked"] is False
        assert provenance["owner_commit"] is None
        assert provenance["commit"] is None
        assert provenance["proof"] == "git_status_porcelain_untracked"
