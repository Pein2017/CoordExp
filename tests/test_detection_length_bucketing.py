...
def test_prefix_rollin_length_provider_is_invariant_to_k_and_random_order(
    ...
    provider = DetectionLengthProvider(dataset)
...
def test_distributed_length_grouped_sampler_shards_and_reshuffles() -> None:
    ...
    rank0 = build_detection_length_grouped_sampler(
        lengths=lengths,
        batch_size=2,
        seed=5,
        drop_last=False,
    ...
    rank1 = build_detection_length_grouped_sampler(
        lengths=lengths,
        batch_size=2,
        seed=5,
        drop_last=False,
...
def test_trainer_mixin_passes_explicit_lengths_without_getitem(tmp_path: Path) -> None:
    ...
    class BucketedTrainer(DetectionLengthGroupedTrainerMixin, BaseTrainer):
        pass

    trainer = BucketedTrainer()
    trainer.detection_length_bucketing = DetectionLengthBucketingConfig(
        enabled=True,
        seed=11,
    )
