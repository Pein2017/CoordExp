...
**Lead-accepted; bounded positive diagnostic result, no promotion.** The lead independently recomputed all four natural-output scores and owner G/L sets, verified16 exact baseline sequences and16 full-vocabulary first-fork tensors, reconstructed coefficients from the original effective readout, and checked unchanged non-coordinate/EOS logits, parameter/runtime identities and completed jobs. [Lead receipt](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-16-endpoint-loop-natural-readout-norm/lead-acceptance.json) and [CPU verifier](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-16-endpoint-loop-natural-readout-norm/lead_verify.py) preserve acceptance. Original candidate research records and result/terminal bytes are retained. The fixed panel and this overnight diagnostic series are closed; no normalization variant, training or follow-up was launched.
...
## Lead closeout: what the evidence now explains

The strongest causal result is that the fixed output-only norm policy changes natural R16 rollouts usefully on two selected images, with28 additional known matches, zero incumbent losses across the four-image panel, and clean termination on the recovered cases. It requires no supplied route and no input-parameter update. Its known FN remain3/17 on417044 and9/27 on477415; this is not scene exhaustion.

The supported sequence is: readout scale changes token competition; greedy argmax crosses a rank boundary; the emitted token then changes subsequent conditioning. The recorded first forks and complete suffixes demonstrate the first and last effects. The earlier exact-return token controls support local trajectory persistence; a specific self-reinforcing circuit is still an inference. Both initial-entry prevention and continued suppression of later recurrence can contribute because the policy stays active for the whole generation.

The strongest surviving alternative to an endpoint-specific defect is broad coordinate-logit reshaping of fragile trajectories. The successful first forks406→405 and811→809 are interior coordinates;999→990 does not rescue351017. All-coordinate scaling also changes competition with syntax/category/EOS. Thus neither endpoint-only causality, input-embedding irrelevance nor a general safe fix has been established. UNKNOWN rows require separate evidence before physical precision claims.

Decision: retain this reproducible causal handle and close the current series. Further work must answer a new question, such as separating early route selection from a need for continued intervention or testing broader retention/coverage; it must not become an automatic strength sweep. No such experiment is launched here.
...
Technical status: valid fixed-policy natural generation and saved-output reduction. Scientific status: lead-accepted bounded positive contribution on two cases, persistent failure on one, no measured damage on one healthy control. No population/deployment or learned-repair promotion.
