...
**Lead-accepted; closed.** Full-row E is negative in all16 supplied/candidate extent combinations under both original and equal-norm readout. This supports conditional **relative** owner/location-sensitive suppression with fixed literal last row C. It does not establish absolute suppression: both A and B candidate probabilities are higher after earlier A than after earlier B, with B increasing more. The shared-x1 first-fork effect has the opposite sign—relative facilitation toward A. These surfaces must remain separate.
...
Root independently accepted the frozen candidate and closed this package. Any successor needs a new distinguishing contrast and a separate lead decision. No escape rollout,0→30 intervention, norm pulse, checkpoint comparison, training or successor is launched.

## Root independent acceptance

Root inspected both the full-image and local A/B/C overlays, confirming distinct dominant owners within this finite contrast. It verified all116 bound files, reproduced the complete reduction and independent-verification receipt exactly, independently rebuilt all16 normalized full-vocabulary row probabilities, and checked all44 native/full positions. No additional model forwards were needed. A single bounded read-only review found no decision-changing blocker, including the forced-only qualification stop. All four owned producers exited0 and are absent; the worker was independently observed idle.

Acceptance is limited to earlier-history dependence, opposite relative effect directions at the first fork and whole row, and the declared numerical/runtime evidence. It does not establish absolute inhibition, an abstract ledger, a unique training cause, a native greedy repair, or population physical recall. Spatial/order differences and synthetic-history compatibility remain alternatives. Original candidate records and hashes remain unchanged.

[Lead receipt](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-18-covered-history-common-tail/lead-acceptance.json) · [CPU verification](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-18-covered-history-common-tail/coordination/lead-verification.json). No successor dispatched; checkpoint-stage probes remain deferred.
