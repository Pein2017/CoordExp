...
**Lead-accepted bounded evidence.** Worker candidate completed 2026-09-18T20:46:45.687245+00:00; independent acceptance is owned by the [lead receipt](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-18-readout-common-component/lead-acceptance.json). Original candidate record bytes are preserved in that receipt.
...
At the first original-versus-full divergence, all25 changes are coordinate-to-coordinate. On that SAME original head state, shared-only selects the full winner in8/25 and centered-only in4/25. Neither component produces an exactly identical full-policy continuation on any of25 boundaries. Shared and centered trajectories coincide in6/25. These are finite selected-case comparisons, not independent trials. Exact continuation identity is a provenance descriptor, not a recovery criterion. Lead same-state margin arithmetic finds that in15/25 first original/full forks neither component alone reverses the selected pair, while their additive increments do. Thus apparent joint necessity can follow from crossing an argmax boundary without nonlinear score synergy. Crossing that boundary also occurs in cases with unchanged or worse recurrence.
...
The lead independently reproduced the saved-output reduction and all-step operator verification exactly, checked bound evidence, and accepted one bounded Luna-max audit with no blocker. Current catalog and routing are maintained by root; the immutable worker terminal retains its pre-acceptance status.
