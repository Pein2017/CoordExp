# Native coordinate-branch completion — lead-accepted
...
At genuine native donut417044 row6/action49 (x1/action54), original R16 FP32/SDPA bs4 runtime, original readout, three fixed branches and width4 row-only search. All prior history including rows3/4 HOLD is unchanged. Root independently accepted this bounded result; the package is closed.
...
Root checked141 binding entries covering135 unique files, reproduced both the complete saved-output reduction and physical scoring JSON exactly, verified all14 final path rankings and the positive all-competitor margins on native greedy, and confirmed the absence of all3 owned producer PIDs with successful exit receipts. Maximum complete-path rescoring discrepancy is4.26365e-6 nats. One bounded producer/contract review found no decision-changing blocker. The worker was independently observed idle; no new model forwards were required for acceptance.

Root viewed the14-row bbox contact sheet, full-image B context and focused branch30 context. It accepts5 repeated-A and4 unvisited-B realizations, with all5 branch30 identities/coverage held unresolved. The physically supported distinction survives legitimate overlap and extent variation; no unmatched output is converted into a physical negative.

This result verifies a local greedy-versus-complete-row difference **within repeated A**, not a retained higher-probability new-owner route. B has an easier conditional suffix, but its lower x1 probability more than offsets that advantage in the retained full rows. The search retains only a small fraction of conditional suffix probability; it cannot establish global sequence optimality, owner-level probability preference, burst-maintenance mechanism, a training cause, or a tied-head diagnosis. No training, architecture change or automatic successor follows.

[Lead receipt](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-18-native-coordinate-branch-completion/lead-acceptance.json) · [CPU verification](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-18-native-coordinate-branch-completion/coordination/lead-verification.json). Original candidate bytes remain preserved; root-owned frontier integration is complete.
