...
Lead-accepted 2026-09-19T03:29:09.473261+00:00. Independent reduction and saved-step verification reproduce the candidate exactly; the bounded review found no blocker. [Acceptance receipt](/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-19-first-coordinate-mediation/lead-acceptance.json).
...
An idle-child dispatch delay occurred before qualification; it consumed no model calls and duplicated no work. Root registered the result in the catalog at acceptance. Predecessors remain immutable. Exact artifacts/reproduction: `/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-19-first-coordinate-mediation/ARTIFACTS.md`.
...
## Root supplementary checks

At original readout, forcing x1 changes later coordinates too: tied first-row x2 changes79→93; untied y2 changes216→247. This is propagation from a discrete choice, not a claim that the whole box differs by only one bin.

A saved-output all-pairs check also rules out merely rearranged numerical repetitions for the selected low-run routes: untied original0 has126 near pairs among31 primary rows, original1 has0 among29; both sign23 choices have0. Tied original0/1 have144/178 near pairs, while both sign23 choices have0. Exact-pair counts and all36 cells are retained in `coordination/lead-all-pairs-check.json`. These supplementary numerical descriptors do not establish distinct physical owners and do not replace the frozen primary metrics.

The separately released [coordinate-pair readout unit](../2026-09-19-coordinate-pair-readout/unit.md) tests persistent score support on the original11 failure boundaries; this accepted package is closed.
