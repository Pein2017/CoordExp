# Glossary and comparison boundaries
...
Historical retrieval aliases: **painted-GT / PVCI** names the July painted-cue, feature/residual intervention and causal-proposal lineage; it is not one immutable architecture. **E0/E1** in its anti-copy studies refer to different painted-row checkpoints, not the later Source or Stable50. **Post-scatter** means after image features replace image-token positions in the language-model input embeddings. **DeepStack** denotes the separate intermediate visual-feature injection channels tested by those units. **Sketch-and-snap** means correcting a corrupted object-coupled cue toward source geometry, not complete geometry-invariant object identity. **Proposal representation** and **proposal feedback/bridge** denote respectively a learned readout and a distinct intervention that consumes it. **Denoising-OFF** removes noisy-history training; zero auxiliary KL alone does not.

Old OKF (`Open Knowledge Format`) types, `active` decisions and `current` headings are dated provenance, not current taxonomy or authorization. Old unit IDs and output paths deliberately keep their literal names for recovery even when their scientific interpretation has been narrowed.
