...
- [Pi Lightweight External Worker Ablation](investigations/pi-lightweight-worker-ablation/) - Stage 0 capability and total-cost screen for bounded, mechanically verifiable CoordExp tasks
...
| Where should the dense-enumeration program look next? | The 2026-07-17/18 transition units establish an immediate, owner-specific, `x1`-led successor effect with strong latest-row dependence but no reliable covered-object ledger, and show that valid boundary probability can be fragmented while greedy picks an isolated extreme token. The matched-objective comparison makes pure cross-entropy the next mechanism baseline; the proposed coordinate-consistency training screen is not promoted. The pending discriminators are the ready same-covered-set earlier-row order-equivalence unit and the pure-cross-entropy own-prefix reachability study; training and architecture remain unauthorized. | [Dense-enumeration research compass](investigations/qwen3-vl-dense-enumeration/compass.md) |
