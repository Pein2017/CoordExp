...
that parse through `LatestDetectionTrainingConfig`.

Current canonical launch configs under
`configs/stage1/recursive_detection_ce_latest/` are still materialized and do
...
Latest detection owns these top-level sections:
...
`custom.*` is rejected for latest detection. Do not extend legacy shared
