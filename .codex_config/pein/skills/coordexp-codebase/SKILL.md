...
- Server-mode smoke (requires GPUs + a local checkpoint):
  - `bash scripts/train_stage2.sh server_gpus=0,1,2 train_gpus=3 config=configs/stage2_ab/smoke/bbox_max60_ckpt1516_b_only_vllm_server.yaml`
...
- `scripts/run_vis.sh`
- `scripts/train_stage2.sh` (canonical; `scripts/stage2_ab_server_train.sh` is a deprecated wrapper)
