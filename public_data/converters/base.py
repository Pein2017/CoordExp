...
class BaseConverter(ABC):
    ...
    def _convert_all_samples(self, annotations: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Adjust total if max_samples is set
        if self.config.max_samples:
            total = min(total, self.config.max_samples)
        
        # Create iterator for samples
        sample_iterator = self._iterate_samples(annotations)
        
        # Use tqdm if available, otherwise fall back to simple progress
        if tqdm is not None:
        ...
        for i, sample_data in enumerate(sample_iterator):
    ...
    def _write_jsonl(self, samples: List[Dict[str, Any]]) -> None:
        ...
        def _round_sample(sample: Dict[str, Any]) -> Dict[str, Any]:
            rounded = dict(sample)
            # ensure width/height are ints
            if "width" in rounded:
                try:
                    rounded["width"] = int(round(float(rounded["width"])))
                except Exception:
                    pass
            if "height" in rounded:
                try:
                    rounded["height"] = int(round(float(rounded["height"])))
                except Exception:
                    pass
            objs = rounded.get("objects") or []
            new_objs = []
            for obj in objs:
                o = dict(obj)
                for key in ("bbox_2d", "poly", "line"):
                    if key in o and o[key] is not None and isinstance(o[key], list):
                        o[key] = _round_list(o[key])
                new_objs.append(o)
            rounded["objects"] = new_objs
            return rounded

        with open(self.config.output_path, "w", encoding="utf-8") as f:
            for sample in samples:
                rounded = _round_sample(sample)
