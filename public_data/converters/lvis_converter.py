...
from .sorting import canonicalize_poly, sort_objects_tlbr
...
class LVISConverter(BaseConverter):
    ...
    def convert_sample(self, image_id: int) -> Optional[Dict[str, Any]]:
        ...
        # ------------------------------------------------------------------
        # Pre-sorting (Qwen3-VL prompt spec compatibility)
        # - Canonicalize polygon vertex order so the first vertex is a stable anchor.
        # - Sort objects top-to-bottom then left-to-right.
        # ------------------------------------------------------------------
        objects = self._presort_objects(objects)
        if not objects:
            return None
    ...
    def _presort_objects(self, objects: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Canonicalize polygon vertices and sort objects (LVIS prep step)."""
        processed: List[Dict[str, Any]] = []
        for obj in objects:
            if obj.get("poly") is not None:
                poly = obj.get("poly")
                if not isinstance(poly, list):
                    self.stats["polygon_skipped"] = self.stats.get("polygon_skipped", 0) + 1
                    continue
                try:
                    canon = canonicalize_poly(poly)
                except Exception:
                    # Keep conversion robust: treat as invalid polygon and drop this object.
                    self.stats["polygon_skipped"] = self.stats.get("polygon_skipped", 0) + 1
                    continue
                if len(canon) < 6 or len(canon) % 2 != 0:
                    self.stats["polygon_skipped"] = self.stats.get("polygon_skipped", 0) + 1
                    continue

                updated = dict(obj)
                updated["poly"] = canon
                # Keep poly_points consistent with any closing-point stripping.
                if "poly_points" in updated:
                    updated["poly_points"] = len(canon) // 2
                processed.append(updated)
            else:
                processed.append(obj)

        return sort_objects_tlbr(processed)
