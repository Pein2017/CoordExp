---
title: Stage-2 Symptom Note
status: historical-diagnostic
scope: stage2
kind: symptom
topics: [stage2, diagnostics, rollout, schedule, historical-stage2-ab]
references:
...
The intent is to preserve a concrete failure pattern from the pre-v2 era so it can be compared against the current clean-prefix design, not to prescribe current operator actions directly.
...
**Historical lessons (config-first at the time):**
...
## 1) Context: what the earlier Stage-2 AB design expected

At the time of this note, `progress/directions/stage2_emish_set_supervision_v1.md` ("Stage-2: EM-ish Training Loop") described the intended schedule as:
...
- Historical config: `configs/stage2_ab/prod/ab_mixed.yaml`
...
- Historical config: `configs/stage2_ab/prod/ab_mixed_continue.yaml`
  - `stage2_ab.schedule.b_ratio: 0.85`
...
Evidence:
- Historical design intent (`progress/directions/stage2_emish_set_supervision_v1.md` at the time): Channel-A hot / Channel-B cold, e.g., 95% / 5%.
- Historical config reality in the analyzed run family:
...
**Recommendation**
- Use the fallback described in `progress/directions/stage2_emish_set_supervision_v1.md`:
