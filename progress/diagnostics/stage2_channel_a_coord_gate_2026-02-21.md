---
title: Stage-2 Channel-A Coord Gate Diagnosis
...
Status note:
- this is a historical diagnosis note from the earlier `stage2_ab` naming era,
- references to `src/trainers/stage2_ab_training.py` describe the code layout at that time,
- the current live Stage-2 implementation is `src/trainers/stage2_two_channel.py`,
- config paths under `configs/stage2_ab/...` are historical provenance handles rather than current repo entrypoints.
...
Status at the time: **Coord gate implemented; em_detach CE-grad bug fixed; awaiting rerun confirmation.**
...
## 5) Historical mitigation in the `stage2_ab` era: `stage2_ab.coord_gate_weight`
...
Current note:
- the current Stage-2 config surface lives under `configs/stage2_two_channel/`,
- use `docs/training/README.md` and `docs/IMPLEMENTATION_MAP.md` to locate the modern equivalents.
...
This was enabled via config blocks under `configs/stage2_ab/prod/*.yaml` at the time.
For current Stage-2 debugging surfaces, use `configs/stage2_two_channel/` and `docs/training/STAGE2_RUNBOOK.md`.

---

## 7) Historical rerun plan (verification at the time)

To verify the gate fix, rerun short experiments (at least to steps 300 and 600):

1) Channel-A only baseline at the time:
   - `configs/stage2_ab/prod/geo_first_a_only.yaml`  (typically `softctx_grad_mode=unroll`)

2) Channel-A only with detached E-step at the time:
   - `configs/stage2_ab/prod/geo_first_a_only_em_detach.yaml`

Do not treat those config paths as current instructions. For analogous modern work, start from `configs/stage2_two_channel/` and the Stage-2 docs stack.
