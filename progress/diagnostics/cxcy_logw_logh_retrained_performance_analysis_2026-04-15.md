---
title: cxcy_logw_logh Retrained Performance Analysis
date: 2026-04-15
status: completed
owner: codex
supersedes:
...
# `cxcy_logw_logh` Retrained Performance Analysis

## Supersession Note

This note is the canonical replacement for the two earlier diagnostics listed in
`supersedes`.

The 2026-04-14 note should not be used for current conclusions because it was
anchored to the wrong checkpoint. The valid performance read for the
`cxcy_logw_logh` family is the retrained checkpoint summarized here.
...
- question:
  - what is the corrected detection quality of the retrained
    `cxcy_logw_logh` checkpoint?
  - what still prevents recovery to the strong 2B baseline band?
...
This is the corrected performance surface for the retrained
`cxcy_logw_logh` checkpoint.

It remains below the strong 2B baseline:
...
Strict same-prefix (`1536` images) comparison against the strong 2B baseline:
...
- a small subset of scenes still contributes a disproportionate share of the
  prediction mass
- this tail materially drags the aggregate metric down relative to baseline
...
- the remaining shortfall vs the strong baseline is therefore not explained by
  duplication alone
...
The absolute localization quality is much lower, especially for small and
medium objects:
...
- boxes are still too weakly calibrated to survive stricter IoU thresholds at
  baseline quality
- the deficit is strongest on small and medium objects, not only on large
  crowded scenes

### 4. The deficit is not only in `logw/logh`; center localization is also weaker
...
- center error is roughly `1.7x` worse than the center-parameterization
  baseline
...
Both center and size are worse, with height and size calibration somewhat more
degraded.

This matters because the strongest in-repo evidence against a "center itself is
the problem" explanation is that the strong 2B baseline here is already a
center-based parameterization run (`center_size_1024`) and still reaches:
...
So a center-based chart can work well in this stack; the regression appears
more specific to the `cxcy_logw_logh` regime than to using center coordinates
at all.

### 5. Token-level coordinate diagnostics look healthy, but detection quality still lags
...
- healthy token-space metrics do not guarantee strong detection geometry after
  full sequence generation
- this points to optimization or decoding behavior rather than a decode
  inversion bug in the retrained checkpoint

## Bottom Line

- the retrained `cxcy_logw_logh` checkpoint is the valid performance surface
  for this family
- corrected same-prefix detection quality is `bbox_AP = 0.20694`,
  `bbox_AP50 = 0.34350`, `bbox_AP75 = 0.21722`
- the remaining gap vs the strong 2B baseline is explained by two interacting
  issues:
  - a duplication-heavy burst tail on a small but influential subset of images
  - a broader localization and size-calibration deficit that persists even off
    the burst subset
