---
title: Stage-2 Channel-A Only Infer/Eval Notes
status: historical-benchmark
scope: stage2-channel-a
kind: benchmark
topics: [stage2, channel-a, eval, infer, repetition-penalty, historical-stage2-ab]
references:
...
Historical config handles:

- `configs/stage2_ab/prod/a_only.yaml`
- `configs/stage2_ab/prod/a_only_iter-1.yaml`

> Historical note: `configs/stage2_ab/prod/a_only_iter-1.yaml` was believed to end with an invalid YAML fragment (`: 1`) at the time of this note.
> Do not treat that as a current repo task; for analogous current work, use the `stage2_two_channel` config tree and resolved run artifacts.
...
## Historical next experiments

To keep comparisons clean and paper-ready:

1) **Verify `n_softctx_iter` in training**
   - At the time, the follow-up was to fix `configs/stage2_ab/prod/a_only_iter-1.yaml` and/or record the resolved config in the run artifact.

2) Run the planned ablation from the then-current Stage-2 design:
...
For current Stage-2 experimentation, map the historical lessons above onto the current `stage2_two_channel` stack rather than trying to reproduce the removed `stage2_ab` config tree directly.
