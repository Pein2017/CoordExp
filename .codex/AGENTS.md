...
* Short inline Python or `jq` is appropriate for one-off queries. Reuse an existing or task-local script when nontrivial inventory, dependency, or validation logic recurs; do not repeatedly regenerate it or create permanent infrastructure for a one-off task. Judge economy by generated code, returned evidence, and rework together, not by the choice of shell syntax.
