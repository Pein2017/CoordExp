---
name: project-memory
description: Maintain concise repository-local continuity in the current cwd when a long-running task changes durable decisions, evidence, blockers, or the next continuation point.
---
...
This is one globally installed skill under `/data/CoordExp/.codex/skills`.
Apply it to the current `cwd` by default: resolve `memories/config.yaml` and its
configured entrypoints relative to that cwd. Use another project root only when
the user explicitly names one. Do not create worktree-local `.codex` copies.
