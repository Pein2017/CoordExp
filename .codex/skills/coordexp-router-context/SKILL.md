...
description: Use when CoordExp work needs current repo routing, docs/spec/code entrypoints, historical evidence boundaries, broad module maps, or phase-specific guidance for CodeGraph MCP, Serena MCP, RTK, raw shell, and navigation.
---
...
4. For code-heavy questions, use CodeGraph as the broad map when a correct local index exists, then switch to Serena once Python files or symbols are known. Do not run a second broad mapper unless the first map was stale, empty, or clearly wrong.
...
- Prefer Serena as the single-tool fallback if tool surfaces conflict: CodeGraph can be stale in worktrees, while Serena tracks the active project and supports exact symbol/reference/edit operations.
...
- Broad-reader ping-pong: do not alternate CodeGraph and Serena for repository-scale discovery. Pick one mapper for the phase, then hand off to exact symbol/file work.
