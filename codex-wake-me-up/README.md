...
The primary `wait_for_event` path is goal-independent. It queues one bounded,
self-identifying pointer in the exact task's existing durable FIFO with
`clientUserMessageId=deliveryId`; the full wake evidence remains in monitor
status. `defer_goal_until_event` is the optional legacy path for callers that
explicitly want to pause and later reactivate an eligible goal.
...
`wait_for_event` is the primary operation. It accepts a loaded or durably stored
exact local task regardless of whether its goal is null, active, paused, or
blocked:
...
before resume. The pointer itself is small and tells the task to inspect status
once; it is not a success claim.
