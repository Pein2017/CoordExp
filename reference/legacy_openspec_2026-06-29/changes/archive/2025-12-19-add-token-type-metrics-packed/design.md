...
- Maintain metric key parity with upstream Qwen3-VL for dashboards: keys without prefix `loss`, `token_acc`, and `{desc,coord,format}_token_acc|token_count`; document intentional delta (aggregate-only vs per-dataset in upstream).
