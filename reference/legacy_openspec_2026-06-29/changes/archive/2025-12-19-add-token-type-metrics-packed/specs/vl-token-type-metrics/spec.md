...
The system SHALL emit stable aggregate metric keys (no prefix): `loss`, `token_acc`, `token_count`, and per-type keys `desc_token_acc`, `desc_entropy`, `desc_token_count`, `coord_token_acc`, `coord_entropy`, `coord_token_count`, `format_token_acc`, `format_entropy`, `format_token_count`. These keys SHALL remain aligned with the upstream Qwen3-VL implementation when both are enabled to ease cross-repo dashboarding.
