...
- [x] 1.1 Encode the locked Stage-1 bbox-only assumption in the spec and docs.
- [x] 1.2 Encode immediate optional Channel-A `A1` support for `bbox_size_aux`.
- [x] 1.3 Validate the draft change artifacts:
  - `openspec validate --type change bbox-log-size-aux-loss --strict --no-interactive`

## 2. Shared Geometry Helper

- [x] 2.1 Add shared decoded-box size-aux helper(s) in
      `src/trainers/teacher_forcing/geometry.py`:
  - matched log-width/log-height loss
  - matched log-area loss
  - optional oversize penalty
- [x] 2.2 Keep helper behavior numerically stable:
  - canonicalize first
  - clamp width/height with `eps`
  - guard empty masks
  - `nan_to_num` returned scalars
- [x] 2.3 Add direct unit coverage for:
...
- [x] 3.1 Add a new pipeline objective module `bbox_size_aux` to the strict
      registry / allowlists / manifest validation.
- [x] 3.2 Reuse the shared helper through a new
...
- [x] 3.3 Thread decoded canonicalized bbox state from `bbox_geo` into
      `bbox_size_aux` so the new plugin does not reimplement a second decode
      path.
- [x] 3.4 Update Stage-2 objective-atom projection / logging so the new
      size-aux terms are emitted explicitly.
- [x] 3.5 Update canonical Stage-2 YAML examples under
...
- [x] 4.1 Ensure `rollout_matching.pipeline.objective[*].name=bbox_size_aux`
      is supported with the same config keys.
- [x] 4.2 Verify rollout-aligned matched teacher-forcing supervision reuses the
      same `bbox_size_aux` plugin implementation without forked loss logic.

## 5. Stage-1 Wiring

- [x] 5.1 Add a typed Stage-1 config block under `custom.bbox_size_aux`.
- [x] 5.2 Add a lightweight Stage-1 aux plugin host around the existing single
      forward / reporter flow, reusing the canonical `bbox_size_aux` plugin
      implementation instead of another feature-specific mixin.
- [x] 5.3 Reuse the shared decoded-box helper once bbox grouping is explicit.
- [x] 5.4 Fail fast if the enabled Stage-1 path cannot construct unambiguous
...
- [x] 6.1 Run targeted config and trainer tests:
...
- [x] 6.2 Add / run Stage-1-focused tests for the new `custom.bbox_size_aux`
      path.
- [x] 6.3 Add a tiny config-path smoke so at least one authored config resolves
