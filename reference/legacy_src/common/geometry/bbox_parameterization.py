...
CXCYWH_SLOT_ORDER = "cxcywh"
CXCYWH_CONVERSION_VERSION = 1
...
def normalize_bbox_format(
    ...
    *,
    ...
    if normalized not in {"xyxy", "cxcy_logw_logh", "cxcywh"}:
        raise ValueError(
            f"{path} must be one of ['cxcy_logw_logh', 'cxcywh', 'xyxy'], got {value!r}"
        )
...
def xyxy_norm_to_cxcywh_norm(points: Sequence[float]) -> list[float]:
    x1, y1, x2, y2 = _canonical_xyxy_norm(points)
    width = max(x2 - x1, BBOX_SIZE_FLOOR)
    height = max(y2 - y1, BBOX_SIZE_FLOOR)
    return [
...
def cxcywh_norm_to_xyxy_norm(points: Sequence[float]) -> list[float]:
    if len(points) != 4:
        raise ValueError(f"bbox_2d must contain 4 values, got {len(points)}")
    cx = min(max(float(points[0]), 0.0), 1.0)
    cy = min(max(float(points[1]), 0.0), 1.0)
    width = min(max(float(points[2]), BBOX_SIZE_FLOOR), 1.0)
    height = min(max(float(points[3]), BBOX_SIZE_FLOOR), 1.0)
    return [
        cx - (width / 2.0),
        cy - (height / 2.0),
        cx + (width / 2.0),
...
def xyxy_norm1000_to_cxcywh_bins(points: Sequence[float | int]) -> list[int]:
    norm = [decode_norm1000_slot(v) for v in points]
    return [quantize_norm1000_slot(v) for v in xyxy_norm_to_cxcywh_norm(norm)]
...
def cxcywh_norm1000_to_xyxy_norm1000(points: Sequence[float | int]) -> list[float]:
    norm = [decode_norm1000_slot(v) for v in points]
    return [float(MAX_BIN) * v for v in cxcywh_norm_to_xyxy_norm(norm)]
