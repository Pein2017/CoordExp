# coordexp-infras Wave 7 / Val200 Validation Packet
...
required for coordexp-infras V1 readiness. The fixed val200 inference/eval run is
...
- Accepted val200 config: `configs/coordexp_infras/infer/qwen3_vl_2b_desc_first_geo_sorted_pure_ce_dora_r16a32_step917_val200.yaml`
...
- Optional full-dataset benchmark config remains available at `configs/coordexp_infras/infer/qwen3_vl_2b_desc_first_geo_sorted_pure_ce_dora_r16a32_benchmark.yaml`, but it is not a required V1 gate.
...
- Adapter smoke command: `CUDA_VISIBLE_DEVICES=4 python -m src.infer --config configs/coordexp_infras/infer/wave7_real_adapter_smoke.yaml`
...
- Production-adapter smoke command: `CUDA_VISIBLE_DEVICES=4 python -m src.infer --config configs/coordexp_infras/infer/wave7_real_production_adapter_smoke.yaml`
...
```bash
CUDA_VISIBLE_DEVICES=1 python -m src.infer --config configs/coordexp_infras/infer/wave7_real_base_single_smoke.yaml
```

Base two-row batched smoke:

```bash
CUDA_VISIBLE_DEVICES=1 python -m src.infer --config configs/coordexp_infras/infer/wave7_real_base_batched_smoke.yaml
```

Adapter-enabled smoke:

```bash
CUDA_VISIBLE_DEVICES=1 python -m src.infer --config configs/coordexp_infras/infer/wave7_real_adapter_smoke.yaml
```

Production-adapter two-row smoke:

```bash
CUDA_VISIBLE_DEVICES=4 python -m src.infer --config configs/coordexp_infras/infer/wave7_real_production_adapter_smoke.yaml
```
