# coordexp-infras Inference Infrastructure Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the coordexp-infras offline inference and benchmark-evaluation infrastructure from the approved OpenSpec baseline, ending in real HF/Qwen smokes and an accepted fixed-val200 scored mAP/mRecall validation gate.
...
**Architecture:** Keep `openspec/changes/build-coordexp-infras-inference-infra/` as the contract authority and use the patched inference decisions as the design rationale. The public entry is thin `src/infer.py`; implementation lives under `src/inference/` and reuses existing `src/qwen`, `src/templates`, `src/artifacts`, `src/config`, and `src/eval` ownership rather than copying training internals or legacy inference code.

**Tech Stack:** Python 3.12, PyTorch, Transformers Qwen3-VL `generate`, PEFT, safetensors, Pydantic or dataclasses consistent with existing config code, pytest, OpenSpec, existing coordexp-infras Qwen/template/artifact helpers.

---

## Source Of Truth

- Worktree: `/data/CoordExp/.worktrees/coordexp-infras`
- OpenSpec change: `openspec/changes/build-coordexp-infras-inference-infra/`
- Proposal: `openspec/changes/build-coordexp-infras-inference-infra/proposal.md`
- Design: `openspec/changes/build-coordexp-infras-inference-infra/design.md`
- Tasks: `openspec/changes/build-coordexp-infras-inference-infra/tasks.md`
- Wave 1 source-study gate note:
  `openspec/changes/build-coordexp-infras-inference-infra/source-studies/2026-07-02-wave1-gate-resolution.md`
- Specs:
  - `openspec/changes/build-coordexp-infras-inference-infra/specs/coordexp-infras-infer-config-runtime/spec.md`
  - `openspec/changes/build-coordexp-infras-inference-infra/specs/coordexp-infras-infer-backend-trace/spec.md`
  - `openspec/changes/build-coordexp-infras-inference-infra/specs/coordexp-infras-infer-prompt-parsing/spec.md`
  - `openspec/changes/build-coordexp-infras-inference-infra/specs/coordexp-infras-infer-scoring-artifacts/spec.md`
  - `openspec/changes/build-coordexp-infras-inference-infra/specs/coordexp-infras-infer-pipeline/spec.md`
  - `openspec/changes/build-coordexp-infras-inference-infra/specs/coordexp-infras-infer-benchmark-smoke/spec.md`
- Decisions: `docs/architecture/proposals/2026-06-27-coordexp-infras/DECISIONS.md`
...
- `configs/coordexp_infras/infer/base.yaml`: shallow inference base.
- `configs/coordexp_infras/infer/<production-leaf>.yaml`: production-like benchmark leaf.
...
- [ ] Run `openspec validate build-coordexp-infras-inference-infra --strict`.
...
- [ ] Create a source-study note under `openspec/changes/build-coordexp-infras-inference-infra/source-studies/`.
...
- [ ] Write probe scripts only under `scripts/probes/coordexp_infras/` if source reading is not enough to prove trace alignment or adapter status behavior.
- [ ] Run probes with sample-limited inputs and save outputs under `temp/coordexp_infras_infer_probes/`.
...
- [ ] Implement inference config loading under `configs/coordexp_infras/infer/`.
...
```bash
openspec status --change build-coordexp-infras-inference-infra --json
openspec validate build-coordexp-infras-inference-infra --strict
python - <<'PY'
from pathlib import Path
assert not Path("src/infer").exists()
assert Path("openspec/changes/build-coordexp-infras-inference-infra/tasks.md").exists()
...
`docs/superpowers/plans/2026-07-02-coordexp-infras-inference-infra-roadmap.md`.
