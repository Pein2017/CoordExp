---
doc_id: docs.architecture.coordexp-infras-decisions
layer: docs
doc_type: architecture-proposal
status: proposal
domain: architecture
summary: Initial resolved design decisions for the coordexp-infras rebuild worktree.
updated: 2026-06-29
---

# coordexp-infras Decisions

This note records resolved discussion outcomes for the `codex/coordexp-infras`
...
coordexp-infras is a from-scratch, production-quality CoordExp training
...
This rule is specifically meant to prevent coordexp-infras from becoming a
...
Transformers is the reusable model substrate. coordexp-infras should freely
...
The model does not own the training loss. coordexp-infras should call the model
...
built-in loss for production training. coordexp-infras disables model-side loss
...
incompatible token types. This is the coordexp-infras equivalent of a
...
to avoid precision issues. In coordexp-infras, the loss path should upcast
...
`ignore_index=-100` and reduce over supervised token sites. coordexp-infras
...
coordexp-infras should not clone MS-Swift's mutable `Template` abstraction as
...
explicit debug/parity profile disables it. coordexp-infras should prefer the
...
`max_seqlen_q/k` as the coordexp-infras Qwen-forward API names.
...
separate "original source order" knob. New coordexp-infras configs should use
...
```text
docs/architecture/proposals/2026-06-27-coordexp-infras/source-studies/dora.md
```
...
For packed supervised training, coordexp-infras should supply explicit
...
height, and width MRoPE ids. The internal coordexp-infras representation may
...
coordexp-infras loss is computed only through `LossRunner`.
...
- never pass `labels` in production training; HF loss is a vanilla CE fallback,
  while coordexp-infras consumes logits and explicitly approved optional outputs
  through `LossRunner`;
...
coordexp-infras should not replace visual embeddings itself in V1. Transformers
Qwen3-VL owns visual feature extraction and the `masked_scatter` replacement of
visual embeddings into language embeddings. coordexp-infras validates
...
DeepSpeed-backed launches, coordexp-infras computes `resolved_grad_accum_steps`
...
floor-style max-step calculation in one helper. coordexp-infras chooses
...
these correspond to `vit`, `aligner`, and `llm`; in coordexp-infras configs we
...
should use the coordexp-infras terms.
...
loaded config and resulting parameter identity. coordexp-infras should detect and
...
output_dir    = avoid in coordexp-infras training config
```
...
it in coordexp-infras terms. MS-Swift configs may be referenced for
...
core coordexp-infras contracts are deliberately non-standard: one packed
...
- HF supports fractional `save_steps`/`eval_steps`, but coordexp-infras should
...
should not be deleted or rewritten before coordexp-infras replacement contracts
...
`docs/architecture/proposals/2026-06-27-coordexp-infras/BLUEPRINT.md`. Each
...
reference, but coordexp-infras should not inherit its broad generic framework
shape.
...
Resolved on 2026-07-02 during coordexp-infras inference/decode planning. Patched
...
Inference configs live under `configs/coordexp_infras/infer/`. Legacy
...
owner for this acceptance path. Given the self-contained coordexp-infras goal,
...
`build-coordexp-infras-inference-infra`. The name is intentionally broader than
...
- `coordexp-infras-infer-config-runtime`
- `coordexp-infras-infer-backend-trace`
- `coordexp-infras-infer-prompt-parsing`
- `coordexp-infras-infer-scoring-artifacts`
- `coordexp-infras-infer-pipeline`
- `coordexp-infras-infer-benchmark-smoke`

Required source studies before coding are: old `/data/CoordExp/src/infer/*` as
reference-only material, current coordexp-infras Qwen training
