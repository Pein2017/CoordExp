...
- **Single-dataset canonical path**: one prepared train/val JSONL pair per run; merge JSONLs offline if you need dataset mixing.
...
**CoordExp training adapter contract**:
- `custom.coord_offset.enabled` defaults to `true`; `coord_offset_adapter` is a core trainable module and is added to `modules_to_save`.
- Base-only training initializes a fresh zero `coord_offset_adapter`.
- Training with `model.adapters` requires the adapter checkpoint to contain `coord_offset_adapter`; missing coord-offset tensors fail fast before training.
- For PEFT LoRA training, the current YAML tuner config owns the active topology. Adapter checkpoints are partial initialization sources: matching tensors load, requested-but-missing tensors stay freshly initialized, extra checkpoint tensors stay inactive, and shape mismatches fail.
