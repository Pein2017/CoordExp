...
- Enabled failure semantics:
  ...
  - `stage2_rollout_correction.correction.rollout_template_family: compact_full` is the new
    ...
    compact-full parser and compact-full rollout-correction target renderer. It must
    not flow through the legacy CoordJSON parser, CoordJSON FN appender, or any
    decode-time compact grammar policy knob.
