...
## Coord-Repel V1 For Compact Teacher Forcing

Coord-repel V1 is an experiment-only Stage-1 compact teacher-forcing auxiliary
loss. It is training/eval forward-pass only and does not change decoding,
attention, KV cache, sampling, or rollout policy.
...
For each eligible `x1` or `y1` coordinate atom, coord-repel compares coordinate
vocabulary probability mass in:

- `B+`: the current selected GT coordinate bin +/- 4;
- `B-`: current wrong top-k coordinate bins outside `B+` intersected with prior
  same-sample, same-role coordinate bands from the selected negative source.
...
where both probabilities are computed over the ordered coordinate-token
vocabulary in fp32. If `B-` is empty, the slot contributes no coord-repel loss.
If `B+` is not supported by the coordinate vocabulary, the run fails fast.

Default `negative_source=last_and_same_desc` means the last previous object plus
all previous objects sharing the normalized duplicate-group key. `all_prior`
exists as a diagnostic/stress mode only and should not be the balanced default.

Coord-repel metrics are emitted under `teacher_forcing/coord_repel/*`, including
raw/weighted loss, active and eligible slot counts, B- empty counts, wrong-top1
intersection rate, B-seen coverage, and weighted-loss-to-main-teacher-forcing
loss ratio. When coord-repel emits metric events, publication to the trainer
metric surface is strict rather than best-effort. These metrics must be present
and inspected before interpreting a smoke or production run; a zero activation
rate means the module is not testing the intended failure mode.

Packed coord-repel configs live under:

```text
configs/stage1/detection_teacher_forcing/coord_repel/
```

The balanced 2B coordexp production candidate is:

```text
configs/stage1/detection_teacher_forcing/coord_repel/prod/packed_len12000_2b_coordexp.yaml
```

It uses COCO `rescale_32_1024_bbox_len12000`, not the legacy max60 dataset,
trains the LLM tower through LoRA while freezing the vision tower and aligner,
and uses static packing with `effective_batch_size: 32`. Production packing keeps
the final underfilled raw pack (`training.packing_drop_last: false`) so the
static raw plan covers the full COCO len12000 dataset before DDP repeat padding.
Before interpreting a production launch, inspect the static plan JSON and require
`raw_missing_sample_count == 0` and `underfill_dropped_sample_count == 0`. On the
8-GPU production launch, the train plan should be aligned to the full optimizer
step boundary as `plan_ws8_drop0_align32.json`.

`compact_box_end` is an independent template ablation axis. Its structural token
row set is:
...
to keep the row-commit ablation separate from the coord-repel loss itself.
