...
NUMERICAL_TRIGGER_RECEIPT = (
    OUTPUT_ROOT
    / "20260830T-image2299-global-target-only-protected-null-distillation-v1"
    / "receipt.json"
)
NUMERICAL_TRIGGER_RECEIPT_SHA256 = "a70a6ffe5a7379b3ad864cd6eb262966a0430106b86b382335e231bc616c819c"
NUMERICAL_TRIGGER_RUNNER_SHA256 = "7f6c69f429e8d24031990caf41978e58e44ee4b293ac215023befad72ddc51e0"
...
def _load_numerical_trigger_receipt() -> dict[str, Any]:
    if (
        not NUMERICAL_TRIGGER_RECEIPT.is_file()
        or _sha256(NUMERICAL_TRIGGER_RECEIPT) != NUMERICAL_TRIGGER_RECEIPT_SHA256
    ):
        raise _hold("immutable global numerical-trigger receipt drifted")
    try:
        receipt = json.loads(NUMERICAL_TRIGGER_RECEIPT.read_text(encoding="utf-8"))
    except (OSError, TypeError, ValueError) as error:
        raise _hold(f"global numerical-trigger receipt is unreadable: {error}") from error
    if not isinstance(receipt, dict):
        raise _hold("global numerical-trigger receipt is not an object")
    return receipt


def _validate_numerical_trigger(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or receipt.get("status") != "global_target_only_infeasible"
        or source.get("sha256") != NUMERICAL_TRIGGER_RUNNER_SHA256
        or not source_path.is_file()
        or _sha256(source_path) != NUMERICAL_TRIGGER_RUNNER_SHA256
        ...
        or solver != {
            "feasible": False,
            "minimum_slack": -1.137212933599585e-06,
            "reason": "dual_solver_did_not_produce_feasible_primal",
            "solver_message": "CONVERGENCE: RELATIVE REDUCTION OF F <= FACTR*EPSMCH",
        }
        or any(key in receipt for key in ("cold_final", "payload", "warm_cold", "warm_final"))
    ):
        raise _hold("global numerical-trigger receipt/source/no-warm binding drifted")
    return {
        "receipt": str(NUMERICAL_TRIGGER_RECEIPT),
        "receipt_sha256": NUMERICAL_TRIGGER_RECEIPT_SHA256,
        "runner_source_snapshot_sha256": NUMERICAL_TRIGGER_RUNNER_SHA256,
        "status_was_misclassified_infeasible": True,
        "solver_reason": "dual_solver_did_not_produce_feasible_primal",
        "minimum_slack": -1.137212933599585e-06,
        "one_solve": True,
...
def _binding_contract() -> dict[str, Any]:
    ...
    numerical_trigger = _validate_numerical_trigger(_load_numerical_trigger_receipt())
    ...
    return inherited | {
        ...
        "numerical_trigger": numerical_trigger,
    }
...
def _binding_receipt() -> dict[str, Any]:
    ...
    return {
        ...
        "numerical_trigger": contract["numerical_trigger"],
...
def _normalized_primal_system(
    constraints: Sequence[Mapping[str, Any]],
    *,
    rank: int,
    row_norms: Mapping[int, float],
    ...
    if not constraints or rank <= 0 or len(selected) != len(selected_set):
        raise ValueError("empty_constraint_basis_or_target_set")
    norms = {int(token): float(value) for token, value in row_norms.items()}
    if (
        set(norms) != selected_set
        or not all(math.isfinite(value) and value > 0.0 for value in norms.values())
    ):
        raise ValueError("target_row_norm_contract")
    ...
    for index, item in enumerate(constraints):
        ...
        if feature.shape != (rank,) or target not in selected_set or target == competitor:
            raise ValueError("malformed_constraint")
        expected_kind = (
            "movable_target_partition" if competitor in selected_set
            else "fixed_non_target_partition_max"
        )
        if trainable != (competitor in selected_set) or kind != expected_kind:
            raise ValueError("competitor_partition_contract")
        start = token_to_row[target] * rank
        matrix[index, start : start + rank] += norms[target] * feature
        if trainable:
            start = token_to_row[competitor] * rank
            matrix[index, start : start + rank] -= norms[competitor] * feature
    ...
    if np.any(impossible) or not np.isfinite(matrix).all() or not np.isfinite(rhs).all():
        raise ValueError("zero_feature_positive_rhs")
    return {
        "matrix": matrix,
        "rhs": rhs,
...
def _legacy_dual_diagnostic(matrix: np.ndarray, rhs: np.ndarray) -> dict[str, Any]:
    diagnostic: dict[str, Any] = {"method": "legacy_l_bfgs_b_dual"}
    try:
        ...
        outcome = minimize(
            ...
            jac=True,
            method="L-BFGS-B",
            bounds=[(0.0, None)] * len(rhs),
            options={"ftol": 1.0e-15, "gtol": 1.0e-10, "maxiter": 20_000, "maxls": 100},
        )
        lam = np.asarray(outcome.x, dtype=np.float64)
        candidate = matrix.T @ lam
        slacks = matrix @ candidate - rhs
        dual_objective = float(rhs @ lam - 0.5 * lam @ gram @ lam)
        diagnostic.update({
            ...
            "normalized_norm_lower_bound": math.sqrt(max(0.0, 2.0 * dual_objective)),
            "primal_candidate_finite": bool(np.isfinite(candidate).all()),
            "primal_candidate_minimum_slack": float(slacks.min(initial=math.inf)),
        })
    except (ArithmeticError, RuntimeError, TypeError, ValueError) as error:
        diagnostic.update({"success": False, "error": f"{type(error).__name__}: {error}"})
    return diagnostic
...
def _solve_target_only_minimum_normalized(
    ...
    *,
    ...
    try:
        system = _normalized_primal_system(
            constraints,
            rank=rank,
            row_norms=row_norms,
            target_token_ids=target_token_ids,
        ...
        return {
            "feasible": False,
            "solver_classification": "numerical_solver_hold",
            "reason": str(error),
        }
    matrix = system["matrix"]
    rhs = system["rhs"]
    selected = system["selected_token_ids"]
    width = int(system["variable_count"])
    primal_system = {
    ...
    legacy = _legacy_dual_diagnostic(matrix, rhs)
    try:
        lp = linprog(
        ...
        return {
            "feasible": False,
            "solver_classification": "numerical_solver_hold",
            "reason": "highs_linprog_exception",
            "primal_system": primal_system,
            "lp_diagnostics": {"method": "highs", "error": f"{type(error).__name__}: {error}"},
            "legacy_dual_diagnostic": legacy,
        }
    lp_x = None if lp.x is None else np.asarray(lp.x, dtype=np.float64)
    lp_finite = lp_x is not None and bool(np.isfinite(lp_x).all())
    lp_minimum_slack = (
        None if not lp_finite else float((matrix @ lp_x - rhs).min(initial=math.inf))
    )
    lp_diagnostics = {
        "method": "highs",
        ...
        "crossover_iterations": int(getattr(lp, "crossover_nit", 0)),
        "primal_finite": lp_finite,
        "minimum_slack": lp_minimum_slack,
    }
    if int(lp.status) == 2 and not lp.success:
        return {
            "feasible": False,
            "solver_classification": "certified_infeasible",
            "reason": "highs_certified_infeasible",
            "constraint_count": len(constraints),
            "variable_count": width,
            "primal_system": primal_system,
    ...
    if (
        not lp.success
        or not lp_finite
        or lp_minimum_slack is None
        or lp_minimum_slack < -2.0e-7
    ):
        return {
            "feasible": False,
            "solver_classification": "numerical_solver_hold",
            "reason": "highs_did_not_produce_feasible_primal",
            "constraint_count": len(constraints),
            "variable_count": width,
            "primal_system": primal_system,
            "lp_diagnostics": lp_diagnostics,
    ...
    try:
        primal = minimize(
            ...
            constraints={
                "type": "ineq",
                "fun": lambda x: matrix @ x - rhs,
                "jac": lambda _x: matrix,
            },
            options={"ftol": 1.0e-12, "maxiter": 20_000, "disp": False},
        )
    except (ArithmeticError, RuntimeError, TypeError, ValueError) as error:
        return {
            "feasible": False,
            "solver_classification": "numerical_solver_hold",
            "reason": "slsqp_exception",
            "primal_system": primal_system,
            "lp_diagnostics": lp_diagnostics,
            "primal_diagnostics": {
                "method": "SLSQP",
    ...
    normalized = np.asarray(primal.x, dtype=np.float64)
    primal_finite = bool(np.isfinite(normalized).all())
    slacks = matrix @ normalized - rhs if primal_finite else np.array([-math.inf])
    minimum_slack = float(slacks.min(initial=math.inf))
    primal_diagnostics = {
        "method": "SLSQP",
        "success": bool(primal.success),
        "status": int(primal.status),
        "message": str(primal.message),
        "iterations": int(primal.nit),
        "primal_finite": primal_finite,
        "minimum_slack": minimum_slack,
        "objective": None if not primal_finite else float(primal.fun),
    }
    if not primal.success or not primal_finite or minimum_slack < -2.0e-7:
        return {
            "feasible": False,
            "solver_classification": "numerical_solver_hold",
            "reason": "slsqp_did_not_produce_feasible_minimum_norm_primal",
            "constraint_count": len(constraints),
            "variable_count": width,
            "primal_system": primal_system,
            "lp_diagnostics": lp_diagnostics,
            "primal_diagnostics": primal_diagnostics,
            "legacy_dual_diagnostic": legacy,
        }
    normalized_rows = normalized.reshape(len(selected), rank)
    return {
        "feasible": True,
        "solver_classification": "feasible_primal_minimum_norm",
        "selected_token_ids": selected,
        ...
        "variable_count": width,
        "primal_system": primal_system,
        "solver_success": bool(primal.success),
        "solver_message": str(primal.message),
        "iterations": int(primal.nit),
        "lp_diagnostics": lp_diagnostics,
        "primal_diagnostics": primal_diagnostics,
        "legacy_dual_diagnostic": legacy,
