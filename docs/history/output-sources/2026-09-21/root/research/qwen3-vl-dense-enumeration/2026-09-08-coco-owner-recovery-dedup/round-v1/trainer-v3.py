    ...
    try:
        ...
        with open_backend_session(frontend.launch) as opened:
            ...
            for update in range(start, args.end_update):
                ...
                require(base._all_true(math.isfinite(local_dedup_grad_sq) and (not replay_receipts or local_dedup_grad_sq > 0), device),
                        "nonfinite or missing eligible replay parameter gradient")
                components = []
                for i in local:
                ...
                if args.check_control_parity:
                    ...
                    parity_ranks = base._dist_values({"rank": rank, **parity})
                    if rank == 0:
                        with (root / "parity.json").open("x") as handle:
                            json.dump(parity_ranks, handle, indent=2)
                        print(json.dumps({"control_parity": parity_ranks}), flush=True)
                    require(all(r["scalar_vs_padded_gradient_relative_l2"] <= 1e-4 and r["component_loss_max_abs"] <= 4e-6 for r in parity_ranks), "lambda0 scalar/padded parity failed")
                ...
                optimizer.step()
                require(base._all_true(all(p._version == frozen[name] for name, p in all_named if not p.requires_grad), device), "frozen parameters changed")
