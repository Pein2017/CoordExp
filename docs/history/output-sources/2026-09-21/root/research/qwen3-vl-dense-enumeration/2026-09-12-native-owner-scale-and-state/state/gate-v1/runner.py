...
def prepare():
    ...
    packet_path = RAW_ROOT / "packet.json"
...
def _run_one_case(qwen, packet, case, directory, receipt):
    ...
    with torch.inference_mode():
        ...
        require(native.stop_reason == "eos" or len(native_suffix) == budget, "native stopped before EOS/full remaining allowance")
        write(
        ...
        for arm in ARMS:
            ...
            stop = "eos" if suffix[-1] == EOS else "length"
            require(stop == "eos" or len(suffix) == budget, "branch stopped before EOS/full remaining allowance")
            if arm == "native_self":
...
def launch(packet_path, out_dir, gate_dir):
    ...
    require(gate_record["packet_sha256"] == file_hash(packet_path), "gate artifact packet differs")
    ...
    require(json.loads((gate_dir / "receipt.json").read_text())["packet_sha256"] == file_hash(packet_path), "gate packet differs")
    out.mkdir(parents=True, exist_ok=False)
...
def consume_panel(packet_path, out_dir, gate_dir):
    ...
    stage_dirs = [gate_dir] + [out / worker["name"] for worker in launch_record["workers"]]
    stage_consumers = []
    all_records = []
    seen_cases = []
