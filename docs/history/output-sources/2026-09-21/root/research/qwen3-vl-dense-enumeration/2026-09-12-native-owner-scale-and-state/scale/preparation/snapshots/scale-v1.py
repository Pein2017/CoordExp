...
SELECTION = ROOT / "preparation" / "selection.json"
ACQUISITION = ROOT / "preparation" / "acquisition.json"
CAP = 3084
EOS = 151645
ROW_START = 151646
ROW_END = 151649
GPUS = (0, 1, 2, 3)
STRATA = ("repeat_or_drift", "missing_or_early_stop", "normal_single_miss")
QUOTAS = {"repeat_or_drift": 25, "missing_or_early_stop": 71, "normal_single_miss": 32}
SELECTION_SALT = "native-owner-scale-raw128-v1"
...
def _history(row: Mapping[str, Any], tokenizer: Any) -> tuple[list[int], str, list[int]]:
...
def candidate_rows(row: Mapping[str, Any], tokenizer: Any) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """Nominate up to two large GT-backed owners geometrically absent from h.
    ...
    """
    h_ids, h_text, omitted_h_tail = _history(row, tokenizer)
    ...
    for gt in row["golden"]["gt"]:
        ...
        (eligible if reason is None else held).append(record)
    eligible.sort(key=lambda x: (-x["c_area_pixels"], x["owner_id"]))
    ...
    for index, item in enumerate(chosen):
        item.update(candidate_index=index, h_token_ids=h_ids, h_token_ids_sha256=digest(h_ids),
                    ...
                    omitted_native_tail_token_count=len(omitted_h_tail),
...
def prepare_selection(output: Path = SELECTION) -> dict[str, Any]:
    ...
    slice_jobs = []
    for name in STRATA:
        slice_jobs.append(next(n["job_id"] for n in nominations if n["stratum"] == name))
    slice_jobs.append(next(n["job_id"] for n in nominations
                           if n["candidate_index"] == 0 and n["job_id"] not in slice_jobs))
    counts = {
    ...
    packet = {
        "schema": "native_owner_scale.selection.v1", "status": "frozen_cpu_no_model_calls",
        ...
        "sources": {"stable50_universe": binding(SOURCE), "producer": binding(Path(__file__).resolve())},
        ...
        "selection_rule": {"salt": SELECTION_SALT, "strata": list(STRATA), "quotas": QUOTAS,
            ...
            "normal_rule": "nonrepeat, nonburden ordinary output with exactly one FN50",
            "within_stratum": "ascending sha256(salt NUL example_id)"},
        "nomination_rule": {"maximum_per_image": 2, "source": "GT-backed literal rows only in v1",
            "history": "maximal literal leading Stable50 complete-row prefix; terminal EOS and any incomplete tail are omitted, never repaired",
...
def validate_selection(path: str | Path = SELECTION) -> dict[str, Any]:
    packet = read(path)
    require(packet["schema"] == "native_owner_scale.selection.v1"
            and packet["status"] == "frozen_cpu_no_model_calls", "selection schema/status")
    for source in packet["sources"].values():
        require(file_hash(source["path"]) == source["sha256"], "selection source changed")
    ...
    require(len(packet["slice_job_ids"]) == len(STRATA) + 1
...
def prepare_acquisition(selection_path: Path = SELECTION, output: Path = ACQUISITION) -> dict[str, Any]:
    ...
    acquisition = {
        "schema": "native_owner_scale.acquisition.v1", "status": "sealed_requires_root_gpu_grant",
        ...
        "modes": {"slice": packet["slice_job_ids"], "full": [j["job_id"] for j in packet["nominations"]]},
...
def validate_acquisition(path: str | Path = ACQUISITION) -> dict[str, Any]:
    packet = read(path)
    require(packet["schema"] == "native_owner_scale.acquisition.v1"
    ...
    require(set(packet["modes"]["slice"]) <= ids and set(packet["modes"]["full"]) == ids,
...
def acquisition_jobs(packet: Mapping[str, Any], mode: str, shard: int) -> list[dict[str, Any]]:
    require(mode in ("slice", "full") and shard in range(len(GPUS)), "acquisition mode/shard")
    selection = validate_selection(packet["sources"]["selection"]["path"])
...
def acquisition_rank(packet_path: Path, output: Path, mode: str, shard: int, physical_gpu: int) -> None:
    ...
    try:
        ...
        require(inspect_dora_adapter_payload(packet["adapter"]["root"], source["model"]["base_model_path"])
                == packet["adapter"], "Stable50 adapter bytes changed")
...
def prepare_training_bank(acquisition_result: Path, reviews_path: Path, output: Path) -> dict[str, Any]:
    ...
    result, reviews = read(acquisition_result), read(reviews_path)
    ...
    for row in result["rows"]:
        ...
        if decision["status"] == "accept":
            ...
            eligible.append(row)
    eligible.sort(key=lambda r: by_job[r["job_id"]]["admission_priority"])
    admitted = eligible[:32]
    if len(admitted) < 16:
    ...
    for row in admitted:
        ...
        common = {"example_id": job["example_id"], "image": job["image"],
            ...
            "case_id": row["job_id"], "candidate_owner_provenance": {"kind": job["source"],
                "owner_id": job["owner_id"], "visual_review": reviews_path.resolve().as_posix()}}
    ...
    runtime = {"world_sizes": [2, 8], "max_rank_seconds": 72000,
        "max_cuda_allocated_bytes": 32 * 1024**3, "max_cuda_reserved_bytes": 32 * 1024**3,
        "max_rss_bytes": 40 * 1024**3, "max_model_forwards_per_rank": 50000,
        "max_image_forwards_per_rank": 50000}
...
def main() -> None:
    ...
    parser.add_argument("command", choices=["prepare-selection", "verify-selection", "prepare-acquisition",
        "verify-acquisition", "acquire-rank", "acquire", "reduce", "prepare-training"])
