...
def main():
 ...
 receipt=read(R/'runtime/Y998/receipt.json');assert receipt['status']=='candidate_complete';assert receipt['panel']==bind(R/'panel.json') and receipt['raw']==bind(R/'runtime/Y998/raw.json');assert receipt['counts']['model_forwards']<=5000
 ...
 raw=read(R/'runtime/Y998/raw.json');assert [i for i,(a,b) in enumerate(zip(raw['supplied137_token_ids'],prior['cells']['C00']['tokens'])) if a!=b]==[7];assert raw['supplied137_token_ids'][7]==tok.convert_tokens_to_ids('<|coord_998|>')==152668
 cell=consumer.consume(p,raw,tok);controls=read(OLD/'result.json')['cells'];coverage=set(cell['joint_match_excluding_supplied137']['covered_owner_ids']);new=set(cell['new_covered_owner_ids']);reference16=set(controls['C01']['new_covered_owner_ids']);assert len(reference16)==16
 comparisons={}
