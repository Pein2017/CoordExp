...
def axis_term(logits, boxes, coords):
    # A nonempty loss keeps its value under no_grad finite-difference evaluation.
    if not boxes:
        return logits.sum()*0
    return raw_axis_validity_hinge(logits, boxes, coordinate_token_ids=coords,
                                   coordinate_bin_values=list(range(1000)), margin=1/999)
    ...
    try:
        for condition in ['tied-shared','tied-split','untied']:
            ...
            def terms(route):
                ...
                axis=axis_term(logits,route['trusted_boxes'],coords)
