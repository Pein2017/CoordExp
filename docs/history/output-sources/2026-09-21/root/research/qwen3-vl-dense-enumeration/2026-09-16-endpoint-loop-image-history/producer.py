"""Fixed image x literal-history contrast; full target prefix supplied at native processor."""
...
ROOT=Path('/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-16-endpoint-loop-image-history')
...
def execute(cell):
 ...
 panel['cases']=panel['original_cases'] if cell=='I00' else panel['donor_cases']
 if cell!='I00':
 ...
 try:
  ...
  for b,item in enumerate(result):
   ...
   if b!=1 or cell=='I00':assert ids==group[b]['generated_token_ids'] and item.stop_reason==group[b]['decode_stop_reason']
  ...
  receipt.update(status='candidate_complete',full_original_batch_identity=cell=='I00',companion_identity=True,common136_identity=True,common136_natural=cell=='I00',history_mask={'batch_position':1,'start':0,'end_exclusive':1233,'free_logits_unrestricted':True},supplied137_identity=True,free_tokens=len(free),stop=target['stop'],target_total_tokens=len(target['token_ids']),elapsed_seconds=time.monotonic()-start,peak_allocated_bytes=torch.cuda.max_memory_allocated(),peak_reserved_bytes=torch.cuda.max_memory_reserved())
  write(out/'raw.json',{'cell':cell,'rows':rows,'free_token_ids':free,'common136_token_ids':panel['common_history_ids'], 'target_image_id':group[1]['image_id'], 'history_supply_interval':[0,1233],'supplied137_token_ids':panel['cells'][cell]['tokens'],'free_text':tok.decode(free,skip_special_tokens=False,clean_up_tokenization_spaces=False),'policy':'native greedy freely after completed row137; no next opener forced; EOS available'})
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('cell',choices=['I00','D00','D10']);execute(p.parse_args().cell)
