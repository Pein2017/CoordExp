...
class Runtime:
    ...
    def _source(
        ...
        for key, path in (
            ...
            if expected is not None and _binding(path) != expected[key]:
        ...
        if receipt.get("condition") != f"{self.args.model}-original":
        ...
        if receipt.get("raw") != _binding(raw_path):
            raise ValueError("source raw binding differs from its receipt")
        if receipt.get("trace") != _binding(trace_path):
            raise ValueError("source trace binding differs from its receipt")
        if expected is None and receipt.get("panel") != _binding(self.args.panel):
            raise ValueError("selected current source is bound to another panel")
    ...
    def capture_source(self, selection_path: Path, manifest_path: Path) -> None:
        ...
        if self.args.boundary_id:
            boundaries = [
                item for item in boundaries if item["id"] == self.args.boundary_id
            ]
        if not boundaries:
