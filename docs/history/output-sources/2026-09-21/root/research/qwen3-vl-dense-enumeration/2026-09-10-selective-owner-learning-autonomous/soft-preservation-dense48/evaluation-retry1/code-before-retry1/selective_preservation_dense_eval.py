...
def prepare():
    require(not OUTPUT.exists(),'occupied dense evaluation output')
...
def validate_receipt(receipt,packet):
    ...
    require(Path(receipt['adapter']['root']).resolve()==(ARM_ROOT/'training/adapter').resolve(),'wrong dense checkpoint path')
    require(file_hash(ARM_ROOT/'training/inputs.json')==receipt['inputs_sha256'],'dense training packet changed')
