...
from probes.dora_owner_learning.selective_preservation_dense import (
    CEILINGS, NUMERICAL_ADMISSION, compensate_for_ddp, file_hash, finalize, global_image_loss,
    gradient_relative_l2, prepare, publish, rank_items, vector_admission,
)
...
def terminal_fixture(tmp_path,*,exit_code=0,bad_rank=None):
    ...
    publish(tmp_path/'serial.json',dict(mechanical_only=True))
    publish(tmp_path/'gradient.json',dict(status='passed',raw_relative_l2=0.,clipped_relative_l2=0.))
    publish(tmp_path/'smoke.json',dict(status='passed',updates=[1,2],step1_top1_rank_equal=True,
        step1_full_vocab_max_abs_errors={'a':5.14984130859375e-5,'b':3.4332275390625e-5},step2_max_KL_by_rank={'0':.01}))
    ref=lambda n:dict(path=str(tmp_path/n),sha256=file_hash(tmp_path/n))
    publish(tmp_path/'provisional.json',dict(status='unsealed',adapter=identity,source_embedding=identity,
        numerical_admission=NUMERICAL_ADMISSION,serial_control=ref('serial.json'),gradient_parity=ref('gradient.json'),two_step=ref('smoke.json')))
...
def test_raw_gradient_gate_detects_scale_hidden_by_global_clipping():
    reference=torch.tensor([30.,40.,-1.],dtype=torch.float32)
    near=reference+torch.tensor([1e-7,-1e-7,0.])
    assert gradient_relative_l2(near,reference)<=1e-5
    wrong=reference/8
    assert gradient_relative_l2(wrong,reference)==pytest.approx(7/8)
    assert gradient_relative_l2(clipped(wrong),clipped(reference))<1e-5  # clipping hides uniform wrong scale
    with pytest.raises(ValueError):gradient_relative_l2(reference[:-1],reference)


def test_observed_serial_DDP_vector_difference_passes_only_revised_gate():
    import numpy as np
    score=dict(top1_id=2,rank_min=4,rank_max=4)
    for error in (5.14984130859375e-5,3.4332275390625e-5):
        observed=np.array([error,0.,0.],dtype=np.float32)
        result=vector_admission(observed,np.zeros(3,dtype=np.float32),score,score)
        assert result['passed'] and result['max_abs_error']>1e-5
    assert not vector_admission(np.array([2e-4],dtype=np.float32),np.zeros(1,dtype=np.float32),score,score)['passed']
    assert not vector_admission(np.zeros(1,dtype=np.float32),np.zeros(1,dtype=np.float32),{**score,'top1_id':3},score)['passed']


def test_actual_saved_serial_DDP_vectors_satisfy_frozen_revised_admission():
    # The actual retained failure is the fixture; this checks the new bound without another model run.
    import numpy as np
    from probes.dora_owner_learning.selective_preservation_dense import OUTPUT,SERIAL_CONTROL
    from src.artifacts import load_canonical_json
    old_scores=load_canonical_json(OUTPUT/'scores'/'step-01.json')
    serial_scores=load_canonical_json(SERIAL_CONTROL/'scores'/'step-01.json')
    for cid,score in old_scores.items():
        image=str(int(cid.split(':')[0].split('_')[-1]))
        a=np.load(OUTPUT/'scores'/f'step-01-{image}.npy',allow_pickle=False)
        b=np.load(SERIAL_CONTROL/'scores'/f'step-01-{image}.npy',allow_pickle=False)
        assert vector_admission(a,b,score,serial_scores[cid])['passed']
