...
FROZEN_TOTAL = 24
ACCEPTED_A = (151646, 22592, 151647, 151648, 151670, 152452, 151763, 152572, 151649)
...
def _event_counts(metric):
    ...
    return {
        ...
        "semantic_malformed": semantic if "semantic_malformed" in (metric or {}) else malformed_drops or int(b.get("malformed") or 0),
...
def _rows(tokens, base_offset):
    # Literal valid-wrapper spans, including invalid geometry; never fixed token chunks.
    out=[];tokens=list(tokens or [])
    for i,t in enumerate(tokens):
        if t!=151646:continue
        j=i+1
        while j<len(tokens) and tokens[j] not in (151646,151647,151648,151649,151645):j+=1
        if j==i+1 or j+6>=len(tokens):continue
        if tokens[j:j+2]!=[151647,151648] or tokens[j+6]!=151649:continue
        if not all(151670<=v<=152669 for v in tokens[j+2:j+6]):continue
        out.append({'offset':base_offset+i,'token_ids':tokens[i:j+7]})
    return out
...
def _runs(rows, exclude_a=True):
    ...
    while i < len(rows):
        j = i + 1
        while j < len(rows) and rows[j]["token_ids"] == rows[i]["token_ids"] and rows[j]["offset"]==rows[j-1]["offset"]+len(rows[j-1]["token_ids"]):
            j += 1
...
def _trajectory(key, cell, exclusions):
    ...
    return {
        ...
        "symmetric_exclusion": sorted(excl),
        "per_trajectory_supplied": exclusion,
        "symmetric_known_accounting": cell.get("known_accounting"),
...
def summarize(doc):
    ...
    def flag(t, name):
        ...
        if name == "geometry_invalid": return bool(t["post_release"]["events"]["geometry_invalid"])
        if name == "malformed": return bool(t["post_release"]["events"]["semantic_malformed"])
        if name == "literal_template_escape": return not bool(t["exact_post_release"]["accepted_a_returns"])
        ...
        rates[str(temp)] = {
            ...
            "descriptors": {name: _wilson(sum(flag(t, name) for t in group), len(group))
                            for name in ("accepted_a_return", "alternate_exact_run_ge3_excluding_a",
                                         "known_gain_candidate", "winner_change", "eos", "cap", "geometry_invalid", "malformed", "literal_template_escape")},
        }
    return {
...
def _selfcheck():
    ...
    assert abs(_wilson(0,8)["wilson95"][1]-0.32440756488388023)<1e-12
    assert _wilson(8,8)["wilson95"][1]==1
    ...
    assert not _runs(_rows(list(ACCEPTED_A)*2+[123]+list(ACCEPTED_A),0),exclude_a=False)
    assert ACCEPTED_A[4:8]==(151670,152452,151763,152572)
