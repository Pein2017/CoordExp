...
OUT = UNIT / "final" / "technical-gate-v2"
PRIOR_OUT = UNIT / "final" / "technical-gate-v1"
...
def make_snapshot(c_reuse: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_id": "technical-gate-v1-corrected-readback",
        ...
        "prior_attempt": {"receipt": binding(PRIOR_OUT / "gate-receipt.json"), "comparison": binding(PRIOR_OUT / "comparison-tied.json")},
    }
...
def compare_route(model_key: str, device: str, c_reuse: dict[str, Any]) -> dict[str, Any]:
    ...
    def position_delta(meta_a: dict[str, Any], meta_b: dict[str, Any], source_index: int) -> int | None:
        ...
        if pos_a is None or pos_b is None:
            return None
        pa_all = torch.tensor(pos_a)
        pb_all = torch.tensor(pos_b)
        source_mask = torch.tensor(mask_a[source_index], dtype=torch.bool)
        target_mask = torch.tensor(mask_b[0], dtype=torch.bool)
        pa = pa_all[:, source_index, source_mask]
        pb = pb_all[:, 0, target_mask]
        if pa.shape != pb.shape:
            return -1
        return int(torch.max(torch.abs(pa.to(torch.int64) - pb.to(torch.int64))).item())
    pos_boundary = position_delta(source_boundary_meta, target_boundary_meta, target_index)
    pos_x1 = position_delta(source_x1_meta, target_x1_meta, target_index)
    ...
    def stage(name: str, source_logits: torch.Tensor, target_logits: torch.Tensor, offset: int) -> dict[str, Any]:
        src = source_logits[target_index, -1]
...
def main() -> None:
    ...
    receipt = {
        ...
        "prior_attempt": {"receipt": binding(PRIOR_OUT / "gate-receipt.json"), "comparison": binding(PRIOR_OUT / "comparison-tied.json"), "prior_model_forwards": 4, "aggregate_model_forwards": 8},
