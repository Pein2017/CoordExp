...
The [fresh128 fixed norm-policy result](experiments/2026-09-17-readout-norm-fresh128/results.md) is independently lead-accepted and [closed](experiments/2026-09-17-readout-norm-fresh128/state.json): known geometric matches578→592 (G30/L16), with net+13 concentrated in9 structurally unhealthy images and+1 in119 healthy images; invalid476→5,caps2→0. The image-bootstrap interval includes zero, and physical benefit remains unresolved after the user stopped exhaustive viewing in favor of selective review. Full paired/shadow artifacts are retained; no training or global-policy promotion. The old two rescue routes also require many later action changes to reproduce their exact recorded sequences; this does not establish their necessity for equivalent physical coverage. The worker separately continues the user's human-review visualization export, with class-aware accounting kept distinct from the frozen class-agnostic result.

The [natural output-norm policy result](experiments/2026-09-16-endpoint-loop-natural-readout-norm/results.md) is independently lead-accepted and [closed](experiments/2026-09-16-endpoint-loop-natural-readout-norm/state.json). Empty-prefix R16 on417044/477415 gains12/16 known owners, retains every incumbent and reaches clean EOS;351017 still loops and healthy7116 retains4/6 with clean EOS. The output-only policy leaves learned/input parameters fixed. Successful first forks are interior coordinates, so broad readout shaping remains an alternative to an endpoint-specific defect. This overnight diagnostic series is stopped: no live jobs, normalization variants, training or deployment. Population efficacy, physical precision and training-origin mechanisms remain unresolved.

The [image x fixed-history result](experiments/2026-09-16-endpoint-loop-image-history/results.md) is independently lead-accepted and [closed](experiments/2026-09-16-endpoint-loop-image-history/state.json): donor7116 changes both trajectories but yields0/6 known free owners under either history. C00 stays invalid; C10 switches to valid-box repetition. This establishes visual-input sensitivity, with long off-policy-history mismatch unresolved. The natural-empty-prefix successor above closes the output-norm test without that foreign-history mismatch; no donor/K/normalization-strength sweep follows.

The [endpoint readout/state result](experiments/2026-09-16-endpoint-loop-readout-state/results.md) is independently lead-accepted and [closed](experiments/2026-09-16-endpoint-loop-readout-state/state.json): equalizing output-row norms changes17 of69 endpoint wins, while52 survive; the same readout supports interior escape decisions. These144 conditional slots establish a norm contribution and state-dependent alignment, not a training-origin cause or repair. The image/history successor above closes that maintenance contrast; no K transplant or coordinate/donor sweep follows.

The [paired one-bin substitution series](experiments/2026-09-16-corner-loop-y1-one-bin-control/results.md) is independently lead-accepted and [closed](experiments/2026-09-16-corner-loop-y1-one-bin-control/state.json): both y1 and y2 999-to998 edits reproduce all1851 C00 free tokens, with no new owners despite changed logits. Together with successful category/full-extent controls, this establishes selective history sensitivity and excludes a guarantee from either arbitrary edits or supplied-box validity alone. The user subsequently authorized overnight first-principles diagnosis of recurring endpoint loops across checkpoints. The readout/state successor above handles the first mechanism discriminator; no further local coordinate sweep follows.

The [one-bin y2 control](experiments/2026-09-16-corner-loop-one-bin-control/results.md) is independently lead-accepted: Y998 returns the exact1851-token C00 free suffix, coverage2/27 with no new owners and cap, despite changed post-intervention logits. Its [state](experiments/2026-09-16-corner-loop-one-bin-control/state.json) is closed/accepted. The subsequent paired y1 control above closes this local substitution series; no coordinate sweep follows.

The [477415/R16 bridge factorial](experiments/2026-09-16-corner-loop-bridge-factorial/results.md) is independently lead-accepted: either single edit recovers the same16 free owners, but all interventions relapse into all999 endpoint loops and cap. Its [state](experiments/2026-09-16-corner-loop-bridge-factorial/state.json) is closed/accepted. The subsequent y2 control above tests a specific still-invalid edit without extending this four-cell result.

The [corner-loop phase1 diagnosis](experiments/2026-09-16-corner-loop-mechanism/results.md) is lead-accepted and closed. Across nine current-checkpoint/image cells,363 saved slot decisions show no cache/full argmax flips. R16 retains the selected477415 exit decisions under P's supplied exit history, which already includes the preceding person/full-canvas row. This establishes conditional availability, not natural access, full-suffix recovery or an embedding-origin cause. The subsequent bridge factorial above tests recent-row sufficiency. [State](experiments/2026-09-16-corner-loop-mechanism/state.json) preserves the phase1 boundary.
