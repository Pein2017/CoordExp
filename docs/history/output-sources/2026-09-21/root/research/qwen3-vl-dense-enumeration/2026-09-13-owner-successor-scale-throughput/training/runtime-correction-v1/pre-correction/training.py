...
def _mine_one(
    ...
    rows = complete_rows(token_ids[:-1] if token_ids and token_ids[-1] == old_training.old.EOS else token_ids)
    receipt: dict[str, Any] = {
        ...
        "first_complete_row": rows[0] if rows else None, "strict_repeat": False,
    }
    if not rows:
        return None, receipt
    row_ids = rows[0]
    ...
    return {"entry": synthetic, **divergence}, receipt
...
def calibrate(
    *, input_path: Path, output_root: Path, sealed_output: Path
    ...
    if common_norm == 0:
        ...
        return result
    model.zero_grad(set_to_none=True)
    events, refresh = _refresh_events(packet, banks, rank=0, world=1)
    if not events:
    ...
    try:
        ...
        for step in packet["schedule"][:updates]:
            if step["refresh_before_update"]:
                refresh_events, observed = _refresh_events(packet, banks, rank=rank, world=world)
            ...
            update_receipts.append({"update": step["update"], "components": components,
        ...
        old_training.publish(run / "terminal.json", {"rank": rank, "status": status, "error": error,
                                                       "arm": arm, "updates": updates,
                                                       "work_plan": work_plan,
