...
def repeat_histories(ids, frozen, tokenizer):
    ...
    if not ids or ids[0] != scale.ROW_START or scale.ROW_END not in ids:
        return []
    prefix, _ = scale._maximal_complete_prefix(list(ids[:-1] if ids and ids[-1] == e.EOS else ids))
...
def consume(packet_path, output):
    ...
    expected=set(packet['conditional_job_ids']) if packet.get('conditional_job_ids') is not None else set(by_job)
