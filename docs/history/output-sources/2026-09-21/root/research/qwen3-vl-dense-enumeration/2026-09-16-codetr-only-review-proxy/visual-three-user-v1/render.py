...
def main():
    ...
    manifest={'purpose':'User-requested three historical errors, source and exact retained detector context',
              ...
              'not_ground_truth':'No corrected GT box is drawn; displayed user rulings supersede historical provisional extent judgments.',
              'sources':[{'path':str(p),'sha256':sha(p)} for p in [candidate_path,decision_path,prediction_path,config_path]],'cases':[]}
    historical={'237954:p11':'accepted; accurate dog box','307814:p0':'accepted; accurate chair box','134520:p0':'wrong; left person hand region'}
    for case in IDS:
        ...
        draw.text((22,718),f"User ruling: {historical[case]}. Neither colored box is GT. Display resize only; no regeneration.",font=font(18),fill='#c8d1df')
        ...
        manifest['cases'].append({'case_id':case,'image_path':c['image_path'],'crop_path':v['image_path'],
                                  ...
                                  'agreement_iou':d['candidate_iou'],'user_ruling':historical[case],
