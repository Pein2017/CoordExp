...
MANIFEST=ROOT.parent/'first-fit-preparation-v1/manifest.json'
...
def execute(name,args,gpu,seconds):
    ...
    return name
publish('producer.json',{'pid':os.getpid(),'cwd':os.getcwd(),'manifest':str(MANIFEST),'scope':'all11 first fit from N16 plus three cold native greedy readbacks'})
receipt={'schema':'training_set_completion.first_fit_phase.v1','phase':'training','status':'running'}
try:
    ...
    assert terminal['status']=='completed' and terminal['updates']==32 and terminal['model_forwards']==352
    receipt['phase']='cold_native_readbacks'
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
        jobs=[pool.submit(execute,'readback-step-'+str(step),['readback','--manifest',str(MANIFEST),'--adapter',str(ROOT/'training/checkpoints'/('step-'+str(step).zfill(5))/'adapter'),'--output',str(ROOT/('readback-step-'+str(step)+'.json')),'--device','cuda:0'],gpu,900) for step,gpu in [(8,1),(16,2),(32,3)]]
        for job in concurrent.futures.as_completed(jobs):job.result()
    receipt.update(status='completed_unscored',phase='terminal',readback_steps=[8,16,32])
    ...
    print('GPU_FIRST_FIT_SUCCESS' if receipt['status']=='completed_unscored' else 'GPU_FIRST_FIT_FAILED',flush=True)
