...
B = HERE.parent
MISLOCATED_CANDIDATE_ROOT = B / "B/fourth-fit-eval-preparation-v1"
...
ANNOTATION_TARGET = B / "target-owners-complete-v5.json"
...
def exact_reuse(row: Mapping[str, Any], index: Mapping[tuple[Any, ...], list[dict[str, Any]]], content_hash: str,
    ...
    if row.get("status") != "parsed_valid":
        return {"state": "parser_known_invalid_output", "source_candidates": [],
                "parser_known_output_status": "invalid_output"}
...
def render_packet(*, step: int, scored_row: Mapping[str, Any], records: Mapping[int, Mapping[str, Any]], target_by_image: Mapping[int, list[Mapping[str, Any]]],
                  annotation_target_by_image: Mapping[int, list[Mapping[str, Any]]],
    ...
    annotation_refs = []
    for ref in annotation_target_by_image[image_id]:
        item = dict(ref)
        item["bbox_pixel_xyxy"] = rr.bins_to_pixels(item["reference_coord_bins_1000"], image.width, image.height)
        annotation_refs.append(item)
    annotation_overlay = packet_dir / "annotation-catalog-v5-overlay.png"
    annotation_list = packet_dir / "annotation-catalog-v5-references.json"
    rr.render_target_overlay(image, annotation_refs, annotation_overlay, image_id)
    write(annotation_list, [{"owner_id": x["owner_id"], "role": x.get("role"), "category": x.get("category"),
                             "reference_coord_bins_1000": x["reference_coord_bins_1000"], "bbox_pixel_xyxy": x["bbox_pixel_xyxy"]} for x in annotation_refs])
    ...
    packet = {"schema": "training_set_completion.fourth_fit_eval_packet.v1", "status": "review_ready_no_physical_conclusion", "image_id": image_id,
              ...
              "source": {"scored_selector": binding(source_selector), "acquisition_manifest": binding(ACQUISITION), "target_catalog_v4": binding(TARGET),
                         "annotation_catalog_v5": binding(ANNOTATION_TARGET),
              ...
              "annotation_catalog_v5_references": json.loads(annotation_list.read_text()),
              "full_raw_rows": raw,
              "rendered": {"original_image": {"path": str(original.resolve()), "sha256": sha(original)},
                           ...
                           "annotation_catalog_v5_overlay": {"path": str(annotation_overlay.resolve()), "sha256": sha(annotation_overlay)},
                           "annotation_catalog_v5_reference_list": binding(annotation_list),
...
def build_step(step: int, scored: Mapping[str, Any], *, records: Mapping[int, Mapping[str, Any]], target_by_image: Mapping[int, list[Mapping[str, Any]]],
    ...
    packets = [render_packet(step=step, scored_row=rows[image_id], records=records, target_by_image=target_by_image, annotation_target_by_image=annotation_target_by_image,
...
def validate_existing(*, target: Mapping[str, Any], annotation_target: Mapping[str, Any]) -> dict[str, Any]:
    """Structural validation only; it makes no image or owner judgment."""
    expected_v4: dict[int, set[str]] = defaultdict(set)
    expected_v5: dict[int, set[str]] = defaultdict(set)
    for ref in target["records"]:
        expected_v4[int(ref["image_id"])].add(str(ref["owner_id"]))
    for ref in annotation_target["records"]:
        expected_v5[int(ref["image_id"])].add(str(ref["owner_id"]))
    summary = {"schema": "training_set_completion.fourth_fit_eval_packet_validation.v1", "validated": True, "steps": []}
    for step, root in ((64, HERE / "parent-step-00064"), (192, HERE / "fourth-step-00192"), (256, HERE / "fourth-step-00256")):
        packets = sorted(root.glob("image-*/packet.json"))
        require(len(packets) == len(COHORT), f"packet count at {step}")
        counts = Counter()
        for path in packets:
            packet = json.loads(path.read_text())
            image_id = int(packet["image_id"])
            require(packet["checkpoint_step"] == step, f"packet step mismatch: {path}")
            require({str(x["owner_id"]) for x in packet["target_catalog_references"]} == expected_v4[image_id], f"v4 references: {path}")
            require({str(x["owner_id"]) for x in packet["annotation_catalog_v5_references"]} == expected_v5[image_id], f"v5 references: {path}")
            raw, rendered, groups = packet["full_raw_rows"], packet["rendered"]["raw_rows"], packet["rendered"]["visual_groups"]
            ids = {str(x["prediction_id"]) for x in raw}
            require(ids == {str(x["prediction_id"]) for x in rendered}, f"raw/rendered IDs: {path}")
            require(ids == {member for group in groups for member in group["member_prediction_ids"]}, f"group IDs: {path}")
            for group in groups:
                require(Path(group["tight_crop"]["path"]).is_file() and Path(group["context_crop"]["path"]).is_file(), f"crop missing: {path}")
            require(Path(packet["rendered"]["annotation_catalog_v5_overlay"]["path"]).is_file(), f"v5 overlay missing: {path}")
            decisions = [json.loads(line) for line in (path.parent / "flat-decisions.jsonl").read_text().splitlines() if line]
            notes = [json.loads(line) for line in (path.parent / "reviewer-notes.jsonl").read_text().splitlines() if line]
            require(len(decisions) == len(raw) == len(notes), f"flat decision/note count: {path}")
            require({str(x["prediction_id"]) for x in decisions} == ids, f"decision IDs: {path}")
            require(all(str(x["review_state"]).startswith("pending_human_review") for x in decisions), f"non-pending draft: {path}")
            for decision in decisions:
                if decision["raw_parser_state"][0] != "parsed_valid":
                    require(decision["exact_reuse"]["state"] == "parser_known_invalid_output", f"dropped-row state: {path}")
            counts.update({"raw_rows": len(raw), "parser_known_invalid_output": sum(x["raw_parser_state"][0] != "parsed_valid" for x in decisions),
                           "exact_reuse_candidates": sum(x["exact_reuse"]["state"] == "exact_literal_reuse_candidate" for x in decisions)})
        summary["steps"].append({"checkpoint_step": step, "image_count": len(packets), **counts})
    return summary
...
def main() -> None:
    ...
    parser.add_argument("--validate", action="store_true", help="Validate canonical packets without consuming any new saved rows.")
    args = parser.parse_args()
    require(HERE.exists(), "output root absent")
    acquisition, target, annotation_target, parent_ledger = (json.loads(path.read_text()) for path in (ACQUISITION, TARGET, ANNOTATION_TARGET, PARENT_LEDGER))
    ...
    require(annotation_target["status"] == "lead-accepted" and annotation_target["atomic_target_count"] == 246, "current v5 annotation catalog changed")
    require(sha(ANNOTATION_TARGET) == "2e5e32cfbcf1632ef3a95744ef9f8b1e3fc8433ff691b3b389417d674bb7802d", "v5 annotation catalog hash changed")
    if args.validate:
        print(json.dumps(validate_existing(target=target, annotation_target=annotation_target), indent=2))
        return
    ...
    annotation_target_by_image: dict[int, list[Mapping[str, Any]]] = {image_id: [] for image_id in COHORT}
    for ref in annotation_target["records"]:
        annotation_target_by_image[int(ref["image_id"])].append(ref)
    ...
    parent_root = HERE / "parent-step-00064"
    results: list[dict[str, Any]] = []
    if not parent_root.exists():
        results.append(build_step(PARENT_STEP, parent_scored, records=records, target_by_image=target_by_image, annotation_target_by_image=annotation_target_by_image, label_index=label_index, rr=rr,
    ...
    for step in FOURTH_STEPS:
        ...
        if not packet_root.exists():
            ...
            results.append(build_step(step, scored, records=records, target_by_image=target_by_image, annotation_target_by_image=annotation_target_by_image, label_index=label_index, rr=rr,
    ...
    pending_receipt = {"schema": "training_set_completion.fourth_fit_final_eval_pending.v1", "status": "pending_final_saved_rows" if pending else "all_saved_rows_consumed",
                       ...
                       "superseded_mislocated_candidate": {"path": str(MISLOCATED_CANDIDATE_ROOT.resolve()),
                                                           "producer_snapshot": binding(MISLOCATED_CANDIDATE_ROOT / "build_final_eval_packets.py")},
