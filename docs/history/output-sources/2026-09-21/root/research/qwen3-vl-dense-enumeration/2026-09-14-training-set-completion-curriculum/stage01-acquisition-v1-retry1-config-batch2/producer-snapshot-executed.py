...
def validate_manifest(path: str | Path = ROOT / "manifest.json", *, verify_large_trees: bool = False) -> dict[str, Any]:
    ...
    for source in manifest["sources"].values():
        require(binding(source["path"]) == source, f"bound source changed: {source['path']}")
...
def worker(*, manifest_path: Path, output: Path, phase: str, shard: int, world_size: int, physical_gpu: int) -> None:
    ...
    try:
        ...
        for job in jobs:
            ...
            case = _candidate_materialized_case(record["case"], manifest["model"]["config"])
...
def readback(*, manifest_path: Path, output: Path, phase: str) -> dict[str, Any]:
    ...
    receipt = {"schema": f"{SCHEMA}.{phase}_readback", "status": "passed", "manifest": binding(manifest_path),
               ...
               "geometry_invalid": sum(len(row["parsed"].get("invalid_predictions", [])) for row in rows),
               "cold_token_text_and_parser_replay": True}
    name = "smoke-readback.json" if phase == "smoke" else "result.json"
    publish(output / name, receipt)
    if phase == "final":
        rows_path = output / "rows.jsonl"
        require(not rows_path.exists(), "merged rows collision")
