...
def consume(panel, runtime_root):
 ...
 for case in panel['cases']:
  ...
  for arm in case['arms']:
   ...
   allowed=ROOT/('producer.py' if arm=='native' else 'producer-v2.py')
   assert rec['producer']==score.bind(allowed)
   ...
   full=view(ids,row['stop']);free=view(ids[end:],row['stop']);free['eos_only_control_text']=(ids[end:]==[151645]);free['semantic_malformed_count']=0 if free['eos_only_control_text'] else free['burden']['malformed'];history=view(ids[:start],'supplied_history');supplied=view(ids[start:end],'supplied_row')
  ...
  results[iid]=arms
 return dict(panel=score.bind(ROOT/'panel.json'),status='candidate',consumer=score.bind(__file__),accepted_parser_scorer=score.bind(SCORE),images=results,physical_review='separate sidecar; known losses are not physical disappearance',cost=dict(batch_executions=len(receipts),model_forwards=sum(x.get('counts',{}).get('model_forwards',x.get('model_forwards',0)) for x in receipts),allocated_gpu_seconds=sum(x.get('elapsed_seconds',0) for x in receipts)))
