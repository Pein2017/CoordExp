...
if __package__ in (None, ""):
    # Preparation copies live below outputs/, so their file parents are not a
    # checkout.  Direct invocation is supported from the selected checkout.
    cwd = Path.cwd()
    if (cwd / "src").is_dir() and (cwd / "probes").is_dir():
...
def _edit_target_scores(
    scores: torch.Tensor, *, target_position: int, supplied_token: int
) -> torch.Tensor:
    """Force one row and prove the other batch rows remain byte-identical."""
    if scores.ndim != 2 or target_position < 0 or target_position >= scores.shape[0]:
        raise ValueError("target score row is outside the batch")
    if supplied_token < 0 or supplied_token >= scores.shape[1]:
        raise ValueError("supplied token is outside the vocabulary")
    transformed = scores.clone()
    transformed[target_position, :] = -torch.inf
    transformed[target_position, supplied_token] = 0
    companion = torch.ones(scores.shape[0], dtype=torch.bool, device=scores.device)
    companion[target_position] = False
    if not torch.equal(scores[companion], transformed[companion]):
...
def _branch_scores(
    ...
    *,
    arm: str,
    target_position: int,
    supplied_token: int | None,
) -> torch.Tensor:
    """Shared native/no-op versus one-target intervention seam."""
    if arm == "native":
        return scores
    if supplied_token is None:
        raise ValueError("intervention requires a supplied token")
    return _edit_target_scores(
        scores, target_position=target_position, supplied_token=supplied_token
    )
...
def _run(
    *, panel_path: Path, image_id: int, arm: str, output_root: Path = OUTPUT_ROOT
    ...
    try:
        ...
        class RowBranchProcessor(LogitsProcessor):
            def __call__(self, ids: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
                ...
                for index, image in enumerate(image_ids):
                    ...
                    supplied_token: int | None = (supplied[offset - start] if index == target_position and start <= offset < end else None)
                    changed = False
                    if index == target_position and arm != "native" and start <= offset < end:
                        supplied_token = supplied[offset - start]
                        edited_scores = _branch_scores(
                            scores,
                            arm=arm,
                            target_position=target_position,
        ...
        # EOS is a valid scientific outcome; diagnostics beyond it are absent,
        # not a technical failure or a reason to extend the free horizon.
        missing_reached = [offset for offset in missing if offset < len(target["token_ids"])]
        if missing_reached:
            raise AssertionError(f"reached sparse capture offsets missing: {missing_reached}")
        receipt["sparse_offsets_not_reached_before_stop"] = missing
...
def cpu_check(output: Path) -> None:
    """Exercise the real score transform and its row isolation contract."""
    ...
    native = _branch_scores(
        scores, arm="native", target_position=1, supplied_token=None
    )
    assert native is scores
    forced = _branch_scores(
        scores, arm="same", target_position=1, supplied_token=2
    )
    assert torch.equal(forced[0], scores[0])
    assert not torch.equal(forced[1], scores[1])
    assert int(torch.argmax(forced[1])) == 2
    assert torch.isneginf(forced[1, 0]) and forced[1, 2] == 0
    # Mutation sensitivity: changing a companion input must be visible in the
    # companion output, while the target edit remains the declared transform.
    mutated = scores.clone()
    mutated[0, 0] = 99
    mutated_forced = _branch_scores(
        mutated, arm="same", target_position=1, supplied_token=2
    )
    assert mutated_forced[0, 0] == 99
    assert torch.equal(mutated_forced[1], forced[1])
    assert len([4, 5, 6][1:]) == 2
    _write(
        path,
        {
            "status": "passed",
            "real_score_transform": "_branch_scores",
