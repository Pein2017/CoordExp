...
def transform_history(
    token_ids: list[int], *, source_width: int, canvas_width: int, tx: int
    ...
    for row_index, start in enumerate(starts):
        ...
        mapped = [
            map_bin(value, source_width=source_width, canvas_width=canvas_width, tx=tx)
            for value in source
        ]
        for index, value in zip(coordinates, mapped, strict=True):
            transformed[index] = COORD_BASE + value
        inverse = [
            inverse_bin(value, source_width=source_width, canvas_width=canvas_width, tx=tx)
            for value in mapped
        ]
