...
GPU="3"
OUT="$BASE/runtime/scaleout-gpu3-tied"
LAUNCH_DIR="$BASE/runtime/launch/scaleout-gpu3-tied"
...
path.write_text(json.dumps({"schema":"first_coordinate_mediation.live.v1","wrapper_shell_pid":int(sys.argv[2]),"gpu":3,"model":"tied","state":"started","started_at_unix":time.time()},indent=2)+"\n")
PYLIVE
CUDA_VISIBLE_DEVICES="$GPU" python3 "$WORKTREE/probes/training_set_completion/first_coordinate/runtime.py" run \
  ...
  --max-seconds 1500 \
  --max-bytes 786432000 \
  --cell-id tied-417044-failure--sign19--x1-1 \
  --cell-id tied-417044-failure--sign21--x1-1 \
  --cell-id tied-417044-failure--sign23--x1-1 \
  --cell-id tied-417044-failure--sign25--x1-1 \
  >"$LOG" 2>&1 &
CHILD=$!
python3 - "$LIVE" "$CHILD" <<'PYLIVE2'
import json, sys, time, pathlib
path=pathlib.Path(sys.argv[1]); data=json.loads(path.read_text())
data.update({"runtime_pid":int(sys.argv[2]),"runtime_command":"CUDA_VISIBLE_DEVICES=3 python3 first_coordinate/runtime.py run scaleout-gpu3-tied","state":"running","runtime_started_at_unix":time.time()})
