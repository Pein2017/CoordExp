...
GPU="4"
OUT="$BASE/runtime/scaleout-gpu4-untied"
LAUNCH_DIR="$BASE/runtime/launch/scaleout-gpu4-untied"
...
path.write_text(json.dumps({"schema":"first_coordinate_mediation.live.v1","wrapper_shell_pid":int(sys.argv[2]),"gpu":4,"model":"untied","state":"started","started_at_unix":time.time()},indent=2)+"\n")
PYLIVE
CUDA_VISIBLE_DEVICES="$GPU" python3 "$WORKTREE/probes/training_set_completion/first_coordinate/runtime.py" run \
  ...
  --max-seconds 1500 \
  --max-bytes 786432000 \
  --cell-id untied-417044-failure--sign19--x1-0 \
  --cell-id untied-417044-failure--sign22--x1-0 \
  --cell-id untied-417044-failure--sign24--x1-0 \
  --cell-id untied-417044-failure--sign26--x1-0 \
  >"$LOG" 2>&1 &
CHILD=$!
python3 - "$LIVE" "$CHILD" <<'PYLIVE2'
import json, sys, time, pathlib
path=pathlib.Path(sys.argv[1]); data=json.loads(path.read_text())
data.update({"runtime_pid":int(sys.argv[2]),"runtime_command":"CUDA_VISIBLE_DEVICES=4 python3 first_coordinate/runtime.py run scaleout-gpu4-untied","state":"running","runtime_started_at_unix":time.time()})
