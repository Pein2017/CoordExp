...
def verify(root):
 ...
 for shard in sorted((root/'runtime').iterdir()):
  ...
  for rp in sorted(shard.glob('*/release.json')):
   ...
   for i,s in enumerate(steps):
    ...
    for j,policy in enumerate(policies):
     ...
     if 151670<=winner<152670:
      assert saved[i,j,winner-151670]==saved[i,j].max()
      if int(saved[i,j].argmax())!=winner-151670:ties.append(dict(offset=i,policy=policy,topk_winner=winner,argmax_winner=int(saved[i,j].argmax())+151670,margin=0.0))
   ...
   reports.append(dict(exact_ties=ties,cell=r['job_id'],steps=len(tokens),formula_max_abs=err,fp64_magnitude_max_abs=magnitude,fp32_change_magnitude_rounding_max_abs=rounded,unchanged_control_equal=equal,trajectory_sha256=hashlib.sha256((rp.parent/'trajectory.pt').read_bytes()).hexdigest()))
