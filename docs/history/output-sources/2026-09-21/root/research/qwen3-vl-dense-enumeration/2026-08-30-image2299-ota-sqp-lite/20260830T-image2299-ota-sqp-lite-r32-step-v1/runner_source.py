...
LEAD_ACCEPTED_R32_SCREEN_RECEIPT = (
    OUTPUT_ROOT / "20260830T-image2299-ota-sqp-lite-r32-screen-v1f" / "receipt.json"
)
LEAD_ACCEPTED_R32_SCREEN_RECEIPT_SHA256 = (
    "31c82b14a5a16ef5bcc0fa71022b6d410d204b8c3e07501dae4678ef4fb2b47b"
)
...
R32_STEP_SURFACE_SHA256 = "717df0c3eb7f58cedcf5ed1316d9cc84e608e81960ec798418f58726ae1dc7a5"
R32_STEP_TENSOR_COUNT = 588
R32_STEP_ELEMENT_COUNT = 35_438_592
R32_STEP_GRAM_RTOL = R32_OLD_GRAM_RTOL
R32_STEP_RHO_TOLERANCE = R32_OLD_RHO_TOLERANCE
R32_STEP_RESOURCE_BOUND = {
    "gpu_count": WORLD_SIZE,
    "minimum_device_memory_bytes": 84_000_000_000,
    "max_peak_cuda_reserved_bytes_per_rank": 64 * 2**30,
    "rank0_gradient_payload_bytes": ACTIVE_GRADIENTS * R32_STEP_ELEMENT_COUNT * 4,
    "teacher_forwards_total_max": 124,
    "natural_decode_generated_tokens_each_max": base.NATURAL_MAX_TOKENS,
    "model_forward_calls_total_max": 124 + 10 * base.NATURAL_MAX_TOKENS,
    "gradient_backwards_total": ACTIVE_GRADIENTS,
    "natural_decodes_total_max": 10,
...
def _step_outcome_status(
    *, r32_step: bool, continuation: bool, selected: bool,
    cold_gate: Mapping[str, Any] | None,
) -> tuple[str, str]:
    if not selected:
        return (
            "r32_step_bounded_negative_no_admissible_radius" if r32_step
            else "continuation_bounded_negative_no_admissible_radius" if continuation
            else "bounded_negative_no_admissible_radius",
            "one_relinearization_eight_radius_panel_had_no_admissible_candidate",
        )
    if cold_gate is None:
        raise OtaSqpHold("HOLD: selected candidate lacks an authoritative cold gate")
    if bool(dict(cold_gate.get("promotion", {})).get("passed")):
        return (
            "r32_step_cold_match_level_promotion" if r32_step
            else "continuation_cold_match_level_promotion" if continuation
            else "cold_match_level_promotion",
            "clean_cold_34_owner_promotion",
        )
    if bool(dict(cold_gate.get("working", {})).get("passed")):
        return (
            "r32_step_selected_working_cold_reproduced" if r32_step
            else "continuation_selected_working_cold_reproduced" if continuation
            else "smoke_selected_working_cold_reproduced",
            "one_matched_r32_step_complete" if r32_step
            else "one_step_continuation_complete" if continuation
            else "one_relinearization_smoke_complete",
        )
...
def _r32_comparator_receipt(
    ...
    counts = dict(receipt.get("counts", {}))
    candidate_panel = list(receipt.get("candidate_panel", ()))
    if (
        receipt.get("schema_version") != SCHEMA_VERSION
        or receipt.get("unit_id") != UNIT_ID
        ...
        or len(candidate_panel) != WORLD_SIZE
        or [int(item.get("rank", -1)) for item in candidate_panel] != list(range(WORLD_SIZE))
        or [float(item.get("radius", math.nan)) for item in candidate_panel] != list(RADII)
        or any(item.get("evaluation_error") is not None for item in candidate_panel)
        or receipt.get("warm_selected_screen") is not None
        or receipt.get("authoritative_cold_gate") is not None
        or counts.get("total") != {
    ...
    if _select_warm_candidate(
        candidate_panel, parent_owner_ids=r16["parent_owner_ids"],
        incumbent_action_min=float(r16["previous_action"]["minimum_margin"]),
        incumbent_divergence=int(r16["relinearization_signature"]["first_natural_divergence"]),
        eta=SAFETY_ETA,
    ) is not None:
        raise OtaSqpHold("HOLD: sealed r16 v3b panel contains an admissible radius")
    return {
        ...
        "radius_panel": {
            "radii": list(RADII), "candidate_count": WORLD_SIZE,
            "admissible_candidate_count": 0,
            "status": "continuation_bounded_negative_no_admissible_radius",
        },
    }
...
def _r32_step_input(receipt_path: Path, receipt_sha256: str) -> dict[str, Any]:
    if (
        Path(receipt_path).resolve() != LEAD_ACCEPTED_R32_SCREEN_RECEIPT.resolve()
        or str(receipt_sha256).lower() != LEAD_ACCEPTED_R32_SCREEN_RECEIPT_SHA256
    ):
        raise OtaSqpHold("HOLD: r32 step requires the lead-accepted v1f tangent receipt")
    receipt = _load_hash_bound_receipt(
        Path(receipt_path), receipt_sha256, label="lead-accepted r32 tangent screen",
    )
    bindings = dict(receipt.get("bindings", {}))
    protocol = dict(receipt.get("protocol", {}))
    snapshot = dict(receipt.get("runner_source_snapshot", {}))
    snapshot_path = Path(str(snapshot.get("path", "")))
    mechanical = dict(receipt.get("mechanical_checkpoint", {}))
    checkpoint = Path(str(mechanical.get("child_checkpoint", "")))
    step_zero = dict(receipt.get("step_zero", {}))
    step_identity = dict(step_zero.get("identity", {}))
    surface = dict(step_zero.get("surface", {}))
    linearization = dict(receipt.get("linearization", {}))
    comparison = dict(receipt.get("comparison", {}))
    counts = dict(receipt.get("counts", {}))
    resources = dict(receipt.get("resources", {}))
    inputs = _r32_inputs(
        Path(str(bindings.get("r16_receipt", ""))),
        str(bindings.get("r16_receipt_sha256", "")),
        Path(str(bindings.get("comparator_receipt", ""))),
        str(bindings.get("comparator_receipt_sha256", "")),
    )
    r16, comparator = inputs["r16"], inputs["comparator"]
    expected_sealed = {
        key: value for key, value in comparator.items() if key not in {"gram", "radius_panel"}
    } | {"gram": comparator["gram"].tolist()}
    if (
        receipt.get("schema_version") != SCHEMA_VERSION
        or receipt.get("unit_id") != UNIT_ID
        or receipt.get("status") != "r32_tangent_gate_open"
        or receipt.get("stop_reason")
        != "added_coordinate_gram_psd_nonzero_and_strictly_improved_feasible_final_min_rho"
        or receipt.get("mode") != "function_preserving_r32_no_update_tangent_screen"
        or receipt.get("run_id") != LEAD_ACCEPTED_R32_SCREEN_RECEIPT.parent.name
        or snapshot_path != receipt_path.parent / "runner_source.py"
        or not snapshot_path.is_file()
        or len(str(snapshot.get("sha256", ""))) != 64
        or base._sha256(snapshot_path) != snapshot.get("sha256")
        or bindings != {
            "comparator_bundle_sha256": V3B_COMPARATOR_SHA256,
            "comparator_receipt": str(LEAD_ACCEPTED_COMPARATOR_V3B_RECEIPT),
            "comparator_receipt_sha256": LEAD_ACCEPTED_COMPARATOR_V3B_RECEIPT_SHA256,
            "image_sha256": IMAGE_SHA256,
            "prompt_token_sha256": PROMPT_TOKEN_SHA256,
            "r16_checkpoint": str(V2_CHECKPOINT),
            "r16_natural_sha256": START_NATURAL_SHA256,
            "r16_receipt": str(LEAD_ACCEPTED_SMOKE_V2_RECEIPT),
            "r16_receipt_sha256": LEAD_ACCEPTED_SMOKE_V2_RECEIPT_SHA256,
            "r16_surface_sha256": V2_SURFACE_SHA256,
        }
        or protocol != {
            "aligner_update": False, "alpha_over_rank": 2.0, "ce": None,
            "fisher": None, "lora_alpha": ALPHA32, "natural_candidate_update": False,
            "optimizer": None, "parameter_update": False, "rank": R32,
            "safety_eta": SAFETY_ETA, "tied_embedding_update": False,
            "trained_state_saved": False, "trust_delta": TRUST_DELTA,
            "use_dora": True, "use_rslora": False, "world_size": 1,
        }
        or receipt.get("sealed_r16_comparator") != expected_sealed
        or counts != {
            "cold_reloads": 1, "gradient_backwards": ACTIVE_GRADIENTS,
            "mechanical_checkpoint_materializations": 1, "natural_decodes": 1,
            "parameter_updates": 0, "teacher_forwards": 2,
            "trained_checkpoint_saves": 0,
        }
        or any(float(resources.get(key, 0)) <= 0 for key in (
            "artifact_bytes_before_receipt", "device_total_memory_bytes",
            "peak_cuda_allocated_bytes", "peak_cuda_reserved_bytes",
        ))
    ):
        raise OtaSqpHold("HOLD: r32 tangent screen schema/source/protocol binding drifted")

    if (
        mechanical.get("source_checkpoint") != str(V2_CHECKPOINT)
        or checkpoint != receipt_path.parent / "checkpoint-r32-step-zero"
        or not checkpoint.is_dir()
        or mechanical != _r32_checkpoint_receipt(V2_CHECKPOINT, checkpoint)
        or dict(mechanical.get("adapter", {})).get("tensor_count") != R32_STEP_TENSOR_COUNT
        or dict(mechanical.get("adapter", {})).get("child_element_count")
        != R32_STEP_ELEMENT_COUNT
    ):
        raise OtaSqpHold("HOLD: r32 step-zero checkpoint/config/payload binding drifted")
    ...
    expected_identity = {
        ...
        "owner_ids": sorted(map(str, r16["parent_owner_ids"])),
        "raw_hard_counters_exact": True,
        "raw_hard_counters": r16["raw_hard_counters"],
        "causal_event_exact": True,
        "causal_event": r16["causal_event"],
        "natural_row_aligned_eos": True,
    }
    if (
        step_identity != expected_identity
        or surface.get("tensor_count") != R32_STEP_TENSOR_COUNT
        or surface.get("element_count") != R32_STEP_ELEMENT_COUNT
        or surface.get("aggregate_sha256") != R32_STEP_SURFACE_SHA256
        or len(surface.get("tensors", ())) != R32_STEP_TENSOR_COUNT
        or hashlib.sha256(
            base.full_root._canonical({"tensors": surface.get("tensors")})
        ).hexdigest() != R32_STEP_SURFACE_SHA256
        or step_zero.get("frozen_surface_sha256") != FROZEN_SURFACE_SHA256
    ):
        raise OtaSqpHold("HOLD: r32 step-zero packet/surface/frozen identity drifted")

    action_tokens = list(map(int, linearization.get("action_tokens", ())))
    cut_bindings = list(linearization.get("cut_bindings", ()))
    live_cut_margins = list(map(float, linearization.get("cut_margins", ())))
    margin_drift = _r32_margin_drift(
        live_target_panel=dict(linearization.get("target_panel", {})),
        live_cut_margins=live_cut_margins, comparator=comparator,
    )
    if (
        linearization.get("action_owner") != comparator["action_owner"]
        or action_tokens != comparator["action_tokens"]
        or linearization.get("action_sha256") != comparator["action_sha256"]
        or linearization.get("action_row_sha256") != comparator["action_row_sha256"]
        or linearization.get("prefix_sha256") != comparator["prefix_sha256"]
        or linearization.get("prefix_eos_sha256") != comparator["prefix_eos_sha256"]
        or cut_bindings != comparator["cut_bindings"]
        or len(live_cut_margins) != CUT_GRADIENTS
        or margin_drift != linearization.get("step_zero_margin_drift")
        or not margin_drift["within_tolerance"]
        or int(linearization.get("gradient_count", -1)) != ACTIVE_GRADIENTS
        or dict(linearization.get("gradient_payload", {})) != {
            "bytes": ACTIVE_GRADIENTS * R32_STEP_ELEMENT_COUNT * 4,
            "dtype": "torch.float32", "shape": [ACTIVE_GRADIENTS, R32_STEP_ELEMENT_COUNT],
            "stored_in_receipt": False,
        }
        or dict(linearization.get("coordinate_split", {})) != {
            "a": 196, "added_a": 8_257_536, "added_b": 9_175_040,
            "b": 196, "live_new_b_parameter_count": 196, "magnitude": 196,
            "new_a_nonzero_gradient_elements": 0,
            "new_b_live_gradient_rows": ACTIVE_GRADIENTS, "old": 18_006_016,
        }
    ):
        raise OtaSqpHold("HOLD: r32 sealed action/cuts/margin/gradient binding drifted")

    gram_records = dict(linearization.get("gram", {}))
    grams: dict[str, np.ndarray] = {}
    for label in ("old_r16_coordinates", "added_coordinates", "full_r32"):
        record = dict(gram_records.get(label, {}))
        try:
            matrix = np.asarray(record.get("matrix"), dtype=np.float64)
            eigenvalues = np.linalg.eigvalsh(matrix)
        except (TypeError, ValueError, np.linalg.LinAlgError) as error:
            raise OtaSqpHold(f"HOLD: r32 sealed {label} Gram is malformed: {error}") from error
        if (
            matrix.shape != (ACTIVE_GRADIENTS, ACTIVE_GRADIENTS)
            or not np.isfinite(matrix).all()
            or not np.allclose(matrix, matrix.T, atol=1e-10, rtol=0.0)
            or record.get("shape") != [ACTIVE_GRADIENTS, ACTIVE_GRADIENTS]
            or record.get("sha256") != base._hash(matrix.tolist())
            or not math.isclose(
                float(record.get("minimum_eigenvalue", math.nan)), float(eigenvalues.min()),
                rel_tol=0.0, abs_tol=SOLVER_TOLERANCE,
            )
            or not math.isclose(
                float(record.get("maximum_eigenvalue", math.nan)), float(eigenvalues.max()),
                rel_tol=0.0, abs_tol=SOLVER_TOLERANCE,
            )
        ):
            raise OtaSqpHold(f"HOLD: r32 sealed {label} Gram identity drifted")
        grams[label] = matrix
    if not np.allclose(
        grams["old_r16_coordinates"] + grams["added_coordinates"],
        grams["full_r32"], atol=1e-9, rtol=1e-12,
    ):
        raise OtaSqpHold("HOLD: r32 sealed coordinate Gram sum drifted")
    old_solver = dict(linearization.get("old_coordinate_solver", {}))
    full_solver = dict(linearization.get("full_r32_solver", {}))
    replayed_old = _solve_final_margin_span(
        comparator["target_margins"], comparator["cut_margins"],
        grams["old_r16_coordinates"], eta=SAFETY_ETA, delta=TRUST_DELTA,
    )
    replayed_full = _solve_final_margin_span(
        comparator["target_margins"], comparator["cut_margins"],
        grams["full_r32"], eta=SAFETY_ETA, delta=TRUST_DELTA,
    )
    gate = _r32_tangent_gate(
        comparator_gram=comparator["gram"], old_gram=grams["old_r16_coordinates"],
        added_gram=grams["added_coordinates"], old_solver=old_solver,
        full_solver=full_solver, comparator_rho=float(comparator["solver"]["rho"]),
    )
    if (
        gate != comparison
        or not comparison.get("added_component_established")
        or comparison.get("status") != "r32_tangent_gate_open"
        or not math.isclose(
            float(old_solver.get("rho", math.nan)), float(replayed_old["rho"]),
            rel_tol=0.0, abs_tol=R32_STEP_RHO_TOLERANCE,
        )
        or not math.isclose(
            float(full_solver.get("rho", math.nan)), float(replayed_full["rho"]),
            rel_tol=0.0, abs_tol=R32_STEP_RHO_TOLERANCE,
        )
    ):
        raise OtaSqpHold("HOLD: r32 sealed Gram/SQP/tangent comparison drifted")

    start = {
        **r16,
        "receipt": str(receipt_path), "receipt_sha256": str(receipt_sha256).lower(),
        "checkpoint": str(checkpoint), "accepted_update_ordinal": 0,
        "surface_sha256": R32_STEP_SURFACE_SHA256,
        "previous_action": {
            "owner": comparator["action_owner"],
            "token_ids": action_tokens[:-1],
            "row_sha256": comparator["action_row_sha256"],
            "minimum_margin": float(linearization["target_panel"]["minimum_margin"]),
        },
        "previous_cuts": cut_bindings,
        "r32_screen_source": {
            "receipt": str(receipt_path), "receipt_sha256": str(receipt_sha256).lower(),
            "runner_source_snapshot": snapshot, "checkpoint": str(checkpoint),
            "surface": surface, "frozen_surface_sha256": FROZEN_SURFACE_SHA256,
        },
    }
    return {
        "bindings": {
            "schema_version": SCHEMA_VERSION, "unit_id": UNIT_ID,
            "world_size": WORLD_SIZE, "r32_screen_receipt": str(receipt_path),
            "r32_screen_receipt_sha256": str(receipt_sha256).lower(),
            "start_checkpoint": str(checkpoint),
            "source_v2_receipt": str(LEAD_ACCEPTED_SMOKE_V2_RECEIPT),
            "source_v2_receipt_sha256": LEAD_ACCEPTED_SMOKE_V2_RECEIPT_SHA256,
            "r16_v3b_receipt": str(LEAD_ACCEPTED_COMPARATOR_V3B_RECEIPT),
            "r16_v3b_receipt_sha256": LEAD_ACCEPTED_COMPARATOR_V3B_RECEIPT_SHA256,
        ...
        "start": start,
        "screen": {
            "action_owner": comparator["action_owner"], "action_tokens": action_tokens,
            ...
            "target_panel": linearization["target_panel"], "cut_bindings": cut_bindings,
            "cut_margins": live_cut_margins,
            "objective_target_margins": comparator["target_margins"],
            "objective_cut_margins": comparator["cut_margins"],
            "full_gram": grams["full_r32"].tolist(), "full_solver": full_solver,
            "comparison": comparison, "counts": counts,
            "r16_radius_panel": comparator["radius_panel"],
        },
    }
...
def _step_trainable_surface(
    model: Any, *, r32_step: bool,
) -> tuple[list[str], list[torch.nn.Parameter]]:
    if r32_step:
        return _r32_trainable_surface(
            model, expected_tensor_count=R32_STEP_TENSOR_COUNT,
            expected_element_count=R32_STEP_ELEMENT_COUNT,
        )
    return base.full_root._trainable_surface(model, dora_all_only=True)
...
def _cold_state(
    ...
    measure_candidate: bool, r32_step: bool = False,
) -> dict[str, Any]:
    ...
    with base.open_backend_session(setup["frontend"].launch) as opened:
        ...
        names, parameters = _step_trainable_surface(model, r32_step=r32_step)
...
def _r32_step_linearization_check(
    *, live_target_panel: Mapping[str, Any], live_cut_margins: Sequence[float],
    live_gram: np.ndarray, live_solver: Mapping[str, Any], screen: Mapping[str, Any],
) -> dict[str, Any]:
    margin_drift = _r32_margin_drift(
        live_target_panel=live_target_panel, live_cut_margins=live_cut_margins,
        comparator={"target_panel": screen["target_panel"], "cut_margins": screen["cut_margins"]},
    )
    sealed_gram = np.asarray(screen["full_gram"], dtype=np.float64)
    if live_gram.shape != sealed_gram.shape or live_gram.shape != (
        ACTIVE_GRADIENTS, ACTIVE_GRADIENTS,
    ):
        raise OtaSqpHold("HOLD: live r32 step Gram shape drifted")
    drift = live_gram - sealed_gram
    scale = max(1.0, float(np.linalg.norm(sealed_gram, ord=2)))
    spectral_relative = float(np.linalg.norm(drift, ord=2)) / scale
    maximum_scaled = float(np.max(np.abs(drift))) / scale
    rho_drift = float(live_solver.get("rho", math.nan)) - float(screen["full_solver"]["rho"])
    if (
        not margin_drift["within_tolerance"]
        or not np.isfinite(live_gram).all()
        or not np.allclose(live_gram, live_gram.T, atol=1e-10, rtol=0.0)
        or spectral_relative > R32_STEP_GRAM_RTOL
        or maximum_scaled > R32_STEP_GRAM_RTOL
        or not math.isfinite(rho_drift)
        or abs(rho_drift) > R32_STEP_RHO_TOLERANCE
        or live_solver.get("eta") != SAFETY_ETA
        or live_solver.get("delta") != TRUST_DELTA
        or not math.isclose(
            float(live_solver.get("ordinary_l2_norm", math.nan)), TRUST_DELTA,
            rel_tol=1e-8, abs_tol=SOLVER_TOLERANCE,
        )
    ):
        raise OtaSqpHold("HOLD: live r32 margins/Gram/rho drifted from the sealed screen")
    return {
        "screen_receipt_sha256": LEAD_ACCEPTED_R32_SCREEN_RECEIPT_SHA256,
        "margin_drift": margin_drift,
        "gram_spectral_relative_drift": spectral_relative,
        "gram_max_absolute_scaled_drift": maximum_scaled,
        "gram_rtol": R32_STEP_GRAM_RTOL,
        "rho_drift": rho_drift,
        "rho_tolerance": R32_STEP_RHO_TOLERANCE,
        "objective_reused_without_tangent_regating": True,
    }
...
def run(
    *, run_id: str, start_receipt: Path | None = None,
    ...
    r32_screen_receipt: Path | None = None,
    r32_screen_receipt_sha256: str | None = None,
) -> Path:
    ...
    r32_step = r32_screen_receipt is not None or r32_screen_receipt_sha256 is not None
    if continuation and (start_receipt is None or start_receipt_sha256 is None):
        raise OtaSqpHold("HOLD: continuation requires both predecessor receipt path and SHA-256")
    if r32_step and (r32_screen_receipt is None or r32_screen_receipt_sha256 is None):
        raise OtaSqpHold("HOLD: r32 step requires both tangent receipt path and SHA-256")
    if continuation and r32_step:
        raise OtaSqpHold("HOLD: r16 continuation and the sole r32 step are mutually exclusive")
    run_id = base._safe_run_id(run_id)
    output = OUTPUT_ROOT / run_id
    started = time.monotonic()
    ...
    r32_screen: Mapping[str, Any] | None = None
    stage = "distributed_initialization"
    start_checkpoint: Path | None = None if continuation or r32_step else START_CHECKPOINT
    supplied_receipt_sha = (
        str(r32_screen_receipt_sha256) if r32_step
        else str(start_receipt_sha256) if start_receipt_sha256 is not None
        else START_RECEIPT_SHA256
    )
    try:
        ...
        owns_output = True
        stage = "hash_bound_input"
        if r32_step:
            r32_contract = base._rank0_call(lambda: _r32_step_input(
                Path(r32_screen_receipt), str(r32_screen_receipt_sha256),
            ))
            bindings, start, r32_screen = (
                r32_contract["bindings"], r32_contract["start"], r32_contract["screen"],
            )
        else:
        ...
        with base.open_backend_session(setup["frontend"].launch) as opened:
            ...
            names, parameters = _step_trainable_surface(model, r32_step=r32_step)
            expected_elements = R32_STEP_ELEMENT_COUNT if r32_step else 18_006_016
            if len(parameters) != 588 or sum(parameter.numel() for parameter in parameters) != expected_elements:
                raise OtaSqpHold(
                    f"HOLD: exact 588/{expected_elements} FP32 language-DoRA surface drifted"
                )
            sentinels = base.full_root._full_root_nontrainable_sentinels(model)
            ...
            stage = "baseline_and_alias_ranking"
            ...
            def baseline_and_aliases(
                *, model: Any = model, tokenizer: Any = tokenizer,
                ...
                if (
                    ...
                    or (r32_step and _ota_state_identity(packet) != start["reference_packet_identity"])
                ):
            ...
            if r32_step:
                assert r32_screen is not None
                if (
                    linearization_manifest["action_owner"] != r32_screen["action_owner"]
                    or linearization_manifest["action_row_sha256"]
                    != r32_screen["action_row_sha256"]
                    or linearization_manifest["action_tokens"] != r32_screen["action_tokens"]
                    or linearization_manifest["action_sha256"] != r32_screen["action_sha256"]
                    or linearization_manifest["prefix_sha256"] != r32_screen["prefix_sha256"]
                    or linearization_manifest["prefix_eos_sha256"]
                    != r32_screen["prefix_eos_sha256"]
                    or linearization_manifest["cuts"] != r32_screen["cut_bindings"]
                ):
                    raise OtaSqpHold(
                        "HOLD: actual r32 step-zero alias ranking/action/G0-G6 drifted from screen"
                    )
            ...
            stage = "rank0_r32_linearization" if r32_step else "rank0_r16_linearization"
            if rank == 0:
                ...
                try:
                    ...
                    objective_target_margins = (
                        list(map(float, r32_screen["objective_target_margins"]))
                        if r32_step else live_target_margins
                    )
                    objective_cut_margins = (
                        list(map(float, r32_screen["objective_cut_margins"]))
                        if r32_step else cut_margins
                    )
                    pre_solver = {
                        **linearization_manifest,
                        "target_margins": objective_target_margins,
                        "cut_margins": objective_cut_margins,
                        "live_target_margins": live_target_margins,
                        "live_cut_margins": cut_margins,
                        "gram": gram.tolist(),
                    ...
                    solution = _solve_final_margin_span(
                        objective_target_margins,
                        objective_cut_margins, gram, eta=SAFETY_ETA, delta=TRUST_DELTA,
                    )
                    r32_screen_check = (
                        _r32_step_linearization_check(
                            live_target_panel=target_panel, live_cut_margins=cut_margins,
                            live_gram=gram, live_solver=solution, screen=r32_screen,
                        )
                        if r32_step else None
                    )
                    ...
                    linearization = {
                        **linearization_manifest,
                        ...
                        "sqp_objective": {
                            ...
                            "cut_margins": objective_cut_margins,
                            "source": (
                                "sealed_r32_tangent_screen_matched_objective"
                                if r32_step else "live_r16_linearization"
                            ),
                        },
                        "r32_screen_match": r32_screen_check,
                        "gradient_payload": {
            ...
            if selected is None:
                ...
                base.full_root._assert_full_root_sentinels(model, sentinels)
                checkpoint = output / (
                    "checkpoint-selected-r32-step" if r32_step
                    else f"checkpoint-selected-update-{int(start['accepted_update_ordinal']) + 1}"
                ...
                if r32_step:
                    ...
                    if (
                        readback.get("parent_checkpoint") != str(start_checkpoint)
                        or readback.get("child_checkpoint") != str(checkpoint)
                        or set(payloads) != {"adapter", "special_token_embeddings"}
                        or int(payloads["adapter"].get("tensor_key_count", -1))
                        != R32_STEP_TENSOR_COUNT
                        or int(payloads["adapter"].get("changed_tensor_count", 0)) <= 0
                        or int(payloads["special_token_embeddings"].get("tensor_key_count", -1)) != 1
                        or int(payloads["special_token_embeddings"].get("changed_tensor_count", -1))
                        != 0
                    ):
                        raise OtaSqpHold("HOLD: selected r32 child checkpoint readback drifted")
                if rank == 0:
        ...
        torch.cuda.empty_cache()
        dist.barrier()
        stage = "authoritative_cold_reload"
        ...
        if selected is None:
            ...
            if (
                ...
                or cold_candidate.get("active_cut_bindings")
                != selected.get("active_cut_bindings")
        ...
        status, stop_reason = _step_outcome_status(
            r32_step=r32_step, continuation=continuation,
            selected=selected is not None, cold_gate=cold_gate,
        )
        ...
        if rank == 0:
            ...
            receipt = {
                ...
                **accounting,
                ...
                "protocol": {
                    "tier": (
                        "matched_r32_single_step" if r32_step
                        else "bounded_one_step_continuation" if continuation
                        else "production_shaped_smoke_only"
                    ),
                    "world_size": WORLD_SIZE,
                    "relinearizations": 1,
                    "foreground_updates_run": int(continuation or r32_step),
                    "maximum_r32_updates": 1 if r32_step else 0,
                    ...
                    "surface": (
                        "588 FP32 r32 language DoRA tensors / 35438592 elements"
                        if r32_step else "588 FP32 language DoRA tensors / 18006016 elements"
                    ),
                    "optimizer": None, "ce": None, "fisher": None,
                    "fixed_gt32_G6_control": {"run": False, "reason": "one-step main arm only"},
                    "r32": (
                        {
                            "gate": "lead_accepted_r32_tangent_gate_open", "run": True,
                            "screen_receipt": str(r32_screen_receipt),
                            "screen_receipt_sha256": str(r32_screen_receipt_sha256).lower(),
                            "updates": 1, "second_update_allowed": False,
                        }
                        if r32_step else {"gate": "not_opened_by_one_step_r16", "run": False}
                    ),
                ...
                "r16_v3b_comparison": (
                    {
                        "receipt": str(LEAD_ACCEPTED_COMPARATOR_V3B_RECEIPT),
                        "receipt_sha256": LEAD_ACCEPTED_COMPARATOR_V3B_RECEIPT_SHA256,
                        **dict(r32_screen["r16_radius_panel"]),
                        "r32_step_status": status,
                        "r32_selected_radius": None if selected is None else float(selected["radius"]),
                        "claim": (
                            "The sealed matched r16 panel had no admissible radius; this field "
                            "reports only the one matched r32 step outcome, not global surface sufficiency."
                        ),
                    }
                    if r32_step else None
                ),
                ...
                "runtime": runtime,
                "claim_boundary": (
                    "One single-image, exact-screen-checkpoint, matched r32/alpha64 OTA-SQP "
                    "eight-radius step only. It is not a second r32 update, optimizer/CE/Fisher "
                    "training, aligner or tied-embedding evidence, global DoRA sufficiency, "
                    "46/46 completion, transfer, or an alias/token promotion lock."
                    if r32_step else None
                ),
            }
            base._atomic_json(output / "receipt.json", receipt)
        ...
        if dist.is_initialized() and dist.get_rank() == 0 and owns_output:
            base._atomic_json(output / "receipt.json", {
                ...
                "hold_type": f"r32_step_{stage}_hold" if r32_step else "ota_step_mechanical_hold",
                "runner_sha256": base._sha256(Path(__file__)),
                "runner_source_snapshot": source_snapshot,
                "start_receipt": (
                    str(r32_screen_receipt) if r32_step
                    else None if start_receipt is None else str(start_receipt)
                ),
                ...
                "error_type": type(error).__name__, "traceback": traceback.format_exc(),
                "r32": {
                    "run": bool(r32_step), "maximum_updates": int(r32_step),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--step-r32", action="store_true",
        help="run exactly one world8 r32/alpha64 step from the sealed tangent screen",
    )
    parser.add_argument(
        "--check-r32-step-input", action="store_true",
        help="CPU-verify the explicit lead-accepted r32 tangent receipt and checkpoint",
    )
    ...
    parser.add_argument("--r32-screen-receipt", type=Path)
    parser.add_argument("--r32-screen-receipt-sha256")
    return parser
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    modes = sum(map(bool, (
        ...
        args.screen_r32, args.check_r32_inputs, args.step_r32, args.check_r32_step_input,
    )))
    if modes > 1:
    ...
    r32_step_values = (args.r32_screen_receipt, args.r32_screen_receipt_sha256)
    r32_step_complete = all(value is not None for value in r32_step_values)
    r32_step_supplied = any(value is not None for value in r32_step_values)
    if args.check_bindings:
        if (
            args.run_id or args.start_receipt or args.start_receipt_sha256
            or r32_supplied or r32_step_supplied
        ):
    ...
    if args.check_r32_inputs:
        if (
            args.run_id or args.start_receipt or args.start_receipt_sha256
            or not r32_complete or r32_step_supplied
        ):
            raise SystemExit(
    ...
    if args.screen_r32:
        ...
        return
    if args.check_r32_step_input:
        if (
            args.run_id or args.start_receipt or args.start_receipt_sha256
            or r32_supplied or not r32_step_complete
        ):
            raise SystemExit(
                "--check-r32-step-input requires --r32-screen-receipt and "
                "--r32-screen-receipt-sha256 only"
            )
        contract = _r32_step_input(*r32_step_values)
        print(json.dumps({
            "status": "verified", **contract["bindings"],
            "start_surface_sha256": contract["start"]["surface_sha256"],
            "frozen_surface_sha256": contract["start"]["frozen_surface_sha256"],
            "action_sha256": contract["screen"]["action_sha256"],
            "full_r32_rho": contract["screen"]["full_solver"]["rho"],
            "r16_radius_panel": contract["screen"]["r16_radius_panel"],
        }, indent=2, sort_keys=True))
        return
    if args.step_r32:
        if (
            ...
            raise SystemExit(
                "--step-r32 requires --r32-screen-receipt, "
                "--r32-screen-receipt-sha256, and --run-id"
            )
        print(run(
            run_id=args.run_id, r32_screen_receipt=args.r32_screen_receipt,
            r32_screen_receipt_sha256=args.r32_screen_receipt_sha256,
        ))
        return
    raise SystemExit(
        "select --check-bindings, --check-start-receipt, --smoke, --continue-one, "
        "--check-r32-inputs, --screen-r32, --check-r32-step-input, or --step-r32"
    )
