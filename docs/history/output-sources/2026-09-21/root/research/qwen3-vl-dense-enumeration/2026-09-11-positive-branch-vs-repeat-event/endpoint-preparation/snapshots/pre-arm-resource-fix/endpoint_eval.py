...
def execute(packet_path: Path, receipt_path: Path, out: Path, *, arm: str, shard: int) -> None:
    ...
    try:
        ...
        selected = [ConditionalJob(**row) for row in packet["conditional_jobs"] if row["rank"] == shard]
...
def merge(packet_path: Path, receipt_path: Path, out: Path, *, arm: str, verify: bool = False) -> dict[str, Any]:
    ...
    conditional = []
    terminals = []
    jobs = {(row["rank"], row["job_id"] if "job_id" in row else f"{row['kind']}:{row['candidate_id']}"): ConditionalJob(**row)
            for row in packet["conditional_jobs"]}
