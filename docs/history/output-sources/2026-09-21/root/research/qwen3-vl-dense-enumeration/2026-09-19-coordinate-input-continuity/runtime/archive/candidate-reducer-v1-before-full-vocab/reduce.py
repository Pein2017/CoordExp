...
def site_metrics(native: dict[str, Any], variant: dict[str, Any], site_index: int) -> dict[str, Any]:
    ...
    n_center = n_coord - n_coord.mean()
    v_center = v_coord - v_coord.mean()
    centered_delta = v_center - n_center
    ...
    centered_linf = float(centered_delta.abs().max())
    return {
        ...
        "centered_logit_change_l2": float(torch.linalg.vector_norm(centered_delta)),
        "centered_logit_change_linf": centered_linf,
        "centered_logit_change_mean_abs": float(centered_delta.abs().mean()),
        "centered_change_over_native_margin": centered_linf / max(abs(native_margin), 1e-12),
        "native_full_winner_is_coordinate": native_full_winner_is_coordinate,
        "centered_change_at_native_winner": (
            float(centered_delta[native_full_winner_coord_index])
            if native_full_winner_is_coordinate
            else None
        ),
        "centered_change_at_replacement": float(centered_delta[new_token - COORD_BASE]),
...
def main() -> None:
    ...
    result = {
        ...
        "contrast_definition": "Each row is a saved +/-1 variant minus its same-state native capture; centered logits subtract the mean over the 1000 coordinate logits at that site; ranks use strict greater-logit counts plus one.",
        "contrasts": contrasts,
        "summary": summary,
