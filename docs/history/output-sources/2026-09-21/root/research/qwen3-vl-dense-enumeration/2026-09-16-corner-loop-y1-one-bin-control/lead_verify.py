...
raw = read(ROOT/'runtime/Y1_998/raw.json'); baseline = read(OLD/'runtime/C00/raw.json')
receipt = read(ROOT/'runtime/Y1_998/receipt.json'); control = read(OLD/'runtime/C00/receipt.json')
...
assert panel['active_cells'] == ['Y1_998']
...
assert delta == [{'offset':1229,'old':152669,'new':152668}]
assert target[:1224] == panel['common_history_ids'] == raw['common136_token_ids']
assert target[1224:1233] == panel['cells']['Y1_998']['tokens'] == raw['supplied137_token_ids']
...
for offset in [1234,1237]:
    ...
    seams.append({'action_offset':offset,'C00_margin':margins[0],'Y1_998_margin':margins[1]})
live = []
...
out = {'status':'lead-accepted','scope':'477415/R16 single y1:999->998 history intervention; exact C00 suffix, no rescue',
       ...
       'limits':['Single image/checkpoint/history and edit.','Numerical distance is not embedding distance.',
                 'Zero-height to positive-height changes geometry semantics.','No embedding-origin identification or stable repair.']}
