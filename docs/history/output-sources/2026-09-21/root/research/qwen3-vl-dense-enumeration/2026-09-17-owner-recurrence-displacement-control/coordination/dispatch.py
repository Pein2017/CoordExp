...
ROOT = Path('/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-09-17-owner-recurrence-displacement-control/coordination')
...
async def main():
    async with aiohttp.ClientSession(connector=aiohttp.UnixConnector(path=SOCKET)) as session, session.ws_connect('http://localhost/', max_msg_size=0) as ws:
        ...
        receipt = {'thread_id': THREAD, 'prior_status': status, 'message_sha256': hashlib.sha256(message.encode()).hexdigest(), 'turn': result['turn'], 'transport': 'existing app-server Unix WebSocket; no setting overrides'}
