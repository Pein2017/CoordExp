...
spec = importlib.util.spec_from_file_location('common_tail_reduction', R / 'reduce.py')
...
for window in read(R / 'panel.json')['windows']:
    folder = R / 'runtime' / window['name']
    readout = torch.load(folder / 'readout.pt', weights_only=True, map_location='cpu')
    tensors = torch.load(folder / 'scores.pt', weights_only=True, map_location='cpu')
    for name, t in tensors.items():
        ...
        expected = reduction['conditions'][window['name']]['candidates'][name]['scores']['equal_norm']['logprob']
        assert abs(likelihood - expected) < 1e-10
        scaled_checks += 1
parity = [p for c in reduction['conditions'].values() for p in c['parity']]
...
receipt = dict(status='passed', bound_files=len(bindings),
               ...
               native_parity_positions=len(parity),
               max_parity_error=max(x['max_abs'] for x in parity),
               max_error_margin_ratio=max(x['twice_error_over_margin'] for x in parity),
