...
**Candidate complete, awaiting root acceptance.** At the earliest physically confirmed recurrence in this bounded review, the emitted repeated-owner row wins both its first competing token and complete-row likelihood against both supported uncovered-owner extents. This finite contrast does **not** show distinct-owner local greedy selection failure. Equal output norms remove coord0's global x1 win at the same hidden state, but do not reverse the repeated-versus-uncovered candidate ordering. Readout-scale sensitivity and surviving contextual angular preference coexist.
