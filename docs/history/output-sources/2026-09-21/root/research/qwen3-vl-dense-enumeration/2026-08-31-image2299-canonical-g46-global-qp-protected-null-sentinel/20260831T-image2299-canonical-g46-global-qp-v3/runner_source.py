...
FINAL_EVALUATOR_LABEL = "canonical-g46-global-qp-final"
...
def _status8_certificate(
    ...
    *,
    ...
    """Admit only a finite status-8 primal whose duality gap independently closes."""
    diagnostics = dict(solved.get("primal_diagnostics", {}))
    certificate: dict[str, Any] = {
        "attempted": diagnostics.get("status") == 8,
        "original_success": diagnostics.get("success"),
        "original_status": diagnostics.get("status"),
        "original_message": diagnostics.get("message"),
        "accepted": False,
    }
    if (
        solved.get("feasible")
        or solved.get("solver_classification") != "numerical_solver_hold"
        or diagnostics.get("status") != 8
        or candidate is None
    ):
        return dict(solved), certificate
    x = np.asarray(getattr(candidate, "x", None), dtype=np.float64)
    if x.ndim != 1 or not np.isfinite(x).all():
        certificate["reason"] = "status8_candidate_not_finite"
        return dict(solved), certificate
    try:
        system = qp._normalized_primal_system(
            constraints,
            rank=rank,
            row_norms=row_norms,
        ...
        certificate["reason"] = f"certificate_system_error:{type(error).__name__}"
        return dict(solved), certificate
    matrix, rhs = system["matrix"], system["rhs"]
    minimum_slack = float((matrix @ x - rhs).min(initial=np.inf))
    primal_objective = float(0.5 * x @ x)
    dual_objective = float(
        dict(solved.get("legacy_dual_diagnostic", {})).get("dual_objective", np.nan)
    )
    ...
    certificate.update(
        {
            ...
            "slack_tolerance": 1.0e-9,
            "duality_gap_tolerance": 1.0e-6,
        }
    )
    if (
        minimum_slack < -1.0e-9
        or not np.isfinite(dual_objective)
        or primal_objective < dual_objective
        or abs(gap) > 1.0e-6
    ):
        certificate["reason"] = "status8_primal_duality_certificate_failed"
        return dict(solved), certificate
    admitted = dict(solved)
    admitted.update(
        {
            "feasible": True,
            "solver_classification": "feasible_primal_duality_certified_status8",
            "selected_token_ids": list(map(int, selected)),
            "normalized_rows": x.reshape(len(selected), rank),
            "scaled_basis_rows": np.stack(
                [
                    x.reshape(len(selected), rank)[index] * float(row_norms[token])
                    for index, token in enumerate(selected)
                ]
            ),
            "normalized_norm": float(np.linalg.norm(x)),
            "minimum_slack": minimum_slack,
            "constraint_count": len(constraints),
            "variable_count": int(system["variable_count"]),
        }
    )
    certificate["accepted"] = True
    return admitted, certificate


def _solve_one_global_qp_with_status8_certificate(
    constraints: Sequence[Mapping[str, Any]],
    *,
    rank: int,
    row_norms: Mapping[int, float],
    ...
    """Capture the helper's sole SLSQP result without invoking any extra solver."""
    captured: dict[str, Any] = {}
    original_minimize = qp.minimize

    def capture_minimize(*args: Any, **kwargs: Any) -> Any:
        outcome = original_minimize(*args, **kwargs)
        if kwargs.get("method") == "SLSQP":
            captured["slsqp"] = outcome
        return outcome

    qp.minimize = capture_minimize
    try:
        ...
        qp.minimize = original_minimize
    return _status8_certificate(
        solved,
        candidate=captured.get("slsqp"),
...
def _cold_verify(*, payload: Path, result: Path) -> None:
    ...
    with base.open_backend_session(setup["frontend"].launch) as opened:
        ...
        if (
            names != names_after
            or any(
                a is not b for a, b in zip(parameters, parameters_after, strict=True)
            )
    ...
    try:
        ...
        with base.open_backend_session(setup["frontend"].launch) as opened:
            ...
            if not production_static["passed"]:
                terminal = "static_route_hold"
                raise _hold(
                    "canonical G46 production parser/global matcher gate failed"
                )
            names, parameters, surface_before, frozen_before = _surface_snapshot(model)
            sentinels = full_root._full_root_nontrainable_sentinels(model)
            base_head = model.get_output_embeddings()
            ...
            phase = "one_global_qp"
            solved, numerical_certificate = (
                _solve_one_global_qp_with_status8_certificate(
                    constraints,
                    rank=basis.shape[1],
