...
def test_reducer_missing_and_extra_score_cells(tmp_path):
    packet = dict(routes=[dict(route_id='required')], groups=[dict(example_id=str(i)) for i in range(32)],
                  witnesses=[{} for _ in range(43)])
    publish(tmp_path / 'inputs.json', packet)
    with pytest.raises(ValueError, match='missing/extra score identities'):
        reduce(tmp_path)
    (tmp_path / 'scores' / 'source').mkdir(parents=True)
    publish(tmp_path / 'scores' / 'source' / 'extra.json', {})
    with pytest.raises(ValueError, match='missing/extra score identities'):
        reduce(tmp_path)
