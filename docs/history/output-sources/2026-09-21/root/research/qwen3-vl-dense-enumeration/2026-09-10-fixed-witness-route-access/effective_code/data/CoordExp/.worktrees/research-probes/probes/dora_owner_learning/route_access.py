...
def execute(output):
    ...
    packet = load_canonical_json(output / 'inputs.json')
    require(not (output / 'execution_started.json').exists(), 'existing model invocation')
    publish(output / 'execution_started.json', dict(pid=os.getpid(), time=time.time(), visible_devices='0'))
    plan = packet['plan']
    ...
    publish(output / 'config_relocation.json', dict(original=plan['sources']['infer_config'], effective_path=str(CONFIG),
    ...
    started, forwards, scored_positions, loads = time.monotonic(), 0, 0, 0
    ...
    try:
        for name in ('source', 'post'):
            ...
            if name == 'post':
                cfg.adapter.path = Path(packet['update_receipt']['saved_adapter']['root'])
        ...
        signal.alarm(0)
        publish(output / 'terminal.json', dict(status=status, error=error, model_loads=loads, model_receipts=model_receipts,
...
def main():
    ...
    p.add_argument('command', choices=['prepare', 'execute', 'reduce'])
