"""Fixed action67, paired history-cache / layer13 residual factorial."""
import argparse
import copy
import inspect
import json
...
def run(panel_path, output, mode, donor_path=None, residual=False, cache_patch=False, rebuild=None):
    ...
    receipt=dict(status='running',mode=mode,residual=residual,cache_patch=cache_patch,
        ...
        intervention=dict(action_offset=67,history_actions=[54,63],layers=list(LAYERS),target=TARGET,
        ...
        def attention_post(layer):
            def hook(module,args,result):
                ...
                tensors['restore'][str(layer)].update(restored_key=entry.keys[:,:,lo:hi,:].detach().cpu().clone(),
                    restored_value=entry.values[:,:,lo:hi,:].detach().cpu().clone(),all_history_restored=True)
                return result
            return hook
...
if __name__=='__main__':
    ...
    p.add_argument('--rebuild',type=Path);p.add_argument('--cpu-check',action='store_true');a=p.parse_args()
    if a.cpu_check:cpu_check(a.output)
    else:run(a.panel,a.output,a.mode,a.donor,a.residual,a.cache_patch,a.rebuild)
