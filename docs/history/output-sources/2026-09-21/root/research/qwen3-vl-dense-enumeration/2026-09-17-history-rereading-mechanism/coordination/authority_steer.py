...
async def main():
    async with aiohttp.ClientSession(connector=aiohttp.UnixConnector(path=SOCKET)) as session, session.ws_connect('http://localhost/', max_msg_size=0) as ws:
        ...
        message = (ROOT/'authority-message.txt').read_text()
        ...
        (ROOT/'authority-steer-receipt.json').write_text(json.dumps(receipt, indent=2)+'\n')
        print(json.dumps({'thread_id': THREAD, 'expected_turn_id': '01a0b00d-4607-7753-8345-98d62eefaa5a', 'steer_result': result, 'receipt': str(ROOT/'authority-steer-receipt.json')}))
