...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v15"
OUTPUT_ROOT = parallel.OUTPUT_ROOT / "projected-owner-corridor"
TRIGGER_RECEIPT = (
    OUTPUT_ROOT / "20260830T-coordinate-working-corridor-v14" / "receipt.json"
)
TRIGGER_SHA256 = "8a76c583ed0f1494f6c8184d20bd2b56ea842ad6506eff15a5359c7fff344ac2"
TRIGGER_SOURCE = TRIGGER_RECEIPT.parent / "runner_source.py"
TRIGGER_SOURCE_SHA256 = "80f822162dfb0b251b0d820b1c1adc8b166bdbdd09ad4df30970ba4e8dbf2e11"
...
START_CHECKPOINT = (
    TRIGGER_RECEIPT.parent / "checkpoint-terminal-step-01"
)
TRIGGER_START_CHECKPOINT = (
    OUTPUT_ROOT / "20260830T-gt26-final-boundary-v13" / "checkpoint-terminal-step-06"
)
NATURAL_TOKEN_SHA256 = "3f5971c5b6293cd1aef5171bb5daf419fdd4c16c7096c609a4cd21bfe062c819"
TRIGGER_INITIAL_NATURAL_SHA256 = "dc2a7a92880a18ac03c30b0d0dd099450ab3c3f371ed292d0c258b068a5542d7"
CLEAN_PARENT_TOKEN_SHA256 = base.PARENT_ROUTE_SHA256
TRIGGER_INITIAL_SURFACE_SHA256 = "53950cb1178269f8f988105505bcab3c04e8efbe1292d31b8887f1cb5344f827"
START_SURFACE_SHA256 = "047367e9b0e334d3816c121de788c20cdc1faef16059d03725bd209491b8a571"
...
TARGET_MARGIN = -9.046401023864746
TARGET_TEACHER_SHA256 = "c4888a6c32b57c5fba26ae5cdef13ed30434359b6e6ae6a9ef65ee6ba2b53b82"
TRIGGER_GUARD_MARGINS = {
    "G0": 0.4106101989746094,
    "G1": 0.21944618225097656,
    "G2": 1.0323419570922852,
    "G3": 1.7416496276855469,
    "G4": 0.4043998718261719,
    "G5": 0.028730392456054688,
}
TRIGGER_INITIAL_TARGET_MARGIN = -3.814697265625e-05
TRIGGER_INITIAL_GUARD_MARGINS = {
...
def _trigger() -> dict[str, Any]:
    ...
    if (
        receipt.get("schema_version") != "image2299.projected_owner_corridor.v14"
        or receipt.get("status") != TRIGGER_STATUS
        or receipt.get("stop_reason") != TRIGGER_STOP
        or int(receipt.get("accepted_step_count", -1)) != 1
        ...
        or int(tail.get("row_index", -1)) != 33 or int(tail.get("token_start", -1)) != 297
        ...
        or bool(warm.get("strict_corridor_gate", {}).get("passed"))
        or warm.get("corridor_gate") != expected_old_gate
        ...
        raise ProjectedCorridorHold("HOLD: v14 trigger state/checkpoint/surface/cold identity drifted")

    accepted = dict(attempts[0])
    accepted_panel = list(accepted.get("candidate_panel", ()))
    accepted_by_rank = {int(candidate.get("rank", -1)): candidate for candidate in accepted_panel}
    selected = dict(accepted.get("selected_broadcast", {}))
    selected_state = dict(selected.get("state", {}))
    initial_tokens = list(map(int, accepted.get("frozen_state", {}).get("generated_token_ids", ())))
    if (
        token_ids_sha256(initial_tokens) != TRIGGER_INITIAL_NATURAL_SHA256
        or list(initial_tokens[-10:-1]) != list(TRIGGER_INITIAL_TAIL)
        or int(accepted.get("attempt", -1)) != 1
        or int(accepted.get("accepted_steps_before", -1)) != 0
        or accepted.get("decision") != "accept_largest_radius_in_priority_class"
        or accepted.get("target_margin") != TRIGGER_INITIAL_TARGET_MARGIN
        or accepted.get("guard_margins") != TRIGGER_INITIAL_GUARD_MARGINS
        or accepted.get("frozen_surface") != initial_surface
        or len(accepted_panel) != WORLD_SIZE or set(accepted_by_rank) != set(range(WORLD_SIZE))
        or tuple(float(accepted_by_rank[rank].get("radius", math.nan)) for rank in range(WORLD_SIZE)) != RADII
        or (selected.get("source_rank"), selected.get("radius"), selected.get("class"))
        != (4, 0.0625, "corridor")
        or accepted_by_rank[4].get("state") != selected_state
        or selected_state.get("generated_token_ids") != tokens
        or selected.get("surface") != terminal_surface
        or not bool(dict(selected_state.get("corridor_gate", {})).get("passed"))
        or bool(dict(selected_state.get("promotion_gate", {})).get("passed"))
    ):
        raise ProjectedCorridorHold("HOLD: v14 accepted near-miss progression drifted")
    ...
    for rank, expected_sha256 in enumerate(expected_hashes):
        candidate = dict(accepted_by_rank[rank])
        ...
        if (
            ...
            or bool(candidate.get("corridor")) != (rank >= 4)
            or not bool(dict(candidate.get("observed_bad_token_guard_gate", {})).get("passed"))
        ):
            raise ProjectedCorridorHold("HOLD: v14 accepted duplicate/near-miss panel drifted")
    ...
    for rank in range(WORLD_SIZE):
        ...
        state_causal = dict(state.get("causal", {}))
        first = dict(state_causal.get("first_event", {}))
        state_rows = list(state_causal.get("rows", ()))
        state_tail = dict(state_rows[-1]) if state_rows else {}
        old_gate = dict(state.get("corridor_gate", {}))
        if (
            state.get("generated_token_ids_sha256") != DUPLICATE_ROUTE_SHA256
            or list(map(int, state.get("generated_token_ids", ())[-10:-1])) != list(DUPLICATE_TAIL)
            or first.get("kind") != "duplicate"
            or int(first.get("event_start", -1)) != 297
            or int(first.get("row_index", -1)) != 33
            or list(map(int, first.get("bad_span_tokens", ()))) != list(DUPLICATE_TAIL)
            or state_evaluation.get("hard_counter_count") != 3
            or state_joint.get("hard_raw_counters") != WORKING_UNSUPPORTED_PERSON_HARD_COUNTERS
            or int(state_joint.get("hard_raw_counters", {}).get("duplicate_person_count_iou95", -1)) != 0
            or ("raw_person_duplicate" in state_tail and state_tail["raw_person_duplicate"] is not False)
            or bool(dict(state.get("promotion_gate", {})).get("passed"))
            or bool(dict(state.get("strict_corridor_gate", {})).get("passed"))
            or old_gate != {
                "passed": False, "kind": "transient_person_near_miss",
                "admission": "working_corridor_optimization_state_only",
                "promotion_eligible": False, "scientific_success": False,
                "debt": {"person_near_miss_tail": True},
            }
            or bool(candidate.get("promotion")) or bool(candidate.get("corridor"))
            or not bool(candidate.get("target_margin_improved"))
            or not math.isfinite(float(candidate.get("target_margin", math.nan)))
            or float(candidate["target_margin"]) <= TARGET_MARGIN
            or not bool(dict(candidate.get("observed_bad_token_guard_gate", {})).get("passed"))
        ):
            raise ProjectedCorridorHold("HOLD: v14 all-duplicate terminal panel drifted")
        panel_receipts.append({
            "rank": rank, "route_sha256": DUPLICATE_ROUTE_SHA256, "kind": "duplicate",
        })
    if (
        int(rejected.get("attempt", -1)) != 2
        or int(rejected.get("accepted_steps_before", -1)) != 1
        or rejected.get("decision") != "no_eligible_radius_restore_current"
        or rejected.get("selected_broadcast") is not None
        or rejected.get("frozen_state") != selected_state
        ...
        or rejected.get("target_pair") != {
            key: value for key, value in current_pair.items() if key != "teacher_tokens"
        }
        or guard_identities(rejected.get("guard_pairs", ())) != guard_identities(current_guards)
        or len(rejected_panel) != WORLD_SIZE or set(by_rank) != set(range(WORLD_SIZE))
        or tuple(float(by_rank[rank].get("radius", math.nan)) for rank in range(WORLD_SIZE)) != RADII
    ):
        raise ProjectedCorridorHold("HOLD: v14 rejected terminal panel evidence drifted")
    return {
        "path": str(TRIGGER_RECEIPT), "sha256": TRIGGER_SHA256,
        "status": TRIGGER_STATUS, "stop_reason": TRIGGER_STOP,
        "accepted_step_count": 1, "start_checkpoint": str(START_CHECKPOINT),
        ...
        "accepted_attempts": [{"attempt": 1, "source_rank": 4, "radius": 0.0625}],
