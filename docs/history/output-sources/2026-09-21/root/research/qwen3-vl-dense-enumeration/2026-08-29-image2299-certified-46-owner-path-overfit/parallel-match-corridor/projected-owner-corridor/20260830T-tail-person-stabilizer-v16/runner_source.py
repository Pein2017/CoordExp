#!/usr/bin/env python3
"""Eight-way DoRA corridor search with a post-description person stabilizer."""
...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v16"
OUTPUT_ROOT = parallel.OUTPUT_ROOT / "projected-owner-corridor"
TRIGGER_RECEIPT = (
    OUTPUT_ROOT / "20260830T-unsupported-person-corridor-v15" / "receipt.json"
)
TRIGGER_SHA256 = "5b1d7bb43a99f48b88ebd88aedce179f53e120ba156697e30a9dfe552a4d6b27"
TRIGGER_SOURCE = TRIGGER_RECEIPT.parent / "runner_source.py"
TRIGGER_SOURCE_SHA256 = "76084c910173c6955be29ebcfc942cd4afa018482fcebca1bccb60ad38904d75"
TRIGGER_STATUS = "bounded_negative_no_match_level_promotion"
TRIGGER_STOP = "max_20_accepted_updates_without_warm_promotion"
START_CHECKPOINT = (
    TRIGGER_RECEIPT.parent / "checkpoint-terminal-step-20"
)
TRIGGER_START_CHECKPOINT = (
    OUTPUT_ROOT / "20260830T-coordinate-working-corridor-v14" / "checkpoint-terminal-step-01"
)
NATURAL_TOKEN_SHA256 = "58d4a7732177fd8a1f2695cf985e8338a9cd6b543609f790702d40ad77722e12"
TRIGGER_INITIAL_NATURAL_SHA256 = "3f5971c5b6293cd1aef5171bb5daf419fdd4c16c7096c609a4cd21bfe062c819"
CLEAN_PARENT_TOKEN_SHA256 = base.PARENT_ROUTE_SHA256
TRIGGER_INITIAL_SURFACE_SHA256 = "047367e9b0e334d3816c121de788c20cdc1faef16059d03725bd209491b8a571"
START_SURFACE_SHA256 = "160b041b6813fe815eaaa6c928f46af63f148db09a9fa318e5c19273b084bfb6"
...
TARGET_POSITION, TARGET_GOOD, TARGET_BAD = 301, 152399, 152596
TARGET_MARGIN = -8.271553993225098
TARGET_TEACHER_SHA256 = "2621f7d6654e303fa4add9cbb63429e5f0d9aff4172a9bf50829abef0b682228"
TRIGGER_GUARD_MARGINS = {
    "G0": 0.3903312683105469,
    "G1": 0.20114898681640625,
    "G2": 1.0900344848632812,
    "G3": 1.7411623001098633,
    "G4": 0.3720436096191406,
    "G5": 0.12572860717773438,
}
GUARD_SPECS = (
    {
...
def _trigger() -> dict[str, Any]:
    ...
    if (
        receipt.get("schema_version") != "image2299.projected_owner_corridor.v15"
        or receipt.get("status") != TRIGGER_STATUS
        or receipt.get("stop_reason") != TRIGGER_STOP
        or int(receipt.get("accepted_step_count", -1)) != 20
        ...
        or len(tokens) != 307 or list(tokens[-10:-1]) != list(PERSON_NEAR_MISS_TAIL)
        ...
        or causal.get("first_event", {}).get("kind") != "near_miss"
        or list(map(int, causal.get("first_event", {}).get("bad_span_tokens", ()))) != list(PERSON_NEAR_MISS_TAIL)
        or tail.get("parse_state") != "valid" or tail.get("decision") != "near_miss"
        ...
        raise ProjectedCorridorHold("HOLD: v15 trigger state/checkpoint/surface/cold identity drifted")
    ...
    if previous_route != TRIGGER_INITIAL_NATURAL_SHA256:
        raise ProjectedCorridorHold("HOLD: v15 accepted progression start route drifted")
    for index, attempt in enumerate(attempts, start=1):
        ...
        if (
            ...
            raise ProjectedCorridorHold("HOLD: v15 accepted progression drifted")
    ...
    if (
        ...
        or dict(attempts[-1].get("selected_broadcast", {})).get("state", {}).get("generated_token_ids") != tokens
        or dict(attempts[-1].get("target_pair", {})).get("pair_position") != TARGET_POSITION
        or dict(attempts[-1].get("target_pair", {})).get("good_token_id") != TARGET_GOOD
        or dict(attempts[-1].get("target_pair", {})).get("bad_token_id") != TARGET_BAD
    ):
        raise ProjectedCorridorHold("HOLD: v15 terminal route/pair progression drifted")
    return {
        "path": str(TRIGGER_RECEIPT), "sha256": TRIGGER_SHA256,
        "status": TRIGGER_STATUS, "stop_reason": TRIGGER_STOP,
        "accepted_step_count": 20, "start_checkpoint": str(START_CHECKPOINT),
...
def _project_direction(
    ...
    *, pair_position: int,
    ...
    if pair_position == LEXICAL_TARGET_POSITION and stabilizer_gradient is not None:
        raise ProjectedCorridorHold("HOLD: lexical target must not use tail-person stabilizer")
    if pair_position > LEXICAL_TARGET_POSITION and g5_gradient is not None:
        raise ProjectedCorridorHold("HOLD: coordinate target must not use G5 gradient")
    auxiliary_gradient = g5_gradient if pair_position == LEXICAL_TARGET_POSITION else stabilizer_gradient
    auxiliary_id = "G5_margin" if pair_position == LEXICAL_TARGET_POSITION else "tail_person_stabilizer_margin"
    if pair_position == LEXICAL_TARGET_POSITION and auxiliary_gradient is None:
        raise ProjectedCorridorHold("HOLD: lexical target requires G5 gradient")
    if pair_position <= LEXICAL_TARGET_POSITION and pair_position != LEXICAL_TARGET_POSITION:
        raise ProjectedCorridorHold("HOLD: unsupported dynamic target position")
    if pair_position > LEXICAL_TARGET_POSITION and auxiliary_gradient is None:
        raise ProjectedCorridorHold("HOLD: coordinate target requires tail-person stabilizer gradient")
    ...
    return direction, {
        "mode": (
            "normalized_unit_target_descent_plus_unit_G5_margin_ascent"
            if pair_position == LEXICAL_TARGET_POSITION
            else "normalized_unit_target_descent_plus_unit_tail_person_stabilizer_margin_ascent"
        ),
...
def _select_candidate(
    ...
    eligible = [
        ...
        and (
            ...
            or (
                base_target_position > LEXICAL_TARGET_POSITION
                and base_stabilizer_margin is not None
                and math.isfinite(float(base_stabilizer_margin))
                and math.isfinite(float(item.get("frozen_stabilizer_margin", math.nan)))
                and float(item["frozen_stabilizer_margin"]) > float(base_stabilizer_margin)
...
def _terminal_gradient_evidence(
    *, model: Any, native_inputs: Mapping[str, Any], pad: int,
    target_pair: Mapping[str, Any], guard_pairs: Sequence[Mapping[str, Any]],
    stabilizer_binding: Mapping[str, Any], parameters: Sequence[torch.nn.Parameter],
) -> dict[str, Any]:
    target_logits = base.full_root._teacher_forced_route_logits(
        model=model, native_inputs=native_inputs, route_tokens=target_pair["teacher_tokens"],
        pad_token_id=pad,
    )
    target_loss = torch.nn.functional.softplus(-_margin(target_logits, target_pair))
    target_gradients = torch.autograd.grad(target_loss, parameters, allow_unused=True)
    del target_logits, target_loss
    natural_logits = base.full_root._teacher_forced_route_logits(
        model=model, native_inputs=native_inputs, route_tokens=guard_pairs[0]["teacher_tokens"],
        pad_token_id=pad,
    )
    stabilizer_margin = _stabilizer_margin(natural_logits, stabilizer_binding)
    stabilizer_gradients = torch.autograd.grad(stabilizer_margin, parameters, allow_unused=True)
    del natural_logits
    reduced: dict[str, list[torch.Tensor]] = {}
    for gradient_id, gradients in (
        ("target_loss", target_gradients),
        ("tail_person_stabilizer_margin", stabilizer_gradients),
    ):
        values = []
        for value in gradients:
            if value is None or not bool(torch.isfinite(value).all().item()):
                raise ProjectedCorridorHold(f"HOLD: disconnected/non-finite {gradient_id} gradient")
            values.append(value.detach())
        reduced[gradient_id] = values
    _direction, projection = _project_direction(
        reduced["target_loss"], None, reduced["tail_person_stabilizer_margin"],
    ...
    return {
        ...
        "margin": float(stabilizer_margin.detach().item()),
        "gradient_norm_squared": projection["fp64_norm_squared"]["tail_person_stabilizer_margin"],
        "target_gradient_dot": projection["fp64_pairwise_dots"]["target_loss"]["tail_person_stabilizer_margin"],
        "directional_derivative": projection["fp64_directional_derivatives"]["tail_person_stabilizer_margin"],
    }
...
def _cold_evaluate(
    ...
    with open_backend_session(setup["frontend"].launch) as opened:
        ...
        if packet["promotion_gate"]["passed"]:
            ...
            stabilizer = _terminal_gradient_evidence(
                model=model, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id),
                target_pair=target_pair, guard_pairs=guard_pairs,
                stabilizer_binding=stabilizer_binding, parameters=parameters,
            )
    ...
    try:
        ...
        with open_backend_session(setup["frontend"].launch) as opened:
            ...
            for attempt in range(1, MAX_ACCEPTED_STEPS + 1):
                ...
                stabilizer_gradients = (
                    torch.autograd.grad(stabilizer_margin_tensor, parameters, allow_unused=True)
                ...
                if stabilizer_gradients is not None:
                    gradient_sets.append(("tail_person_stabilizer_margin", stabilizer_gradients))
                ...
                direction, projection = _project_direction(
                    ...
                    reduced_gradients.get("tail_person_stabilizer_margin"),
                    pair_position=int(target_pair["pair_position"]),
                )
            ...
            if promoted:
                ...
                warm_stabilizer = _terminal_gradient_evidence(
                    model=model, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id),
                    target_pair=terminal_target_pair, guard_pairs=terminal_guard_pairs,
                    stabilizer_binding=terminal_stabilizer_binding, parameters=parameters,
                )
                if (
        ...
        receipt = {
            ...
            "protocol": {
                ...
                "projection": "at lexical position 298, equal unit target-loss descent plus G5 ascent; at coordinate positions >298, equal unit target-loss descent plus tail-person stabilizer ascent",
                "selection": "proper Parent superset first; otherwise largest working-corridor observed-bad-token-safe radius strictly improving the frozen-current target and tail-person stabilizer while keeping natural token 8987 at position 298; G5 must strictly improve only at lexical position 298; all six candidate-on-policy guards stay positive",
                ...
                "gradient_objectives": {
                    ...
                    "post_description": ["target_loss", "tail_person_stabilizer_margin"],
                },
