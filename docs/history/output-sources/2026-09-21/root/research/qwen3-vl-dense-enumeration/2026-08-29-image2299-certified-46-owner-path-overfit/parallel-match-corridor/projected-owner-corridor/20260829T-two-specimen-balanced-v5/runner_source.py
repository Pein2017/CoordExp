...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v5"
...
def _project_direction(
    ...
    target_norm_sq = _fp64_dot(target_gradient, target_gradient)
    h_norm_sq = _fp64_dot(guard_gradient, guard_gradient)
    if (
        not math.isfinite(target_norm_sq) or not math.isfinite(h_norm_sq)
        or target_norm_sq <= 0.0 or h_norm_sq <= 0.0
    ):
        raise ProjectedCorridorHold("HOLD: zero/non-finite target or guard gradient")
    target_norm, guard_norm = math.sqrt(target_norm_sq), math.sqrt(h_norm_sq)
    desired_target = [-value.detach() / target_norm for value in target_gradient]
    desired_guard = [value.detach() / guard_norm for value in guard_gradient]
    raw = [
        (target_value + guard_value).detach()
        for target_value, guard_value in zip(desired_target, desired_guard, strict=True)
    ]
    raw_norm = math.sqrt(max(0.0, _fp64_dot(raw, raw)))
    if not math.isfinite(raw_norm) or raw_norm <= 0.0:
        raise ProjectedCorridorHold("HOLD: normalized target and guard directions cancel")
    direction = [(value / raw_norm).detach() for value in raw]
    direction_norm = math.sqrt(max(0.0, _fp64_dot(direction, direction)))
    h_dot_direction = _fp64_dot(guard_gradient, direction)
    g_dot_direction = _fp64_dot(target_gradient, direction)
    cosine = _fp64_dot(target_gradient, guard_gradient) / (target_norm * guard_norm)
    if (
        not all(math.isfinite(value) for value in (direction_norm, h_dot_direction, g_dot_direction, cosine))
        or abs(direction_norm - 1.0) > PROJECTION_TOLERANCE
        or h_dot_direction <= 0.0 or g_dot_direction >= 0.0
    ):
        raise ProjectedCorridorHold("HOLD: balanced direction does not improve both contextual margins")
    return direction, {
        "mode": "unit_gradient_bisector", "fp64_target_norm_squared": target_norm_sq,
        "fp64_h_norm_squared": h_norm_sq, "fp64_target_guard_cosine": cosine,
        "direction_norm": direction_norm,
        "fp64_h_dot_direction": h_dot_direction,
        "fp64_target_g_dot_direction": g_dot_direction,
...
def _select_candidate(
    ...
    eligible = [
        ...
        and float(item["guard_margin"]) > max(GUARD_MARGIN_FLOOR, base_guard_margin)
    ]
    ...
    try:
        ...
        with open_backend_session(setup["frontend"].launch) as opened:
            ...
            baseline_target_margin, baseline_guard_margin = _teacher_margins(
                model=model, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id),
                target_pair=initial_target_pair, guard_pair=guard_pair,
            )
            ...
            for attempt in range(1, MAX_ACCEPTED_STEPS + 1):
                ...
                candidate_summary = {
                    ...
                    "guard_preserved": candidate_guard_margin > max(GUARD_MARGIN_FLOOR, base_guard_margin),
                    "specimen_identity": (
            ...
            warm_target_margin, warm_guard_margin = _teacher_margins(
                model=model, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id),
                target_pair=terminal_target_pair, guard_pair=guard_pair,
            )
            if accepted_steps == 0:
        ...
        cold = base._rank0_call(lambda: _cold_evaluate(
            ...
            target_pair=terminal_target_pair, guard_pair=guard_pair, label="projected-corridor-cold",
        ))
        if (
        ...
        receipt = {
            ...
            "bindings": {
                ...
                "initial_target_pair": {key: value for key, value in initial_target_pair.items() if key != "teacher_tokens"},
                "terminal_target_pair": {key: value for key, value in terminal_target_pair.items() if key != "teacher_tokens"},
            ...
            "protocol": {
                ...
                "projection": "unit-norm target-descent plus unit-norm guard-ascent, renormalized once",
