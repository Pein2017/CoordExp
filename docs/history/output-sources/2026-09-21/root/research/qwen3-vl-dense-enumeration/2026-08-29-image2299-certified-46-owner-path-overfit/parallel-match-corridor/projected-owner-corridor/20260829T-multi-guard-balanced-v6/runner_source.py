#!/usr/bin/env python3
"""Eight-way DoRA corridor search with three fixed owner guards."""
...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v6"
OUTPUT_ROOT = parallel.OUTPUT_ROOT / "projected-owner-corridor"
TRIGGER_RECEIPT = (
    OUTPUT_ROOT / "20260829T-two-specimen-balanced-v5" / "receipt.json"
)
TRIGGER_SHA256 = "e6f2a7e0b514465303e9fd70b1d1aab4a331e4cd269fe621d8664db7b1701fab"
TRIGGER_SOURCE = TRIGGER_RECEIPT.parent / "runner_source.py"
TRIGGER_SOURCE_SHA256 = "5d5f8749abb0199b8ee04ee79d1b3eee3d7cd5c860913025a9732a1b726bc707"
...
START_CHECKPOINT = (
    TRIGGER_RECEIPT.parent / "checkpoint-terminal-step-11"
)
TRIGGER_START_CHECKPOINT = (
    parallel.OUTPUT_ROOT / "20260829T-parallel-match-corridor-v1c" / "checkpoint-terminal-step-36"
)
...
TRIGGER_INITIAL_SURFACE_SHA256 = "6c4e8a11fe27e87ef7e361da1c63c15a1784cab1cb2fb5cdfe61048964428c00"
START_SURFACE_SHA256 = "1d611df1925267ea8b5a14a5e56d38eeb228b5916e99825686273138ac5bedb7"
...
FAILED_CANDIDATE_SHA256 = "9f91c9dd89fd4fe5e161f276725860ae464da7783b6800873cfb954b93b6e462"
FAILED_LOST_OWNER_IDS = ("gt:2299:7", "gt:2299:19")
FAILED_GAINED_OWNER_IDS = ("gt:2299:34", "gt:2299:35")
OBSERVED_TAIL = (151646, 48731, 151647, 151648, 152596, 152097, 152617, 152138, 151649)
WORLD_SIZE = 8
MAX_ACCEPTED_STEPS = 14
PRIOR_ACCEPTED_STEPS = 47
...
def _trigger() -> dict[str, Any]:
    ...
    last = dict(attempts[-1]) if len(attempts) == 12 else {}
    ...
    if (
        receipt.get("schema_version") != "image2299.projected_owner_corridor.v5"
        or receipt.get("status") != TRIGGER_STATUS
        or receipt.get("stop_reason") != TRIGGER_STOP
        or int(receipt.get("accepted_step_count", -1)) != 11
        ...
        or warm_final.get("guard_margin") != TRIGGER_GUARD_MARGIN
        or cold_record.get("guard_margin") != TRIGGER_GUARD_MARGIN
        ...
        or int(last.get("attempt", -1)) != 12
        or int(last.get("accepted_steps_before", -1)) != 11
        ...
        raise ProjectedCorridorHold("HOLD: v5 trigger state/checkpoint/surface identity drifted")
    for item in panel:
        ...
        if (
            ...
            or not bool(item.get("guard_preserved"))
            or bool(item.get("promotion")) or bool(item.get("corridor"))
            or bool(item.get("specimen_identity"))
            or failed_evaluation.get("exact_target_prefix") != 40
            or failed_evaluation.get("hard_counter_count") != 0
            or failed_evaluation.get("matched_target_owner_count") != 33
            or state.get("generated_token_ids_sha256") != FAILED_CANDIDATE_SHA256
            or token_ids_sha256(failed_tokens) != FAILED_CANDIDATE_SHA256
            or base._exact_prefix(clean_parent_tokens, failed_tokens) != 40
            or {owner for owner in owners if owner not in failed_matched} != set(FAILED_LOST_OWNER_IDS)
            or {owner for owner in failed_matched if owner not in owners} != set(FAILED_GAINED_OWNER_IDS)
            ...
            or failed_tokens[4 * base.ROW_TOKENS:5 * base.ROW_TOKENS] != list(GUARD_SPECS[1]["failed_row"])
            or failed_tokens[6 * base.ROW_TOKENS:7 * base.ROW_TOKENS] != list(GUARD_SPECS[0]["current_row"])
            or failed_tokens[21 * base.ROW_TOKENS:22 * base.ROW_TOKENS] != list(GUARD_SPECS[2]["failed_row"])
        ):
            raise ProjectedCorridorHold("HOLD: v5 rejected radius panel identity drifted")
    _guard_bindings(tokens)
    return {
        "path": str(TRIGGER_RECEIPT), "sha256": TRIGGER_SHA256,
        "status": TRIGGER_STATUS, "stop_reason": TRIGGER_STOP,
        "accepted_step_count": 11, "start_checkpoint": str(START_CHECKPOINT),
        ...
        "failed_candidate": {
            "attempt": 12, "ranks": list(range(WORLD_SIZE)), "radii": list(RADII), "lcp": 40,
            ...
            "rows": {
                "4": list(GUARD_SPECS[1]["failed_row"]),
                "6": list(GUARD_SPECS[0]["current_row"]),
                "21": list(GUARD_SPECS[2]["failed_row"]),
            },
        },
    }
...
def _guard_bindings(tokens: Sequence[int]) -> list[dict[str, Any]]:
    ...
    if token_ids_sha256(natural) != NATURAL_TOKEN_SHA256:
        raise ProjectedCorridorHold("HOLD: exact guard teacher identity drifted")
    guards = []
    for spec in GUARD_SPECS:
        start = int(spec["row_index"]) * base.ROW_TOKENS
        position = int(spec["position"])
        if (
            natural[start:start + base.ROW_TOKENS] != list(spec["current_row"])
            ...
            raise ProjectedCorridorHold(f"HOLD: {spec['guard_id']} exact Parent-prefix guard drifted")
        guards.append({
            ...
            "current_row": list(spec["current_row"]), "failed_row": list(spec["failed_row"]),
            "earliest_divergence_position": position,
            "real_prefix_sha256": spec["real_prefix_sha256"],
            "teacher_tokens": natural, "teacher_tokens_sha256": NATURAL_TOKEN_SHA256,
        })
    return guards


def _target_binding(tokens: Sequence[int]) -> dict[str, Any]:
    token_sha256 = token_ids_sha256(tokens)
    pair = parallel._frozen_pair(tokens, parallel.ALIAS_TOKENS)
    if token_sha256 == NATURAL_TOKEN_SHA256:
        expected = (TARGET_POSITION, TARGET_GOOD, TARGET_BAD, TARGET_TEACHER_SHA256)
        state = "unmatched_tail"
    elif token_sha256 == CLEAN_PARENT_TOKEN_SHA256:
        expected = (297, parallel.ALIAS_TOKENS[0], base.EOS, pair["teacher_tokens_sha256"])
        state = "clean_parent33"
    else:
        raise ProjectedCorridorHold("HOLD: target teacher is outside the two exact on-policy specimens")
    if (
        pair["pair_position"], pair["good_token_id"], pair["bad_token_id"],
        pair["teacher_tokens_sha256"],
    ) != expected:
        raise ProjectedCorridorHold("HOLD: dynamic target teacher pair identity drifted")
    pair["specimen"] = state
...
def _project_direction(
    ...
    guard_gradients: Sequence[Sequence[torch.Tensor]],
) -> tuple[list[torch.Tensor], dict[str, Any]]:
    if len(guard_gradients) != len(GUARD_SPECS):
        raise ProjectedCorridorHold("HOLD: projection requires exactly three guard gradients")
    names = ("target_loss", "G0_margin", "G1_margin", "G2_margin")
    gradients = [target_gradient, *guard_gradients]
    ...
    raw = [
        (-units[0][index] + units[1][index] + units[2][index] + units[3][index]).detach()
    ...
    if (
        ...
        or any(derivatives[f"G{index}_margin"] <= 0.0 for index in range(3))
    ):
        raise ProjectedCorridorHold("HOLD: balanced direction does not improve target and all guards")
    return direction, {
        "mode": "equal_angular_target_plus_three_guards",
...
def _select_candidate(
    ...
    eligible = [
        ...
        and dict(item.get("state", {})).get("generated_token_ids_sha256")
        in {NATURAL_TOKEN_SHA256, CLEAN_PARENT_TOKEN_SHA256}
        ...
        and all(
            ...
            and float(item["guard_margins"][guard_id]) > float(base_guard_margins[guard_id])
...
def _teacher_margins(
    *, model: Any, native_inputs: Mapping[str, Any], pad: int,
    ...
    if (
        len(guard_pairs) != 3
        or {str(pair.get("guard_id")) for pair in guard_pairs} != {"G0", "G1", "G2"}
        or len({pair.get("teacher_tokens_sha256") for pair in guard_pairs}) != 1
    ):
        raise ProjectedCorridorHold("HOLD: exact three-guard teacher set drifted")
...
def _cold_evaluate(
    ...
    target_pair: Mapping[str, Any], guard_pairs: Sequence[Mapping[str, Any]], label: str,
) -> dict[str, Any]:
...
def run(*, run_id: str) -> Path:
    ...
    try:
        ...
        guard_pairs = _guard_bindings(trigger["natural_token_ids"])
        ...
        with open_backend_session(setup["frontend"].launch) as opened:
            ...
            baseline_target_margin, baseline_guard_margins = _teacher_margins(
                model=model, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id),
                target_pair=initial_target_pair, guard_pairs=guard_pairs,
            )
            if (
                baseline_target_margin != TARGET_MARGIN
                or baseline_guard_margins.get("G0") != TRIGGER_GUARD_MARGIN
                or set(baseline_guard_margins) != {"G0", "G1", "G2"}
            ...
            for attempt in range(1, MAX_ACCEPTED_STEPS + 1):
                ...
                target_pair = _target_binding(current["evaluation"]["generated_token_ids"])
                ...
                raw_guard_gradients = [
                    torch.autograd.grad(
                        guard_margin_tensors[str(pair["guard_id"])], parameters,
                        allow_unused=True, retain_graph=index < len(guard_pairs) - 1,
                    )
                    for index, pair in enumerate(guard_pairs)
                ]
                del guard_logits
                target_gradient: list[torch.Tensor] = []
                guard_gradients: list[list[torch.Tensor]] = [[] for _ in guard_pairs]
                for parameter_index, target_value in enumerate(target_gradients):
                    values = [target_value, *(gradient[parameter_index] for gradient in raw_guard_gradients)]
                    if any(
                    ...
                    for guard_index, value in enumerate(reduced[1:]):
                        guard_gradients[guard_index].append(value)
                direction, projection = _project_direction(target_gradient, guard_gradients)
                ...
                candidate_summary = {
                    ...
                    "specimen_identity": (
                        candidate_packet["evaluation"]["generated_token_ids_sha256"]
                        in {NATURAL_TOKEN_SHA256, CLEAN_PARENT_TOKEN_SHA256}
                    ),
            ...
            warm_target_margin, warm_guard_margins = _teacher_margins(
                model=model, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id),
                target_pair=terminal_target_pair, guard_pairs=guard_pairs,
            )
            if accepted_steps == 0:
        ...
        cold = base._rank0_call(lambda: _cold_evaluate(
            ...
            target_pair=terminal_target_pair, guard_pairs=guard_pairs, label="projected-corridor-cold",
        ))
        if (
        ...
        receipt = {
            ...
            "protocol": {
                ...
                "projection": "unit target-loss descent plus three unit guard-margin ascents, summed then renormalized once",
                "selection": "proper Parent superset with zero debt before auxiliaries; otherwise largest exact-specimen radius strictly improving target and all three guards",
                "fixed_specimen": "non-promotion steps may alternate only between exact Parent33+EOS and exact Parent33+observed-tail+EOS",
                "acceptance": "match-level proper Parent superset only; never canonical-token identity",
                "dynamic_guards": False, "fixed_guard_count": 3,
            },
