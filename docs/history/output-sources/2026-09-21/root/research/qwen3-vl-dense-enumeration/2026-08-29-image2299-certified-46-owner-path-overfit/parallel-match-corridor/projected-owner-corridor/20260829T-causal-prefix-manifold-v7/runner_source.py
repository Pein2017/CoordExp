#!/usr/bin/env python3
"""Eight-way DoRA causal-prefix corridor search with three owner guards."""
...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v7"
OUTPUT_ROOT = parallel.OUTPUT_ROOT / "projected-owner-corridor"
TRIGGER_RECEIPT = (
    OUTPUT_ROOT / "20260829T-multi-guard-balanced-v6" / "receipt.json"
)
TRIGGER_SHA256 = "f0088c6339adb209fcb9ab52faf7a111ada0c1b8b416e8e53f578ee2a20498c1"
TRIGGER_SOURCE = TRIGGER_RECEIPT.parent / "runner_source.py"
TRIGGER_SOURCE_SHA256 = "fbd8ddfb9ff94bdbaa065880ea48d55bdf63598a6aca37d9aaf8b32a47bb4482"
...
TRIGGER_START_CHECKPOINT = (
    OUTPUT_ROOT / "20260829T-two-specimen-balanced-v5" / "checkpoint-terminal-step-11"
)
...
TRIGGER_INITIAL_SURFACE_SHA256 = "1d611df1925267ea8b5a14a5e56d38eeb228b5916e99825686273138ac5bedb7"
START_SURFACE_SHA256 = "a0b9add91f6e7b07e81175b1c4d98ccef97de8bcd112658effa16e9156c1c3ef"
...
REJECTED_CANDIDATE_SHA256 = "3548b24ea7257c14c63e2712df9cb0e644a19f22e6a1865c434d351c7b957b9a"
REJECTED_LCP = 223
REJECTED_TOKEN_DIFFS = ((223, 152059, 152050), (232, 152228, 152201))
OBSERVED_TAIL = (151646, 48731, 151647, 151648, 152596, 152097, 152617, 152138, 151649)
WORLD_SIZE = 8
MAX_ACCEPTED_STEPS = 14
PRIOR_ACCEPTED_STEPS = 53
...
def _trigger() -> dict[str, Any]:
    ...
    last = dict(attempts[-1]) if len(attempts) == 7 else {}
    ...
    initial_pair = dict(bindings.get("initial_target_pair", {}))
    source = dict(receipt.get("runner_source_snapshot", {}))
    if (
        receipt.get("schema_version") != "image2299.projected_owner_corridor.v6"
        ...
        or (
            initial_pair.get("pair_position"), initial_pair.get("good_token_id"),
            initial_pair.get("bad_token_id"), initial_pair.get("teacher_tokens_sha256"),
        ...
        or int(last.get("attempt", -1)) != 7
        or int(last.get("accepted_steps_before", -1)) != 6
        ...
        raise ProjectedCorridorHold("HOLD: v6 trigger state/checkpoint/surface identity drifted")
    ...
    for item in panel:
        ...
        token_diffs = [
            ...
            for index, (incumbent, candidate) in enumerate(zip(tokens, failed_tokens, strict=False))
        ...
        if (
            ...
            or bool(item.get("promotion")) or not bool(item.get("corridor"))
            or bool(item.get("specimen_identity"))
            or failed_evaluation.get("exact_target_prefix") != REJECTED_LCP
            or failed_evaluation.get("hard_counter_count") != evaluation.get("hard_counter_count")
            or failed_evaluation.get("matched_target_owner_count") != 33
            or not bool(failed_evaluation.get("all_parent_owners_retained"))
            ...
            or len(failed_tokens) != len(tokens)
            or base._exact_prefix(tokens, failed_tokens) != REJECTED_LCP
            or token_diffs != list(REJECTED_TOKEN_DIFFS)
            or set(failed_matched) != set(owners)
            or failed_joint.get("hard_raw_counters") != joint.get("hard_raw_counters")
            or failed_joint.get("raw_counters") != joint.get("raw_counters")
            or failed_causal.get("causal_hard_counter_count") != causal.get("causal_hard_counter_count")
            or failed_causal.get("locked_owner_ids") != causal.get("locked_owner_ids")
            or dict(state.get("corridor_gate", {})) != dict(warm.get("corridor_gate", {}))
            or failed_promotion.get("added_owner_ids") != []
            or failed_promotion.get("matched_owner_ids") != failed_matched
            or failed_tokens[24 * base.ROW_TOKENS:25 * base.ROW_TOKENS]
            != [151646, 8987, 151647, 151648, 152386, 151762, 152459, 152050, 151649]
            or failed_tokens[25 * base.ROW_TOKENS:26 * base.ROW_TOKENS]
            != [151646, 8987, 151647, 151648, 152400, 151987, 152488, 152201, 151649]
        ):
            raise ProjectedCorridorHold("HOLD: v6 rejected radius panel identity drifted")
        rejected_tokens = failed_tokens
    if rejected_tokens is None:
        raise ProjectedCorridorHold("HOLD: v6 rejected radius panel absent")
    prefix_gate = _causal_prefix_gate(rejected_tokens, tokens)
    if not prefix_gate["passed"]:
        raise ProjectedCorridorHold("HOLD: v6 rejected route is not on the causal-prefix manifold")
    ...
    return {
        ...
        "rejected_candidate": {
            "attempt": 7, "ranks": list(range(WORLD_SIZE)), "radii": list(RADII),
            "lcp": REJECTED_LCP, "token_ids_sha256": REJECTED_CANDIDATE_SHA256,
            "token_ids": rejected_tokens,
            "token_diffs": [list(item) for item in REJECTED_TOKEN_DIFFS],
            "matched_owner_ids": sorted(owners), "causal_prefix_gate": prefix_gate,
        },
    }
    ...
    try:
        ...
        receipt = {
            ...
            "protocol": {
                ...
                "selection": "proper Parent superset before auxiliaries; otherwise largest corridor-feasible causal-prefix radius strictly improving the current target and all guards",
                "causal_prefix_manifold": f"tokens 0..{MAX_GUARD_POSITION} equal the immutable v6 incumbent; later tokens may vary",
                "acceptance": "match-level proper Parent superset only; never canonical-token identity",
                "dynamic_teachers": True, "fixed_guard_count": 3,
            },
