#!/usr/bin/env python3
"""Eight-way DoRA corridor search with a dynamic coordinate-owner guard."""
...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v18"
OUTPUT_ROOT = parallel.OUTPUT_ROOT / "projected-owner-corridor"
TRIGGER_RECEIPT = (
    OUTPUT_ROOT / "20260830T-behavioral-person-gate-v17" / "receipt.json"
)
TRIGGER_SHA256 = "4e356af58356210c346b89f8232587cee6324a12fda347817a14d33187694f79"
TRIGGER_SOURCE = TRIGGER_RECEIPT.parent / "runner_source.py"
TRIGGER_SOURCE_SHA256 = "33afcb871fd45a86f7c278b67a837da83b2a145d99ed4b5ad3fbbc2a657a5b8e"
...
START_CHECKPOINT = (
    OUTPUT_ROOT / "20260830T-tail-person-stabilizer-v16" / "checkpoint-terminal-step-05"
)
...
GUARD_SPECS = (
    ...
    {
        "guard_id": "G6", "row_index": 25, "owner": "gt:2299:17",
        "observed_owner": "gt:2299:32", "cascade_gained_owner_ids": ("gt:2299:32",),
        "position": 229, "bad_token_id": 152399,
        "failed_row": (151646, 8987, 151647, 151648, 152399, 152094, 152472, 152441, 151649),
    },
)
ACTIVE_GUARD_IDS = ("G5", "G6")
FEASIBILITY_GUARD_IDS = ("G0", "G1", "G2", "G3", "G4", "G5")
...
def _trigger() -> dict[str, Any]:
    ...
    warm_record, cold_record = map(dict, (receipt.get("warm_final", {}), receipt.get("cold", {})))
    warm, cold = map(dict, (warm_record.get("packet", {}), cold_record.get("packet", {})))
    evaluation, cold_evaluation = map(dict, (warm.get("evaluation", {}), cold.get("evaluation", {})))
    causal, cold_causal = map(dict, (warm.get("causal_ledger", {}), cold.get("causal_ledger", {})))
    ...
    legacy_guard_gate = {
        "guard_ids": expected_guard_gate["guard_ids"][:6],
        "bindings": expected_guard_gate["bindings"][:6],
        "lcp_with_incumbent": expected_guard_gate["lcp_with_incumbent"],
        "passed": True,
    }
    ...
    if (
        receipt.get("schema_version") != "image2299.projected_owner_corridor.v17"
        ...
        or saved.get("readback") != {"reused_predecessor": True}
        ...
        or bindings.get("natural_token_ids_sha256") != NATURAL_TOKEN_SHA256
        or current_pair != _target_binding(tokens)
        or guard_identities(current_guards) != guard_identities(expected_guards[:6])
        or (
        ...
        or dict(evaluation.get("joint_gate", {})).get("hard_raw_counters")
        != WORKING_UNSUPPORTED_PERSON_HARD_COUNTERS
        or causal.get("first_event", {}).get("kind") != "duplicate"
        or list(map(str, causal.get("locked_owner_ids", ())))[25:26] != ["gt:2299:17"]
        ...
        or cold_evaluation.get("generated_token_ids_sha256") != NATURAL_TOKEN_SHA256
        or cold_causal.get("first_event") != causal.get("first_event")
        ...
        or warm_record.get("observed_bad_token_guard_gate") != legacy_guard_gate
        or cold_record.get("observed_bad_token_guard_gate") != legacy_guard_gate
        or not expected_guard_gate["passed"]
        or cold_record.get("target_pair") != current_pair
        or guard_identities(cold_record.get("guard_pairs", ())) != guard_identities(expected_guards[:6])
        or initial_surface.get("aggregate_sha256") != START_SURFACE_SHA256
        ...
        raise ProjectedCorridorHold("HOLD: v17 trigger state/checkpoint/surface/cold identity drifted")

    rejected = attempts[0]
    rejected_panel = list(rejected.get("candidate_panel", ()))
    rejected_by_rank = {int(item.get("rank", -1)): item for item in rejected_panel}
    current_row = [151646, 8987, 151647, 151648, 152400, 151987, 152488, 152201, 151649]
    swapped_row = [151646, 8987, 151647, 151648, 152399, 152094, 152472, 152441, 151649]
    expected_swapped_owners = set(owners) - {"gt:2299:17"} | {"gt:2299:32"}
    if (
        tokens[225:234] != current_row
        or int(rejected.get("attempt", -1)) != 1
        or int(rejected.get("accepted_steps_before", -1)) != 0
        ...
        or any(
            int(item.get("candidate_lcp", -1)) != 229
            or not bool(item.get("target_margin_improved"))
            or float(item.get("base_target_margin", math.nan)) != TARGET_MARGIN
            or not math.isfinite(float(item.get("target_margin", math.nan)))
            or float(item["target_margin"]) <= TARGET_MARGIN
            or bool(item.get("promotion")) or bool(item.get("corridor"))
            or not bool(dict(item.get("observed_bad_token_guard_gate", {})).get("passed"))
            or list(map(int, item.get("state", {}).get("generated_token_ids", ())))[225:234]
            != swapped_row
            or list(map(int, item.get("state", {}).get("generated_token_ids", ())))[
                STABILIZER_POSITION:STABILIZER_POSITION + 1
            ] != [STABILIZER_PERSON_TOKEN]
            or set(map(str, item.get("state", {}).get("evaluator", {}).get("matched_target_owner_ids", ())))
            != expected_swapped_owners
            or list(map(str, item.get("state", {}).get("causal", {}).get("locked_owner_ids", ())))[
                25:26
            ] != ["gt:2299:32"]
            or item.get("state", {}).get("causal", {}).get("first_event", {}).get("kind")
            != "duplicate"
            or item.get("state", {}).get("causal", {}).get("raw_person_duplicate") is not False
            or _observed_bad_token_guard_gate(
                item.get("state", {}).get("generated_token_ids", ()), tokens,
            )["passed"]
            for item in rejected_panel
        )
    ):
        raise ProjectedCorridorHold("HOLD: v17 rejected gt17-to-gt32 panel drifted")
    return {
        "path": str(TRIGGER_RECEIPT), "sha256": TRIGGER_SHA256,
        "status": TRIGGER_STATUS, "stop_reason": TRIGGER_STOP,
        "accepted_step_count": 0, "attempt_count": 1, "start_checkpoint": str(START_CHECKPOINT),
        ...
        "predecessor_bindings": bindings, "cold_reproduced": True,
        "rejected_panel_count": WORLD_SIZE,
        "rejected_swap": {"lcp": 229, "lost_owner": "gt:2299:17", "gained_owner": "gt:2299:32"},
...
def _project_direction(
    ...
    g6_gradient: Sequence[torch.Tensor] | None = None,
    ...
    *, pair_position: int,
    ...
    if stabilizer_gradient is not None:
        raise ProjectedCorridorHold("HOLD: tail-person stabilizer is diagnostic only")
    if pair_position == LEXICAL_TARGET_POSITION:
        if g5_gradient is None or g6_gradient is not None:
            raise ProjectedCorridorHold("HOLD: lexical target requires only the G5 gradient")
        auxiliary_gradient, auxiliary_id = g5_gradient, "G5_margin"
    else:
        if g5_gradient is not None or g6_gradient is None:
            raise ProjectedCorridorHold("HOLD: coordinate target forbids G5 and requires G6")
        auxiliary_gradient, auxiliary_id = g6_gradient, "G6_margin"
    ...
    return direction, {
        "mode": f"normalized_unit_target_descent_plus_unit_{auxiliary_id}_ascent",
...
def _select_candidate(
    ...
    if (
        ...
        or base_target_position < LEXICAL_TARGET_POSITION
        or any(
        ...
        or not math.isfinite(float(base_guard_margins["G6"]))
    ):
    ...
    active_guard_id = "G5" if base_target_position == LEXICAL_TARGET_POSITION else "G6"
    eligible = [
        ...
        == STABILIZER_PERSON_TOKEN
        and math.isfinite(float(item["frozen_guard_margins"].get(active_guard_id, math.nan)))
        and float(item["frozen_guard_margins"][active_guard_id])
        > float(base_guard_margins[active_guard_id])
...
def _teacher_margins(
    *, model: Any, native_inputs: Mapping[str, Any], pad: int,
    ...
    if (
        ...
        raise ProjectedCorridorHold("HOLD: exact seven-guard teacher set drifted")
    ...
    try:
        ...
        with open_backend_session(setup["frontend"].launch) as opened:
            ...
            if (
                ...
                or {
                    ...
                    for guard_id in TRIGGER_GUARD_MARGINS
                } != TRIGGER_GUARD_MARGINS
                ...
                or not all(baseline_guard_margins[guard_id] > 0.0 for guard_id in FEASIBILITY_GUARD_IDS)
            ...
            for attempt in range(1, MAX_ACCEPTED_STEPS + 1):
                ...
                active_guard_id = "G5" if pair_position == LEXICAL_TARGET_POSITION else "G6"
                active_guard_gradients = torch.autograd.grad(
                    guard_margin_tensors[active_guard_id], parameters, allow_unused=True,
                )
                reduced_gradients: dict[str, list[torch.Tensor]] = {}
                gradient_sets = [
                    ("target_loss", target_gradients),
                    (f"{active_guard_id}_margin", active_guard_gradients),
                ]
                ...
                direction, projection = _project_direction(
                    ...
                    reduced_gradients.get("G6_margin"), pair_position=pair_position,
                )
                ...
                candidate_summary = {
                    ...
                    "active_guard_improved": (
                        math.isfinite(float(candidate_frozen_guard_margins.get(active_guard_id, math.nan)))
                        and float(candidate_frozen_guard_margins[active_guard_id])
                        > float(base_guard_margins[active_guard_id])
                    ),
                    "active_guard_id": active_guard_id,
                    "guards_improved": {
                        guard_id: (
        ...
        receipt = {
            ...
            "protocol": {
                ...
                "projection": "at lexical position 298, equal unit target-loss descent plus dynamic G5 ascent; at coordinate positions >298, equal unit target-loss descent plus dynamic G6 ascent; both directional derivatives must have the required strict sign",
                "selection": "proper Parent superset first; otherwise largest working-corridor radius passing all seven observed-bad-token guards, strictly improving the frozen-current target and the position-specific frozen-current guard G5/G6, keeping natural token 8987 at position 298, and keeping all seven candidate-on-policy guards positive; stabilizer margin is diagnostic only",
                ...
                "dynamic_teachers": True, "fixed_guard_count": 7,
                "active_guard_ids": list(ACTIVE_GUARD_IDS),
                "feasibility_guard_ids": list(FEASIBILITY_GUARD_IDS),
                "gradient_objectives": {
                    "lexical_position_298": ["target_loss", "G5_margin"],
                    "post_description": ["target_loss", "G6_margin"],
                },
