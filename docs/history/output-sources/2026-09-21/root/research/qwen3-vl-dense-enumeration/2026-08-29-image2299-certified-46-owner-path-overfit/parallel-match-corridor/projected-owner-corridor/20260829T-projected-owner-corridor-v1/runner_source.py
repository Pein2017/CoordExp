#!/usr/bin/env python3
"""Eight-way DoRA corridor search projected against the observed row-15 failure."""
...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v1"
OUTPUT_ROOT = parallel.OUTPUT_ROOT / "projected-owner-corridor"
TRIGGER_RECEIPT = (
    parallel.OUTPUT_ROOT / "20260829T-parallel-match-corridor-v1c" / "receipt.json"
)
TRIGGER_SHA256 = "1a53a9888e3af61f4bafb8e8af0077199b04ba2ab7fd6b16ed80dbdce9c58f16"
TRIGGER_STATUS = "bounded_negative_no_match_level_promotion"
TRIGGER_STOP = "no_feasible_margin_improving_radius"
START_CHECKPOINT = TRIGGER_RECEIPT.parent / "checkpoint-terminal-step-36"
...
GUARD_POSITION, GUARD_GOOD, GUARD_BAD = 141, 152183, 152178
GUARD_ROW_INDEX, GUARD_OWNER = 15, "gt:2299:26"
GUARD_ROW = (151646, 8987, 151647, 151648, 152103, 152078, 152183, 152275, 151649)
FAILED_GUARD_ROW = (151646, 8987, 151647, 151648, 152103, 152078, 152178, 152275, 151649)
OBSERVED_TAIL = (151646, 48731, 151647, 151648, 152596, 152097, 152617, 152138, 151649)
WORLD_SIZE = 8
MAX_ACCEPTED_STEPS = 14
RADII = parallel.RADII
GUARD_DROP_TOLERANCE = 1e-6
...
def _trigger() -> dict[str, Any]:
    ...
    last = list(receipt.get("attempts", ()))
    last = dict(last[-1]) if last else {}
    panel = list(last.get("candidate_panel", ()))
    rank7 = next((dict(item) for item in panel if int(item.get("rank", -1)) == 7), {})
    failed_event = dict(dict(dict(rank7.get("state", {})).get("causal", {})).get("first_event", {}))
    pair = dict(last.get("frozen_pair", {}))
    ...
    if (
        ...
        or int(receipt.get("accepted_step_count", -1)) != 36
        ...
        or pair.get("pair_position") != TARGET_POSITION
        or pair.get("good_token_id") != TARGET_GOOD
        or pair.get("bad_token_id") != TARGET_BAD
        or pair.get("teacher_tokens_sha256") != TARGET_TEACHER_SHA256
        or float(last.get("base_frozen_pair_margin", math.nan)) != TARGET_MARGIN
        or int(last.get("attempt", -1)) != 37
        or float(rank7.get("radius", math.nan)) != RADII[7]
        or failed_event.get("kind") != "near_miss"
        or int(failed_event.get("event_start", -1)) != GUARD_ROW_INDEX * base.ROW_TOKENS
        or int(failed_event.get("row_index", -1)) != GUARD_ROW_INDEX
        or failed_event.get("near_miss_owner") != GUARD_OWNER
        or list(map(int, failed_event.get("bad_span_tokens", ()))) != list(FAILED_GUARD_ROW)
    ):
        raise ProjectedCorridorHold("HOLD: trigger does not bind the terminal Parent33 guard specimen")
    ...
    if (
        ...
        raise ProjectedCorridorHold("HOLD: guard rows do not first diverge at absolute position 141")
    return {
        "path": str(TRIGGER_RECEIPT), "sha256": TRIGGER_SHA256,
        "status": TRIGGER_STATUS, "stop_reason": TRIGGER_STOP,
        "accepted_step_count": 36, "start_checkpoint": str(START_CHECKPOINT),
...
def _select_candidate(
    ...
    eligible = [
        ...
        and float(item["guard_margin"]) >= base_guard_margin - GUARD_DROP_TOLERANCE
    ]
        ...
        with open_backend_session(setup["frontend"].launch) as opened:
            ...
            for attempt in range(1, MAX_ACCEPTED_STEPS + 1):
                ...
                candidate_summary = {
                    ...
                    "guard_preserved": candidate_guard_margin >= base_guard_margin - GUARD_DROP_TOLERANCE,
                    "specimen_identity": (
