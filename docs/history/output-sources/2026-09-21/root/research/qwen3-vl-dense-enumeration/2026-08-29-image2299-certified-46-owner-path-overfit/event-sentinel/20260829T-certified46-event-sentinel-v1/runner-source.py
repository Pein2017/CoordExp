...
EVENT_SENTINEL_TRIGGER_RECEIPT = OUTPUT_ROOT / "frontier" / "20260829T-certified46-frontier-v1" / "receipt.json"
EVENT_SENTINEL_TRIGGER_SHA256 = "515967d91521a8b6337b7c69ffb467c0a9761458a610d8ed186c9ecfa1cd3a99"
EVENT_SENTINEL_TRIGGER_STATUS = "phase2_negative_bounded"
EVENT_SENTINEL_TARGET_OWNER = "gt:2299:32"
EVENT_SENTINEL_EVENT_START = 297
EVENT_SENTINEL_RADII = (1, 2, 4, 8, 16, 32, 64)
...
def _event_sentinel_trigger() -> dict[str, Any]:
    if (
        not EVENT_SENTINEL_TRIGGER_RECEIPT.is_file()
        or _sha256(EVENT_SENTINEL_TRIGGER_RECEIPT) != EVENT_SENTINEL_TRIGGER_SHA256
    ):
        raise Certified46Hold("HOLD: Phase-2 event-sentinel trigger receipt identity drifted")
    receipt = json.loads(EVENT_SENTINEL_TRIGGER_RECEIPT.read_text(encoding="utf-8"))
    if (
        receipt.get("status") != EVENT_SENTINEL_TRIGGER_STATUS
        or receipt.get("stop_reason") != "same_first_causal_event_three_independent_branches"
        or receipt.get("final_champion_checkpoint") != str(PARENT_CHECKPOINT)
        or len(receipt.get("final_champion_owner_ids", ())) != 33
    ):
        raise Certified46Hold("HOLD: Phase-2 receipt does not authorize the event sentinel")
    return {
        "path": str(EVENT_SENTINEL_TRIGGER_RECEIPT),
        "sha256": EVENT_SENTINEL_TRIGGER_SHA256,
        "status": EVENT_SENTINEL_TRIGGER_STATUS,
        "stop_reason": "same_first_causal_event_three_independent_branches",
        "champion_checkpoint": str(PARENT_CHECKPOINT),
        "champion_owner_count": 33,
    }
...
def _event_sentinel_plan(*, plan: Mapping[str, Any], parent_tokens: Sequence[int]) -> dict[str, Any]:
    """Freeze the one causal EOS-to-target-token contrast selected by the frontier scorer."""
    event = plan.get("event")
    teacher = list(map(int, plan.get("teacher_tokens", ())))
    target_row = list(map(int, plan.get("target_row_tokens", ())))
    if (
        not isinstance(event, Mapping)
        or event.get("kind") != "premature_eos"
        or int(event.get("event_start", -1)) != EVENT_SENTINEL_EVENT_START
        or list(map(int, event.get("bad_span_tokens", ()))) != [EOS]
        or str(plan.get("target_owner")) != EVENT_SENTINEL_TARGET_OWNER
        or len(target_row) != ROW_TOKENS
        or teacher != [*map(int, parent_tokens[:EVENT_SENTINEL_EVENT_START]), *target_row]
        or int(plan.get("span_start", -1)) != EVENT_SENTINEL_EVENT_START
    ):
        raise Certified46Hold("HOLD: event sentinel scorer plan drifted from Parent EOS/gt32 event")
    return {
        "target_owner": EVENT_SENTINEL_TARGET_OWNER,
        "event_start": EVENT_SENTINEL_EVENT_START,
        "teacher_tokens": teacher,
        "target_row_tokens": target_row,
        "target_token_id": target_row[0],
        "bad_token_id": EOS,
        "supervision_positions": [EVENT_SENTINEL_EVENT_START],
    }


def _apply_event_radius(
    *, parameters: Sequence[torch.nn.Parameter], originals: Sequence[torch.Tensor], direction: Sequence[torch.Tensor], multiplier: int,
) -> None:
    if multiplier not in EVENT_SENTINEL_RADII or len(parameters) != len(originals) or len(parameters) != len(direction):
        raise Certified46Hold("HOLD: malformed event-sentinel radius application")
    with torch.no_grad():
        for parameter, original, gradient in zip(parameters, originals, direction, strict=True):
            if parameter.shape != original.shape or parameter.shape != gradient.shape:
                raise Certified46Hold("HOLD: event-sentinel direction surface drifted")
            parameter.copy_(original - (LEARNING_RATE * multiplier) * gradient)


def _event_sentinel_decision(trials: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    by_radius = {int(trial["radius_multiplier"]): trial for trial in trials}
    if set(by_radius) != {0, *EVENT_SENTINEL_RADII}:
        raise Certified46Hold("HOLD: event-sentinel radius receipt is incomplete")
    baseline = int(by_radius[0]["target_row_common_prefix"])
    positive = [by_radius[radius] for radius in EVENT_SENTINEL_RADII]
    supported = [
        trial for trial in positive
        if bool(trial["parent_owners_retained"])
        and int(trial["exact_parent_prefix"]) == EVENT_SENTINEL_EVENT_START
        and int(trial["target_row_common_prefix"]) > baseline
    ]
    if supported:
        chosen = min(supported, key=lambda trial: int(trial["radius_multiplier"]))
        return {"decision": "DORA_EVENT_DIRECTION_SUPPORTED", "selected_radius_multiplier": int(chosen["radius_multiplier"])}
    if all(
        not bool(trial["parent_owners_retained"])
        and int(trial["target_row_common_prefix"]) <= baseline
        for trial in positive
    ):
        return {"decision": "TIED_ROW_GATE_OPEN", "selected_radius_multiplier": None}
    return {"decision": "INCONCLUSIVE_SAFE_MARGIN_ONLY", "selected_radius_multiplier": None}
...
def _event_sentinel_direction(
    *, model: Any, native_inputs: Mapping[str, Any], plan: Mapping[str, Any], parameters: Sequence[torch.nn.Parameter], pad: int,
) -> tuple[list[torch.Tensor], dict[str, Any]]:
    for parameter in parameters:
        parameter.grad = None
    teacher = list(map(int, plan["teacher_tokens"]))
    position = int(plan["event_start"])
    logits = full_root._teacher_forced_route_logits(
        model=model, native_inputs=native_inputs, route_tokens=teacher, pad_token_id=pad,
    )
    loss, allocation = _frontier_loss(
        logits, teacher, span_start=position, span_end=position,
        worker_index=dist.get_rank(), worker_count=WORLD_SIZE,
    )
    gradients = torch.autograd.grad(loss, parameters, allow_unused=True)
    for parameter, gradient in zip(parameters, gradients, strict=True):
        if gradient is None or not bool(torch.isfinite(gradient).all().item()):
            raise Certified46Hold("HOLD: disconnected/non-finite event-sentinel gradient")
        parameter.grad = gradient.detach()
        dist.all_reduce(parameter.grad, op=dist.ReduceOp.SUM, group=dist.group.WORLD)
    norm = torch.nn.utils.clip_grad_norm_(parameters, GRAD_CLIP)
    if not bool(torch.isfinite(norm).item()) or float(norm) <= 0:
        raise Certified46Hold("HOLD: event-sentinel gradient is zero/non-finite")
    return [parameter.grad.detach().clone() for parameter in parameters], {
        "loss_contribution": float(loss.detach()), "allocation": allocation,
        "gradient_norm_pre_clip": float(norm), "gradient_clip": GRAD_CLIP,
    }


def _event_sentinel_teacher_margins(
    *, model: Any, native_inputs: Mapping[str, Any], plan: Mapping[str, Any], pad: int,
) -> dict[str, Any]:
    teacher = list(map(int, plan["teacher_tokens"]))
    position, target, bad = int(plan["event_start"]), int(plan["target_token_id"]), int(plan["bad_token_id"])
    with torch.inference_mode():
        logits = full_root._teacher_forced_route_logits(
            model=model, native_inputs=native_inputs, route_tokens=teacher, pad_token_id=pad,
        )
    if logits.ndim != 2 or not (0 <= position < logits.shape[0]) or not bool(torch.isfinite(logits).all().item()):
        raise Certified46Hold("HOLD: event-sentinel teacher logits drifted")
    values = logits[position]
    target_logit = values[target]
    nontarget = values.clone()
    nontarget[target] = -torch.inf
    best_nontarget = int(nontarget.argmax().item())
    return {
        "position": position, "target_token_id": target, "bad_token_id": bad,
        "target_vs_bad_eos_margin": float((target_logit - values[bad]).item()),
        "target_vs_best_nontarget_margin": float((target_logit - nontarget[best_nontarget]).item()),
        "best_nontarget_token_id": best_nontarget,
    }
...
def _event_sentinel_observation(
    *, model: Any, tokenizer: Any, native_inputs: Mapping[str, Any], pad: int, target: Mapping[str, Any],
    raw_example: Any, parent_tokens: Sequence[int], parent_owners: Sequence[str], route: Mapping[str, Any],
    plan: Mapping[str, Any], radius_multiplier: int, label: str,
) -> dict[str, Any]:
    tokens = _natural_tokens(model=model, native_inputs=native_inputs, pad=pad)
    causal = _causal_locked_ledger(tokenizer=tokenizer, tokens=tokens, raw_example=raw_example)
    natural = _natural_evaluation(
        tokenizer=tokenizer, tokens=tokens, target=target, route_tokens=route["tokens"], raw_example=raw_example,
        parent_owners=parent_owners, label=label,
    )
    event = causal.get("first_event")
    event_start = int(plan["event_start"])
    locked = tuple(map(str, causal.get("locked_owner_ids", ())))
    return {
        "radius_multiplier": radius_multiplier,
        "teacher_margins": _event_sentinel_teacher_margins(
            model=model, native_inputs=native_inputs, plan=plan, pad=pad,
        ),
        "natural_token_sha256": token_ids_sha256(tokens),
        "exact_parent_prefix": _exact_prefix(tokens, parent_tokens),
        "causal_locked_owner_ids": list(locked), "causal_locked_owner_count": len(locked),
        "parent_owners_retained": set(parent_owners).issubset(locked),
        "target_row_common_prefix": _exact_prefix(tokens[event_start:], plan["target_row_tokens"]),
        "first_causal_event_kind": None if not isinstance(event, Mapping) else event.get("kind"),
        "first_causal_event_start": None if not isinstance(event, Mapping) else event.get("event_start"),
        "global_hard_counter_count": int(natural["hard_counter_count"]),
        "causal_hard_counter_count": int(causal["causal_hard_counter_count"]),
    }


def _run_event_sentinel_stage(*, stage: str, run_id: str) -> Path:
    rank = dist.get_rank()
    output = OUTPUT_ROOT / stage / run_id
    try:
        _rank0_call(lambda: _prepare_frontier_output(output))
        dist.barrier()
        trigger = _rank0_call(_event_sentinel_trigger)
        _admission, setup, target = _bindings()
        parent_tokens = _parent_tokens()
        with open_backend_session(setup["frontend"].launch) as opened:
            if type(opened) is not HFBackendSession or opened._model is None or opened._tokenizer is None:
                raise Certified46Hold("HOLD: event sentinel requires concrete FP32 HF backend")
            model, tokenizer = opened._model, opened._tokenizer
            model.eval()
            native_inputs, prompts, _grids, _media = opened._materialize_native_inputs(setup["requests"][:1])
            if token_ids_sha256(prompts[0]) != PROMPT_TOKEN_SHA256:
                raise Certified46Hold("HOLD: event-sentinel live prompt identity drifted")
            parent_ledger = _causal_locked_ledger(tokenizer=tokenizer, tokens=parent_tokens, raw_example=setup["raw_example"])
            _assert_parent_causal_ledger(parent_ledger)
            parent_owners = tuple(map(str, parent_ledger["locked_owner_ids"]))
            baseline = _rank0_call(lambda: _phase2_observation(
                model=model, tokenizer=tokenizer, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id), target=target,
                raw_example=setup["raw_example"], parent_owners=parent_owners, rejected_offset=0, route=None,
            ))
            if baseline["generated_token_ids"] != parent_tokens:
                raise Certified46Hold("HOLD: event-sentinel baseline is not exact Parent")
            _assert_parent_causal_ledger(baseline["causal_ledger"])
            frontier = baseline.get("frontier_plan")
            if not isinstance(frontier, Mapping):
                raise Certified46Hold("HOLD: event-sentinel baseline has no causal frontier plan")
            plan = _event_sentinel_plan(plan=frontier, parent_tokens=parent_tokens)
            route = _rank0_call(lambda: _compile_dynamic_route(
                target=target, natural_tokens=parent_tokens, causal_ledger=parent_ledger,
                preferred_owner=EVENT_SENTINEL_TARGET_OWNER,
            ))
            _rank0_call(lambda: _admit_route(
                tokenizer=tokenizer, route=route, target=target, raw_example=setup["raw_example"], label="event-sentinel-route",
            ))
            names, parameters = full_root._trainable_surface(model, dora_all_only=True)
            if len(parameters) != 588 or sum(parameter.numel() for parameter in parameters) != 18_006_016:
                raise Certified46Hold("HOLD: exact 588/18006016 FP32 DoRA surface drifted")
            ...
            try:
                baseline_trial = _rank0_call(lambda: _event_sentinel_observation(
                    model=model, tokenizer=tokenizer, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id), target=target,
                    raw_example=setup["raw_example"], parent_tokens=parent_tokens, parent_owners=parent_owners, route=route,
                    plan=plan, radius_multiplier=0, label="event-sentinel-radius-0",
                ))
                if (
                    baseline_trial["exact_parent_prefix"] != len(parent_tokens)
                    or tuple(baseline_trial["causal_locked_owner_ids"]) != parent_owners
                    or baseline_trial["target_row_common_prefix"] != 0
                ):
                    raise Certified46Hold("HOLD: event-sentinel baseline observation drifted")
                trials.append(baseline_trial)
                direction, direction_record = _event_sentinel_direction(
                    model=model, native_inputs=native_inputs, plan=plan, parameters=parameters, pad=int(tokenizer.pad_token_id),
                )
                _surface_agreement(full_root._full_root_surface_snapshot(names, parameters)[1], dist.group.WORLD)
                full_root._assert_full_root_sentinels(model, sentinels)
                if _frozen_surface(model, names) != frozen_before:
                    raise Certified46Hold("HOLD: event-sentinel gradient mutated frozen surface")
                for multiplier in EVENT_SENTINEL_RADII:
                    full_root._restore_parameters(parameters, originals)
                    if full_root._full_root_surface_snapshot(names, parameters)[1] != initial_surface:
                        raise Certified46Hold("HOLD: event-sentinel radius did not restart at Parent")
                    _apply_event_radius(parameters=parameters, originals=originals, direction=direction, multiplier=multiplier)
                    _surface_agreement(full_root._full_root_surface_snapshot(names, parameters)[1], dist.group.WORLD)
                    full_root._assert_full_root_sentinels(model, sentinels)
                    if _frozen_surface(model, names) != frozen_before:
                        raise Certified46Hold("HOLD: event-sentinel radius mutated frozen surface")
                    trials.append(_rank0_call(lambda multiplier=multiplier: _event_sentinel_observation(
                        model=model, tokenizer=tokenizer, native_inputs=native_inputs, pad=int(tokenizer.pad_token_id), target=target,
                        raw_example=setup["raw_example"], parent_tokens=parent_tokens, parent_owners=parent_owners, route=route,
                        plan=plan, radius_multiplier=multiplier, label=f"event-sentinel-radius-{multiplier}",
                    )))
                    full_root._restore_parameters(parameters, originals)
                    if full_root._full_root_surface_snapshot(names, parameters)[1] != initial_surface:
                        raise Certified46Hold("HOLD: event-sentinel failed to restore Parent after radius")
                    full_root._assert_full_root_sentinels(model, sentinels)
                    if _frozen_surface(model, names) != frozen_before:
                        raise Certified46Hold("HOLD: event-sentinel restore mutated frozen surface")
            finally:
                full_root._restore_parameters(parameters, originals)
            if full_root._full_root_surface_snapshot(names, parameters)[1] != initial_surface:
                raise Certified46Hold("HOLD: event-sentinel exit did not restore Parent DoRA surface")
            full_root._assert_full_root_sentinels(model, sentinels)
            if _frozen_surface(model, names) != frozen_before:
                raise Certified46Hold("HOLD: event-sentinel exit mutated frozen surface")
            runtime = opened.receipt.to_artifact_dict()
        if rank == 0:
            decision = _event_sentinel_decision(trials)
            receipt = {
                "schema_version": "image2299.certified_46_owner_path_overfit.event_sentinel.v1",
                "status": "event_sentinel_complete", "stage": stage, "run_id": run_id, "unit_id": UNIT_ID,
                "runner_sha256": _sha256(Path(__file__)), "phase2_trigger": trigger,
                "bindings": {"parent_checkpoint": str(PARENT_CHECKPOINT), "parent_route_sha256": PARENT_ROUTE_SHA256, "target": str(TARGET_PATH), "target_sha256": TARGET_SHA256},
                "protocol": {"surface": "588 FP32 DoRA tensors / 18006016 elements", "objective": "one-token CE over full vocabulary at Parent position 297", "gradient": "one world-allreduced clipped DoRA direction from Parent", "radii": list(EVENT_SENTINEL_RADII), "radius_application": "each exact Parent snapshot minus multiplier times existing LR direction; no optimizer chain", "checkpoint_saved": False, "promotion": False},
                "parent_owners": list(parent_owners), "initial_surface": initial_surface, "frozen_surface": frozen_before,
                "direction": direction_record, "trials": trials, "decision": decision, "runtime": runtime,
            }
        _rank0_call(lambda: _atomic_json(output / "receipt.json", receipt))
        dist.barrier()
        return output
        ...
        if rank == 0 and output.exists():
            _atomic_json(output / "failure.json", {
                "schema_version": "image2299.certified_46_owner_path_overfit.event_sentinel.v1.failure", "status": "mechanical_failure",
                "stage": stage, "run_id": run_id, "error_type": type(error).__name__, "error": str(error), "traceback": traceback.format_exc(),
            })
        raise


def run_stage(*, stage: str, run_id: str) -> Path:
    if stage not in {"no-update", "one-update", "foreground", "frontier", "phase2", "event-sentinel"}:
    ...
    if stage in {"frontier", "phase2", "event-sentinel"}:
        try:
            return (
                _run_event_sentinel_stage(stage=stage, run_id=run_id)
                if stage == "event-sentinel" else _run_frontier_stage(stage=stage, run_id=run_id)
            )
        finally:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--stage", choices=("no-update", "one-update", "foreground", "frontier", "phase2", "event-sentinel"), required=True)
