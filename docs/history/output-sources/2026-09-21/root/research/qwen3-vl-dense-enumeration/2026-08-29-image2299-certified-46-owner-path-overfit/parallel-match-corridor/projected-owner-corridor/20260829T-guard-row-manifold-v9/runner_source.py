...
SCHEMA_VERSION = "image2299.projected_owner_corridor.v9"
OUTPUT_ROOT = parallel.OUTPUT_ROOT / "projected-owner-corridor"
TRIGGER_RECEIPT = (
    OUTPUT_ROOT / "20260829T-active-g3-release-v8" / "receipt.json"
)
TRIGGER_SHA256 = "dac2d07e9c07a2681ce5c209192fe13e7e82ffcda253938902c0301070a0f3df"
TRIGGER_SOURCE = TRIGGER_RECEIPT.parent / "runner_source.py"
TRIGGER_SOURCE_SHA256 = "13ff88e2bc25408ef1d3f5992a6b3a1d20e6576691387ab40a97baa5a4f936bf"
TRIGGER_STATUS = "bounded_negative_no_match_level_promotion"
TRIGGER_STOP = "no_feasible_target_g3_improving_guard_positive_radius"
START_CHECKPOINT = (
    TRIGGER_RECEIPT.parent / "checkpoint-terminal-step-11"
)
TRIGGER_START_CHECKPOINT = (
    OUTPUT_ROOT / "20260829T-causal-prefix-manifold-v7" / "checkpoint-terminal-step-13"
)
NATURAL_TOKEN_SHA256 = "028c73b65a15eebf124298b7570277b4ff1188aee575e50085654e5066296c3f"
TRIGGER_INITIAL_NATURAL_SHA256 = "c17b74f9cd4145845674a2faa41aa9bebd100acdd9cd9c45b807117b3de05bb8"
CLEAN_PARENT_TOKEN_SHA256 = base.PARENT_ROUTE_SHA256
TRIGGER_INITIAL_SURFACE_SHA256 = "eaa50bc87bd4445083c53d2d545c5557a45a810f48b3f92f9e9bc3e7dae535c0"
START_SURFACE_SHA256 = "34091dab45d5f2d4fbd0477f8e556dc2f29ad355e4d121dc4e916f08ca971f39"
...
TARGET_MARGIN = -1.2520217895507812
TARGET_TEACHER_SHA256 = "d452c804a0d05a7c8d014b4b53c3019241321c612cd24c28fb272b6c9da0ad2c"
TRIGGER_GUARD_MARGINS = {
    "G0": 0.373504638671875,
    "G1": 0.1648387908935547,
    "G2": 0.6965389251708984,
    "G3": 1.742386817932129,
}
GUARD_SPECS = (
    {
...
REJECTED_CANDIDATE_SHA256 = "d3a8f8cb5adca0846e41cadb62a798ad40a33d4542f26bc16da583b158149576"
REJECTED_LCP = 104
REJECTED_ROW_INDEX = 11
INCUMBENT_ROW = (151646, 8987, 151647, 151648, 151987, 151952, 152045, 152169, 151649)
REJECTED_ROW = (151646, 8987, 151647, 151648, 151987, 151939, 152045, 152169, 151649)
...
def _trigger() -> dict[str, Any]:
    ...
    if (
        receipt.get("schema_version") != "image2299.projected_owner_corridor.v8"
        ...
        or [int(item.get("attempt", -1)) for item in attempts] != list(range(1, 13))
        or [int(item.get("accepted_steps_before", -1)) for item in attempts] != list(range(12))
        ...
        raise ProjectedCorridorHold("HOLD: v8 trigger state/checkpoint/surface identity drifted")
    rejected_tokens: list[int] | None = None
    expected_matched = set(map(str, evaluation.get("matched_target_owner_ids", ())))
    expected_hard_count = int(evaluation.get("hard_counter_count", -1))
    expected_corridor = dict(warm.get("corridor_gate", {}))
    if expected_matched != set(owners) or expected_hard_count != 2:
        raise ProjectedCorridorHold("HOLD: v8 incumbent owner/counter identity drifted")
    for item in panel:
        ...
        guard_margins = dict(item.get("guard_margins", {}))
        if (
            rank not in range(WORLD_SIZE)
            or float(item.get("radius", math.nan)) != RADII[rank]
            ...
            or set(guard_margins) != set(FEASIBILITY_GUARD_IDS)
            or not all(math.isfinite(float(value)) and float(value) > 0.0 for value in guard_margins.values())
            or bool(item.get("promotion")) or not bool(item.get("corridor"))
            or bool(dict(item.get("causal_prefix_gate", {})).get("passed"))
            or int(dict(item.get("causal_prefix_gate", {})).get("lcp_with_incumbent", -1)) != REJECTED_LCP
            or int(item.get("candidate_lcp", -1)) != REJECTED_LCP
            or failed_evaluation.get("exact_target_prefix") != REJECTED_LCP
            or failed_evaluation.get("hard_counter_count") != expected_hard_count
            or failed_evaluation.get("matched_target_owner_count") != len(owners)
            ...
            or tokens[REJECTED_ROW_INDEX * base.ROW_TOKENS:(REJECTED_ROW_INDEX + 1) * base.ROW_TOKENS] != list(INCUMBENT_ROW)
            or failed_tokens[REJECTED_ROW_INDEX * base.ROW_TOKENS:(REJECTED_ROW_INDEX + 1) * base.ROW_TOKENS] != list(REJECTED_ROW)
            or [(index, old, new) for index, (old, new) in enumerate(zip(tokens, failed_tokens, strict=True)) if old != new]
            != [(REJECTED_LCP, INCUMBENT_ROW[5], REJECTED_ROW[5])]
            or set(failed_matched) != expected_matched
            or failed_joint.get("hard_raw_counters") != REJECTED_HARD_COUNTERS
            or failed_joint.get("raw_counters") != {**REJECTED_HARD_COUNTERS, "valid_prediction_count": 34}
            or failed_causal.get("causal_hard_counter_count") != 1
            or list(map(str, failed_causal.get("locked_owner_ids", ()))) != owners
            or failed_causal.get("first_event") != causal.get("first_event")
            or dict(state.get("corridor_gate", {})) != expected_corridor
            or failed_promotion.get("added_owner_ids") != []
            or failed_promotion.get("matched_owner_ids") != failed_matched
        ):
            raise ProjectedCorridorHold("HOLD: v8 rejected radius panel identity drifted")
        if not _guard_row_manifold_gate(failed_tokens, tokens)["passed"]:
            raise ProjectedCorridorHold("HOLD: v8 rejected route left the guard-row manifold")
        rejected_tokens = failed_tokens
    if rejected_tokens is None:
        raise ProjectedCorridorHold("HOLD: v8 rejected radius panel absent")
    guard_row_gate = _guard_row_manifold_gate(rejected_tokens, tokens)
    if not guard_row_gate["passed"] or guard_row_gate["lcp_with_incumbent"] != REJECTED_LCP:
        raise ProjectedCorridorHold("HOLD: v8 rejected route is not the row-11 manifold opening")
    ...
    return {
        ...
        "rejected_candidate": {
            "attempt": 12, "ranks": list(range(WORLD_SIZE)), "radii": list(RADII),
            "lcp": REJECTED_LCP, "token_ids_sha256": REJECTED_CANDIDATE_SHA256,
            "token_ids": rejected_tokens, "row_index": REJECTED_ROW_INDEX,
            "incumbent_row": list(INCUMBENT_ROW), "candidate_row": list(REJECTED_ROW),
            "matched_owner_ids": sorted(expected_matched),
            "guard_row_manifold_gate": guard_row_gate,
        },
    }
