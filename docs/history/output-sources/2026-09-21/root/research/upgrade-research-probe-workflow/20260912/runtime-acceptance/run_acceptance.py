...
def main():
    ...
    parser.add_argument('--restored-source-gate', type=Path)
    ...
    try:
        ...
        from probes.logit_lens.base import (_stage_source_gate, SOURCE_GATE_STUDY,
            SOURCE_GATE_STUDY_SHA256, SOURCE_GATE_RECEIPT_SHA256)
        ...
        if args.restored_source_gate is None:
            gate_root, gate = _stage_source_gate(output)
        else:
            restoration = args.restored_source_gate.resolve()
            restoration_receipt = restoration / 'restoration-receipt.json'
            gate_root = output / 'source-gate'
            gate_files = []
            for relative, expected in (
                (SOURCE_GATE_STUDY, SOURCE_GATE_STUDY_SHA256),
                (Path('outputs/probes/coordexp_swift/special_token_embeddings_roundtrip/receipt.json'), SOURCE_GATE_RECEIPT_SHA256),
            ):
                original_gate_file = restoration / relative
                assert digest_file(original_gate_file) == expected
                destination = gate_root / relative
                destination.parent.mkdir(parents=True, exist_ok=True)
                shutil.copyfile(original_gate_file, destination)
                assert digest_file(destination) == expected
                gate_files.append(dict(source=str(original_gate_file), staged=str(destination), sha256=expected))
            gate = dict(root=str(gate_root), files=gate_files,
                restoration_receipt=str(restoration_receipt), restoration_receipt_sha256=digest_file(restoration_receipt))
