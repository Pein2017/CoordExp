#!/usr/bin/env python3
"""Small paired diagnostic for dynamic HF and materialized BF16 HF detections.
...
The pair matcher is class-aware and one-to-one: all same-class candidates are
sorted by descending pixel IoU, then dynamic and materialized prediction index,
and greedily consumed.  Thus a changed output sequence never causes a
positional zip of boxes.
"""
...
def _pair_predictions(
    ...
    for left_index in left_valid_indices:
        ...
        for right_index in right_valid_indices:
            ...
            if right_box is None:
                continue
            candidates.append((_iou(left_box, right_box), left_index, right_index))
    ...
    return {
        "matching_algorithm": (
            "same normalized class; all valid pixel-box candidates sorted by "
...
def _paired_score_stats(matches: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for index, left in enumerate(valid_matches):
        for right in valid_matches[index + 1 :]:
            left_dyn = float(left["dynamic_score"])
            right_dyn = float(right["dynamic_score"])
            left_mat = float(left["materialized_score"])
            right_mat = float(right["materialized_score"])
            if (left_dyn - right_dyn) * (left_mat - right_mat) < 0:
                inversions += 1
    return {
        "matched_score_count": len(deltas),
        "delta_materialized_minus_dynamic": _distribution(deltas),
        "rank_inversion_pair_count": inversions,
    }
...
def _aggregate(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    score_rank_inversions = sum(
        int(row["scores"]["paired"].get("rank_inversion_pair_count", 0)) for row in rows
    )
    ...
    return {
        ...
        "native_scores": {
            ...
            "rank_inversion_pair_count": score_rank_inversions,
        },
        "gt_correctness": gt,
    }
...
def main() -> int:
    ...
    evaluation_ok = all(result.get("status") == "passed" for result in evaluation.values())
        ...
        output_dir / "analysis_receipt.json",
        {
            ...
            "evaluation": {
                key: {
                    ...
                    if field in {"status", "returncode", "metrics_path", "stdout_path", "stderr_path"}
                }
