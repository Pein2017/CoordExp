...
- Stage-1 static dataset packing uses contiguous greedy grouping. It appends
  samples to the current packed sequence until the next sample would exceed the
  cap, then starts a new packed sequence. This preserves source order and matches
  the intended "A+B+C, then D" semantics for length-capped packed examples.
- Stage-1 production configs should use `training.packing_drop_last: false`
  unless a deterministic tail drop is explicitly part of the experiment. Static
  plan artifacts record raw/aligned coverage counts, underfilled-tail drops, DDP
  truncation, and DDP repeat padding so launches can audit data coverage.
- Stage-1 static train packing aligns the final plan to the optimizer-step
  boundary (`world_size * gradient_accumulation_steps`), not only to DDP world
  size. For the coord-repel production leaf on 8 GPUs with `effective_batch_size:
  32`, the train cache plan is named `plan_ws8_drop0_align32.json`.
...
| Stage-1 compact teacher forcing | `12000` | `32` for coord-repel production candidates | static dataset packing with exact atom remap | Requires `objective.target_ir.exact_packing_mapping.enabled=true`; emits one merged teacher-forcing IR per physical packed row. |
| Stage-1 compact recursive detection latest | `12000` | `128` | disabled | Packing remains disabled for recursive sidecars until recursive target-position offset rewriting is implemented and validated. |
...
Latest compact teacher forcing supports the packed regime only through exact
atom-position remapping, described below. Recursive sidecar packing and the
experimental `packing.padding_free_packed=true` runtime mode remain disabled.
...
validated, recursive-detection configs must not enable dataset/static packing or
padding-free packed runtime. The retired recursive-detection config root is archived under
...
## Stage-1 Compact Teacher-Forcing Static Packing

Compact teacher forcing supports ordinary static dataset packing only when exact
atom remapping is explicitly enabled:
...
The collator receives one raw packed row as a list of original encoded samples,
derives cumulative physical offsets from source lengths, and emits one merged
`TeacherForcingTargetIR` per physical packed row. Every atom has `batch_index`,
`target_position`, and `logit_position` rewritten to the physical packed tensor
coordinates before objective math runs.

Guardrails:

- `training.per_device_train_batch_size` must be `1`; the effective batch counts
  physical packed rows.
- Padding must be right-side only. Exact remap assumes the first source sample
  starts at physical position 0.
- Full logits are required. `training.use_logits_to_keep` is rejected.
- Encoded-sample cache is rejected until cache replay carries the exact packed
  sidecar contract.
- `packing.padding_free_packed=true` is rejected. Static packing still uses the
  upstream ms-swift padding-free/FlashAttention varlen path internally; the
  config key names a separate experimental runtime mode.

Static-packing cache identity distinguishes this surface from ordinary SFT via
`teacher_forcing_target_ir_exact_mapping_v1`; merged IR metadata records the
per-row remap contract as `teacher_forcing_atoms_v1`.
...
- Latest compact recursive detection surfaces keep packing and encoded-sample cache fail-fast until recursive sidecar target-position rewriting is explicitly implemented and validated.
- Compact-full token-row runs train 1002 rows through the persisted
  ...
  structural rows `<|object_ref_start|>` and `<|box_start|>`. Compact-box-end
  runs train 1003 rows by adding `<|box_end|>` as a structural row. The module
  name is historical; the current contract is token-row adaptation, not
  coord-only adaptation.
...
packing_drop_last: true          # historical probe only; production should prefer false for full raw coverage
eval_packing: true
```
