...
operator-facing keys through
`EMITTED_STAGE1_SET_CONTINUATION_METRICS` and `numeric_metric_payload`.
Internal runtime counters such as exact branch-batch scheduler details,
`batch_loss`, `batch_size`, and per-position scratch values may exist in
trainer-local dictionaries but are not part of the public compact schema.

The additive Phase-1 schema is
`stage1_set_continuation_metrics_v3`. It preserves all legacy v2 public keys,
including bare `rmp/*`, `loss/rmp*`, `mp/*`, and `stop/*` keys, and adds the
teacher-forced branch diagnostics under `setcont/rmp/*`. Set-continuation
metrics remain flat allow-listed scalars in Phase 1; they are not converted to
`MetricEvent`.
...
The `mp/padding_free_pack_forwards` and
`mp/padding_free_pack_rows_mean` keys are padding-free packing/runtime counters,
not objective metrics.
...
ET-RMP-CE v3 adds Phase-1 teacher-forced branch diagnostics:
...
These diagnostics are prefix-conditioned, teacher-forced ET-RMP-CE branch
metrics. They diagnose whether model mass and ranks favor valid continuation
children, teacher-path children, and coarse branch-type buckets (`desc_text`,
`coord`, `structural`, `other`). They are not rollout parse metrics, parse
validity, duplicate-control metrics, or mAP.

For a branch node with valid child set `V`, full-vocabulary model distribution
`p`, teacher child token `t`, normalized valid-child target distribution `q`,
and logits `z`, v3 uses:
...
- `teacher_path_child_prob = p[t]`
- `teacher_path_child_rank` is a 1-based full-vocabulary competition rank for the teacher token `t`
- `positive_child_rank` is a 1-based full-vocabulary competition rank for the highest-logit valid child in `V`; lower is better
- Ties receive the same rank because only logits strictly greater than the target logit are counted: `rank = 1 + count(scores > target_score)`
- `valid_child_effective_count = exp(-sum_{v in V} p_valid[v] * log(clamp_min(p_valid[v], eps)))`
- `top1_invalid_rate = mean(1[argmax(z) not in V])`
- `top1_valid_not_teacher_rate = mean(1[argmax(z) in V and argmax(z) != t])`
- `valid_child_mass_p10` is the empirical 10th percentile over branch-node valid masses using the implementation's tensor quantile convention

Denominators:

- `setcont/rmp/branch_node_count` covers support, mass, rank, and top-1 branch metrics
- `setcont/rmp/effective_count_node_count` covers `setcont/rmp/valid_child_effective_count_mean`
- `setcont/rmp/balance_node_count` covers aggregate `setcont/rmp/balance_kl_mean`
- `setcont/rmp/<type>_branch_node_count` covers `setcont/rmp/support_loss_<type>_mean`
- `setcont/rmp/<type>_balance_node_count` covers `setcont/rmp/balance_kl_<type>_mean`

Zero-denominator behavior:

- Count keys are emitted with `0`.
- Mean and rate keys for empty groups are omitted.
- Missing groups must not be represented as misleading `0.0` means.
- Legacy `rmp/*` zero-denominator behavior remains separate from the additive
  `setcont/rmp/*` contract.
