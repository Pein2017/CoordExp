...
Recursive-detection CE currently logs implemented flat reporter keys, including:

- `loss/recursive_detection_ce`
- `recursive_detection_ce/batch_size`

Do not treat recursive-detection CE as publishing additive canonical
`MetricEvent` identities unless the recursive detection path explicitly emits
them. The current source-reviewed `MetricEvent` contract applies to the
aggregate coord-token identities documented above, with legacy aliases generated
only by `flatten_metric_events` for registered identities.
