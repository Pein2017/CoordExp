...
RETRY_OUTPUT = AUTONOMOUS / 'soft-preservation-dense48/training-retry1'
SERIAL_CONTROL = AUTONOMOUS / 'dense48-parity-serial'
NUMERICAL_ADMISSION = 'serial_dense48_global_gradient_v1'
...
def gradient_relative_l2(observed, reference):
    require(observed.shape == reference.shape and observed.dtype == reference.dtype == torch.float32,
            'serial gradient shape/dtype identity')
    a,b=observed.double(),reference.double()
    require(bool(torch.isfinite(a).all() and torch.isfinite(b).all()) and float(b.norm())>0,'finite nonzero reference gradient')
    return float((a-b).norm()/b.norm())


def vector_admission(values, reference, observed_score, reference_score):
    import numpy as np
    require(values.shape==reference.shape and np.isfinite(values).all() and np.isfinite(reference).all(),'entry vector shape/finite')
    error=float(np.max(np.abs(values.astype(np.float64)-reference.astype(np.float64))))
    rank_equal=all(observed_score[k]==reference_score[k] for k in ('top1_id','rank_min','rank_max'))
    return dict(max_abs_error=error,top1_rank_equal=rank_equal,passed=error<=1e-4 and rank_equal)
...
def prepare(output):
    ...
    control_receipt=load_canonical_json(SERIAL_CONTROL/'mechanical_receipt.json')
    require(control_receipt['status']=='mechanical_control_completed' and control_receipt['not_for_natural_evaluation'] and
            control_receipt['updates']==1 and control_receipt['resources']['model_forwards']==54,'serial mechanical control binding')
    serial_files=[SERIAL_CONTROL/'mechanical_receipt.json',SERIAL_CONTROL/'state_saved_before_comparisons.json',
                  SERIAL_CONTROL/'gradient_layout.json',SERIAL_CONTROL/'raw_global_gradient.safetensors',
                  SERIAL_CONTROL/'clipped_global_gradient.safetensors',SERIAL_CONTROL/'scores'/'step-01.json']
    serial_files += [SERIAL_CONTROL/'scores'/f"step-01-{c['image_id']}.npy" for c in wide['cases']]
    for path in serial_files:sources[str(path)]=file_hash(path)
    serial_control=dict(path=str(SERIAL_CONTROL/'mechanical_receipt.json'),sha256=file_hash(SERIAL_CONTROL/'mechanical_receipt.json'))
    ...
    packet=dict(schema_version='selective_preservation_dense.inputs.v1',cases=wide['cases'],trajectories=wide['trajectories'],
        ...
        numerical_admission=NUMERICAL_ADMISSION,serial_control=serial_control,
        gradient_relative_l2_tolerance=1e-5,entry_vector_max_abs_tolerance=1e-4,
...
def execute_rank(output):
    ...
    with (run/'execution.log').open('x') as log,redirect_stdout(log),redirect_stderr(log):
      try:
        ...
        require(gradient_layout==load_canonical_json(SERIAL_CONTROL/'gradient_layout.json'),'serial selected-gradient layout differs')
        ...
        def score_phase(step):
            ...
            if rank==0:
              try:
                scores={};parity={};top1_ranks={};wide_descriptive={}
                serial_scores=load_canonical_json(SERIAL_CONTROL/'scores'/'step-01.json') if step==1 else {}
                for c,prompt,inputs in score_inputs:
                    ...
                    if step==1:
                        reference=np.load(SERIAL_CONTROL/'scores'/f"step-01-{c['image_id']}.npy",allow_pickle=False)
                        admission=vector_admission(values,reference,s,serial_scores[c['case_id']])
                        parity[c['case_id']]=admission['max_abs_error'];top1_ranks[c['case_id']]=admission['top1_rank_equal']
                        old=np.load(packet['step1_comparators'][c['case_id']],allow_pickle=False)
                        wide_descriptive[c['case_id']]=float(np.max(np.abs(values-old)))
                publish(output/'scores'/f'step-{step:02d}.json',scores)
                if step==1:
                    passed=all(d<=1e-4 for d in parity.values()) and all(top1_ranks.values())
                    publish(output/'step1-vector-parity.json',dict(status='passed' if passed else 'failed',
                        numerical_admission=NUMERICAL_ADMISSION,max_abs_errors=parity,tolerance=1e-4,
                        top1_rank_equal=top1_ranks,wide31_descriptive_max_abs_errors=wide_descriptive,
                        serial_control=packet['serial_control']))
                    require(passed,'step1 serial dense48 full-vocabulary vector parity failed')
                result=dict(ok=True,scores=scores,step1_parity=parity,step1_top1_rank_equal=all(top1_ranks.values()))
        ...
        dose=[];step1_parity={}
        step1_gradient_records={}
        def save_gradient_snapshot(label):
            from safetensors.torch import save_file,load_file
            vector=torch.cat([p.grad.detach().flatten() for _,p in named]).cpu().contiguous()
            path=output/f'step1-{label}-gradient.safetensors'
            save_file({'gradient':vector},str(path))
            require(torch.equal(load_file(str(path))['gradient'],vector),'step1 gradient disk reload')
            serial_path=SERIAL_CONTROL/f'{label}_global_gradient.safetensors'
            reference=load_file(str(serial_path))['gradient']
            return dict(path=str(path),sha256=file_hash(path),serial_path=str(serial_path),serial_sha256=file_hash(serial_path),
                        relative_l2=gradient_relative_l2(vector,reference),l2_float64=float(vector.double().norm()))
        for step in range(1,24):
            ...
            if step==1:
                snapshot=None
                if rank==0:
                  try:snapshot=dict(ok=True,record=save_gradient_snapshot('raw'))
                ...
                step1_gradient_records['raw']=snapshot['record']
            ...
            if step==1:
                snapshot=None
                if rank==0:
                  try:snapshot=dict(ok=True,record=save_gradient_snapshot('clipped'))
                  except BaseException as exc:snapshot=dict(ok=False,error=f'{type(exc).__name__}: {exc}')
                snapshot=gather(snapshot)[0]
                require(snapshot['ok'],f"clipped gradient snapshot failed: {snapshot.get('error')}")
                step1_gradient_records['clipped']=snapshot['record']
            ...
            if step==1:
                # All comparison decisions are deferred until raw/clipped/Adam/adapter evidence is durable.
                snapshot=None
                if rank==0:
                  try:
                    optimizer_path=output/'step1-optimizer.pt';torch.save(optimizer.state_dict(),optimizer_path)
                    restored=torch.load(optimizer_path,map_location='cpu',weights_only=True)
                    live=optimizer.state_dict();require(restored['param_groups']==live['param_groups'],'step1 optimizer config reload')
                    require(all(torch.equal(restored['state'][k][field],value.cpu()) for k,st in live['state'].items() for field,value in st.items()),'step1 optimizer tensors reload')
                    diagnostic_adapter=_save_adapter_only(model,source_root=Path(config.adapter.path),output=output/'step1-diagnostic-adapter')
                    state_snapshot=dict(mechanical_diagnostic_only=True,raw_gradient=step1_gradient_records['raw'],
                        clipped_gradient=step1_gradient_records['clipped'],optimizer=dict(path=str(optimizer_path),sha256=file_hash(optimizer_path)),
                        adapter=diagnostic_adapter,rank_raw_gradient_hashes=gradient_hashes,
                        rank_adapter_optimizer_hashes=[list(x) for x in state_hashes],raw_gradient_norm=raw_norm,clipped_gradient_norm=clipped_norm,
                        serial_control=packet['serial_control'],gradient_layout_sha256=file_hash(run/'gradient_layout.json'))
                    publish(output/'step1-state-snapshot.json',state_snapshot)
                    raw_error=step1_gradient_records['raw']['relative_l2'];clipped_error=step1_gradient_records['clipped']['relative_l2']
                    passed=raw_error<=1e-5 and clipped_error<=1e-5 and len(set(gradient_hashes))==1 and len(set(state_hashes))==1
                    parity=dict(status='passed' if passed else 'failed',numerical_admission=NUMERICAL_ADMISSION,
                        raw_relative_l2=raw_error,clipped_relative_l2=clipped_error,tolerance=1e-5,
                        exact_raw_gradient_rank_identity=len(set(gradient_hashes))==1,
                        exact_adapter_optimizer_rank_identity=len(set(state_hashes))==1,
                        state_snapshot=dict(path=str(output/'step1-state-snapshot.json'),sha256=file_hash(output/'step1-state-snapshot.json')),
                        serial_control=packet['serial_control'])
                    publish(output/'step1-gradient-parity.json',parity)
                    snapshot=dict(ok=True,passed=passed,parity=parity)
                  except BaseException as exc:snapshot=dict(ok=False,error=f'{type(exc).__name__}: {exc}')
                snapshot=gather(snapshot)[0]
                require(snapshot['ok'],f"step1 state snapshot failed: {snapshot.get('error')}")
                require(snapshot['passed'],'step1 serial dense48 raw/clipped gradient or rank identity parity failed; snapshots preserved')
            ...
            if step==1:
                step1_parity=score_result['step1_parity'];step1_top1_rank_equal=score_result['step1_top1_rank_equal']
            ...
            if rank==0:
                ...
                if step==2:
                    publish(output/'two-step-smoke.json',dict(status='passed',updates=[1,2],initial_reference_stats=refstats,
                        ...
                        step1_top1_rank_equal=step1_top1_rank_equal,numerical_admission=NUMERICAL_ADMISSION,
                        gradient_parity_sha256=file_hash(output/'step1-gradient-parity.json'),
        ...
        if rank==0:
          try:
            ...
            provisional=dict(schema_version='selective_preservation_dense.training.v1',status='unsealed',adapter=adapter,
                ...
                numerical_admission=NUMERICAL_ADMISSION,serial_control=packet['serial_control'],
                gradient_parity=dict(path=str(output/'step1-gradient-parity.json'),sha256=file_hash(output/'step1-gradient-parity.json')),
...
def finalize(output):
    ...
    require(provisional['numerical_admission']==NUMERICAL_ADMISSION,'sealed numerical admission revision')
    for ref in (provisional['serial_control'],provisional['gradient_parity'],provisional['two_step']):
        require(file_hash(ref['path'])==ref['sha256'],'numerical admission evidence hash')
    gradient=load_canonical_json(Path(provisional['gradient_parity']['path']))
    smoke=load_canonical_json(Path(provisional['two_step']['path']))
    require(gradient['status']=='passed' and gradient['raw_relative_l2']<=1e-5 and gradient['clipped_relative_l2']<=1e-5,
            'gradient admission did not pass')
    require(smoke['status']=='passed' and smoke['updates']==[1,2] and smoke['step1_top1_rank_equal'] and
            all(e<=1e-4 for e in smoke['step1_full_vocab_max_abs_errors'].values()) and
            any(k>1e-8 for k in smoke['step2_max_KL_by_rank'].values()),'two-step admission did not pass')
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=RETRY_OUTPUT)
    ...
    if args.command=='prepare':
        ...
        require(r['status']=='completed' and r['global_model_forwards']==1248 and r['numerical_admission']==NUMERICAL_ADMISSION,'sealed receipt incomplete')
        for ref in r['rank_terminals']+[r['launcher_exit'],r['serial_control'],r['gradient_parity'],r['two_step']]:
            require(file_hash(ref['path'])==ref['sha256'],'terminal/launcher/admission binding changed')
