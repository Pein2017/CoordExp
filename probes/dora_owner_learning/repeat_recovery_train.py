...
def prepare(manifest_path: Path, output: Path) -> dict[str, Any]:
    ...
    commands = {
        ...
        "smoke_A": f"CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m probes.dora_owner_learning.repeat_recovery_train launch --input {output / 'inputs.json'} --arm A --mode smoke --output-root {RAW_ROOT / 'smoke-A-retry1'}",
        "cold_A": f"CUDA_VISIBLE_DEVICES=0 python -m probes.dora_owner_learning.repeat_recovery_train cold-check --input {output / 'inputs.json'} --output-root {RAW_ROOT / 'smoke-A-retry1'}",
