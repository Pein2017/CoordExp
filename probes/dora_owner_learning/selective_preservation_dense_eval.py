"""Frozen Source/KL10/wide31/dense48 natural130 read with isolated native shards."""
...
ARM_ROOT=ROOT/'2026-09-10-selective-owner-learning-autonomous/soft-preservation-dense48'
OUTPUT=ARM_ROOT/'evaluation-retry1'
TRAINING=ARM_ROOT/'training-retry1'
ADMISSION='serial_dense48_global_gradient_v1'
SERIAL_CONTROL=ARM_ROOT.parent/'dense48-parity-serial/mechanical_receipt.json'
PREFLIGHT=ARM_ROOT.parent/'full-support-preflight.json'
OLD_PREFLIGHT=ARM_ROOT.parent/'support32-preflight.json'
WIDE_ROOT=ARM_ROOT.parent/'soft-preservation-wide31'
DENSE17=[25274,49327,64010,90862,101636,114340,152252,158044,203986,422969,474979,511251,532132,540107,548337,568311,575627]
GPUS=(1,3,4,5)
COUNTS=(33,33,32,32)
...
def prepare():
    require(not OUTPUT.exists() or {p.name for p in OUTPUT.iterdir()}=={'code-before-retry1'},'occupied dense evaluation output')
    ...
    old_support=json.loads(OLD_PREFLIGHT.read_text())['proposed_support31']['retained_image_ids']
    support=[str(i) for i in sorted(set(map(int,old_support))|set(DENSE17))]
    require(len(support)==48 and not set(map(int,support))&{r['image_id'] for r in records},'support/evaluation separation')
    wide=json.loads((WIDE_ROOT/'evaluation/consumer.json').read_text());require([r['example_id'] for r in wide]==[r['example_id'] for r in records],'wide31 control130 ordering')
    paths=[KL10_ROOT/'evaluation/manifest.json',KL10_ROOT/'dev112/manifest.json',KL10_ROOT/'evaluation/execution/consumer.json',KL10_ROOT/'dev112/consumer.json',PREFLIGHT,OLD_PREFLIGHT,WIDE_ROOT/'evaluation/consumer.json']
    packet=dict(schema='selective_preservation_dense.eval.v1',arm='soft-preservation-dense48',records=records,shards=shards,
        ...
        wide31_controls=wide,support_image_ids=support,support_action_states=5848,support_preflight_sha256=file_hash(PREFLIGHT),source_files={**old18['source_files'],**old112['source_files'],**{str(p):file_hash(p) for p in paths}},
        scope='Same130 exposeddevelopment images; fourshards of onecandidate, no newselection or confirmation.')
    original_path=ARM_ROOT/'evaluation/manifest.json';original=json.loads(original_path.read_text())
    require(packet==original,'retry changed original130 evaluation payload')
    packet.update(attempt='training-retry1',numerical_admission=ADMISSION,original_manifest_sha256=file_hash(original_path))
    packet['source_files'][str(original_path)]=file_hash(original_path)
    validate_packet(packet);OUTPUT.mkdir(parents=True,exist_ok=True);publish(OUTPUT/'manifest.json',packet)
...
def validate_packet(packet):
    require(packet['schema']=='selective_preservation_dense.eval.v1' and packet['arm']=='soft-preservation-dense48','wrong dense arm/schema')
    require(packet['attempt']=='training-retry1' and packet['numerical_admission']==ADMISSION,'wrong retry admission identity')
    original_path=ARM_ROOT/'evaluation/manifest.json';require(file_hash(original_path)==packet['original_manifest_sha256'],'old frozen130 manifest changed')
    original=json.loads(original_path.read_text())
    for key in ('records','shards','kl10_controls','wide31_controls','support_image_ids','support_action_states'):
        require(packet[key]==original[key],'retry changed original frozen payload')
    ...
    require(packet['shards']==expected_shards(packet['records']),'changed dense shard assignment')
    ...
    require(packet['wide31_controls']==json.loads((WIDE_ROOT/'evaluation/consumer.json').read_text()),'retained wide31 outcomes changed')
...
def validate_receipt(receipt,packet):
    require(receipt['schema_version']=='selective_preservation_dense.training.v1' and receipt['status']=='completed','wrong/incomplete dense receipt')
    require(receipt['updates']==23 and receipt['stop_reason']=='fixed_steps','dense fixed23 required')
    require(receipt['lambda_kl']==receipt['lambda_support_kl']==10.,'both frozen KL weights10 required')
    require(receipt['support_image_ids']==packet['support_image_ids'] and len(set(receipt['support_image_ids']))==48,'wrong support48 identity')
    require(receipt['support_action_states']==packet['support_action_states']==5848,'wrong fullstate support count')
    require(Path(receipt['adapter']['root']).resolve()==(TRAINING/'adapter').resolve(),'wrong dense retry checkpoint path')
    require(file_hash(TRAINING/'inputs.json')==receipt['inputs_sha256'],'dense training packet changed')
    validate_global_completion(receipt)
    validate_numerical_admission(receipt)
    return validate_source_receipt(receipt,packet)


def validate_numerical_admission(receipt):
    require(receipt['numerical_admission']==ADMISSION,'wrong numerical admission revision')
    evidence={}
    for key,path in [('serial_control',SERIAL_CONTROL),('gradient_parity',TRAINING/'step1-gradient-parity.json'),('two_step',TRAINING/'two-step-smoke.json')]:
        ref=receipt[key];require(Path(ref['path']).resolve()==path.resolve() and file_hash(path)==ref['sha256'],'admission evidence path/bytes changed')
        evidence[key]=json.loads(path.read_text())
    serial=evidence['serial_control'];require(serial['status']=='mechanical_control_completed' and serial['updates']==1 and serial['not_for_natural_evaluation'] is True,'wrong serial mechanical control')
    gradient=evidence['gradient_parity'];require(gradient['status']=='passed' and all(0<=gradient[k]<=1e-5 for k in ('raw_relative_l2','clipped_relative_l2')),'direct gradient parity failed')
    two=evidence['two_step'];require(two['status']=='passed' and two['updates']==[1,2] and
        len(two['step1_full_vocab_max_abs_errors'])==2 and all(0<=v<=1e-4 for v in two['step1_full_vocab_max_abs_errors'].values()) and
        two['step1_top1_rank_equal'] is True,'same-objective entry vector/rank parity failed')
    require(set(two['step2_max_KL_by_rank'])==set(map(str,range(8))) and any(v>1e-8 for v in two['step2_max_KL_by_rank'].values()) and
        two['reduced_gradient_adapter_optimizer_rank_identity'] is True and two['frozen_bytes_unchanged'] is True,'two-step nonzero-KL/rank-state admission failed')


def validate_global_completion(receipt):
    ranks=receipt['rank_terminals'];require(len(ranks)==8 and {r['rank'] for r in ranks}==set(range(8)),'exact eight rank receipts required')
    require(receipt['global_model_forwards']==1248,'global1248 forward accounting')
    forwards=0
    for r in ranks:
        require(file_hash(r['path'])==r['sha256'],'rank terminal bytes changed')
        terminal=json.loads(Path(r['path']).read_text())
        require(terminal['rank']==r['rank'] and terminal['status']=='completed' and terminal['updates']==23,'incomplete/misbound rank terminal')
        require(terminal['model_forwards']==([216,168]+[144]*6)[r['rank']],'wrong per-rank forward accounting')
        forwards+=terminal['model_forwards']
    require(forwards==receipt['global_model_forwards'],'global/rank forward count mismatch')
    launch=receipt['launcher_exit'];require(file_hash(launch['path'])==launch['sha256'],'launcher exit bytes changed')
    require(json.loads(Path(launch['path']).read_text())['exit_code']==0,'torchrun launcher did not exit0')
...
def score_terminal_entries(qwen,packet,receipt,run):
    ...
    for frozen in packet['records'][:2]:
        ...
        require(all(abs(v)<=1e-5 for v in deltas.values()) and observed['A_id']==saved['target_id'] and all(observed[k]==saved[k] for k in ('rank_min','top1_id')),'dense saved/cold entry mismatch')
...
def require_complete_shards(terminals,packet):
    ...
    for t in terminals:
        i=t['shard'];require(t['status']=='completed' and t['manifest_sha256']==digest(packet) and t['gpu']==GPUS[i] and t['continuations']==COUNTS[i]
            and t['model_loads']==1 and t['score_forwards']==(2 if i==0 else 0) and t['new_tokens']<=COUNTS[i]*3084 and t['elapsed_seconds']<3600,'incomplete/wrong dense shard')
    require(len({t['training_receipt_sha256'] for t in terminals})==1,'mixed candidate receipts')


def reduce_records(cold,packet):
    controls=indexed(packet['kl10_controls'],'example_id');wide=indexed(packet['wide31_controls'],'example_id');old=indexed(packet['records'],'example_id');result={}
    for split in ('train','guard','dev112','dev128_descriptive'):
        ...
        result[split]=dict(images=len(selected),source=aggregate_scores([old[r['example_id']]['baseline_score'] for r in selected]),
            kl10=aggregate_scores([controls[r['example_id']]['score'] for r in selected]),wide31=aggregate_scores([wide[r['example_id']]['score'] for r in selected]),dense48=aggregate_scores([r['score'] for r in selected]),
            versus={label:{t:{k:sum(len(owner_change(r['score'][t]['owners'],(old[r['example_id']]['baseline_score'] if label=='source' else (controls if label=='kl10' else wide)[r['example_id']]['score'])[t]['owners'])[k]) for r in selected)
                for k in ('gained','lost','retained')} for t in ('50','60','80')} for label in ('source','kl10','wide31')},eos=sum(r['stop_reason']=='im_end' for r in selected))
    per_image={}
    for r in cold:
        ...
        per_image[eid]=dict(split=p['split'],versus_source=r['owner_changes'],versus_kl10={t:owner_change(r['score'][t]['owners'],control['score'][t]['owners']) for t in ('50','60','80')},
            versus_wide31={t:owner_change(r['score'][t]['owners'],wide[eid]['score'][t]['owners']) for t in ('50','60','80')},
            source=p['baseline_score'],kl10=control['score'],wide31=wide[eid]['score'],dense48=r['score'])
        if p['split']=='train':
            owner=p['bridge_case']['owner'];per_image[eid].update(training_readout=r['training_readout'],target_boxes={label:target_boxes(parsed,scored,owner)
                for label,parsed,scored in [('source',p['baseline'],p['baseline_score']),('kl10',control['parsed'],control['score']),('wide31',wide[eid]['parsed'],wide[eid]['score']),('dense48',r['parsed'],r['score'])]})
    return dict(schema='selective_preservation_dense.eval_reduction.v1',images=130,splits=result,per_image=per_image,
...
def execute(shard):
    ...
    try:
        ...
        require(terminal['continuations']==len(records),'complete assigned dense shard required');terminal['status']='completed'
