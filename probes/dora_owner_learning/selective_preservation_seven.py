"""Seven admitted entrance labels,47 full-preservation references and81 fixed updates."""
...
from .selective_preservation_dense import optimizer_hash,compensate_for_ddp
from .selective_preservation_strong import OUTPUT as PRIOR

OUTPUT=ROOT/'2026-09-10-selective-owner-learning-autonomous/positive7-support47-81/training'
BRANCH=OUTPUT.parents[1]/'positive-branch-expansion'
NUMERICAL_ADMISSION='ddp_path_reuse_positive7_support47_v1'
CEILINGS=[588,574,574,574,574,574,574,410]
MASKS={'368':(97,139,102,134),'7116':(22,46,27,41),'252411':(1,31,11,21),
       '465695':(1,29,9,21),'529411':(1,21,11,11),'538814':(4,10,9,5),'540567':(1,49,10,40)}
NEW={'252411':('1676022',2026090602),'465695':('1612233',2026090601),'529411':('2147505',2026090603),
     '538814':('1121312',2026090602),'540567':('1491504',2026090601)}


def rank_items(packet,rank):
    require(0<=rank<8,'rank range');items=[]
    if rank<7:
        c=packet['cases'][rank];t=packet['trajectories'][c['case_id']]
        items.append(dict(key=c['case_id'],kind='entrance',case=c,action_ids=t['action_ids'],positions=t['preservation_positions'],suffix_start=t['suffix_start']))
    for c in packet['support_cases'][rank::8]:
        items.append(dict(key=c['example_id'],kind='support',case=c,action_ids=c['action_ids'],positions=list(range(len(c['action_ids'])))))
    require(len(items)==(7 if rank<7 else 5),'local54 assignment')
    return items


def global_image_loss(logits,targets,item,reference_logp):
    from src.losses import aligned_token_logprobs
    ids=item['action_ids'];checked_ids(ids,'im_end')
    require(logits.ndim==2 and logits.shape[0]==len(ids) and targets.tolist()==ids and
            reference_logp.shape==(len(item['positions']),logits.shape[1]),'causal/reference alignment')
    require(logits.dtype==reference_logp.dtype==torch.float32,'FP32 objective')
    if item['kind']=='entrance':
        c=item['case'];i=c['action_index']
        require(c['state_ids']==ids[:i] and len(c['state_ids'])==i and ids[i]==c['target_token_id'] and
                item['positions']==[j for j in range(len(ids)) if j<i or j>=item['suffix_start']], 'generic entrance/mask identity')
        ce=-aligned_token_logprobs(logits[i:i+1],targets[i:i+1]).sum()
    else:
        require(item['kind']=='support' and item['positions']==list(range(len(ids))),'full support mask');ce=None
    ref=reference_logp.detach();student=torch.log_softmax(logits[item['positions']],-1)
    kl=(ref.exp()*(ref-student)).sum(-1).mean()
    return ((ce+10*kl)/7 if ce is not None else (100/47)*kl),ce,kl


class SevenScorer(torch.nn.Module):
...
def build_data():
    old=load_canonical_json(PRIOR/'inputs.json')
    require(file_hash(BRANCH/'inputs.json')=='710c21601156a6e04a3ad195d71f824203c0a2f7eb3cf080dd0b7da79180eb4f' and
            file_hash(BRANCH/'reduction.json')=='4bc95c3d736dab4d2614d087c67c24a36477f621ea12a14605fb180d6c712728','lead-admitted conditional evidence identity')
    branch=load_canonical_json(BRANCH/'inputs.json');reduced=load_canonical_json(BRANCH/'reduction.json')
    variants={v['variant_id']:v for v in branch['variants']};results={r['variant_id']:r for r in reduced['results']}
    cases=list(old['cases']);trajectories=dict(old['trajectories']);sources=dict(old['source_files'])
    for image,(owner,seed) in NEW.items():
        key=f'{image}:{owner}:{seed}';v=variants[key];r=results[key]
        require(r['eligibility']['candidate_eligible'] and reduced['prospective_selection_by_lowest_seed'][r['case_id']]==key and
                r['actual_first_row']['full_global_target_assignment']['50'],'frozen selected variant must pass actual first-row gate')
        path=BRANCH/'rows'/f'{image}-{seed}.json';actual=load_canonical_json(path);b=v['boundary'];g=v['case']['group']
        ids=checked_ids(actual['action_ids'],'im_end');end=r['actual_first_row']['end_token_exclusive'];i=b['entry_index'];target=b['forced_ids'][0]
        require(ids==b['extension_ids']+actual['free_ids'] and ids[:i]==v['source_ids'][:i]==b['prefix_ids'] and ids[i]==target and
                ids[end-1]==151649 and ids[:end]==r['actual_first_row']['row_token_ids'],'actual conditional trajectory/row boundary')
        entry=dict(action_index=i,state_ids=b['prefix_ids'],A_id=target,B_id=v['source_ids'][i])
        c=dict(case_id=v['case']['case_id'],example_id=v['case']['example_id'],image_id=image,owner_id=owner,category=v['case']['category'],
               action_index=i,target_token_id=target,state_ids=b['prefix_ids'],prefix_token_ids=b['prefix_ids'],prompt_token_ids=g['prompt_token_ids'],
               entrance=entry,group=g,admitted_variant_id=key,actual_first_row_iou=r['actual_first_row']['direct_target_iou'])
        cases.append(c);positions=[j for j in range(len(ids)) if j<i or j>=end]
        trajectories[c['case_id']]=dict(action_ids=ids,suffix_start=end,preservation_positions=positions,preservation_count=len(positions),
            excluded_positions=list(range(i,end)),actual_row_token_ids=ids[:end],source_record=dict(path=str(path),sha256=file_hash(path)),
            reference_scope='Actual single-token-release Source continuation, not sampled witness geometry')
        sources[str(path)]=file_hash(path)
    cases.sort(key=lambda c:int(c['image_id']))
    require([c['image_id'] for c in cases]==list(MASKS),'frozen sorted seven positives')
    for c in cases:
        t=trajectories[c['case_id']];expected=MASKS[c['image_id']]
        require((c['action_index'],len(t['action_ids']),t['suffix_start'],len(t['preservation_positions']))==expected and
                t['excluded_positions']==list(range(expected[0],expected[2])) and len(t['action_ids'])-1 in t['preservation_positions'],'seven real-token masks')
    support=[c for c in old['support_cases'] if c['image_id']!='529411']
    require(len(support)==47 and sum(len(c['action_ids']) for c in support)==5838 and
            not {c['image_id'] for c in support}&{c['image_id'] for c in cases},'remove only529411 conflict; no backfill')
    require(sum(len(trajectories[c['case_id']]['preservation_positions']) for c in cases)==273,'local KL states')
    for path in [PRIOR/'inputs.json',BRANCH/'inputs.json',BRANCH/'reduction.json']:sources[str(path)]=file_hash(path)
    return old,cases,trajectories,support,sources


def objective_fixture(cases,trajectories):
    # Real token identities/intervals, with artificial logits that isolate objective weights.
    from src.losses import aligned_token_logprobs
    errors=[];checked=[]
    for c in cases:
        t=trajectories[c['case_id']];ids=t['action_ids'];positions=t['preservation_positions'];i=c['action_index'];vocab=152670
        logits=torch.full((len(ids),vocab),-1000.,dtype=torch.float32);logits[:,:2]=math.log(.5)
        logits[i,:]=-1000.;logits[i,0]=math.log(.75);logits[i,c['target_token_id']]=math.log(.25);logits.requires_grad_(True)
        ref=torch.full((len(positions),vocab),-1000.);ref[:,0]=math.log(.8);ref[:,1]=math.log(.2)
        item=dict(kind='entrance',case=c,action_ids=ids,positions=positions,suffix_start=t['suffix_start'])
        loss,ce,kl=global_image_loss(logits,torch.tensor(ids),item,ref)
        expected=(math.log(4)+10*(.8*math.log(1.6)+.2*math.log(.4)))/7
        require(abs(float(loss.detach())-expected)<2e-6,'mean7 positive/local objective')
        loss.backward()
        expected_ce=-.75/7;expected_local=(-.3)*(10/(7*len(positions)))
        errors.append(abs(float(logits.grad[i,c['target_token_id']])-expected_ce));errors.append(abs(float(logits.grad[positions[0],0])-expected_local))
        excluded=[j for j in t['excluded_positions'] if j!=i]
        require(not excluded or int(torch.count_nonzero(logits.grad[excluded]))==0,'excluded nonentry logit weights')
        require(float(logits.grad[-1].abs().sum())>0 and errors[-1]<1e-6 and errors[-2]<1e-6,'EOS/local/CE weighting')
        changed=logits.detach().clone()
        if excluded:changed[excluded]=123.
        require(torch.equal(global_image_loss(changed,torch.tensor(ids),item,ref)[0],loss.detach()),'excluded nonentry invariance')
        checked.append(dict(image_id=c['image_id'],entry_index=i,excluded=t['excluded_positions'],preserved=len(positions),EOS_preserved=True))
        del logits,ref,loss,ce,kl,changed
    support_ids=[151646,65,151645]
    support_logits=torch.full((3,152670),-1000.);support_logits[:,:2]=math.log(.5);support_logits.requires_grad_(True)
    support_ref=torch.full_like(support_logits,-1000.);support_ref[:,0]=math.log(.8);support_ref[:,1]=math.log(.2)
    support_item=dict(kind='support',action_ids=support_ids,positions=[0,1,2])
    support_loss,_,_=global_image_loss(support_logits,torch.tensor(support_ids),support_item,support_ref)
    support_loss.backward();support_error=abs(float(support_logits.grad[-1,0])-(-.3)*(100/(47*3)))
    require(support_error<1e-6,'100/47 mean-image full-state support gradient')
    errors.append(support_error)
    local=[];counts=[]
    for rank in range(8):
        pieces=[torch.tensor([(j+1)/100,(-1)**j*.2],dtype=torch.float64)*(100/47) for j in range(rank,47,8)]
        if rank<7:pieces.insert(0,torch.tensor([rank+1.,2.-rank],dtype=torch.float64)/7)
        local.append(sum(pieces));counts.append(len(pieces))
    expected=sum(local);average=torch.stack([8*x for x in local]).mean(0);clip=lambda x:x*min(1.,1/float(x.norm()))
    require(torch.allclose(expected,average) and not torch.allclose(average/8,expected) and
        not torch.allclose(torch.stack([8*x/n for x,n in zip(local,counts)]).mean(0),expected) and
        not torch.allclose(torch.stack([clip(8*x) for x in local]).mean(0),clip(expected)),'DDP compensation/rank mean/preclip sensitivity')
    # Unequal state counts must remain image means, not a pooled state mean.
    values=[.3,.01];lengths=[21,134]
    equal=sum(values)/7;pooled=sum(v*n for v,n in zip(values,lengths))/sum(lengths)*2/7
    require(abs(equal-pooled)>1e-3 and abs(1/7-.5)>.1,'state pool/old-half coefficient sensitivity')
    return dict(status='passed',real_token_masks=checked,max_gradient_weight_error=max(errors),positive_CE_weight=1/7,
                local_KL_image_weight=10/7,support_KL_image_weight=100/47,old_half_weight_rejected=True,
                state_pooling_rejected=True,compensation_and_clip_checks=True,excluded_nonentry_invariance=True)


def prepare(output):
    require(not output.exists(),'occupied seven-positive arm')
    old,cases,trajectories,support,sources=build_data();proof=load_canonical_json(PRIOR/'receipt.json')
    require(proof['status']=='completed' and proof['numerical_admission']=='ddp_path_reuse_support100_v1' and proof['actual_support_uses']==1104,'prior accepted DDP path')
    for ref in [proof['two_step'],proof['launcher_exit']]:require(file_hash(ref['path'])==ref['sha256'],'prior path proof bytes')
    require(load_canonical_json(Path(proof['two_step']['path']))['status']=='passed' and load_canonical_json(Path(proof['launcher_exit']['path']))['exit_code']==0,'prior DDP settlement')
    fixture=objective_fixture(cases,trajectories)
    sources[str(PRIOR/'receipt.json')]=file_hash(PRIOR/'receipt.json')
    for path,sha in sources.items():require(file_hash(path)==sha,f'frozen input changed: {path}')
    output.mkdir(parents=True,exist_ok=False);publish(output/'objective-fixture.json',fixture)
    packet=dict(schema_version='selective_preservation_seven.inputs.v1',cases=cases,trajectories=trajectories,support_cases=support,
        positive_image_ids=[c['image_id'] for c in cases],support_image_ids=[c['image_id'] for c in support],local_KL_states=273,
        positive_trajectory_tokens=325,support_action_states=5838,total_cached_states=6111,removed_support_image_ids=['529411'],
        model=old['model'],train_source=old['train_source'],source_files=sources,optimizer=OPTIMIZER,clip_gradient_norm=1.,
        lambda_kl=10.,lambda_support_kl=100.,updates=81,numerical_admission=NUMERICAL_ADMISSION,
        prior_path_proof=dict(path=str(PRIOR/'receipt.json'),sha256=file_hash(PRIOR/'receipt.json')),
        objective_fixture=dict(path=str(output/'objective-fixture.json'),sha256=file_hash(output/'objective-fixture.json')),
        positive_admission=dict(authority='Root fifth frozen unit',conditional_input_sha256=file_hash(BRANCH/'inputs.json'),conditional_reduction_sha256=file_hash(BRANCH/'reduction.json')),
        objective='mean7CE+10mean7localKL+100mean47supportKL; globalimage losses times8 before DDP mean; singleclip afterreduction.',
        dose_interpretation='81=ceil(23*7/2), coefficient-time heuristic only; no Adam/clip equivalence or pure fixed-dose diversity claim.',
        bounds=dict(model_loads=8,reference_forwards=54,train_forwards=4374,entry_score_forwards=14,model_forwards=4442,
                    rank_model_forwards=CEILINGS,reference_cache_bytes=3731865480,rank_seconds=3600,distributed_timeout_seconds=600))
    for rank in range(8):rank_items(packet,rank)
    publish(output/'inputs.json',packet)
    files=[Path(__file__),Path(__file__).with_name('tests')/'test_selective_preservation_seven.py',CONFIG,
        Path(__file__).with_name('selective_preservation_strong.py'),Path(__file__).with_name('selective_preservation_dense.py'),
    ...
    publish(output/'code_identity.json',dict(files=records,git_head=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),git_status=subprocess.check_output(['git','status','--short'],text=True)))
    return packet
...
def execute_rank(output):
    ...
    with (run/'execution.log').open('x') as log,redirect_stdout(log),redirect_stderr(log):
      try:
        ...
        require(sum(s['reference_count'] for s in refstats)==54 and sum(s['cache_bytes'] for s in refstats)==3731865480,'global reference bounds')
        ...
        for step in range(1,82):
                ...
                if rank==0:
                  try:
                    ...
                    agreement=dict(status='passed' if passed else 'failed',update=step,numerical_admission=NUMERICAL_ADMISSION,
                        ...
                        state_snapshot=dict(path=str(state_path),sha256=file_hash(state_path)),prior_path_proof=packet['prior_path_proof'],objective_fixture=packet['objective_fixture'])
            ...
            final_scores=score_phase(step)['scores'] if step==81 else None
            ...
            if rank==0:
                ...
                if step==2:
                    publish(output/'two-step-smoke.json',dict(status='passed' if step2_KL_passed else 'failed',updates=[1,2],initial_reference_stats=refstats,
                        ...
                        prior_path_proof=packet['prior_path_proof'],objective_fixture=packet['objective_fixture'],
        ...
        require(counters['model_forwards']==CEILINGS[rank] and counters['train_forwards']==81*len(materialized),'rank counters incomplete')
        ...
        if rank==0:
          try:
            ...
            provisional=dict(schema_version='selective_preservation_seven.training.v1',status='unsealed',adapter=adapter,
                ...
                updates=81,stop_reason='fixed_steps',optimizer=OPTIMIZER,lambda_kl=10.,lambda_support_kl=100.,clip_gradient_norm=1.,
                positive_image_ids=packet['positive_image_ids'],support_image_ids=packet['support_image_ids'],support_action_states=5838,
                local_KL_states=273,positive_trajectory_tokens=325,total_cached_states=6111,removed_support_image_ids=['529411'],positive_admission=packet['positive_admission'],
                ...
                numerical_admission=NUMERICAL_ADMISSION,prior_path_proof=packet['prior_path_proof'],objective_fixture=packet['objective_fixture'],
...
def finalize(output):
    ...
    require({p.name for p in (output/'ranks').iterdir() if p.is_dir()}=={f'rank{r}' for r in range(8)},'eight rank coverage')
    terminals=[];refs=[]
    for rank in range(8):
        path=output/'ranks'/f'rank{rank}'/'terminal.json';t=load_canonical_json(path)
        require(t['status']=='completed' and t['rank']==rank and t['updates']==81 and t['model_forwards']==CEILINGS[rank] and
                t['counters']['support_KL_trajectories']==(486 if rank<7 else 405) and
                t['counters']['supervised_target_tokens']==(81 if rank<7 else 0),'81-step rank counts')
    ...
    require(p['numerical_admission']==NUMERICAL_ADMISSION and p['updates']==81 and len(p['cases'])==7 and len(p['support_image_ids'])==47,'seven recipe')
    for ref in [p['prior_path_proof'],p['objective_fixture'],p['two_step']]:require(file_hash(ref['path'])==ref['sha256'],'admission proof hash')
    fixture=load_canonical_json(Path(p['objective_fixture']['path']));smoke=load_canonical_json(Path(p['two_step']['path']))
    require(fixture['status']=='passed' and fixture['old_half_weight_rejected'] and fixture['state_pooling_rejected'] and fixture['compensation_and_clip_checks'],'objective fixture')
    require(smoke['status']=='passed' and smoke['updates']==[1,2] and smoke['initial_reference_count']==54 and smoke['initial_KL_zero'] and
            any(x>1e-8 for x in smoke['step2_max_KL_by_rank'].values()) and smoke['reduced_gradient_adapter_optimizer_rank_identity'] and smoke['frozen_bytes_unchanged'],'current two-step gate')
    for ident in [p['adapter'],p['source_embedding']]:
        for f in ident['files']:require(file_hash(Path(ident['root'])/f['relative_path'])==f['sha256'],'adapter bytes')
    positive=sum(t['counters']['supervised_target_tokens'] for t in terminals);support=sum(t['counters']['support_KL_trajectories'] for t in terminals)
    require(positive==567 and support==3807 and sum(t['model_forwards'] for t in terminals)==4442,'global counts')
    receipt={**p,'status':'completed','rank_terminals':refs,'launcher_exit':dict(path=str(exit_path),sha256=file_hash(exit_path)),
        'actual_positive_uses':positive,'actual_support_uses':support,'global_model_forwards':4442,'provisional_sha256':file_hash(output/'provisional.json'),
        'resources':dict(model_loads=8,reference_forwards=sum(t['counters']['reference_forwards'] for t in terminals),
            ...
            peak_cuda_allocated_bytes_max_rank=max(t['state']['peak_cuda_allocated_bytes'] for t in terminals),peak_cuda_reserved_bytes_max_rank=max(t['state']['peak_cuda_reserved_bytes'] for t in terminals),
            peak_rss_bytes_max_rank=max(t['state']['peak_rss_bytes'] for t in terminals))}
    publish(output/'receipt.json',receipt);return receipt


def launch(output):
    require(os.environ.get('CUDA_VISIBLE_DEVICES')=='0,1,2,3,4,5,6,7' and not (output/'launcher_owner.json').exists(),'single eight-GPU launch')
    command=[sys.executable,'-m','torch.distributed.run','--standalone','--nnodes=1','--nproc-per-node=8','-m','probes.dora_owner_learning.selective_preservation_seven','rank','--output',str(output)]
    publish(output/'launcher_owner.json',dict(pid=os.getpid(),command=command,started=time.time(),processes=subprocess.check_output(['nvidia-smi','--query-compute-apps=pid,gpu_uuid,used_memory','--format=csv,noheader'],text=True)))
    started=time.monotonic()
    ...
    r=finalize(output);print(json.dumps(dict(status='completed',resources=r['resources'])));return 0


def main():
    parser=argparse.ArgumentParser();parser.add_argument('command',choices=['prepare','rank','launch','verify']);parser.add_argument('--output',type=Path,default=OUTPUT);a=parser.parse_args()
    if a.command=='prepare':p=prepare(a.output);print(json.dumps(dict(positives=p['positive_image_ids'],support=len(p['support_image_ids']),bounds=p['bounds'])))
    elif a.command=='rank':execute_rank(a.output)
    elif a.command=='launch':raise SystemExit(launch(a.output))
    else:
        r=load_canonical_json(a.output/'receipt.json');require(r['status']=='completed' and r['updates']==81 and r['global_model_forwards']==4442,'sealed81-step receipt')
        for ref in r['rank_terminals']+[r['launcher_exit'],r['prior_path_proof'],r['objective_fixture'],r['two_step']]:require(file_hash(ref['path'])==ref['sha256'],'sealed evidence')
        for ident in [r['adapter'],r['source_embedding']]:
            for f in ident['files']:require(file_hash(Path(ident['root'])/f['relative_path'])==f['sha256'],'sealed model bytes')
