"""Frozen seven-positive81 arm with exactly three additional original Source references."""
...
from .route_access import ROOT,SOURCE_ROOT,CONFIG,checked_ids,publish
...
from .selective_preservation_seven import OUTPUT as PRIOR,global_image_loss as seven47_loss,objective_fixture as prior_positive_fixture

OUTPUT=ROOT/'2026-09-10-selective-owner-learning-autonomous/positive7-support50-81/training'
PREFLIGHT=OUTPUT.parents[1]/'full-support-preflight.json'
NUMERICAL_ADMISSION='ddp_path_reuse_positive7_support50_v1'
CEILINGS=[670,656,574,574,574,574,574,492]
NEW_REFERENCES={'73843':(10,1372),'360573':(95,1415),'545632':(81,1365)}
...
def rank_items(packet,rank):
    ...
    require(len(items)==[8,8,7,7,7,7,7,6][rank],'current57 assignment')
    return items


def global_image_loss(logits,targets,item,reference_logp):
    loss,ce,kl=seven47_loss(logits,targets,item,reference_logp)
    return ((100/50)*kl if item['kind']=='support' else loss),ce,kl


class StableScorer(torch.nn.Module):
...
def validate_new_references(added,cards):
    require(len(added)==3 and {c['image_id'] for c in added}==set(NEW_REFERENCES),'exact three additional references')
    for c in added:
        card=cards[c['image_id']];ids=checked_ids(c['action_ids'],'im_end')
        require(card['admitted'] and card['stop_reason']=='im_end' and card['parser_drops']==0 and
                ids==card['action_ids'] and digest(ids)==card['action_ids_sha256'] and
                c['example_id']==card['example_id'] and digest(c['prompt_token_ids'])==card['prompt_ids_sha256'],
                'new reference must be exact original Source, not candidate or modified tokens')
        require((len(ids),len(ids)+len(c['prompt_token_ids']))==NEW_REFERENCES[c['image_id']],'new Source lengths')


def build_data():
    old=load_canonical_json(PRIOR/'inputs.json');full=load_canonical_json(PREFLIGHT)
    cards={c['image_id']:c for c in full['all254_cases']};plan=load_canonical_json(ROOT/'2026-09-10-fixed-witness-route-access/inputs.json')['plan']
    groups=indexed(plan['population']['groups'],'example_id');old_ids={c['image_id'] for c in old['support_cases']}
    dev=SOURCE_ROOT.parent/'qwen3-vl-2b-sft256-source-dev128-natural-v1'
    dev_ids={str(int(Path(r['image_path']).stem)) for r in rows(dev/'gt_vs_pred.jsonl')}
    require(not set(NEW_REFERENCES)&(old_ids|set(old['positive_image_ids'])|dev_ids),'added support overlap')
    added=[]
    for image in NEW_REFERENCES:
        card=cards[image];g=groups[card['example_id']]
        require(file_hash(g['image_path'])==card['image_content_sha256']==g['image_content_sha256'] and
                card['executed_media_sha256']==g['executed_media_sha256'],'new Source image/media identity')
        added.append(dict(image_id=image,example_id=card['example_id'],action_ids=card['action_ids'],prompt_token_ids=g['prompt_token_ids'],group=g,
                          reference_origin='original_Source_train256_natural_EOS',source_action_ids_sha256=card['action_ids_sha256']))
    validate_new_references(added,cards)
    support=sorted(old['support_cases']+added,key=lambda c:int(c['image_id']))
    require(len(support)==50 and sum(len(c['action_ids']) for c in support)==6024,'support50 complete-state count')
    for c in old['support_cases']:require(next(x for x in support if x['image_id']==c['image_id'])==c,'old47 reference modified')
    sources={**old['source_files'],**full['source_files'],str(PRIOR/'inputs.json'):file_hash(PRIOR/'inputs.json'),str(PREFLIGHT):file_hash(PREFLIGHT)}
    return old,support,added,cards,sources


def objective_fixture(old,support,added,cards):
    validate_new_references(added,cards)
    positive_proof=prior_positive_fixture(old['cases'],old['trajectories'])
    require(positive_proof['status']=='passed','unchanged seven positive/local fixture')
    for c in old['cases']:
        t=old['trajectories'][c['case_id']];x=torch.zeros(len(t['action_ids']),152670);ref=torch.log_softmax(x[t['preservation_positions']],-1)
        item=dict(kind='entrance',case=c,action_ids=t['action_ids'],positions=t['preservation_positions'],suffix_start=t['suffix_start'])
        a=seven47_loss(x,torch.tensor(t['action_ids']),item,ref);b=global_image_loss(x,torch.tensor(t['action_ids']),item,ref)
        require(all(torch.equal(u,v) for u,v in zip(a,b)),'positive/local contribution changed')
    ids=added[0]['action_ids'];x=torch.full((len(ids),152670),-1000.);x[:,:2]=math.log(.5);x.requires_grad_(True)
    ref=torch.full_like(x,-1000.);ref[:,0]=math.log(.8);ref[:,1]=math.log(.2)
    item=dict(kind='support',action_ids=ids,positions=list(range(len(ids))))
    loss,ce,kl=global_image_loss(x,torch.tensor(ids),item,ref)
    expected=2*(.8*math.log(1.6)+.2*math.log(.4));require(ce is None and abs(float(loss.detach())-expected)<1e-6,'support50 rather than47 denominator')
    loss.backward();error=abs(float(x.grad[-1,0])-(-.3)*2/len(ids));require(error<1e-6,'full-state EOS support50 gradient')
    local=[];counts=[]
    for rank in range(8):
        pieces=[torch.tensor([(j+1)/100,(-1)**j*.2],dtype=torch.float64)*2 for j in range(rank,50,8)]
    ...
    expected_global=sum(local);averaged=torch.stack([compensate_for_ddp(v) for v in local]).mean(0);clip=lambda v:v*min(1.,1/float(v.norm()))
    require(counts==[8,8,7,7,7,7,7,6] and torch.allclose(expected_global,averaged) and
            not torch.allclose(averaged/8,expected_global) and not torch.allclose(torch.stack([8*v/n for v,n in zip(local,counts)]).mean(0),expected_global) and
            not torch.allclose(torch.stack([clip(8*v) for v in local]).mean(0),clip(expected_global)),'DDP compensation/rank-mean/preclip sensitivity')
    return dict(status='passed',positive_and_local_unchanged=True,support_denominator=50,support_KL_image_weight=2.,
                old47_denominator_rejected=True,new_Source_EOS_references_verified=True,compensation_and_clip_checks=True,
                max_gradient_weight_error=max(error,positive_proof['max_gradient_weight_error']),
                new_references=[dict(image_id=c['image_id'],action_length=len(c['action_ids']),full_sequence=len(c['action_ids'])+len(c['prompt_token_ids']),action_ids_sha256=digest(c['action_ids'])) for c in added])


def prepare(output):
    require(not output.exists(),'occupied stable50 output')
    old,support,added,cards,sources=build_data();proof=load_canonical_json(PRIOR/'receipt.json')
    require(proof['status']=='completed' and proof['updates']==81 and proof['global_model_forwards']==4442 and
            proof['numerical_admission']=='ddp_path_reuse_positive7_support47_v1','prior81 DDP path proof')
    for ref in [proof['two_step'],proof['launcher_exit']]:require(file_hash(ref['path'])==ref['sha256'],'prior proof bytes')
    require(load_canonical_json(Path(proof['two_step']['path']))['status']=='passed' and load_canonical_json(Path(proof['launcher_exit']['path']))['exit_code']==0,'prior complete path')
    fixture=objective_fixture(old,support,added,cards);sources[str(PRIOR/'receipt.json')]=file_hash(PRIOR/'receipt.json')
    ...
    packet={k:old[k] for k in ['cases','trajectories','positive_image_ids','local_KL_states','positive_trajectory_tokens','model','train_source','positive_admission']}
    packet.update(schema_version='selective_preservation_stable.inputs.v1',support_cases=support,support_image_ids=[c['image_id'] for c in support],
        added_support_image_ids=list(NEW_REFERENCES),support_action_states=6024,total_cached_states=6297,
        source_files=sources,optimizer=OPTIMIZER,clip_gradient_norm=1.,lambda_kl=10.,lambda_support_kl=100.,updates=81,
        numerical_admission=NUMERICAL_ADMISSION,prior_path_proof=dict(path=str(PRIOR/'receipt.json'),sha256=file_hash(PRIOR/'receipt.json')),
        objective_fixture=dict(path=str(output/'objective-fixture.json'),sha256=file_hash(output/'objective-fixture.json')),
        objective='mean7CE+10mean7localKL+100mean50supportKL; exactoldsevenlabels/localmasks81; globallynormalizedloss*8 beforeDDPmean andclipafterreduction.',
        selection_disclosure='Exactly73843/360573/545632 selected adaptively from exposed Seven81 new caps; references are original Source cleanEOS, not candidate paths. No failed output removed from evaluation.',
        bounds=dict(model_loads=8,reference_forwards=57,train_forwards=4617,entry_score_forwards=14,model_forwards=4688,
                    rank_model_forwards=CEILINGS,reference_cache_bytes=3845451960,rank_seconds=3600,distributed_timeout_seconds=600))
    for rank in range(8):rank_items(packet,rank)
    publish(output/'inputs.json',packet)
    files=[Path(__file__),Path(__file__).with_name('tests')/'test_selective_preservation_stable.py',CONFIG,
        Path(__file__).with_name('selective_preservation_seven.py'),Path(__file__).with_name('selective_preservation_dense.py'),
...
def execute_rank(output):
    ...
    with (run/'execution.log').open('x') as log,redirect_stdout(log),redirect_stderr(log):
      try:
        ...
        require(sum(s['reference_count'] for s in refstats)==57 and sum(s['cache_bytes'] for s in refstats)==3845451960,'global reference bounds')
        ...
        if rank==0:
          try:
            ...
            provisional=dict(schema_version='selective_preservation_stable.training.v1',status='unsealed',adapter=adapter,
                ...
                positive_image_ids=packet['positive_image_ids'],support_image_ids=packet['support_image_ids'],support_action_states=6024,
                local_KL_states=273,positive_trajectory_tokens=325,total_cached_states=6297,removed_support_image_ids=['529411'],added_support_image_ids=packet['added_support_image_ids'],positive_admission=packet['positive_admission'],selection_disclosure=packet['selection_disclosure'],
...
def finalize(output):
    ...
    for rank in range(8):
        ...
        require(t['status']=='completed' and t['rank']==rank and t['updates']==81 and t['model_forwards']==CEILINGS[rank] and
                t['counters']['support_KL_trajectories']==(567 if rank<2 else 486) and t['counters']['supervised_target_tokens']==(81 if rank<7 else 0),'current rank counts')
        terminals.append(t);refs.append(dict(rank=rank,path=str(path),sha256=file_hash(path)))
    require(len({t['state']['adapter_hash'] for t in terminals})==len({t['state']['optimizer_hash'] for t in terminals})==1,'terminal rank states')
    p=load_canonical_json(output/'provisional.json');require(p['numerical_admission']==NUMERICAL_ADMISSION and p['updates']==81 and len(p['cases'])==7 and len(p['support_image_ids'])==50,'stable50 recipe')
    for ref in [p['prior_path_proof'],p['objective_fixture'],p['two_step']]:require(file_hash(ref['path'])==ref['sha256'],'admission evidence')
    fixture=load_canonical_json(Path(p['objective_fixture']['path']));smoke=load_canonical_json(Path(p['two_step']['path']))
    require(fixture['status']=='passed' and fixture['support_denominator']==50 and fixture['positive_and_local_unchanged'] and
            fixture['old47_denominator_rejected'] and fixture['new_Source_EOS_references_verified'] and fixture['compensation_and_clip_checks'],'current objective/reference fixture')
    require(smoke['status']=='passed' and smoke['updates']==[1,2] and smoke['initial_reference_count']==57 and smoke['initial_KL_zero'] and
    ...
    require(positive==567 and support==4050 and sum(t['model_forwards'] for t in terminals)==4688,'global counts')
    receipt={**p,'status':'completed','rank_terminals':refs,'launcher_exit':dict(path=str(exit_path),sha256=file_hash(exit_path)),
        'actual_positive_uses':positive,'actual_support_uses':support,'global_model_forwards':4688,'provisional_sha256':file_hash(output/'provisional.json'),
        'resources':dict(model_loads=8,reference_forwards=sum(t['counters']['reference_forwards'] for t in terminals),train_forwards=sum(t['counters']['train_forwards'] for t in terminals),
            entry_score_forwards=sum(t['counters']['entry_score_forwards'] for t in terminals),cumulative_rank_model_seconds=sum(t['cumulative_model_seconds'] for t in terminals),
            max_rank_model_seconds=max(t['cumulative_model_seconds'] for t in terminals),reference_cache_bytes=sum(t['state']['reference_cache_bytes'] for t in terminals),
...
def launch(output):
    ...
    command=[sys.executable,'-m','torch.distributed.run','--standalone','--nnodes=1','--nproc-per-node=8','-m','probes.dora_owner_learning.selective_preservation_stable','rank','--output',str(output)]
...
def main():
    ...
    if a.command=='prepare':p=prepare(a.output);print(json.dumps(dict(support=len(p['support_image_ids']),added=p['added_support_image_ids'],bounds=p['bounds'])))
    elif a.command=='rank':execute_rank(a.output)
    elif a.command=='launch':raise SystemExit(launch(a.output))
    else:
        r=load_canonical_json(a.output/'receipt.json');require(r['status']=='completed' and r['global_model_forwards']==4688 and r['updates']==81,'sealed current receipt')
        for ref in r['rank_terminals']+[r['launcher_exit'],r['prior_path_proof'],r['objective_fixture'],r['two_step']]:require(file_hash(ref['path'])==ref['sha256'],'sealed proof')
        for ident in [r['adapter'],r['source_embedding']]:
            for f in ident['files']:require(file_hash(Path(ident['root'])/f['relative_path'])==f['sha256'],'sealed model')
