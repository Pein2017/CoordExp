...
def _row_text(description="cat", coords=(100, 100, 300, 300)):
    return (
        f"<|object_ref_start|>{description}<|object_ref_end|><|box_start|>"
        + "".join(f"<|coord_{value}|>" for value in coords)
        + "<|box_end|>"
    )


def _mine_fixture(monkeypatch, tmp_path, generated_ids, *, row_decodes=None,
                  journal_name="refresh.jsonl"):
    from src.qwen import generation

    h_plus = [71, 72]
    repeated_row = [ROW_START, 101, REF_END, BOX_START, 201, 202, 203, 204, ROW_END]
    credible_row = [ROW_START, 102, REF_END, BOX_START, 201, 202, 203, 204, ROW_END]
    history_text = _row_text()
    decodes = {tuple(repeated_row): history_text, **(row_decodes or {})}
    ...
    class Tokenizer:
        ...
        def decode(self, ids, *, skip_special_tokens=False):
            ...
            if list(ids) == h_plus:
                return history_text
            key = tuple(ids)
            value = decodes.get(key)
            if isinstance(value, BaseException):
                raise value
            assert value is not None, key
            return value

    def generate(*_, extensions, budgets, **__):
        assert extensions == (h_plus,)
        assert budgets == (t.MAX_NEW_TOKENS - len(h_plus),)
        return [SimpleNamespace(token_ids=tuple(generated_ids), stop_reason="length")]

    monkeypatch.setattr(generation, "generate_continuations", generate)
    package = {
        "package_id": "package-1",
        "h_plus_token_ids": h_plus,
        "h_text": history_text,
        "credible_c_token_ids": credible_row,
        "case": {"row_id": "row-1"},
        "golden": {
            "example_id": "example-1", "gt": [], "image_height": 100,
            "image_path": "/image.jpg", "image_width": 100,
            "row_id": "row-1", "row_index": 0,
        },
        "c_record": {
            "record_id": "c-1", "example_id": "example-1",
            "prompt_token_ids_sha256": "a" * 64,
            "image": {"executed_media_sha256": "b" * 64},
        },
    }
    c_entry = {
        "inputs": {}, "prompt_ids": [11, 12],
        "record": {"record_id": "c-1", "target_token_ids": credible_row},
    }
    journal = tmp_path / journal_name
    result = t._mine_one(
        object(), SimpleNamespace(tokenizer=Tokenizer()), package, c_entry,
        journal_path=journal, phase="train", rank=0, before_update=161,
    )
    rows = [json.loads(line) for line in journal.read_text().splitlines()]
    return result, rows, repeated_row


def test_mine_uses_first_valid_repeat_even_when_later_suffix_is_incomplete(monkeypatch, tmp_path):
    normal, normal_journal, row = _mine_fixture(
        monkeypatch, tmp_path, [ROW_START, 101, REF_END, BOX_START, 201, 202, 203, 204,
                               ROW_END, EOS], journal_name="normal.jsonl",
    )
    tailed, tailed_journal, _ = _mine_fixture(
        monkeypatch, tmp_path, [*row, ROW_START, 999], journal_name="tailed.jsonl",
    )
    normal_event, normal_receipt = normal
    tailed_event, tailed_receipt = tailed
    assert tailed_event == normal_event
    assert {key: tailed_event[key] for key in (
        "prefix_token_ids", "divergence_index", "repeat_token_id", "credible_token_id"
    )} == {
        "prefix_token_ids": [ROW_START], "divergence_index": 1,
        "repeat_token_id": 101, "credible_token_id": 102,
    }
    assert tailed_receipt["first_complete_row"] == row
    assert tailed_receipt["strict_repeat"] is True
    assert tailed_receipt["repeated_history_iou"] == 1.0
    assert tailed_receipt["divergence"] == normal_receipt["divergence"]
    assert [entry["kind"] for entry in tailed_journal] == [
        "generated_refresh", "parsed_outcome"
    ]
    assert tailed_journal[0]["generated_token_ids"] == [*row, ROW_START, 999]
    assert tailed_journal[0]["h_plus_token_ids"] == [71, 72]
    assert tailed_journal[0]["continuation_budget"] == t.MAX_NEW_TOKENS - 2
    assert tailed_journal[0]["total_allowance"] == 3084
    assert tailed_journal[0]["stop_reason"] == "length"
    assert tailed_journal[1]["receipt"]["first_row_outcome"] == "active_strict_repeat"
    assert normal_journal[1]["receipt"]["first_row_outcome"] == "active_strict_repeat"


@pytest.mark.parametrize(("generated", "outcome"), [
    ([], "inactive_empty_or_eos_only"),
    ([EOS], "inactive_empty_or_eos_only"),
    ([ROW_START, 101, REF_END], "inactive_incomplete_first_row"),
    ([999, ROW_START, 101, REF_END, BOX_START, 201, 202, 203, 204, ROW_END],
     "inactive_malformed_first_token"),
    ([ROW_START, 101, ROW_START, 101, REF_END, BOX_START, 201, 202, 203, 204, ROW_END],
     "inactive_nested_first_row"),
])
def test_mine_malformed_immediate_material_is_inactive_without_resynchronizing(
    monkeypatch, tmp_path, generated, outcome,
):
    (event, receipt), journal, _ = _mine_fixture(monkeypatch, tmp_path, generated)
    assert event is None
    assert receipt["strict_repeat"] is False
    assert receipt["first_row_outcome"] == outcome
    assert journal[1]["receipt"]["first_row_outcome"] == outcome


def test_mine_valid_nonrepeat_and_parser_invalid_first_rows_are_inactive(monkeypatch, tmp_path):
    nonrepeat_ids = [ROW_START, 111, REF_END, BOX_START, 211, 212, 213, 214, ROW_END]
    (event, receipt), _, _ = _mine_fixture(
        monkeypatch, tmp_path, [*nonrepeat_ids, EOS],
        row_decodes={tuple(nonrepeat_ids): _row_text(coords=(600, 600, 800, 800))},
        journal_name="nonrepeat.jsonl",
    )
    assert event is None and receipt["first_row_outcome"] == "inactive_nonrepeat"
    assert receipt["max_history_iou"] == 0.0

    invalid_ids = [ROW_START, 112, REF_END, BOX_START, 221, 222, 223, 224, ROW_END]
    (event, receipt), journal, _ = _mine_fixture(
        monkeypatch, tmp_path, [*invalid_ids, EOS],
        row_decodes={tuple(invalid_ids): _row_text(description="")},
        journal_name="invalid.jsonl",
    )
    assert event is None and receipt["first_row_outcome"] == "inactive_invalid_or_dropped"
    assert receipt["first_row_parse"] == "invalid_or_dropped"
    assert journal[1]["event_active"] is False


def test_mine_journals_full_generation_before_unexpected_parse_failure(monkeypatch, tmp_path):
    row = [ROW_START, 113, REF_END, BOX_START, 231, 232, 233, 234, ROW_END]
    journal = tmp_path / "fatal.jsonl"
    with pytest.raises(RuntimeError, match="decode fault"):
        _mine_fixture(
            monkeypatch, tmp_path, [*row, ROW_START, 999],
            row_decodes={tuple(row): RuntimeError("decode fault")},
            journal_name=journal.name,
        )
    entries = [json.loads(line) for line in journal.read_text().splitlines()]
    assert len(entries) == 1 and entries[0]["kind"] == "generated_refresh"
    assert entries[0]["generated_token_ids"] == [*row, ROW_START, 999]
    assert entries[0]["rank"] == 0 and entries[0]["before_update"] == 161


def test_completed_update_journal_is_append_only_scalar_evidence(tmp_path):
    path = tmp_path / "updates.jsonl"
    counters = {"optimizer_steps": 1, "model_forwards": 10, "image_forwards": 2,
                "model_loads": 1, "refreshes": 7}
    t._journal_completed_update(
        path, rank=0, arm="B", update=1, components={"fork": 0.25},
        raw_gradient_norm=3.5, replays=12, active_local_forks=4, counters=counters,
    )
    counters["optimizer_steps"] = 2
    t._journal_completed_update(
        path, rank=0, arm="B", update=2, components={"fork": 0.5},
        raw_gradient_norm=2.5, replays=13, active_local_forks=3, counters=counters,
    )
    rows = [json.loads(line) for line in path.read_text().splitlines()]
    assert [row["update"] for row in rows] == [1, 2]
    assert rows[0]["counters"]["optimizer_steps"] == 1
    assert rows[1]["counters"]["optimizer_steps"] == 2
