...
ROOT = B / 'third-complete-bank-preparation-v2'
...
def build(*, catalog_path: Path = CATALOG, output: Path = ROOT) -> dict[str, Any]:
    ...
    # Admissions grounded in a parent step-16 row retain that native position.
    # Step-32-only admissions have no parent-16 order and are appended below.
    source_by_proposal = {str(row['proposal_id']): row for row in source_rows}
    for entry in first_admissions['entries']:
        source = source_by_proposal.get(str(entry['reference_proposal_id']))
        if source and source.get('step') == 16:
            source = dict(source)
            source['owner_id'] = str(entry['owner_id'])
            source_by_image[int(source['image_id'])].append(source)
