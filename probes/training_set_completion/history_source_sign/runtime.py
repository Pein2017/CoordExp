...
class Runtime:
    def __init__(self, args: argparse.Namespace, cell: dict[str, Any] | None = None) -> None:
        ...
        if args.mode == "qualify":
            ...
            self.cell = cell
            repair = int(getattr(args, "repair", 0))
            self.job_dir = args.output_root / "runtime" / str(cell["id"])
            if repair:
                self.job_dir = args.output_root / "runtime" / f"{cell['id']}--repair-{repair}"
    ...
    def run(self) -> None:
        ...
        try:
            result = self.qualify() if self.args.mode == "qualify" else self._run_cell()
            if self.args.mode == "qualify" and not bool(result.get("passed")):
