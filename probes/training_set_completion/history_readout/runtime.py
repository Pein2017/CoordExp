...
class Runtime(component.Runtime):
    ...
    def _normalise_jobs(self, plan: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for raw_cell in planned:
            ...
            # Keep the original source binding immutable.  The synthetic
            # boundary below is deliberately edited for the supplied history;
            # aliasing these two dictionaries would make the inherited native
            # identity check compare the source against its own replacement.
            boundary = copy.deepcopy(boundary_original)
    ...
    def _run_job(self, job: dict[str, Any]) -> dict[str, Any]:
        ...
        # The inherited runtime records the cell receipt before this history
        # metadata rewrite.  Refresh that artifact binding so the shard ledger
        # remains a complete, content-addressed manifest.
        receipt_binding = _binding(receipt_path)
        refreshed = False
        for index, item in enumerate(self.ledger["artifacts"]):
            if item.get("path") == receipt_binding["path"]:
                self.ledger["artifacts"][index] = receipt_binding
                refreshed = True
        if not refreshed:
            raise RuntimeError(f"rewritten cell receipt is absent from artifact ledger: {receipt_path}")
