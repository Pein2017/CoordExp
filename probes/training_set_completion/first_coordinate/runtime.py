    ...
    def _run_job(self, job: dict[str, Any]) -> dict[str, Any]:
        ...
        class Mediation(LogitsProcessor):
            def __call__(_self, tokens: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
                ...
                if override_applied:
                    # This is the semantic seam: inspect the native prefix and
                    # the frozen slot role before constructing the carrier.  A
                    # late post-generation check would allow a misplaced
                    # override to masquerade as the first x1 mediation.
                    generated_prefix = tokens[target_index, width:].tolist()
                    if generated_prefix != list(EXPECTED_PRELUDE):
                        raise NeedsLead(
                            "offset-five carrier reached with a changed target prelude"
                        )
                    slot_offset = source_end + forced_offset
                    x1_slots = [
                        slot
                        for slot in boundary.get("target_slots", [])
                        if int(slot.get("offset", -1)) == slot_offset
                    ]
                    if len(x1_slots) != 1 or x1_slots[0].get("role") != "x1":
                        raise NeedsLead(
                            "offset-five carrier slot is not the frozen first x1 role"
                        )
                    if int(x1_slots[0].get("row_delay", -1)) != 0:
                        raise NeedsLead(
                            "offset-five carrier slot is not the frozen box start"
                        )
