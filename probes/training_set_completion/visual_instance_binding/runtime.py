...
class Runtime:
    ...
    def _snapshot_sources(self) -> None:
        ...
        sources += [Path(_input_identity.__code__.co_filename), Path(preserve_source.__code__.co_filename)]
        snap_dir.mkdir()
        entries = []
        for index, source in enumerate(dict.fromkeys(path.resolve() for path in sources)):
            ...
            if source.suffix.lower() in {".py", ".md", ".sh"}:
                target = preserve_source(source, run_root=self.output,
                                         relative_name=Path("source-snapshot") / name)
            else:
