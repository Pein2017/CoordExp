...
def verify(root):
    ...
    for shard in sorted((root/'runtime').iterdir()):
        ...
        for rp in sorted(shard.glob('*/release.json')):
            ...
            boundary=boundaries[r['boundary_id']];native=boundary['native_tokens'][boundary['source_row']['end']:];n=min(len(tokens),len(native));source_equal=tokens[:n]==native[:n]
            q=r['qualification']
            if q:
                ...
                if r['policy']=='original':assert source_equal
            reports.append(dict(cell=r['job_id'],steps=len(tokens),formula_max_abs=error,decomposition_max_abs=decomposition,original_source_overlap_equal=source_equal if r['policy']=='original' else None,source_overlap_tokens=n if r['policy']=='original' else None,trajectory_sha256=hashlib.sha256((rp.parent/'trajectory.pt').read_bytes()).hexdigest()))
