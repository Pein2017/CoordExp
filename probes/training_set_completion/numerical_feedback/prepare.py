...
def main():
 ...
 source=Path('/data/CoordExp/public_data/coco/rescale_32_1024_bbox_len12000_xy_sorted/val.coord.jsonl');records={x['image_id']:x for x in map(json.loads,source.open()) if x['image_id'] in [885,5586,632]};cases=[];runtime=R/'new-three.runtime.jsonl'
 for i,iid in enumerate([885,5586,632]):
  row=copy.deepcopy(records[iid]);image=(source.parent/row['images'][0]).resolve();im=Image.open(image);row['images']=[os.path.relpath(image,R)]
  assert str(image).startswith('/data/CoordExp/public_data/coco/rescale_32_1024_bbox/images/')
 ...
 q=load_qwen_components_from_options(QwenLoadOptions(base_model=p['configs']['untied']['model']['base_model'],dtype='fp32',attn_implementation='sdpa',patch_embed_linearization='enabled',load_model=False))
 config=InferConfig.model_validate(p['configs']['untied'])
 for i,case in enumerate(cases):
  row=case['input_record'];raw=raw_example_from_jsonl_row(row,jsonl_path=runtime,row_number=i+1,raw_line=json.dumps(row));plan,=plan_examples([raw],config=config,components=q,row_indices=[i]);im=plan.image
  case['row_id']=str(raw.example_id);case['image_plan']=dict(backend_prompt_token_count=len(plan.prompt.expected_executed_prompt_token_ids),image_content_sha256=im.image_content_sha256,logical_transform_id=im.logical_transform_id,merged_visual_tokens=im.merged_visual_tokens,observed_image_grid_thw=list(im.expected_image_grid_thw))
