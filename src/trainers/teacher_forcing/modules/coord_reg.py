...
def run_coord_reg_module(
    *,
    ...
    if not isinstance(coord_slot_weights, torch.Tensor) or int(coord_slot_weights.numel()) != int(target_bins.numel()):
        coord_slot_weights = torch.ones_like(target_bins, dtype=torch.float32, device=context.logits.device)
    coord_slot_weights = coord_slot_weights.to(device=context.logits.device, dtype=torch.float32)
    coord_weight_denom = coord_slot_weights.sum().clamp(min=1e-6)
    ...
    if weights["coord_ce_weight"] != 0.0:
        ...
        coord_ce = ((coord_ce_per * coord_slot_weights) / coord_weight_denom).sum().to(dtype=context.logits.dtype)
    ...
    if weights["coord_soft_ce_weight"] != 0.0 or weights["coord_w1_weight"] != 0.0:
        ...
        coord_soft_ce = ((dist.soft_ce_per_token * coord_slot_weights) / coord_weight_denom).sum().to(dtype=context.logits.dtype)
        coord_w1 = ((dist.w1_per_token * coord_slot_weights) / coord_weight_denom).sum().to(dtype=context.logits.dtype)
    ...
    if weights["coord_gate_weight"] != 0.0:
        ...
        coord_gate = ((gate_per_token * coord_slot_weights) / coord_weight_denom).sum().to(dtype=context.logits.dtype)
    ...
    if (
        ...
        if weights["coord_entropy_weight"] != 0.0:
            ...
            coord_entropy = ((-(p * p.log()).sum(dim=-1) * coord_slot_weights) / coord_weight_denom).sum().to(dtype=context.logits.dtype)
        ...
        if weights["coord_el1_weight"] != 0.0:
            coord_el1 = (((probs * diff.abs()).sum(dim=-1) * coord_slot_weights) / coord_weight_denom).sum().to(dtype=context.logits.dtype)
        ...
        if weights["coord_ehuber_weight"] != 0.0:
            ...
            coord_ehuber = (((probs * huber).sum(dim=-1) * coord_slot_weights) / coord_weight_denom).sum().to(dtype=context.logits.dtype)
