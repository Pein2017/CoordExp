...
from src.trainers.rollout_correction.residual_set import CorrectionEvent
...
def build_stage2_teacher_forcing_target_ir(
    *,
    ...
    surface = str(meta.get("stage2_surface", "rollout_correction") or "rollout_correction")
    ...
    for target in object_targets:
        ...
        tags = {"stage2", "rollout_correction", source}
        if source == "fn":
            tags.add("fn")
    ...
    for desc_position in _absolute_tail_positions(
        ...
        atoms.append(
            SupervisionAtom(
                ...
                loss_tags=frozenset(
                    {"stage2", "rollout_correction", "description"}
                ),
                loss_weight=1.0,
                coord_role=None,
                provenance={
                    ...
                    "context": str(meta.get("rollout_context", "rollout_context")),
                },
            )
        )
    ...
    metadata = {
        "stage": "stage2",
        "stage2_surface": surface,
        "stage2_context": str(meta.get("rollout_context", "rollout_context")),
...
def _residual_atom_provenance(
    *,
    ...
    if event.metadata.get("correction_builder") is not None:
        provenance["correction_builder"] = str(event.metadata["correction_builder"])
