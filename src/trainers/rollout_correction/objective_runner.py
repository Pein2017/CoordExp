...
def run_stage2_objective_pipelines(
    *,
    ...
    if str(channel) in {"A", "B", "channel_a", "channel_b"}:
        raise ValueError(
            "stage2_rollout_correction objective execution has no A/B channel; "
            "use channel='rollout_correction'."
        )
    objective_specs_ctx = list(objective_specs or [])
    ...
    *,
    ...
    for key, value in pipeline_metrics_ctx.items():
        ...
        if not (
            ...
            or key_s.startswith("stage2_rollout_correction/residual_set/")
            or key_s == "loss/stage2_rollout_correction/stage2_trie_ce"
