    ...
    *,
    ...
    run_stage2_ddp_monitored_barrier(
        ...
        missing_barrier_label=(
            "torch.distributed.monitored_barrier is required for bounded stage2_rollout_correction DDP barriers"
        ),
        timeout_error_label="stage2_rollout_correction DDP barrier timed out",
    ...
    *,
    ...
    if wait_timeout_cfg is None:
        ...
        try:
            ...
            raise ValueError(
                "stage2_rollout_correction.correction.producer_wait_timeout_s must be a float/int when set"
            ) from exc
    ...
    phase_config = resolve_rollout_correction_ddp_phase_config(
        owner,
...
def accumulate_rollout_correction_producer_item(
    *,
...
def consume_rollout_correction_queue_item(
    *,
    ...
    owner._stage2_append_post_rollout_segments(channel="rollout_correction", segments=segs)
    return accumulate_rollout_correction_producer_item(
        segs=segs,
        metrics=metrics,
        raw_n=int(raw_n),
...
def prepare_rollout_correction_pipeline_pack_step(
    *,
    ...
    is_last_pack = (int(seen_raw) >= int(total_segments_target)) and (
        not bool(owner._stage2_post_rollout_buffer(channel="rollout_correction"))
    )
    if bool(is_last_pack):
        ddp_phase_barrier_fn(
            "rollout_correction_pipeline_before_final_sync_backward",
...
def finalize_rollout_correction_pipeline_step(
    *,
    ...
    if thread_obj.is_alive():
        raise RuntimeError(
            "stage2_rollout_correction rollout-correction producer thread did not terminate cleanly after pipeline step"
        )
    ...
    if int(total_segments_target) > 0 and int(seen_raw) != int(total_segments_target):
        raise ValueError(
            "stage2_rollout_correction rollout-correction pipeline produced unexpected raw-rollout count: "
    ...
    if loss_total is None:
        raise AssertionError("stage2_rollout_correction rollout-correction pipelined step produced no packs")
    return loss_total
...
def run_rollout_correction_train_one_pack(
    *,
    ...
    owner._assert_single_packed_forward(
        batch,
        where="stage2_rollout_correction/packed_forward",
    )
    ...
    if bool(shadow_zero_loss):
        batch["_stage2_rollout_correction_shadow_pack"] = True
    ...
    with cm:
        ...
        try:
            with loss_ctx:
                with owner._rollout_correction_disable_average_tokens_across_devices_for_packed_step(
                    dist=dist,
                    ddp_rank=int(ddp_rank),
                    ddp_world_size=int(ddp_world_size),
                    where="stage2_rollout_correction/train_one_pack",
                ):
    ...
    if backend == "nccl":
        ...
        if torch.cuda.is_available():
            try:
                ...
                pass
        raise RuntimeError(
            "stage2_rollout_correction DDP pack-count gather with NCCL requires a CUDA device"
        )
    ...
    *,
    ...
    if callable(all_gather_object):
        ...
        if len(counts) != int(ddp_world_size):
            raise RuntimeError(
                "stage2_rollout_correction rollout-correction DDP pack scheduling gathered unexpected world size: "
    ...
    if not callable(all_gather):
        raise RuntimeError(
            "stage2_rollout_correction rollout-correction DDP pack scheduling requires torch.distributed.all_gather_object or all_gather"
        )

    device = _rollout_correction_pack_count_gather_device(owner=owner, model=model, dist=dist)
...
def build_rollout_correction_pack_schedule(
    *,
    ...
    pack_counts = gather_rollout_correction_local_pack_counts(
        owner=owner,
        model=model,
        local_pack_count=int(local_pack_count),
        dist=dist,
    ...
    *,
    ...
    def _trace(phase: str, *, extra: Mapping[str, Any] | None = None) -> None:
        ...
        trace_payload = {
            ...
            "rollout_correction_buffer_size": int(
                len(owner._stage2_post_rollout_buffer(channel="rollout_correction"))
            ),
        }
    ...
    _trace("rollout_correction_non_pipeline_before_flush")
    owner._stage2_flush_train_monitor_dump(global_step=target_log_step)
    if not isinstance(segments, list):
        raise ValueError(
            "stage2_rollout_correction step mode expected post-rollout segments as a list; "
            f"got {type(segments).__name__}"
        )

    batch_metrics = dict(batch_metrics) if isinstance(batch_metrics, Mapping) else {}
    batch_metrics["stage2_rollout_correction/train_monitor_dump_written"] = float(
        1.0
    ...
    _trace(
        "rollout_correction_non_pipeline_after_flush",
        extra={
            "train_monitor_dump_written": float(
                batch_metrics["stage2_rollout_correction/train_monitor_dump_written"]
            )
        },
    )
    ...
    owner._stage2_append_post_rollout_segments(channel="rollout_correction", segments=segments)
    _trace("rollout_correction_non_pipeline_after_append")
    local_packs: list[
        tuple[Sequence[tuple[dict[str, Any], dict[str, Any], int]], dict[str, float]]
    ] = []
    while owner._stage2_post_rollout_buffer(channel="rollout_correction"):
        t_pack0 = time.perf_counter()
        selected, pack_metrics = owner._stage2_pop_post_rollout_pack(channel="rollout_correction")
    ...
    _trace("rollout_correction_non_pipeline_before_prepare_barrier")
    ...
    ddp_phase_barrier_fn(
        "rollout_correction_non_pipeline_after_prepare",
        timeout_s=float(ddp_phase_prepare_timeout_s),
    )
    _trace("rollout_correction_non_pipeline_after_prepare_barrier")

    schedule = build_rollout_correction_pack_schedule(
        owner=owner,
        model=model,
    ...
    if not schedule.has_global_packs:
        raise ValueError(
            "stage2_rollout_correction rollout-correction step mode produced zero post-rollout packs on all ranks; "
            "check rollout parsing, fallback policy, and target construction"
        )
    _trace(
        "rollout_correction_non_pipeline_after_pack_schedule",
    ...
    if (
        ...
        raise RuntimeError(
            "stage2_rollout_correction rollout-correction DDP pack schedule found a rank with zero local packs "
    ...
    for slot in schedule.slots:
        with owner._stage2_stage_wallclock_ctx("sft"):
            ...
            if slot.is_empty:
                _trace(
                    "rollout_correction_non_pipeline_empty_pack_slot",
                    extra={
                ...
                if bool(sync_gradients):
                    _trace(
                        "rollout_correction_non_pipeline_before_final_sync_backward_barrier",
                        extra={
                            "slot_index": int(slot.slot_index),
                            "global_slot_count": int(schedule.global_slot_count),
                            "local_pack_count": int(schedule.local_pack_count),
                    ...
                    ddp_phase_barrier_fn(
                        "rollout_correction_non_pipeline_before_final_sync_backward",
                        timeout_s=float(ddp_phase_final_sync_timeout_s),
                    )
                    _trace(
                        "rollout_correction_non_pipeline_after_final_sync_backward_barrier",
                        extra={
                            "slot_index": int(slot.slot_index),
                            "global_slot_count": int(schedule.global_slot_count),
                            "local_pack_count": int(schedule.local_pack_count),
                ...
                loss_pack = run_rollout_correction_train_one_pack(
                    owner=owner,
                    model=model,
    ...
    if loss_total is None:
        raise AssertionError("stage2_rollout_correction rollout-correction step mode produced no packs")
    return loss_total
...
def run_rollout_correction_pipeline_learning_loop(
    *,
    ...
    while (not producer_done) or owner._stage2_post_rollout_buffer(channel="rollout_correction"):
        while (not producer_done) and (buf_total_len < int(prefill_target_len)):
            try:
                ...
                pending_buf = int(len(owner._stage2_post_rollout_buffer(channel="rollout_correction")))
                raise RuntimeError(
                    "stage2_rollout_correction rollout-correction pipeline stalled while waiting for producer output; "
            ...
            seen_segments, seen_raw, buf_total_len = consume_rollout_correction_queue_item(
                owner=owner,
                item=item,
                rollout_static=rollout_static,
                pending_totals=pending_totals,
        ...
        if not owner._stage2_post_rollout_buffer(channel="rollout_correction"):
            continue
        ...
        with owner._stage2_stage_wallclock_ctx("sft"):
            ...
            step_totals_pack, is_last_pack = prepare_rollout_correction_pipeline_pack_step(
                owner=owner,
                selected=selected,
                pending_totals=pending_totals,
                seen_raw=int(seen_raw),
    ...
    return finalize_rollout_correction_pipeline_step(
        thread_obj=th,
        owner=owner,
    ...
    try:
        for off in range(0, int(len(raw_samples)), int(rollout_decode_bs)):
            ...
            with owner._stage2_stage_wallclock_ctx("rollout"):
                segs, m = owner._prepare_rollout_correction_inputs(chunk, _segments_only=True)
            raw_n = int(len(chunk))
