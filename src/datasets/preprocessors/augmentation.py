...
class AugmentationPreprocessor(BasePreprocessor):
    ...
    def _augment_record(self, rec: ConversationRecord) -> ConversationRecord:
        ...
        # NOTE: Production `apply_augmentations` returns `images_bytes: list[dict]`.
        # Some tests/stubs return a list of paths; skip width/height update in that case.
        if images_bytes:
            first = images_bytes[0]
            if isinstance(first, dict):
                b0 = first.get("bytes")
                if b0 is None:
                    raise KeyError("apply_augmentations images_bytes[0] missing 'bytes'")
                if not isinstance(b0, (bytes, bytearray)):
                    raise TypeError(
                        "apply_augmentations images_bytes[0]['bytes'] must be bytes"
                    )
