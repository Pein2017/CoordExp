...
class PrefixDenoisingTrainingDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        self.detection_template_id = str(detection_template_id or "compact")
        self.object_ordering = str(object_ordering or "sorted")
    ...
    def _build_sample(self, base_idx: int, *, epoch: int) -> HybridPrefixDenoisingSample:
        return build_hybrid_prefix_denoising_sample(
            ...
            detection_template_id=self.detection_template_id,
            object_ordering=self.object_ordering,
            object_ordering_seed=_mix_seed(self.seed, int(epoch), int(base_idx)),
            epoch=int(epoch),
    ...
    def _build_one(base_idx: int) -> tuple[int, PrefixDenoisingPackingEstimate]:
        return (
            ...
            estimate_hybrid_prefix_denoising_packing(
                ...
                detection_template_id=detection_template_id,
                object_ordering=object_ordering,
                object_ordering_seed=_mix_seed(seed, 0, base_idx),
...
def _default_eligibility_fingerprint(
    *,
    ...
    return {
        ...
        "system_prompt": system_prompt,
        "seed": int(seed),
        "detection_template_id": str(detection_template_id or "compact"),
        "object_ordering": str(object_ordering or "sorted"),
