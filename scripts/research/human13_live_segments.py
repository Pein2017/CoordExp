...
class MaterializedSegments:
    ...
    def preflight(
        ...
        *,
        enforce_a4_aggregate: bool = False,
    ) -> None:
        if enforce_a4_aggregate:
    ...
    *,
    ...
    for image in manifest.images:
        ...
        h_rows = tuple(
            ...
            if row.owner_id in {o.owner_id for o in image.owners if o.stratum == "H"}
        )

        def add(
            role: str,
            suffix: Sequence[int],
            bindings: Sequence[Human13EncodedRowBinding],
            label: str,
        ) -> LogicalPanelSegment:
            ids = prompt + tuple(int(x) for x in suffix)
            segment = _segment(
                skeleton, image_id, role, f"human13:{image_id}:{label}", ids, bindings
            )
            return segment
        ...
        for row in h_rows:
            segments.append(
                add(
                    "h1_independent",
                    clean + tuple(row.token_ids),
                    _bindings(
                        "h",
                        (row,),
                        prompt_count + len(clean),
                        unit_by_row={row.row_id: row.owner_id},
                    ),
                    f"h1:{row.row_id}",
                )
            )
        ...
        for row in h_rows:
            donor = next(
                (t for t in image.trajectories if t.trajectory_id == row.trajectory_id),
                None,
            )
            if donor is None:
            ...
            segments.append(
                add(
                    "a6_donor_h1",
                    donor_prefix + tuple(row.token_ids),
                    _bindings(
                        "h",
                        (row,),
                        prompt_count + len(donor_prefix),
                        unit_by_row={row.row_id: row.owner_id},
                    ),
                    f"a6:{row.row_id}",
                )
        ...
        owner_by_row = {
            ...
            for row_id in getattr(owner, "source_row_ids", ())
        }
        segments.append(
            add(
                "source_replay",
        ...
        gt_suffix = tuple(
            token
            for owner in image.owners
            for token in owner_tokens.get(owner.owner_id, ())
        )
        ...
        for event in image.duplicate_events:
            dup_ids = tuple(event.decision_prefix_token_ids) + (
            ...
            segments.append(
                add(
                    "duplicate_event",
                    dup_ids,
                    (_duplicate_binding(event, prompt_count),),
                    event.event_id,
                )
            )
        ...
        for trajectory in image.trajectories:
            for row in trajectory.rows:
                ...
                candidate_rows[row.row_id] = SimpleNamespace(
                    row_id=row.row_id,
                    token_ids=tuple(
                        trajectory.raw_token_ids[row.token_start : row.token_end]
                    ),
                )
        for row_id in image.candidate_row_ids:
            ...
            owner_id = next(
                (
                    owner.owner_id
                    for owner in image.owners
                    if row_id in getattr(owner, "sampled_row_ids", ())
                ),
                row_id,
            )
            candidate = add(
                ...
                _bindings(
                    ...
                    (row,),
                    prompt_count + len(clean),
                    unit_by_row={row.row_id: owner_id},
                ),
                f"a4:{row.row_id}",
            )
    ...
    # Every independently encoded segment must fit.  The stronger same-image
    # aggregate gate belongs only to A4; otherwise an intentionally omitted A4
...
def _bindings(
    ...
    offset: int,
    *,
    unit_by_row: Mapping[str, str] | None = None,
    ...
    for row in rows:
        length = (
            len(row.token_ids)
            if hasattr(row, "token_ids")
            else int(row.token_end - row.token_start)
        )
        if length <= 0:
            continue
        unit_id = (unit_by_row or {}).get(row.row_id, row.row_id)
        mask = (
            tuple(row.target_token_mask)
            if hasattr(row, "target_token_mask")
            else (True,) * length
        )
        out.append(
            Human13EncodedRowBinding(
                family, unit_id, row.row_id, cursor, cursor + length, mask
            )
        )
        cursor += length
    return tuple(out)
...
def _replay_bindings(
    ...
    removed = {
        ...
        if row.row_id in set(source.duplicate_row_ids)
    ...
    for raw_index in range(
        len(
            source.raw_token_ids[
                : source.terminal_token_index or len(source.raw_token_ids)
            ]
        )
    ):
...
def _duplicate_binding(event: Any, prompt_count: int) -> Human13EncodedRowBinding:
    ...
    start = prompt_count
    return Human13EncodedRowBinding(
        "duplicate",
        event.event_id,
        event.duplicate_row_id,
        start,
        start + length,
        (False,) * (length - 1) + (True,),
    )
...
def _full_gt_bindings(
    owners: Sequence[Any], owner_tokens: Mapping[str, Sequence[int]], prompt_count: int
) -> tuple[Human13EncodedRowBinding, ...]:
    cursor = prompt_count
    out = []
    for owner in owners:
...
def _clone(
    ...
    segment_id: str,
    input_ids: tuple[int, ...],
    bindings: tuple[Human13EncodedRowBinding, ...],
) -> Any:
    ...
    values.update(
        example_id=segment_id, input_ids=input_ids, human13_row_bindings=bindings
    )
