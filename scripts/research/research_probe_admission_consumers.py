...
SUPPORT_CONSUMER = ACTIVE_RESEARCH_PROBES_ROOT / "scripts/research/run_natural_boundary_support_completion.py"
SUPPORT_CONSUMER_TEST = ACTIVE_RESEARCH_PROBES_ROOT / "tests/research/test_run_natural_boundary_support_completion.py"
SUPPORT_MERGER = ACTIVE_RESEARCH_PROBES_ROOT / "scripts/research/merge_natural_boundary_support_completion.py"
SUPPORT_MERGER_TEST = ACTIVE_RESEARCH_PROBES_ROOT / "tests/test_merge_natural_boundary_support_completion.py"
...
SUPPORT_PLAN = Path("/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-08-06-natural-boundary-routing-history-replication/support-completion-plan-v1/plan.json")
SUPPORT_CENSUS = Path("/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-08-06-natural-boundary-routing-history-replication/cpu-census-v2/admission-census.json")
SUPPORT_COMPATIBILITY_RECEIPT = Path(
    "/data/CoordExp/outputs/research-probe-infras/"
    "2026-08-09-production-shaped-admission-inputs-v3/"
    "support-cpu-compatibility.json"
)
CROSSOVER_RUNNER = ACTIVE_RESEARCH_PROBES_ROOT / "scripts/research/run_s_k10_h20_crossover_shard.py"
CROSSOVER_RUNNER_TEST = ACTIVE_RESEARCH_PROBES_ROOT / "tests/research/test_run_s_k10_h20_crossover_shard.py"
CROSSOVER_SEALER = ACTIVE_RESEARCH_PROBES_ROOT / (
    "scripts/research/seal_s_k10_h20_crossover_pre_gpu_receipt.py"
)
CROSSOVER_FINALIZER = ACTIVE_RESEARCH_PROBES_ROOT / "scripts/research/finalize_s_k10_h20_crossover.py"
CROSSOVER_FINALIZER_TEST = ACTIVE_RESEARCH_PROBES_ROOT / "tests/research/test_finalize_s_k10_h20_crossover.py"
CROSSOVER_SOURCE_PREFLIGHT = Path("/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-08-07-s-k10-h20-natural-crossover/pre-gpu-evidence-v5/source-preflight-preseal.json")
CROSSOVER_ACCEPTED_EVIDENCE = Path("/data/CoordExp/outputs/research/qwen3-vl-dense-enumeration/2026-08-07-s-k10-h20-natural-crossover/evidence-v4/evidence.json")
...
def support_binding_requests(*, consumer: Path = SUPPORT_CONSUMER, consumer_test: Path = SUPPORT_CONSUMER_TEST, plan: Path = SUPPORT_PLAN, census: Path = SUPPORT_CENSUS, merger: Path = SUPPORT_MERGER, merger_test: Path = SUPPORT_MERGER_TEST, adapter_test: Path = SUPPORT_ADAPTER_TEST, compatibility_receipt: Path = SUPPORT_COMPATIBILITY_RECEIPT) -> tuple[RegularFileBinding | AbsoluteExecutableBinding | StrictValueBinding, ...]:
    """Bind support source, tests, plan/census, and a read-only verification file."""

    return (
        RegularFileBinding("support_consumer", consumer), RegularFileBinding("support_consumer_test", consumer_test),
        RegularFileBinding("support_plan", plan), RegularFileBinding("support_census", census),
        RegularFileBinding("support_merger", merger), RegularFileBinding("support_merger_test", merger_test),
        ...
        RegularFileBinding(
            "consumer_admission_adapter", CONSUMER_ADMISSION_ADAPTER
        ),
        ...
        StrictValueBinding("support_mechanics_boundary", {"legacy_merger_closed": False}),
        *_command_binding_requests(compatibility_receipt, "support"),
    )


def crossover_binding_requests(*, runner: Path = CROSSOVER_RUNNER, runner_test: Path = CROSSOVER_RUNNER_TEST, sealer: Path = CROSSOVER_SEALER, finalizer: Path = CROSSOVER_FINALIZER, finalizer_test: Path = CROSSOVER_FINALIZER_TEST, source_preflight: Path = CROSSOVER_SOURCE_PREFLIGHT, accepted_evidence: Path = CROSSOVER_ACCEPTED_EVIDENCE, compatibility_receipt: Path = CROSSOVER_COMPATIBILITY_RECEIPT) -> tuple[RegularFileBinding | AbsoluteExecutableBinding, ...]:
    """Bind crossover production/test surfaces and opaque public receipts."""

    return (
        RegularFileBinding("crossover_runner", runner), RegularFileBinding("crossover_runner_test", runner_test),
        RegularFileBinding("crossover_sealer", sealer),
        RegularFileBinding("crossover_finalizer", finalizer), RegularFileBinding("crossover_finalizer_test", finalizer_test),
        RegularFileBinding("crossover_source_preflight", source_preflight), RegularFileBinding("crossover_accepted_evidence", accepted_evidence),
...
def build_support_cpu_evidence(*, output_root: Path, compatibility_receipt_path: Path, plan_path: Path, census_path: Path, consumer_path: Path, consumer_test_path: Path, merger_path: Path, merger_test_path: Path, adapter_path: Path = Path(support.__file__).resolve(), adapter_test_path: Path = SUPPORT_ADAPTER_TEST) -> StageEvidence:
    ...
    receipt = _read_hashed_receipt(compatibility_receipt_path, "support CPU compatibility receipt")
    _require_exact_receipt(
        receipt, schema="research_probe_admission.support_cpu_compatibility.v1", kind="support_cpu_compatibility",
        files={"consumer": consumer_path, "consumer_test": consumer_test_path, "merger": merger_path, "merger_test": merger_test_path, "adapter": adapter_path, "adapter_test": adapter_test_path, "plan": plan_path, "census": census_path, "generator": ADMISSION_ACCEPTANCE_DRIVER},
        selectors={"consumer_validator": "validate_execution_plan", "model_free_finalizer": "materialize_legacy_shard_receipts", "downstream_validator": "merge_support_receipts"},
    )
    _require_flags(receipt, {"consumer_validator_ran": True, "model_free_finalizer_ran": True, "downstream_validator_accepted": True, "model_loaded": False, "gpu_used": False, "scientific_interpretation": False})
    verification = _publish_fresh_receipt(output_root, "support_cpu_verification.json", {"schema_version": "research_probe_admission.consumer.support_cpu.v2", "kind": "verified_support_cpu_compatibility_receipt", "compatibility_receipt": _file_identity(compatibility_receipt_path), "claim_boundary": "mechanics_only_model_free_cpu_receipt_validation"})
    return StageEvidence(
        producer_binding="support_consumer", validator_binding="support_merger", output_files=(RegularFileBinding("support_cpu_verification", verification),),
        assertions={"production_entrypoint_resolved": True, "consumer_validator_ran": True, "model_free_finalizer_ran": True, "downstream_validator_accepted": True, "model_loaded": False, "gpu_used": False},
        detail={"compatibility_receipt": str(compatibility_receipt_path.resolve(strict=True)), "mode": "receipt_adapter"},
    )


def validate_support_bounded_terminal(*, terminal_path: Path, plan_path: Path, schedule: Mapping[str, Any], slot_roots: Sequence[Path], execution_identity: Mapping[str, Any], admission_bindings: BindingManifest, worker_path: Path, executable_path: Path, worker_argv: Sequence[str], worker_cwd: Path, worker_exit_receipts: Sequence[Path], runtime_receipts: Sequence[Path], expected_checkpoint: str, expected_config_fingerprint: str) -> dict[str, Any]:
    ...
    if len(slot_roots) != checked_schedule["slot_count"] or len(worker_exit_receipts) != len(slot_roots) or len(runtime_receipts) != len(slot_roots):
        raise ConsumerAdmissionError("bounded terminal requires one terminal, exit, and runtime receipt per scheduled slot")
    terminal = _read_hashed_receipt(terminal_path, "bounded terminal")
    forbidden = {"legacy_merger_closed", "legacy_merger", "merger_closure", "eight_shard_closure"}
    if forbidden & set(terminal) or "legacy" in str(terminal.get("claim_boundary", "")).lower():
        raise ConsumerAdmissionError("bounded terminal must not claim legacy eight-shard merger closure")
    if terminal.get("kind") != "bounded_terminal_materialization" or terminal.get("claim_boundary") != "bounded_live_mechanics_only_no_scientific_receipt_or_outcome_interpretation":
        raise ConsumerAdmissionError("terminal is not the bounded mechanics-only materialization")
    if terminal.get("logical_plan_file_sha256") != plan.file_sha256 or terminal.get("logical_plan_content_sha256") != plan.content_sha256 or terminal.get("schedule_sha256") != checked_schedule["content_sha256"]:
        raise ConsumerAdmissionError("bounded terminal plan or schedule binding drifted")
    expected_ids = [context.context_id for context in plan.contexts]
    if terminal.get("context_count") != len(expected_ids) or terminal.get("context_ids") != expected_ids or len(set(expected_ids)) != len(expected_ids):
        raise ConsumerAdmissionError("bounded terminal context denominator is incomplete or foreign")
    ...
    for index, (slot_root, exit_path, runtime_path) in enumerate(zip(slot_roots, worker_exit_receipts, runtime_receipts, strict=True)):
        diagnostics = ExecutionEvidenceJournal.inspect_diagnostics(slot_root)
        if diagnostics.snapshot.terminal is None or tuple(diagnostics.snapshot.completed_work_item_ids) != tuple(checked_schedule["slots"][index]["context_ids"]):
            raise ConsumerAdmissionError("bounded terminal requires a complete terminal slot journal")
        expected_identity = support._slot_identity(execution_identity=execution_identity, schedule=checked_schedule, slot_index=index)
        ...
        if (
            ...
            raise ConsumerAdmissionError("runtime receipt is foreign or does not bind logical cuda:0")
        if runtime["runtime_identity"].get("checkpoint") != expected_checkpoint:
            raise ConsumerAdmissionError("runtime receipt checkpoint differs from the bound expectation")
        if runtime["runtime_identity"].get("config_fingerprint") != expected_config_fingerprint:
            raise ConsumerAdmissionError("runtime receipt config fingerprint differs from the bound expectation")
        ...
        if (
            ...
            or exit_receipt.get("schedule_sha256")
            != checked_schedule["content_sha256"]
            ...
            or exit_receipt.get("journal_inspection", {}).get("status")
            != "validated"
            ...
            raise ConsumerAdmissionError("worker-exit receipt does not close the exact bounded slot")
        ...
        slot_terminal_digests.append(str(diagnostics.snapshot.terminal["content_sha256"]))
    ...
    if terminal.get("record_digests") != record_digests or terminal.get("payload_fingerprints") != payload_fingerprints or terminal.get("slot_terminal_digests") != slot_terminal_digests or terminal.get("slot_plan_fingerprints") != slot_plan_fingerprints:
        raise ConsumerAdmissionError("bounded terminal digest arrays differ from journal diagnostics")
    return {"schema_version": "research_probe_admission.consumer.support_vertical.v2", "kind": "bounded_terminal_validator", "terminal": _file_identity(terminal_path), "runtime_receipts": runtime_identities, "worker_exit_receipts": worker_exit_identities, "slot_journals": slot_journal_identities, "logical_plan_file_sha256": plan.file_sha256, "schedule_sha256": checked_schedule["content_sha256"], "durable_work_item_count": len(expected_ids), "legacy_merger_closed": False, "model_runtime_loaded": True, "scientific_interpretation": False, "claim_boundary": "bounded_terminal_mechanics_only_never_legacy_eight_shard_merger_closure"}


def build_support_vertical_evidence(*, output_root: Path, terminal_path: Path, plan_path: Path, schedule: Mapping[str, Any], slot_roots: Sequence[Path], execution_identity: Mapping[str, Any], admission_bindings: BindingManifest, worker_path: Path, executable_path: Path, worker_argv: Sequence[str], worker_cwd: Path, worker_exit_receipts: Sequence[Path], runtime_receipts: Sequence[Path], expected_checkpoint: str, expected_config_fingerprint: str) -> StageEvidence:
    validation = validate_support_bounded_terminal(terminal_path=terminal_path, plan_path=plan_path, schedule=schedule, slot_roots=slot_roots, execution_identity=execution_identity, admission_bindings=admission_bindings, worker_path=worker_path, executable_path=executable_path, worker_argv=worker_argv, worker_cwd=worker_cwd, worker_exit_receipts=worker_exit_receipts, runtime_receipts=runtime_receipts, expected_checkpoint=expected_checkpoint, expected_config_fingerprint=expected_config_fingerprint)
    outputs = _publish_vertical_snapshot(output_root, validation, terminal_path, runtime_receipts, worker_exit_receipts, slot_roots)
    return StageEvidence(producer_binding="support_worker", validator_binding="consumer_admission_adapter", output_files=tuple(RegularFileBinding(f"support_vertical_{index}", path) for index, path in enumerate(outputs)), assertions={"production_entrypoint_executed": True, "model_runtime_loaded": True, "terminal_finalizer_completed": True, "downstream_validator_accepted": True, "durable_work_item_count": validation["durable_work_item_count"]}, detail={"consumer": "natural_boundary_support", "legacy_merger_closed": False, "mode": "receipt_adapter_snapshot"})


def build_crossover_cpu_evidence(*, output_root: Path, compatibility_receipt_path: Path, source_preflight_path: Path, accepted_evidence_path: Path, runner_path: Path, runner_test_path: Path, sealer_path: Path, finalizer_path: Path, finalizer_test_path: Path) -> StageEvidence:
    ...
    receipt = _read_hashed_receipt(compatibility_receipt_path, "crossover CPU compatibility receipt")
    _require_exact_receipt(receipt, schema="research_probe_admission.crossover_cpu_compatibility.v1", kind="crossover_cpu_compatibility", files={"runner": runner_path, "runner_test": runner_test_path, "sealer": sealer_path, "finalizer": finalizer_path, "finalizer_test": finalizer_test_path, "source_preflight": source_preflight_path, "accepted_evidence": accepted_evidence_path, "generator": ADMISSION_ACCEPTANCE_DRIVER}, selectors={"source_preflight_validator": "preflight_crossover_sources", "model_free_finalizer": "finalize", "downstream_validator": "validate_evidence"})
    _require_flags(receipt, {"source_preflight_validated": True, "model_free_finalizer_ran": True, "downstream_validator_accepted": True, "model_loaded": False, "gpu_used": False, "scientific_interpretation": False})
    ...
    if receipt.get("replay_status") in {"not_replayed", "read_only_wrapper"}:
        raise ConsumerAdmissionError("crossover compatibility receipt says the finalizer was not replayed")
    verification = _publish_fresh_receipt(output_root, "crossover_cpu_verification.json", {"schema_version": "research_probe_admission.consumer.crossover_cpu.v2", "kind": "verified_crossover_cpu_compatibility_receipt", "compatibility_receipt": _file_identity(compatibility_receipt_path), "claim_boundary": "mechanics_only_model_free_cpu_receipt_validation"})
    return StageEvidence(producer_binding="crossover_runner", validator_binding="crossover_finalizer", output_files=(RegularFileBinding("crossover_cpu_verification", verification),), assertions={"production_entrypoint_resolved": True, "consumer_validator_ran": True, "model_free_finalizer_ran": True, "downstream_validator_accepted": True, "model_loaded": False, "gpu_used": False}, detail={"compatibility_receipt": str(compatibility_receipt_path.resolve(strict=True)), "mode": "receipt_adapter"})


def _require_exact_receipt(receipt: Mapping[str, Any], *, schema: str, kind: str, files: Mapping[str, Path], selectors: Mapping[str, str]) -> None:
    if receipt.get("schema_version") != schema or receipt.get("kind") != kind or receipt.get("exit_code") != 0:
        raise ConsumerAdmissionError("compatibility receipt schema, kind, or zero exit status differs")
    ...
    for name, path in files.items():
        if actual_files[name] != _file_identity(path):
            raise ConsumerAdmissionError(f"compatibility receipt {name} raw source binding drifted")
    if receipt.get("selectors") != dict(selectors):
        raise ConsumerAdmissionError("compatibility receipt command/test selectors differ")
    _require_command_results(receipt, selectors)


def _require_command_results(receipt: Mapping[str, Any], selectors: Mapping[str, str]) -> None:
    ...
    if not isinstance(results, Mapping) or set(results) != set(selectors):
        raise ConsumerAdmissionError("compatibility receipt command_results set differs")
    for name, selector in selectors.items():
        result = results[name]
        if not isinstance(result, Mapping) or set(result) != {"executable", "cwd", "argv", "selector", "return_code", "passed", "stdout", "stderr", "receipt", "generator"}:
            raise ConsumerAdmissionError("command result schema differs")
        if result["selector"] != selector or result["return_code"] != 0 or isinstance(result["passed"], bool) or not isinstance(result["passed"], int) or result["passed"] < 1:
            raise ConsumerAdmissionError("command result selector, exit, or passed count differs")
        ...
        if argv[0] != str(executable):
            raise ConsumerAdmissionError("command argv does not invoke the bound executable")
        ...
        command_receipt = _read_hashed_receipt(
            command_receipt_path, "command receipt"
        )
        expected_receipt = {
...
def _command_binding_requests(
    ...
    for command_name in sorted(results):
        ...
        if not isinstance(executable, Mapping) or not isinstance(
            ...
            raise ConsumerAdmissionError("compatibility command executable is malformed")
        ...
        for field in ("stdout", "stderr", "receipt", "generator"):
            ...
            bindings.append(
                RegularFileBinding(
                    f"{binding_prefix}_{field}", Path(identity["path"])
                )
            )
...
def _identity_to_live_file(value: Any, label: str) -> Path:
    if not isinstance(value, Mapping) or set(value) != {"path", "raw_sha256", "byte_count"}:
...
def _regular_directory(value: Any, label: str) -> Path:
    ...
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode) or not path.is_absolute():
        raise ConsumerAdmissionError(f"{label} must be an absolute non-symlink directory")
...
def _require_flags(receipt: Mapping[str, Any], expected: Mapping[str, bool]) -> None:
    for name, value in expected.items():
        if receipt.get(name) is not value:
            raise ConsumerAdmissionError(f"compatibility receipt flag {name} is not {str(value).lower()}")
...
def _require_production_identity(
    ...
    entries = {
        str(item["name"]): item
        for item in manifest.to_mapping()["bindings"]
    }
    required = {
        "support_consumer",
        "admission_core",
    ...
    if worker != SUPPORT_WORKER.resolve(strict=True):
        raise ConsumerAdmissionError("vertical worker is not the current production worker")
    ...
    _require_runtime_source_bindings(entries, execution_identity["consumer_runtime_source"])
    ...
    if (
        ...
        or executable_binding.get("sha256") != _file_identity(executable_path)["raw_sha256"]
    ):
    ...
    if list(worker_argv[:3]) != [
        ...
        raise ConsumerAdmissionError("worker argv does not invoke the bound production worker")
    ...
    if (
        ...
        raise ConsumerAdmissionError("worker runtime policy is not single-device cuda:0")
...
def _require_base_model_binding(
    ...
    if (
        ...
        raise ConsumerAdmissionError("base model denominator differs from admission binding")
    bound_files = {
    ...
    if bound_files != observed_files:
        raise ConsumerAdmissionError("base model file bytes differ from admission binding")
...
def _require_runtime_source_bindings(
    ...
    if any(path.is_symlink() or not path.is_file() for path in live_paths):
        raise ConsumerAdmissionError("consumer runtime tree contains a non-regular Python file")
    bound = {
    ...
    live = {str(path.resolve(strict=True)): support.file_sha256(path) for path in live_paths}
    if bound != live:
        raise ConsumerAdmissionError("consumer runtime Python denominator is not admission-bound")
    aggregate_rows = [
        {
    ...
    if not isinstance(critical_files, Mapping):
        raise ConsumerAdmissionError("consumer runtime critical-file identity is malformed")
    for reference in critical_files.values():
        if (
            not isinstance(reference, Mapping)
            or bound.get(str(reference.get("path"))) != reference.get("sha256")
        ):
            raise ConsumerAdmissionError("consumer runtime critical file is not admission-bound")
...
def _require_source_gate_bindings(
    ...
    if not expected or bound != expected:
        raise ConsumerAdmissionError("embedding source-gate files are not admission-bound")


def _file_identity(path: Path) -> dict[str, Any]:
    candidate = _regular_leaf(path, "bound file")
    raw = candidate.read_bytes()
    return {"path": str(candidate), "raw_sha256": hashlib.sha256(raw).hexdigest(), "byte_count": len(raw)}
...
def _publish_fresh_receipt(root: Path, filename: str, body: Mapping[str, Any]) -> Path:
    ...
    expected = json.dumps(payload, ensure_ascii=True, sort_keys=True, separators=(",", ":"), allow_nan=False).encode("utf-8")
    path = root / filename
    if root.exists() or root.is_symlink():
        try:
            ...
            raise ConsumerAdmissionError("existing receipt root cannot be inspected") from exc
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode) or [item.name for item in children] != [filename]:
            raise ConsumerAdmissionError("existing receipt root is not the exact single-file publication shape")
        existing = _regular_leaf(path, "existing verification receipt")
        if existing.read_bytes() != expected:
            raise ConsumerAdmissionError("existing verification receipt differs from exact publication bytes")
        return existing
...
def _publish_vertical_snapshot(root: Path, validation: Mapping[str, Any], terminal: Path, runtimes: Sequence[Path], exits: Sequence[Path], slot_roots: Sequence[Path]) -> tuple[Path, ...]:
    ...
    *,
    ...
    for path in sorted(root.rglob("*"), key=lambda item: item.relative_to(root).as_posix()):
        ...
        if path.is_dir():
            continue
        if not path.is_file():
            raise ConsumerAdmissionError("vertical snapshot contains a non-regular file")
    ...
    if require_complete and observed != expected_relative_paths:
        raise ConsumerAdmissionError("vertical snapshot output denominator is incomplete")
...
def _publish_canonical_copy(target: Path, source: Path) -> None:
    try:
        ...
        raise ConsumerAdmissionError("vertical evidence source is not canonical JSON") from exc
...
def _regular_leaf(path: Path, label: str) -> Path:
    ...
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise ConsumerAdmissionError(f"{label} must be a declared regular non-symlink file")
    try:
        return path.resolve(strict=True)
    except OSError as exc:
...
__all__ = ["ConsumerAdmissionError", "build_crossover_cpu_evidence", "build_support_cpu_evidence", "build_support_vertical_evidence", "crossover_binding_requests", "support_binding_requests", "validate_support_bounded_terminal"]
