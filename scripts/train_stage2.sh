...
if [[ $# -gt 0 ]]; then
  ...
  echo "[ERROR] Example: server_gpus=0,1,2,3,4,5,6 train_gpus=7 config=configs/stage2_rollout_correction/base.yaml bash scripts/train_stage2.sh" >&2
  exit 2
fi
...
export CONFIG="${config:-${CONFIG:-configs/stage2_rollout_correction/base.yaml}}"
