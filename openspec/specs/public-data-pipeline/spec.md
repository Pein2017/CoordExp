...
- **WHEN** the dataset plugin runs `convert`
- **THEN** the plugin derives `bbox_2d` from the polygon (or emits `poly` when supported and enabled), and the record remains compliant with `docs/data/CONTRACT.md`.
...
Validation SHALL enforce the structural requirements in `docs/data/CONTRACT.md` for both `bbox_2d` and `poly` geometries.
