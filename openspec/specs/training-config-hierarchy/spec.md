# training-config-hierarchy Specification
...
- raw ownership checks MUST verify that canonical migrated leaves still author:
  ...
  - `training.run_name`
  - `training.artifact_subdir`
- named regression suites MUST be stabilized or partitioned before they are
  used as parity evidence when unrelated pre-existing failures already exist.
