# coordexp-infras-infer-config-runtime Specification

## Purpose
Canonical coordexp-infras inference configuration contract for strict YAML
...
Inference configs SHALL live under `configs/coordexp_infras/infer/`.
Legacy `configs/infer/*` files MUST be treated as reference-only and MUST NOT
be loaded as canonical coordexp-infras V1 inference configs unless a later
compatibility change explicitly promotes them.

#### Scenario: coordexp-infras config path
- **WHEN** `python -m src.infer --config configs/coordexp_infras/infer/base.yaml`
  is invoked
- **THEN** the config loader treats the file as a coordexp-infras inference
  config candidate
...
coordexp-infras inference SHALL require CUDA for every non-dry execution path.
