...
- `infer.model_checkpoint` (string; either a merged/full checkpoint, or a
  local adapter-directory shorthand whose `adapter_config.json` resolves the
  base model path),
...
When the inference engine emits `gt_vs_pred.jsonl`, it SHALL preserve
prediction order so downstream visualization can trace model interpretation
order faithfully.
