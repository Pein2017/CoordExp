...
- evaluator-selected scenes intended for shared review rendering MUST
  materialize canonical visualization records with:
...
- when current evaluator logic already computes matching for a selected scene,
  it MUST preserve and normalize that matching under the canonical `matching`
...
- **WHEN** a canonical visualization record is materialized for overlay or
  audit
...
- **THEN** composition fails fast rather than silently pairing the wrong
  scenes.

#### Scenario: Evaluator remaps GT indices into canonical GT order
- **GIVEN** an evaluator-selected scene whose source-local GT indices are
  defined over a post-validation GT array
...
When detection evaluation is invoked from training under DDP and only rank 0
computes the final evaluation outputs, non-zero ranks MUST NOT be required to
materialize the full combined evaluation payload solely to support rank-0
scoring.

Normative behavior:
- The train-time evaluation path MAY use shard-first or scalar-first
  coordination.
- Rank 0 MUST remain the owner of final heavy evaluation work and final
  train-time summary emission when the integration is rank-0-owned.
- Any rank-efficient transport optimization MUST preserve final metric meaning
  and the supported train-time summary outputs for that integration.
- This requirement does NOT broaden train-time evaluation into full offline
  evaluator artifact parity.
...
- **THEN** non-zero ranks are not required to materialize the full combined
  payload solely for rank-0 scoring
- **AND** rank 0 still produces the canonical train-time summary outputs for
  that integration.
