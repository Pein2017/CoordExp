# encoded-training-cache Specification

## Purpose
Define the deterministic encoded-sample cache contract for eligible training
datasets, including config, fingerprinting, reuse, and provenance.
## Requirements
...
- Cache hits MUST remain functionally equivalent whether the accessed shard was
  already resident or had to be reloaded after eviction.
- The chosen residency policy MUST NOT change the encoded payload contract
  delivered to downstream training code.
...
- **WHEN** a worker traverses enough samples to touch more shards than the
