...
- [x] 1.2 Update `progress/directions/full_idea_v3.md` so the locked v1 contract is explicit:
...
- [x] 1.3 Validate the new change artifacts with `openspec validate --type change stage2-b-v3-k2-triage --strict --no-interactive`.
...
- [x] 2.2 Keep existing `stage2_ab.channel_b` knobs intact:
  ...
  - `ddp_phase_timeout_s`
- [x] 2.3 Add a per-call rollout decode override seam that works across both HF and vLLM backends:
  - anchor greedy override
  - explorer stochastic override
  - fail-fast when a backend/runtime cannot honor the requested policy
- [x] 2.4 Update canonical Stage-2 YAML profiles under `configs/stage2_two_channel/` to author the new v3 fields explicitly for the selected implementation profile.
- [x] 2.5 Add config-contract and rollout-contract tests covering accepted keys, missing required fields, rejection of misplaced legacy keys, and dual-policy decode support.
...
- [x] 3.2 Reuse the existing per-run v2 cleanup path independently for each rollout:
  ...
  - sequential dedup
  - Hungarian matching
- [x] 3.3 Add a deterministic one-to-one max-IoU anchor/explorer association helper with stable tie-breaks.
- [x] 3.3a Add a unit case where a `2x2` association problem has equal IoU totals across multiple assignments and verify the lexicographically smallest sorted pair list is chosen.
- [x] 3.4 Project association results into side-specific action states:
  - `anchor_gt_backed`
  - `recovered_fn`
  - `shielded_anchor`
  - `dead_anchor`
  - `dead_explorer`
- [x] 3.5 Keep DDP/runtime control flow symmetric across ranks for the dual-rollout path.

## 4. Target Construction and Weighting

- [x] 4.1 Build the final teacher-forced target as an **anchor-edited** clean sequence, preserving anchor order.
- [x] 4.2 Keep shielded anchor objects as neutral context only.
- [x] 4.3 Remove dead anchor objects from the positive prefix.
- [x] 4.4 Detect recovered GT objects as:
  - missed in anchor
  - hit in explorer
- [x] 4.5 Keep recovered GTs on the existing FN-injection path and add the configured higher **per-object desc+geo+coord** weight.
- [x] 4.6 Thread recovered-FN metadata through the rollout segment payload so `token_ce`, `bbox_geo`, and `coord_reg` can distinguish recovered FN objects from ordinary FN objects.
- [x] 4.7 Do not add recovered-prefix distillation or any second teacher-forced payload in this implementation slice.
...
- [x] 7.1 Run targeted config and trainer unit tests:
...
- [x] 7.2 Run a config materialization check for the selected Stage-2 profile with the new v3 fields.
- [x] 7.3 Run a short Stage-2 smoke that exercises the dual-rollout B-step path and verifies:
...
- [x] 7.4 Validate the OpenSpec change again before implementation handoff:
