...
> Attempt 6 executed nothing and is immutable. Attempt 7 MUST use a new absent
> `-r7` target and new paths, and MUST deterministically re-render and compare
> all three role-config byte streams, digests, and resolved fingerprints against
> that root before freezing; no identity may be copied forward. Attempts 1-6 and
