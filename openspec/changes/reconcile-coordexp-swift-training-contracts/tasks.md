...
- [ ] 3.4 Keep attempt 3 frozen at implementation commit `037ab6683f9eeeb99157960f9fcf5bb3176a7044`, manifest SHA-256 `c3e4e93d997c87ad26379b0246f5536aec4f96afbc9a59be16985572a718cf42`, and packet SHA-256 `70b4f4cc7b235db0f21dcf5e3ade68d06b5db923a4876ceee6ba92ceed07ca02`; record its pre-launch `HOLD` without editing either frozen file. Close only its two P1s test-first: (a) in `reconcile_exact_resume_probe.py`, route only the resumed parent through a synchronous held-parent wrapper that calls the real pipeline checkpoint handler and then blocks after committed step 1 while control/child remain `src.train`; (b) add the separate experiment-local packet executor and schema-v2 `execution_contract`. Preserve production `src/`, strict admission, resume-compatibility behavior, the public `execute`/CLI surface, and the six-command order.
- [ ] 3.5 Re-freeze the successor packet and schema-v2 manifest against the post-repair commit, absent target, exact configs/commands, and original `world_size=2`/two-GPU/three-arm numeric bounds. The manifest `execution_contract` MUST bind an immutable signed pre-cost review receipt path without binding the review hash. Publish at that path a signed `coordexp-swift-reconcile-resume-probe-pre-cost-review-v1` receipt containing `status: READY`, exact implementation commit, manifest SHA-256, packet SHA-256, independent reviewer identity, and `receipt_payload_sha256`; resolve every P0/P1. Self-declared manifest `READY` is never authority. Any missing, `HOLD`, stale, invalid, replaced, or non-independent review blocks before marker creation; any frozen-input mutation requires re-freeze/re-review, not another user prompt.
- [ ] 3.6 Execute only through `reconcile_exact_resume_packet_executor.py` after the executor validates that review. It MUST claim the attempt marker with `O_EXCL`, run exactly once in order `setup`, `success.uninterrupted_control`, `success.resumed_child`, `rank_failure`, `interruption`, `verification`, stop on the first failure with no retry, and accept exactly one Popen-like process per launch. Every post-launch sampler/artifact/timeout/bound/nonzero/exception path MUST TERM, KILL if necessary, reap, and confirm process-group absence keyed by leader PID/starttime before a successful terminal outcome; cleanup failure is recorded in a failed signed outer receipt. Retain and validate the exact physical-index-to-GPU-UUID map before marker and before the first GPU command; count a GPU rank row only when its UUID is selected and PID/starttime belongs to the current command descendant tree. Require CPU ranks `[0, 1]` for both success commands plus `rank_failure` and `interruption`, GPU ranks `[0, 1]` only for both success commands, and no rank rows for `setup`/`verification`. The outer receipt MUST also bind review, launcher/runtime and command-group identities, exact argv observations, bounded artifact-tree summaries, rank coverage, process/GPU/resource maxima, cleanup/stop outcome, and the inner verifier receipt when reached. The success evidence MUST bind the matched branch pair and compare next input/pack identity; pre-forward trainable/optimizer/scheduler/scaler/RNG/cursor state; objective/loss fields; and resulting trainable parameters after the first post-resume optimizer update under the declared exact policy.
- [ ] 3.7 Gate Wave 3 only when the successor packet's external signed review receipt is valid and `READY`, and the signed outer receipt proves exact six-command order, marker ownership, launcher/runtime identity, bounded artifact-tree summaries, per-command rank coverage, selected-UUID/descendant-owned GPU attribution, and successful process-group cleanup. The matched success-pair and failure/interruption inner receipts MUST be target-bound, every required rank and branch accounted for, focused artifact verification green, and the claim limited to same-world-size optimizer-step-boundary continuation; manifest self-readiness, publication/admission, exit zero, or an inner receipt alone is insufficient.
