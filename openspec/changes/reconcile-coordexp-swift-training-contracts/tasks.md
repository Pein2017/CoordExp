...
- [ ] 3.4 Keep all attempt 1-4 evidence immutable. Attempt 3 remains bound to implementation `037ab6683f9eeeb99157960f9fcf5bb3176a7044`, manifest `c3e4e93d997c87ad26379b0246f5536aec4f96afbc9a59be16985572a718cf42`, and packet `70b4f4cc7b235db0f21dcf5e3ade68d06b5db923a4876ceee6ba92ceed07ca02` with pre-launch `HOLD`. Attempt 4 remains bound to implementation `5d68a41081aecc3282cb44ce70bcb5bcdcc7c19e`, manifest `b3538fb6167186cd5063f343a447ef1ed8f3024dc07f96284c7628c1ab08d3a9`, and packet `45afe6475e2aa3cae6e106bc446725de4b60197b77ba3d0a3a8ff45263242a87`: setup and control returned zero with confirmed cleanup, but the signed outer receipt stopped at `packet_executor.missing_gpu_rank` and ran no later command/retry because host NVML PIDs cannot be verifiably joined to the executor's container `/proc` namespace. Replace only that GPU evidence gate test-first: preserve the physical-index-to-UUID and occupancy preflight, make NVML process rows optional observations, and require target-bound receipt + `run.json` + canonical `logging.jsonl` GPU coverage. Preserve production `src/`, CPU measurement modes, strict admission, resume compatibility, the public `execute`/CLI surface, and the six-command order.
...
- [ ] 3.6 Re-freeze Attempt 5 against the post-repair commit, a new absent target, exact configs/commands, and the original `world_size=2`/two-GPU/three-arm numeric bounds; never reuse Attempt 4's target or frozen inputs. The contract MUST preserve per-rank CPU coverage for both success commands and command-tree aggregate CPU coverage for the four model-free commands, bind `gpu_measurement_source: torch_allocator_high_water`, and require exact GPU ranks `[0, 1]` through signed success receipt + `run.json` + `logging.jsonl` joins. Retain the physical-index-to-UUID map and occupancy preflight, but never use NVML process rows as a rank gate. Obtain a new independent signed `READY` review bound to the exact Attempt-5 commit/manifest/packet; any mutation requires re-freeze/re-review, not another user prompt.
- [ ] 3.7 Execute Attempt 5 only through `reconcile_exact_resume_packet_executor.py` after review validation. Keep `O_EXCL`, exact order, no retry, one owned process group, cleanup, CPU modes, and bounded artifact summaries. After each success command returns zero, cleanup is confirmed, and artifact summary completes, validate the target-bound GPU join before resource success. For control require signed role/config/run-dir bindings, completed `world_size=2`/`completed_steps=2`, exact train steps 1 and 2 once each, exact rank/topology attestations, and maxima across both rows. For resumed require the signed receipt to span an authenticated controlled parent exit at step 1 plus parent train step 1 and a completed child at step 2 plus child train step 2, merging both lifetimes. Reject bad receipt bindings/digests, run state/topology, rows/steps/ranks/fields, non-finite/boolean/negative/non-integer-valued metrics, or bound excess; fake NVML rows cannot satisfy missing artifacts. The signed outer receipt MUST retain source label, artifact paths/SHA-256, row/step inventory, raw allocated/reserved maxima, conservative maxima, and rank-to-local/logical/physical/UUID mapping. Gate Wave 3 only when outer and inner receipts prove the full contract; exit zero alone is insufficient.
