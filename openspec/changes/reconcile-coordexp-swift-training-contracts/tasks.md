...
- [ ] 3.4 Keep all attempt 1-3 receipts immutable, including attempt 3 at implementation commit `037ab6683f9eeeb99157960f9fcf5bb3176a7044`, manifest SHA-256 `c3e4e93d997c87ad26379b0246f5536aec4f96afbc9a59be16985572a718cf42`, and packet SHA-256 `70b4f4cc7b235db0f21dcf5e3ade68d06b5db923a4876ceee6ba92ceed07ca02`; record its pre-launch `HOLD` without editing either frozen file. Close its two P1s and the discovered CPU-evidence contract defect test-first: (a) route only the resumed parent through the probe-local synchronous held-parent wrapper while all success launches remain real two-rank `torchrun`; (b) add the experiment-local packet executor and schema-v2 `execution_contract`; (c) prove an unranked real CPU subprocess tree is accepted with `RANK`/`LOCAL_RANK` absent, command-tree peak RSS is a concurrent-snapshot sum rather than a sum of per-PID high-water marks, aggregate bound exceedance and a missing owned sample fail closed, and both success commands still require CPU/GPU ranks `[0, 1]`. Preserve production `src/`, strict admission, resume compatibility, the public `execute`/CLI surface, and the six-command order; do not turn model-free arms into `torchrun`.
- [ ] 3.5 Bump only the experiment-local rank-failure and interruption arm receipts to schema v2 and add exhaustive semantic tests. Every rank-failure injection MUST record exact `expected_ranks: [0, 1]`, `serialized_ranks: [0, 1]`, injection-derived `published_ranks`, and its exact production `error_code`; the frozen representative missing rank 1 has published `[0]` and `error_code: training_state.incomplete_rank_set`. Every interruption boundary MUST record exact `expected_ranks: [0, 1]`, serialized/published ranks, and `rank_state_boundary_reached`; frozen `stop_after=1` has serialized/published `[]` and boundary false. The verifier MUST compare these exact fields and reject missing, stale-schema, re-signed-tampered, and digest-tampered receipts.
- [ ] 3.6 Re-freeze the successor packet and schema-v2 manifest against the post-repair commit, absent target, exact configs/commands, and original `world_size=2`/two-GPU/three-arm numeric bounds. Its `execution_contract` MUST declare CPU mode `per_rank`, `required_cpu_ranks: [0, 1]`, and the per-rank CPU bound for both success commands; CPU mode `command_tree_aggregate`, `required_cpu_ranks: []`, and the frozen `max_cpu_rss_command_tree_bytes` bound for setup, rank failure, interruption, and verification; and `required_gpu_ranks: [0, 1]` only for both success commands. It also MUST bind the immutable signed pre-cost review path without binding the review hash. Publish there a signed `coordexp-swift-reconcile-resume-probe-pre-cost-review-v1` receipt containing `status: READY`, exact implementation commit, manifest SHA-256, packet SHA-256, independent reviewer identity, and `receipt_payload_sha256`; resolve every P0/P1. Self-declared manifest `READY` is never authority. Any missing, `HOLD`, stale, invalid, replaced, or non-independent review blocks before marker creation; any frozen-input mutation requires re-freeze/re-review, not another user prompt.
- [ ] 3.7 Execute only through `reconcile_exact_resume_packet_executor.py` after it validates that review. It MUST claim the marker with `O_EXCL`, run exactly once in order `setup`, `success.uninterrupted_control`, `success.resumed_child`, `rank_failure`, `interruption`, `verification`, stop on first failure with no retry, and own exactly one Popen-like process tree at a time. Every post-launch sampler/artifact/timeout/bound/nonzero/exception path MUST TERM, KILL if necessary, reap, and confirm process-group absence by leader PID/starttime before success. For aggregate-mode commands, record and enforce `cpu_rss_command_tree_max_bytes` as the maximum concurrent RSS sum over exact owned PID/starttime rows in one sampler snapshot, with at least one owned sample; never sum independent per-PID maxima. Retain per-rank CPU/GPU maxima and required ranks `[0, 1]` for the true two-rank success commands. The signed outer receipt MUST bind the review, launcher/runtime and command-group identities, argv, bounded artifact trees, CPU modes/bounds/maxima, rank coverage, cleanup/stop outcome, and inner verifier receipt. Gate Wave 3 only when that outer receipt and target-bound inner receipts prove the exact resource contract and semantic arm fields, matched first-update success comparison, selected-UUID/descendant GPU attribution, and cleanup; publication/admission, exit zero, manifest self-readiness, or an inner receipt alone is insufficient.
