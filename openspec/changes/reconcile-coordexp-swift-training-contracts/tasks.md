...
- [ ] 3.4 Keep all attempt 1-5 evidence immutable. Attempt 3 remains bound to implementation `037ab6683f9eeeb99157960f9fcf5bb3176a7044`, manifest `c3e4e93d997c87ad26379b0246f5536aec4f96afbc9a59be16985572a718cf42`, and packet `70b4f4cc7b235db0f21dcf5e3ade68d06b5db923a4876ceee6ba92ceed07ca02` with pre-launch `HOLD`. Attempt 4 remains bound to implementation `5d68a41081aecc3282cb44ce70bcb5bcdcc7c19e`, manifest `b3538fb6167186cd5063f343a447ef1ed8f3024dc07f96284c7628c1ab08d3a9`, and packet `45afe6475e2aa3cae6e106bc446725de4b60197b77ba3d0a3a8ff45263242a87`: setup and control returned zero with confirmed cleanup, but the signed outer receipt stopped at `packet_executor.missing_gpu_rank` and ran no later command/retry because host NVML PIDs cannot be verifiably joined to the executor's container `/proc` namespace. Replace only that GPU evidence gate test-first: preserve the physical-index-to-UUID and occupancy preflight, make NVML process rows optional observations, and require target-bound receipt + `run.json` + canonical `logging.jsonl` GPU coverage. Preserve production `src/`, CPU measurement modes, strict admission, resume compatibility, the public `execute`/CLI surface, and the six-command order. Attempt 5 remains bound to implementation `9ef726085d2118dca90b373ccde4a88d068bf516`, manifest `ee9227fdb310f7e9a2d629000df137ac5dbc2b472461a83f729df56909d58c24`, packet `48b7bb924f960bb5f65a87dfa4be4b9f5289b79b9e02b5375b253dd7b4caf897`, target root `reconcile_exact_resume_2026-08-13-r5`, and a signed independent pre-cost `HOLD` at `receipts/wave-3-attempt-5-pre-cost-review.json`; it executed no command, marker, cache, model, `torchrun`, GPU allocation, or artifact-target mutation and MUST NOT be edited, re-signed, or reused. Its two P0s are (a) all three role-config `expected_sha256` values were copied forward from the Attempt-4 target although the rendered role configs embed `run.artifact_root` and the child's `resume.checkpoint_dir`, and (b) the executor bound the interrupted `resumed_parent` `run.json` to status `running`, which production `RunWriter` never writes because a `SIGTERM`-terminated held parent never reaches `finalize()`. Fix (b) test-first by binding the real controlled-exit status `initialized` with a null `completed_at`, keeping the exact progress, checkpoint-event, and topology checks and rejecting `completed`, `failed`, and `running`.
- [ ] 3.5 Preserve and verify the existing experiment-local rank-failure and interruption schema-v2 receipts and exhaustive semantic tests. Every rank-failure injection MUST record exact `expected_ranks: [0, 1]`, `serialized_ranks: [0, 1]`, injection-derived `published_ranks`, and its exact production `error_code`; the frozen representative missing rank 1 has published `[0]` and `error_code: training_state.incomplete_rank_set`. Every interruption boundary MUST record exact `expected_ranks: [0, 1]`, serialized/published ranks, and `rank_state_boundary_reached`; frozen `stop_after=1` has serialized/published `[]` and boundary false. The verifier MUST compare these exact fields and reject missing, stale-schema, re-signed-tampered, and digest-tampered receipts.
- [ ] 3.6 Freeze a successor Attempt 6 against the post-repair commit, a new absent `-r6` target, exact configs/commands, and the original `world_size=2`/two-GPU/three-arm numeric bounds; never reuse Attempt 4's target or frozen inputs. The contract MUST preserve per-rank CPU coverage for both success commands and command-tree aggregate CPU coverage for the four model-free commands, bind `gpu_measurement_source: torch_allocator_high_water`, and require exact GPU ranks `[0, 1]` through signed success receipt + `run.json` + `logging.jsonl` joins. Retain the physical-index-to-UUID map and occupancy preflight, but never use NVML process rows as a rank gate. Never reuse Attempt 5's `-r6`-absent inputs either: deterministically render all three role configs against the `-r6` artifact root and compare the rendered bytes and SHA-256 against the manifest before freezing, because the rendered configs embed `run.artifact_root` and the child's `resume.checkpoint_dir`; no role-config digest may be copied forward from any earlier attempt. Bind the interrupted `resumed_parent` `run.json` to the real controlled-exit status `initialized` with a null `completed_at`. Bind the accumulation invariant the artifact-progress contract assumes, separating its three authorities. Config identity: the manifest and packet MUST record the absolute base-config path `/data/CoordExp/.worktrees/CoordExp-swift/configs/coordexp_swift/smoke/qwen3_vl_2b_desc_first_geo_sorted_pure_ce_dora_llm_12000_accelerate2_ebs2_1step.yaml` with SHA-256 `44c2cd2a6442917595e6426061e021e2989542114b7ce7cdb97cd37edb4f609f` plus each rendered role config's bytes and SHA-256, and before freezing every rendered role config MUST be loaded through production `load_train_config` and MUST resolve `effective_batch_size` to `2`. Launch topology: `world_size` is not a config field, so the frozen launch argv and manifest MUST bind `world_size = 2` (setup `--world-size 2` and the two-rank `torchrun` success commands) and that binding MUST be verified against the frozen argv, never read from the YAML. Derived accumulation: for each of `uninterrupted_control`, `resumed_parent`, and `resumed_child`, invoke production `resolve_effective_batch_runtime(config, world_size=2)` on the loaded config with the argv-bound world size and require `resolved_grad_accum_steps: 1`, hence expected consumed progress `1` at step 1 and `2` at step 2; record each role's config bytes/digest and the resolved fingerprint and its digest in the manifest and packet. The independent signed `READY` review MUST verify each binding against its own authority; this round keeps the experiment-local packet fixed at one pack per step and does not generalize the executor to arbitrary accumulation. Obtain a new independent signed `READY` review bound to the exact Attempt-6 commit/manifest/packet; any mutation requires re-freeze/re-review, not another user prompt.
- [ ] 3.7 Execute Attempt 6 only through `reconcile_exact_resume_packet_executor.py` after review validation. Keep `O_EXCL`, exact order, no retry, one owned process group, cleanup, CPU modes, and bounded artifact summaries. After each success command returns zero, cleanup is confirmed, and artifact summary completes, validate the target-bound GPU join before resource success. For control require signed role/config/run-dir bindings, completed `world_size=2`/`completed_steps=2`, exact train steps 1 and 2 once each, exact rank/topology attestations, and maxima across both rows. For resumed require the signed receipt to span an authenticated controlled parent exit at step 1 plus parent train step 1 and a completed child at step 2 plus child train step 2, merging both lifetimes; the parent `run.json` MUST carry the durable controlled-exit state `initialized` with a null `completed_at`, and `completed`/`failed`/`running` fail closed. Before marker creation and again in setup validation, revalidate all three accumulation authorities: the bound absolute base-config path/SHA-256 and each role config's bytes/digest with `load_train_config` resolving `effective_batch_size` to `2`; the frozen argv/manifest binding `world_size = 2`; and production `resolve_effective_batch_runtime(config, world_size=2)` returning `resolved_grad_accum_steps: 1`, which is what makes top-level `consumed_packs` 1/2 and each event's `committed_progress.consumed_packs` equal to its step correct. Any drift stops the attempt without retry and requires re-freeze and re-review. Reject bad receipt bindings/digests, run state/topology, rows/steps/ranks/fields, non-finite/boolean/negative/non-integer-valued metrics, or bound excess; fake NVML rows cannot satisfy missing artifacts. The signed outer receipt MUST retain source label, artifact paths/SHA-256, row/step inventory, raw allocated/reserved maxima, conservative maxima, and rank-to-local/logical/physical/UUID mapping. Gate Wave 3 only when outer and inner receipts prove the full contract; exit zero alone is insufficient.
...
> verifiable mapping. Attempt 5 MUST therefore use a new absent target and
> re-freeze/re-review an artifact-derived torch-allocator GPU contract; the
...
> Attempt 6 MUST use a new absent `-r6` target and MUST deterministically
> re-render and compare all three role-config byte streams and digests against
> that root before freezing; no digest may be copied forward. Attempts 1-5 and
