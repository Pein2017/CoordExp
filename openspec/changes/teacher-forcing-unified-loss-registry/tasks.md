...
- [x] 0.5 Sync all documents to use:
...
- [x] 1.1 Implement token-type partitioning utilities (`struct/desc/coord/eos`) with deterministic, packing-safe behavior.
- [x] 1.2 Implement registry contexts (`gt/self_context/rollout`) and rollout object subsets (`matched/fp/fn`) with FP-neutral + EOS-enforced masking.
- [x] 1.3 Implement canonical loss components (`struct_ce/desc_ce/coord_token_ce/coord_reg/geo`) and shared helpers (bbox canonicalization + SmoothL1+CIoU).
- [x] 1.4 Make `full_idea` Channel-B semantics the canonical registry behavior:
  - matched prefix: struct-only CE,
  - FP spans: fully masked out,
  - FN injected: struct+desc CE (default),
  - closure/EOS: supervised.
- [x] 1.5 Provide explicit ablation knobs to represent rollout-matching supervision variants (e.g., FN `desc` weight = 0,
...
- [x] 2.1 Add typed YAML keys for ST modes in `src/config/schema.py` under `stage2_ab`:
  - `coord_ctx_embed_mode: soft|st|hard`
  - `coord_decode_mode: exp|st`
  with strict unknown-key fail-fast.
- [x] 2.2 Implement ST-embedding for Channel-A coord-slot context embeddings (hard forward + soft grads).
- [x] 2.3 Implement ST coord decode for geometry loss (hard forward + soft grads).
- [x] 2.4 Add at least one Stage-2 Two-Channel smoke config that enables ST explicitly (to “see the difference” without changing defaults).
- [x] 2.5 Add rollout-matching SFT typed config for `rollout_matching.coord_decode_mode: exp|st` (ST decode for geo when enabled).

## 3. Config Surface + Schema Contracts (Pipeline)

- [x] 3.1 Add a typed YAML surface for `stage2_ab.pipeline` (objective + diagnostics module lists) in `src/config/schema.py` with strict unknown-key fail-fast.
- [x] 3.2 Define module-spec validation rules (required `name`, optional `enabled/weight/channels/config`) and fail fast on duplicates and unknown module names.
- [x] 3.3 Enforce single-mode precedence: when `stage2_ab.pipeline` is present, disallow objective-affecting flat knobs (fail fast with guidance to move them into module configs).
- [x] 3.4 Emit a stable pipeline checksum + resolved module list in trainer init logs (include config path, run_name, seed context for reproducibility).
- [x] 3.5 Add `stage2_ab.text_gate_weight: float` (default `0.0`) to the typed Stage-2 Two-Channel schema so bi-directional gate supervision can be enabled without requiring a pipeline declaration (pipeline mode still disallows flat knobs).
...
- [x] 4.1 Introduce a `TeacherForcingContext` contract that supports multiple registry contexts (`gt/self_context/rollout`) and packing-aware segment meta, and exposes standardized derived views (token-type masks, coord/bbox groups, subset masks) so modules do not depend on trainer-specific raw meta formats.
- [x] 4.2 Implement a pipeline executor that runs objective modules (loss) and diagnostics modules (metrics) in config order.
- [x] 4.3 Preserve Stage-2 Two-Channel forward strategy semantics:
...
- [x] 4.4 Factor shared forward preamble utilities (Qwen3-VL padding-free packing `position_ids` fix, `use_cache=false`, logits slicing guard) into reusable helpers and reuse in Stage-2 Two-Channel and rollout-aligned trainers.
- [x] 4.5 Unify rollout-context metadata into a single “rich meta” contract that can be produced by both Stage-2 Channel-B and rollout-matching SFT, including (minimum):
...
- [x] 4.6 Refactor Stage-2 Channel-B and rollout-matching SFT batch-prep code to use shared helpers for:
...
- [x] 5.1 Implement `token_ce` objective module using registry masks and `full_idea` semantics:
  - matched prefix: struct-only CE,
  - FP spans: fully masked out,
  - FN injected: struct+desc CE (desc supervised by default),
  - closure/EOS: supervised (EOS-enforced).
- [x] 5.2 Implement `bbox_geo` objective module using CoordExp decode + bbox canonicalization + SmoothL1 + CIoU:
  - Channel-A (`self_context`): identity-aligned GT objects,
  - Channel-B (`rollout`): matched + FN injected objects (FP excluded).
- [x] 5.3 Implement `coord_reg` objective module(s) for optional coord regularizers (coord CE, soft-CE, W1, entropy, coord_gate, text_gate, EL1/EHuber), with explicit per-term weights and strict validation.
- [x] 5.4 Make rollout-matching previous defaults expressible by module config (e.g., FN `desc` weight 0, matched-prefix struct CE off) without forking trainer code.
- [x] 5.5 Ensure module-emitted metric keys follow the unified registry naming contract.

## 6. Diagnostics Modules + Best-Effort Isolation

- [x] 6.1 Implement coord diagnostics (e.g., `coord_diag/*`) as a diagnostics module with best-effort exception isolation (warn once, skip/disable on failure).
- [x] 6.2 Ensure time/packing telemetry remains intact and consistent with existing Stage-2 Two-Channel and rollout-aligned logging conventions.
- [x] 6.3 Add a minimal diagnostics-only module example that is safe to toggle for ablations without changing the objective.

## 6B. Eval-Step Detection Metrics (COCO mAP)

- [x] 6B.0 Activate COCO mAP eval by default for Stage-2 trainers:
...
- [x] 6B.1 Make Stage-2 Two-Channel `eval_step` compute the same rollout matching evaluator metrics as Stage-2 Rollout-Aligned, including COCO mAP when `rollout_matching.eval_detection.enabled=true`.
- [x] 6B.2 Ensure `rollout_matching.eval_detection` behavior is consistent across both Stage-2 trainers (same metric keys, same masking/parse strictness, same failure surfacing).
- [x] 6B.3 Add an eval-step regression test that asserts `rollout/mAP` is always present when COCO eval is enabled:
...
- [x] 6B.4 Refactor eval-step COCO evaluation to reuse the offline detection evaluator implementation:
...
- [x] 7.1 Extend `_PendingStage2Log` aggregation to accept module-emitted metrics while preserving counter vs gauge semantics (avoid grad-accum dilution for sparse gauge keys).
- [x] 7.2 Preserve the Stage-2 Two-Channel `Trainer.log()` DDP contract (no rank-conditional collectives) while merging pipeline metrics into the optimizer-step log line.

## 7C. Metrics Normalization + Naming (Make Scalars Comparable)

- [x] 7C.1 Audit emitted metrics across Stage-1, Stage-2 Two-Channel, and Stage-2 Rollout-Aligned to classify each key as:
...
- [x] 7C.2 Enforce a single naming contract for reduction semantics:
...
- [x] 7C.4 Ensure that any sum/count helpers used only for building means (e.g., semantic similarity sums) do not leak into final logs unless explicitly named as counters.
- [x] 7C.5 Add a metrics-contract test that asserts:
...
- [x] 7B.1 Refactor Stage-1 auxiliary loss mixins to reuse the unified registry + module implementations (no duplicated loss math).
- [x] 7B.2 Ensure Stage-1 static-only packing policy remains enforced (no dynamic iterable packing) and that the shared registry/pipeline code is packing-safe under dataset-level static packing.
- [x] 7B.3 (Optional) Introduce a Stage-1 pipeline declaration surface if it reduces MRO/mixin complexity; otherwise keep mixins as thin wrappers over shared modules.
  - Decision: keep Stage-1 mixins as thin wrappers over shared registry modules; no new Stage-1 pipeline declaration surface added.

## 8. YAML Migration + Documentation

- [x] 8.1 Add a Stage-2 Two-Channel smoke config that uses `stage2_ab.pipeline` explicitly while matching default weights/behavior.
- [x] 8.2 Update `docs/training/METRICS_LOSSES.md` to document:
...
- [x] 9.1 Add unit tests for pipeline parsing and validation:
...
- [x] 9.1B Add a **default-manifest resolution** test:
...
- [x] 9.3 Add focused tests for ST modes (forward differs vs `soft/exp`, gradients flow through expectation path).
- [x] 9.3B Add tests for `full_idea` rollout-context masking semantics:
  - matched prefix: `desc` CE masked, struct CE on,
  - FN injected: `desc` CE on by default,
  - FP spans: fully masked out,
  - closure/EOS: supervised.
- [x] 9.3C Add a minimal Stage-1 regression test for registry-backed dedup:
...
- [x] 9.3D Add a minimal metrics-contract test:
...
- [x] 9.4 Run targeted validation commands and record results:
