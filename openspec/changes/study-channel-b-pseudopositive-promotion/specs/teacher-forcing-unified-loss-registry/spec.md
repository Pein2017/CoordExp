...
- `pseudo_positive`: selected unmatched anchor objects promoted for coord-only positive supervision.
- `shielded_anchor`: unmatched anchor objects kept as neutral context.
...
- `pseudo_positive` objects MUST NOT create desc CE or matched-prefix structure CE,
...
#### Scenario: Pseudo-positive object uses anchor geometry and no new text CE
...
- **AND** it contributes no desc CE and no matched-prefix structure CE.
...
### Requirement: Gate terms respect context-specific masking and pseudo-positive coord-only participation
The unified loss registry SHALL keep pseudo-positive spans coord-positive and text-neutral when coord regularization gate terms are enabled.
...
- selected `pseudo_positive` anchors MUST NOT contribute to `text_gate`, desc-side supervision, or matched-prefix structure supervision,
- shielded and dead unmatched anchors MUST remain outside both `coord_gate` and `text_gate`.

#### Scenario: Pseudo-positive spans are coord-positive but text-neutral under coord regularization
- **WHEN** `stage2_ab.channel_b.pseudo_positive.enabled=true`
...
- **AND** they do not contribute to `text_gate` or other text-side supervision.
...
- shielded unmatched extras MUST stay outside matched-prefix struct masks, desc supervision, and positive bbox/coord groups,
- selected pseudo-positive anchors MUST populate positive bbox/coord groups only,
