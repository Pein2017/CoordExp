...
- [ ] 1.1 Add a shared offline bbox-format derivation module in `public_data/scripts/` that runs as a separate `public_data/` workflow branch and emits derived split artifacts under `bbox_formats/<format>/`.
- [ ] 1.2 Implement first-pass `cxcy_logw_logh = [cx, cy, logw, logh]` derivation from canonical preset `xyxy` sources, exporting `train.jsonl` / `val.jsonl` plus matching `train.coord.jsonl` / `val.coord.jsonl` when those splits exist.
- [ ] 1.3 Stamp derived records with `prepared_bbox_format`, slot-order metadata, canonical source lineage, and conversion-version metadata, and add focused tests that round-trip a known canonical box through the offline branch.
...
- [ ] 2.2 Add validation coverage for `bbox_formats/<format>/` branches, including missing-manifest, missing split JSONL outputs, provenance-mismatch, and malformed-geometry failures.
...
- [ ] 3.1 Refactor `src/datasets/` so offline-prepared non-canonical bbox branches are consumed directly and the supported online bbox-format conversion path is removed for that workflow.
...
- [ ] 3.3 Add regression tests covering the real failure mode: derived `cxcy_logw_logh` records render one-pass model-facing bbox slots only and never undergo a second runtime conversion.
...
- [ ] 5.1 Update Stage-1 `cxcy_logw_logh` configs to point at offline-prepared derived branch JSONLs rather than relying on runtime conversion.
- [ ] 5.2 Update `docs/data/PREPARATION.md`, `docs/data/CONTRACT.md`, and `docs/training/STAGE1_OBJECTIVE.md` to document the canonical `xyxy` source flow plus the separate offline bbox-format branch workflow.
