...
- [ ] 1.3 Retire canonical-parent usage of `configs/stage2_ab/base_rollout_matching_sft.yaml`, `configs/stage2_ab/prod/base.yaml`, and `configs/stage2_ab/base_smoke_runtime.yaml`; decide and document whether iterative variants remain (migrated one-hop) or are relocated to `configs/stage2_ab/legacy/`.
...
- [ ] 2.6 Land YAML relocation + `src/sft.py` normalized injection + `scripts/train_stage2.sh` normalized preflight together before enabling legacy-path fail-fast (single coordinated cutover).
...
- [ ] 3.4 Enable strict validators only after canonical leaves are migrated and any legacy/experimental profiles are parked under `configs/stage2_ab/legacy/`.
...
- [ ] 5.4 Audit profile discovery tooling/globs so canonical runs include only `configs/stage2_ab/prod/*.yaml` + `configs/stage2_ab/smoke/*.yaml` and exclude `configs/stage2_ab/legacy/*.yaml`.
