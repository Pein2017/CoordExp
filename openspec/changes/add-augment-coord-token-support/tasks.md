...
- [ ] 2.2 Regression smoke: run `scripts/inspect_chat_template.py` (or dataset builder) on a coord-token sample with augmentation enabled to confirm no crashes and tokens remain.

## 3. Documentation
- [ ] 3.1 Update DATA_JSONL_CONTRACT.md (augmentation note) or add a short doc blurb explaining coord-token augmentation behaviour and config gating.
