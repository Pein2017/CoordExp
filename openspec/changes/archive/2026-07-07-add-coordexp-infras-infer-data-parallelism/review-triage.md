...
- Objective: converge and verify the
  `add-coordexp-infras-infer-data-parallelism` OpenSpec change through the V1
...
- Source of truth: this change directory plus the active baseline inference
  contract in `openspec/changes/build-coordexp-infras-inference-infra/`.
...
- P1: The baseline dependency on
  `openspec/changes/build-coordexp-infras-inference-infra/` was implicit.
...
```bash
git diff --check -- openspec/changes/add-coordexp-infras-infer-data-parallelism
```
