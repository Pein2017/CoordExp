...
coordexp-infras inference currently supports HF batched decoding inside one
...
accepted baseline in `openspec/changes/build-coordexp-infras-inference-infra/`:
...
- Treat `openspec/changes/build-coordexp-infras-inference-infra/` as the current
...
- `coordexp-infras-infer-data-parallel-runtime`: controller/worker launch,
  CUDA binding, rank planning, deterministic row sharding, and strict merge
  orchestration for data-parallel inference.

### Modified Capabilities

- `coordexp-infras-infer-config-runtime`: production inference now has explicit
  CUDA availability and visible-device semantics for data-parallel launch.
- `coordexp-infras-infer-pipeline`: pipeline orchestration gains multi-rank
  shard planning, worker execution, and top-level merge behavior while keeping
  `generation.batch_size` per device.
- `coordexp-infras-infer-scoring-artifacts`: scored artifacts and provenance
  gain strict shard-to-merged validation and parallelism metadata.
- `coordexp-infras-infer-backend-trace`: trace/provenance records may include
  rank/device identity while preserving backend-neutral decode records.
- `coordexp-infras-infer-benchmark-smoke`: acceptance now requires a real
...
- `configs/coordexp_infras/infer/**` for production and smoke leaves that rely
