...
coordexp-infras V1 has a functional local training and inference backbone, but
...
- `coordexp-infras-pack-cache-semantic-identity`: source-code identity,
  provenance, and invalidation behavior for deterministic supervised packing
  caches.
- `coordexp-infras-checkpoint-handoff-readiness`: canonical checkpoint handoff
...
coordexp-infras specs. This change hardens contracts from the active
`rebuild-coordexp-infras-training-infra` baseline without treating archived or
...
- OpenSpec/docs surfaces that define the first two upgrade waves from
  `docs/superpowers/plans/2026-07-06-coordexp-infras-architecture-upgrade-roadmap.md`.
