...
coordexp-infras now has a rebuilt training path, but the repository still lacks
...
coordexp-infras checkpoints through batched HF/Qwen generation, token-trace
...
- Add a strict `InferConfig` schema rather than reusing `TrainConfig`; inference
  configs live under `configs/coordexp_infras/infer/`.
...
- Reuse coordexp-infras Qwen/template/model-composition semantics: no-resize
...
- `coordexp-infras-infer-config-runtime`: strict inference config, resolved
...
- `coordexp-infras-infer-backend-trace`: backend-neutral decode request/result
...
- `coordexp-infras-infer-prompt-parsing`: training-aligned prompt construction,
  Qwen no-resize image-plan evidence, compact object-box-closed parsing,
  parser diagnostics, and geometry conversion contracts.
- `coordexp-infras-infer-scoring-artifacts`: selected-token scoring,
...
- `coordexp-infras-infer-pipeline`: end-to-end inference orchestration, batching,
  artifact writing, counters, failure policy, and separation from metric
  reduction.
- `coordexp-infras-infer-benchmark-smoke`: real HF/Qwen smoke requirements,
  adapter-enabled smoke, evaluator-consumer compatibility, and final full
  benchmark acceptance gates.

### Modified Capabilities

- None. This change adds the first coordexp-infras inference/eval contract set.
...
- Affected configs: new inference configs under `configs/coordexp_infras/infer/`;
