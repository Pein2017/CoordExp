# coordexp-infras Inference Infrastructure Design

## Context

coordexp-infras has rebuilt the training infrastructure around local ownership
...
`docs/architecture/proposals/2026-06-27-coordexp-infras/DECISIONS.md`. Legacy
...
- provide a strict inference config and thin `python -m src.infer --config CONFIG.yaml`
  entrypoint;
- reuse coordexp-infras Qwen/template/adapter/artifact ownership instead of
...
coordexp-infras. After implementation, rollback means disabling the new
`configs/coordexp_infras/infer/*` entrypoints and keeping legacy inference
