...
- [x] 1.2 Study owner-boundary paths in current coordexp-infras Qwen/template/config/artifact/adapter code for prompt rendering, tokenizer ids, no-resize image planning, image batch materialization, resolved config writing, manifest writing, adapter reload, and special-token embedding delta validation.
