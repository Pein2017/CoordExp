...
coordexp-infras inference currently exposes a reserved `vllm` backend name but
...
- `coordexp-infras-infer-execution-model`: content-addressed base/DoRA/selected-
  token-delta composition, identity, atomic publication, cache validation, and
  worker handoff.

### Modified Capabilities

- `coordexp-infras-infer-config-runtime`: accept a strict vLLM backend block,
...
- Operational impact: composed vLLM models are derived caches under
  `model_cache/coordexp_infras/vllm_materialized/`, not canonical training
