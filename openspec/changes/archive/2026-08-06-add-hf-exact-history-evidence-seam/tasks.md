...
- [x] 1.1 Confirm explicit user approval before changing runtime code, tests,
  research scripts, configs, or launching model work; verify the exact
  `research-probe-infras` implementation worktree and preserve the source
  `research-probes` worktree and unrelated dirt.
- [x] 1.2 In `ms`, run
  ...
  appears.
- [x] 1.3 Add failing interface tests under `tests/inference/` for a
...
- [x] 1.4 Add failing inference tests for at-use Qwen position construction,
...
- [x] 1.5 Add failing receipt tests for observed parameter-dtype counts and
...
- [x] 2.1 Add the frozen `HFExactHistory` and `HFChosenTokenEvidence` public
...
- [x] 2.2 Implement `special_token_ids`, one-request
...
- [x] 2.3 Implement `teacher_forced_evidence` over the full
...
- [x] 2.5 Make the new `tests/inference/` slice green and run the pre-existing
...
- [x] 3.1 Migrate only the boundary-scoring evidence path in
...
- [x] 3.2 Run the frozen locality smoke for Source and transition step 36 in a
...
- [x] 3.3 Migrate only `_score_state_target` evidence construction in
...
- [x] 3.4 Run the frozen owner smoke for Source and transition step 36 in a new
  ...
  receipts.
- [x] 3.5 Only after both parity checks pass, remove superseded scoring and
...
- [x] 3.6 Verify that no new config file, artifact schema, reducer, owner policy,
...
- [x] 3.7 **Gate One:** run focused inference tests, the existing continuation
...
- [x] 4.1 Resolve only in-scope Gate One findings and rerun the smallest test or
...
- [x] 4.2 Run the complete default-collected test suite plus
  `tests/inference/`,
...
- [x] 4.3 Repeat the four representative Source/transition smokes from a clean
...
- [x] 4.4 Inspect the final diff and dependency surface: scientific choices
...
- [x] 4.5 **Gate Two:** run `openspec validate
