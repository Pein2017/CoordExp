...
coordexp-infras needs a clean, self-contained training infrastructure baseline so
...
- Establish a fresh OpenSpec baseline for coordexp-infras, with the previous
...
- `coordexp-infras-config-runtime`: strict resolved config, run identity,
  inheritance, cadence, runtime backend status, and planned-step schedule
  contracts.
- `coordexp-infras-data-template-encoding`: raw example shape, JSONL loading,
...
- `coordexp-infras-packing-forward`: no-padding packed-sequence construction,
...
- `coordexp-infras-supervision-losses`: `TokenAtom` / `TokenSpan` /
...
- `coordexp-infras-adapters-embeddings-optim`: DoRA/LoRA source-study gates,
...
- `coordexp-infras-training-artifacts`: `SupervisedTrainer`, `TrainRuntime`,
...
- `coordexp-infras-vertical-smoke`: the permanent tiny Qwen3-VL single-image
