...
coordexp-infras's training path is functional, but its infrastructure currently
...
- Migrate checked-in `configs/coordexp_infras/prod/` and smoke profiles directly
...
None. This change deliberately simplifies the existing coordexp-infras
training capabilities instead of introducing a new framework or telemetry
subsystem.

### Modified Capabilities

- `coordexp-infras-config-runtime`: Make Accelerate the sole runtime, write one
...
- `coordexp-infras-training-artifacts`: Replace the manifest/receipt/metric-event
  estate with a single rank-zero run record, wide-row `logging.jsonl`, bounded
  trainer results, and minimal checkpoint aliases.
- `coordexp-infras-pack-cache-semantic-identity`: Make cache payload versions
  disposable and rebuild-only while preserving strict semantic invalidation,
  completeness, and ordering checks.
- `coordexp-infras-checkpoint-handoff-readiness`: Remove the handoff manifest and
  ...
  boundary.
- `coordexp-infras-infer-config-runtime`: Replace `checkpoint-final` metadata
...
- Config areas: active `configs/coordexp_infras/prod/` and
  `configs/coordexp_infras/smoke/`; `configs/coordexp_infras/deepspeed/` is
  removed.
