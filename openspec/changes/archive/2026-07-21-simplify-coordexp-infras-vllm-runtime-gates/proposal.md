...
coordexp-infras can already materialize the required Qwen3-VL base, DoRA
...
- `coordexp-infras-infer-config-runtime`: replace exact receipt qualification
  with live operational support and explicit absolute path ownership.
- `coordexp-infras-infer-execution-model`: retain strict A+B+C structural
  composition while making behavioral comparison receipts optional.
- `coordexp-infras-infer-backend-trace`: make actual engine/decode/replay
  evidence authoritative and historical qualification informational.
- `coordexp-infras-infer-pipeline`: keep execution-model resolution in the
  controller while removing composition-fidelity binding from its ownership.
- `coordexp-infras-infer-benchmark-smoke`: distinguish optional parity studies
