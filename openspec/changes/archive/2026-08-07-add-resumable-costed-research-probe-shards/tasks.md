...
- [x] 1.1 Record the active consumer, merger, analyzer, tests, sealed plan, and
...
- [x] 1.2 Add failing journal interface tests for validated record payload
  ...
  schema version 1.
- [x] 1.3 Add failing consumer-adapter tests for separate logical plan and
...
- [x] 2.1 Add immutable public record and attempt view types plus a complete
...
- [x] 2.2 Expose last durable sequence, work-item identifier, attempt identifier,
  and record digest while keeping OS exit code and signal outside the journal
  owner.
- [x] 2.3 Pass the focused artifact suite and journal corruption/failure-injection
...
- [x] 3.1 Implement the consumer-specific adapter under `scripts/research/`
...
- [x] 3.2 Implement the versioned deterministic LPT schedule using
...
- [x] 3.3 Materialize a CPU schedule mechanics receipt for the accepted
...
- [x] 4.1 Create one exact-identity execution journal per physical slot, bind
...
- [x] 4.2 Adapt the existing support-context scoring boundary so each complete
...
- [x] 4.3 Add an explicit continuation path that opens only the exact schedule,
  ...
  outcome.
- [x] 4.4 Add scorer/model cleanup and best-effort `SIGTERM` attempt-failure
...
- [x] 5.1 Implement a pure materializer that loads all validated slot records,
  ...
  fields.
- [x] 5.2 Refuse terminal receipt publication for missing, duplicate, foreign,
  invalid, or merger-ineligible observations; publish mechanics diagnostics
  without assigning scientific meaning.
- [x] 5.3 Prove all eight materialized receipts pass the current unchanged
  merger; pass the unchanged analyzer regression on its owned census-v3 input;
  preserve ledger denominator, support rule, calibration, checkpoint, prefix,
  candidate, and outcome semantics; and separately record any pre-existing
  prior-support-ledger to census-v3 bridge incompatibility without repairing or
  reinterpreting it.

## 6. Attempt And Exit Diagnostics

- [x] 6.1 Add a parent launcher that records child PID, physical slot, attempt
...
- [x] 7.2 Require byte-for-byte equality and equal SHA-256 for every canonical
...
- [x] 7.3 Preserve a strict CPU equivalence receipt with input identities,
...
- [x] 8.1 Write a compact interface note describing logical plan, physical
  schedule, slot journals, explicit continuation, terminal materialization,
  diagnostics, failure states, and operator commands.
- [x] 8.2 Produce an exact-digest-bound adapter patch or consumer example for
...
- [x] 8.3 Recheck the active worktree and all named sealed roots against the
...
- [x] 9.1 Check live GPU ownership and headroom, select one explicit device,
...
- [x] 9.2 Launch the real consumer path, wait until at least one context record
...
- [x] 9.3 Materialize the bounded terminal receipt, close the scorer cleanly,
...
- [x] 10.1 Run focused journal and research-adapter tests, relevant inference
...
- [x] 10.2 Run separate standards and intent-contract audits; resolve every P0
...
- [x] 10.3 Freeze a self-excluding fixed-tree manifest over implementation,
  interface note, adapter patch/example, tests, CPU/GPU mechanics receipts, and
  audit verdicts; verify it from a fresh process.
- [x] 10.4 Stage only the explicit change-owned paths and create a local fixed
