...
- [x] 1.1 Add an interface-level test that imports the complete minimal
...
- [x] 1.2 Add a one-shot producer characterization proving strict exclusive
  publication refuses an occupied result path and preserves the original bytes
  and hash.
- [x] 1.3 Add a two-producer journal characterization using a common immutable
  ...
  interpretation.
- [x] 1.4 Record the Image2299 and historical Human13 mapping used by the guide:
...
- [x] 2.1 Extend the existing lazy `src.artifacts` facade with only
  ...
  deep owner module.
- [x] 2.2 Replay the import regression and prove the public surface remains
  lazy, CPU-only, and behavior-identical to direct imports from the deep
  owners.
- [x] 2.3 Replay the one-shot and independent-journal characterizations through
...
- [x] 3.1 Add `docs/RESEARCH_PROBE_INFRA_BASE.md` with the one-shot,
...
- [x] 3.2 Add one routing link from `docs/AGENT_INDEX.md` and one link at the
  existing infra-lane paragraph in `docs/BRANCH_AND_WORKTREE_POLICY.md`.
  Preserve that policy as the sole lifecycle authority.
- [x] 3.3 Add the promotion checklist requiring two live cross-direction
...
- [x] 3.4 Run a documentation residue check and remove any wording that
...
- [x] 4.2 Run targeted import/compile/style checks for `src/artifacts`, the new
  test, and the changed docs; inspect the exact diff for accidental source,
  worktree-lifecycle, or active-probe edits.
- [x] 4.3 Run `openspec validate establish-research-probe-infra-base --strict`,
...
- [x] 4.4 Freeze the resulting diff and obtain one bounded read-only
  ...
  checks.
- [x] 4.5 Submit the verified infra-base change for user review.  Do not merge
